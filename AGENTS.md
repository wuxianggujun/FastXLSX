...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  bold/italic/direct-color font 和 solid fill styles /
...
- `tests/test_worksheet_event_reader.cpp` 通过 CTest 注册为
  ...
  value text token，token 中的 string_view 只在 source worksheet XML buffer 生命周期内
  有效；当前测试覆盖 qualified local-name、inline text raw value、metadata pass-through
  和 malformed boundary 拒绝。这不是 public `WorksheetReader`、完整 XML parser /
  schema validation、namespace repair、relationship repair、row/cell transformer、
  `PackageEditor` stream rewrite handoff 或低内存大文件编辑完成。
- `tests/test_worksheet_transformer.cpp` 通过 CTest 注册为
  ...
  duplicate selector 和 empty payload preflight。这不是 public transformer、worksheet
  writer、dimension recalculation、dependency repair、stream rewrite output、
  `PackageEditor` / `EditPlan` commit 或低内存大文件编辑完成。
...
- `tests/test_package_editor.cpp` 通过 CTest 注册为 `fastxlsx.package_editor`，
  ...
  preservation。这只是 internal Patch MVP / handoff 回归，不是
  public `PackageEditor`、随机 cell 编辑、sharedStrings 索引迁移、style id 迁移、
  styles 合并、table/drawing 语义同步或大文件 streaming worksheet transformer。
...
- 把 `src/zip_store_writer.*`、`src/package_writer.*`、`src/package_reader.*` 或
  ...
  会在打开输出路径前拒绝需要 Zip64 的 entry count 或单 entry 未压缩大小，以及超过
