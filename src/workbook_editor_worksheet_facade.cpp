...
void WorksheetEditor::copy_cell_values(
    CellRange source, WorksheetCellReference destination)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    try {
        ...
        session->copy_cell_values(source,
            detail::CellPosition {destination.row, destination.column});
        state.clear_last_edit_error();
    } catch (const FastXlsxError& error) {
        state.record_last_edit_error(error);
...
void WorksheetEditor::copy_cell_values(std::string_view source_range_reference,
    std::string_view destination_cell_reference)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    CellRange source {};
    ...
    copy_cell_values(source,
        WorksheetCellReference {destination.row, destination.column});
}

void WorksheetEditor::copy_cell_values_from(const WorksheetEditor& source_sheet,
    CellRange source, WorksheetCellReference destination)
{
    WorkbookEditor& destination_owner = owner();
    WorkbookEditor::Impl& state = *destination_owner.impl_;
    try {
        ...
        if (&source_owner != &destination_owner) {
            throw FastXlsxError(
                "WorksheetEditor::copy_cell_values_from() requires both worksheets to belong to the same WorkbookEditor");
        }
        detail::validate_worksheet_editor_cell_range(source);
        detail::validate_worksheet_editor_cell_coordinate(
            destination.row, destination.column);
        ...
        destination_session->copy_cell_values_from(*source_session, source,
            detail::CellPosition {destination.row, destination.column});
        state.clear_last_edit_error();
    } catch (const FastXlsxError& error) {
        state.record_last_edit_error(error);
    ...
    std::string_view destination_cell_reference)
{
    ...
    copy_cell_values_from(source_sheet, source,
        WorksheetCellReference {destination.row, destination.column});
}

void WorksheetEditor::move_cell_values(
    CellRange source, WorksheetCellReference destination)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    try {
        ...
        session->move_cell_values(source,
            detail::CellPosition {destination.row, destination.column});
        state.clear_last_edit_error();
    } catch (const FastXlsxError& error) {
        state.record_last_edit_error(error);
...
void WorksheetEditor::move_cell_values(std::string_view source_range_reference,
    std::string_view destination_cell_reference)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    CellRange source {};
    ...
    move_cell_values(source,
        WorksheetCellReference {destination.row, destination.column});
}

void WorksheetEditor::move_cell_values_from(WorksheetEditor& source_sheet,
    CellRange source, WorksheetCellReference destination)
{
    WorkbookEditor& destination_owner = owner();
    WorkbookEditor::Impl& state = *destination_owner.impl_;
    try {
        ...
        if (&source_owner != &destination_owner) {
            throw FastXlsxError(
                "WorksheetEditor::move_cell_values_from() requires both worksheets to belong to the same WorkbookEditor");
        }
        detail::validate_worksheet_editor_cell_range(source);
        detail::validate_worksheet_editor_cell_coordinate(
            destination.row, destination.column);
        ...
        destination_session->move_cell_values_from(*source_session, source,
            detail::CellPosition {destination.row, destination.column});
        state.clear_last_edit_error();
    } catch (const FastXlsxError& error) {
        state.record_last_edit_error(error);
    ...
    std::string_view destination_cell_reference)
{
    ...
    move_cell_values_from(source_sheet, source,
        WorksheetCellReference {destination.row, destination.column});
}
...
void WorksheetEditor::move_cell_style(std::uint32_t source_row,
    std::uint32_t source_column, std::uint32_t destination_row,
    std::uint32_t destination_column)
{
    WorkbookEditor::Impl& state = *owner().impl_;
...
void WorksheetEditor::move_cell_style(std::string_view source_cell_reference,
    std::string_view destination_cell_reference)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    detail::WorksheetEditorCellCoordinate source {};
...
void WorksheetEditor::move_cell_styles(
    CellRange source, WorksheetCellReference destination)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    try {
        ...
        session->move_cell_styles(source,
            detail::CellPosition {destination.row, destination.column});
        state.clear_last_edit_error();
    } catch (const FastXlsxError& error) {
        state.record_last_edit_error(error);
...
void WorksheetEditor::move_cell_styles(std::string_view source_range_reference,
    std::string_view destination_cell_reference)
{
    WorkbookEditor::Impl& state = *owner().impl_;
    CellRange source {};
...
void WorksheetEditor::move_cell_styles_from(WorksheetEditor& source_sheet,
    CellRange source, WorksheetCellReference destination)
{
    WorkbookEditor& destination_owner = owner();
    WorkbookEditor::Impl& state = *destination_owner.impl_;
    try {
        ...
        if (&source_owner != &destination_owner) {
            throw FastXlsxError(
                "WorksheetEditor::move_cell_styles_from() requires both worksheets to belong to the same WorkbookEditor");
        }
        detail::validate_worksheet_editor_cell_range(source);
        detail::validate_worksheet_editor_cell_coordinate(
            destination.row, destination.column);
        ...
        destination_session->move_cell_styles_from(*source_session, source,
            detail::CellPosition {destination.row, destination.column});
        state.clear_last_edit_error();
    } catch (const FastXlsxError& error) {
        state.record_last_edit_error(error);
    ...
    std::string_view destination_cell_reference)
{
    ...
    move_cell_styles_from(source_sheet, source,
        WorksheetCellReference {destination.row, destination.column});
}
