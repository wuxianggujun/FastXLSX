...
- Patch 已覆盖 catalog、事务式空白 worksheet add、关系闭合 worksheet remove、sheetData/cell replacement、worksheet-local internal/external hyperlink、窄 rename、formula audit/recalculation、core/app properties 和 media bytes replacement。Internal hyperlink 只改 worksheet XML，不创建 `.rels`；external hyperlink 同步 worksheet XML 与 worksheet `.rels`。Data-validation editing 仍是后续独立切片。当前没有 public existing-workbook worksheet clone。新增表可同会话用 Patch 填充/rename/hyperlink，但需保存重开后才能 In-memory materialize。
