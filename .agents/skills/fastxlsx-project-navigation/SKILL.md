...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  ...
  `TwoColorScaleRule`、`TableOptions`、`TableTotalsFunction`、
  `ImageFormat`、`ImageInfo`、`ImageEditAs`、`ImageOptions`、`read_image_info()`、
  `WorkbookWriter::add_style()`、`CellView::with_style()`、
  `WorksheetWriter::add_conditional_color_scale()`、`WorksheetWriter::add_image()`
...
- OPC 当前已有内部 manifest / relationships / `PartIndex` / `RelationshipGraph`
  ...
  prompt/error attributes）、external/internal hyperlinks、two-color conditional color
  scales、streaming-only tables 和
