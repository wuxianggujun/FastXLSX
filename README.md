...
- **Patch**：`WorkbookEditor`，面向已有 workbook 的 part-level rewrite，并支持受限的 basic classic note、writer-compatible data-validation add/remove、conditional-format add 与 table metadata lifecycle；未修改和未知 part 默认保留。
...
- `WorkbookEditor::add_conditional_color_scale()` / `add_conditional_data_bar()` / `add_conditional_icon_set()` 可向 planned worksheet 追加 writer-compatible conditional formatting；color scale 支持 two-/three-color，三类 API 均支持 single range、`span` 与 initializer-list multi-range overload。Patch 对现有 conditional formats 只做 bounded structural/priority audit，再从 effective maximum priority + 1 分配新 priority；advanced/custom、`dxf`/styles、formula/cellIs 与 multiple-rule source payload 原样保留且不形成 editable projection，结构或 priority 异常明确 fail。该 edit 支持 planned rename/added worksheet，只改 worksheet XML，不改 cells、relationships、content types、styles/dxf、calc metadata 或 linked objects；`pending_worksheet_edits()` 通过 `conditional_format_count` 报告最终追加数，失败不污染状态且可 retry。
