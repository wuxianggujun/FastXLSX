...
- 当前 streaming conditional formatting 测试覆盖 two-/three-color color scale 和
  basic data bar。
...
- data bar 覆盖 `<cfRule type="dataBar">`、两个 `<cfvo>`、一个 `<color rgb>`、
  numeric/percentile endpoints、multi-range `sqref`、与 color scale 共享 worksheet-local
...
- worksheet metadata：列宽、冻结窗格、自动筛选、合并单元格、data validations、
  external/internal hyperlinks、two-/three-color conditional color scales、basic data bars 和 tables。
- dimension tracking。
...
ShowValue 和多区域 AppliesTo。它们是本地 QA/排障工具，不接入默认 CTest/CI，也不是运行时依赖。
