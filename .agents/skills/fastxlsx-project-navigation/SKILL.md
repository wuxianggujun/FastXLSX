...
description: "导航 FastXLSX 项目架构和当前实现状态。用于理解修改位置、判断文档中的 API/模块是否已实现、调整公共架构、添加核心模块、审查可编辑高性能 OpenXML 设计边界、Streaming/Patch/In-memory 三路径、EditPlan 联动分析，或避免把路线图内容误当作现有代码。"
---
...
仍是空骨架。
如果任务涉及 Phase 4、Patch、preservation、dependency policy 或 In-memory，
先按 `docs/TASK_BREAKDOWN.md` 选择子任务编号和并行/串行边界，不要直接执行笼统的
“任务 4”。

## 项目事实

- FastXLSX 是 C++20 / MSVC 2026 优先的可编辑高性能 XLSX/OpenXML 引擎。
- 项目方向是共享 OpenXML/OPC 底座，提供 Streaming、Patch、In-memory 三条 API
  路径；流式写入是大文件性能主线，编辑能力也是核心主线。
- 参考库范围不限 C++。`docs/ARCHITECTURE.md` 和 `docs/PERFORMANCE_TARGETS.md`
  记录跨语言 XLSX 参考边界、取舍和 benchmark 证据；参考其他库时必须说明吸收的是
  In-memory UX、Streaming writer、Patch editor、OPC package 语义、兼容性 fixture 还是
  benchmark 经验。
- 当前阶段计划的默认推进线是 `C0 -> C7`；`P*` 只保留为历史索引和能力切片，不再是
  默认执行顺序。不要把 `P4.0` 当成下一步入口：C0 校准基线，C1 硬化当前
  `WorkbookEditor` Patch facade，C2 做 preservation / dependency audit，C3 走 public
  editor 决策门，C4 走小文件 In-memory，C5 走大 worksheet 低内存 rewrite，C6 维持支撑线，
  C7 处理 release / packaging。Phase 2 writer/backend/sharedStrings/benchmark 工作作为
  并行支撑线继续硬化。不要把性能硬化解释成“编辑能力以后再说”，也不要把 Patch 做成
  streaming writer 的事后补丁。
...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  ...
  `VerticalAlignment`、`CellFont`、`CellFill`、`CellStyle`、`DocumentProperties`、`DataValidationRule`、
  ...
  `DataBarRule`、`IconSetStyle`、`IconSetValueType`、`IconSetRule`、
  `TableOptions`、`TableTotalsFunction`、
  ...
  `WorksheetWriter::add_image()` path / memory-source overloads、
  `WorkbookEditor`、`WorkbookEditorOptions`、`WorkbookEditorWorksheetCatalogEntry`、
  `WorkbookEditorWorksheetEditSummary`、`WorksheetEditor`、`WorksheetEditorOptions`、
  `WorksheetCellReference`、`WorksheetCellUpdate`、`WorksheetCellSnapshot`、
  `CellPatchMissingCellPolicy` 和 `FastXlsxError`；`Workbook::set_document_properties()` 和
  `WorkbookWriterOptions::document_properties` 是当前基础 docProps 配置入口。
- `WorkbookEditor::replace_cells(..., CellPatchMissingCellPolicy::Insert)` 是当前
  public targeted-cell point-upsert 入口；不要把内部
  `PackageEditor::replace_or_insert_worksheet_cells*` 写成 public API，也不要再引用已移除的
  separate upsert-named public wrapper。
  当前 targeted-cell transformer 已有 completed-target fast path：strict replace
  在全部 targets 命中且 source stream 越过最后 target 坐标后停止 tail
  replacement lookup；Insert/upsert 在全部 targets 发射后停止 pending-target /
  replacement lookup，但仍扫描 source XML。这是大 worksheet tail pass-through
  热路径优化，不是 metadata repair 或完整 low-memory random editing。当前已有
  internal `WorksheetCellIndex` source-offset 索引和 indexed rewrite planning
  基础，可用 compact `{row, column, range}` 主索引把 source worksheet cells
  映射到 `<c>` byte ranges，并校验 / 排序有界 target set；旧 `cells()` map
  只作为 lazy diagnostic snapshot 存在，不再是 benchmark / rewrite 热路径。
  transformer actions 也会暴露 source XML offset。当前还有
  internal materialized indexed slicer，可在 worksheet XML bytes 与 index 完全匹配时
  按 source ranges 拼接 strict existing-cell replacements。当前还有 internal
  `PackageEntryChunk` byte-range emitter，可按 offset/size 切出 memory/file staged
  chunks，包含跨 chunk 边界的切片。当前还已有 internal PackageEditor
  chunk-backed indexed strict-replace slicer prototype，可用 staged chunks + prebuilt
  index 或 target-only preplanned ranges 拼接 existing-cell replacement payload；当前
  `fastxlsx_bench_package_editor_cell_replacement` 还提供 opt-in
  `--rewrite-strategy indexed-staged` schema-v2 benchmark/prototype 路径，验证
  benchmark 生成的 staged chunks + indexed/preplanned ranges。该 benchmark 现在使用
  target-only range planner，只保存请求 target cells 的 `<c>` byte ranges。它们不是
  public API，也不是当前 public Patch 的默认 algorithm switch、PackageEditor
  source-entry ZIP seek 或完整 indexed worksheet rewrite 能力。
- `WorkbookWriter` / `WorksheetWriter` / `CellView` 是当前流式 writer 写入骨架；
  公式、行高、列宽、冻结窗格、自动筛选和合并单元格属于当前写入骨架，
  不等同完整 Phase 3。
- 当前 public docProps 配置会写出基础 `docProps/core.xml` 和 `docProps/app.xml`
  小型 XML part；内部 `PackageEditor` 另有 core/app docProps Patch 小切片。
  这不是 `docProps/custom.xml` 创建/编辑或完整 document properties public API；
  当前内部回归只证明 core/app docProps Patch helper 重写 package metadata 时会
  保留已有 `docProps/custom.xml`、custom-properties package relationship、content type
  override 和 unknown bytes。
- 当前已有 streaming-only number-format + wrap-text + limited horizontal/vertical alignment + bold/italic font + solid fill
  styles 基础切片：`StyleId` 是 workbook-local handle，`CellStyle` 当前支持
  `number_format`、可选 `CellAlignment::wrap_text`、可选 `HorizontalAlignment` /
  `VerticalAlignment` 和可选 `CellFont::bold` / `CellFont::italic`、可选
  `CellFill` solid foreground ARGB，`WorkbookWriter::add_style()`
  会注册样式并让 `close()` 写 `xl/styles.xml`、workbook styles relationship 和 content
  type override，`CellView::with_style()` 让 cell 写 `s="N"`。alignment 只写
  `applyAlignment="1"` / `<alignment .../>` attributes，不计算行高。这不是完整 styles、
  full font control、full fill/pattern control、border/full alignment、date cell type、dxf-backed conditional formatting、
  rich text 或 existing-file style preservation；当前 bold/italic font 只写 `<fonts>` 中的
  `<b/>` / `<i/>`、`fontId` 和 `applyFont="1"`；当前 solid fill 只写 solid `<patternFill>`、
  `fgColor rgb`、`fillId` 和 `applyFill="1"`。当前 two-/three-color color scale 和
  basic data bar 和 basic 3Arrows icon set 是 worksheet metadata，不是 styles registry 或 `dxfs` 支持。
- `PackageReader` 当前有 `src/package_reader.*` 内部 ZIP entry reader
  和 content-types / relationships ingestion 基础；它能索引和读取
  stored/no-compression entries，包括 unknown entries；`FASTXLSX_ENABLE_MINIZIP_NG=ON`
  构建还能通过 minizip-ng 读取 DEFLATE entries，默认构建仍拒绝 compressed input。
  读取 entry 时会校验解压后 payload CRC，拒绝非法 ZIP entry name（绝对路径、尾部斜杠、
  反斜杠、query/fragment components、空段、dot 段或 parent 段）、local header CRC/method/name/size mismatch、encrypted flags、data descriptor entries、Zip64 和损坏 metadata 或 payload bytes，也会拒绝 owner part 缺失的 source-owned
  `.rels`，包括根级 `_rels/foo.xml.rels` owner relationship entry，并建立内部 `PartIndex` /
  ...
  `.rels` metadata ingestion 和 `RelationshipGraph` 挂回；workbook sheet catalog
  resolver 还会先验证 package `_rels/.rels` 中存在且仅存在一个 internal
  `officeDocument` relationship，当前窄实现只接受解析到 `/xl/workbook.xml` 的
  target；相对、绝对和 dot-segment package target（例如 `xl/./workbook.xml`）
  都从 package root 解析，且不会把 package root 建模成真实 `PartName`；
  缺失、重复、external、带 query/fragment 或非固定 target 会在 lookup
  阶段失败；当前 reader/source-catalog by-name Patch 回归还覆盖 workbook-owned 绝对 worksheet
  ...
  relationship repair、pruning 或 public API。
  它不复制或重写 package，也不是 public editing API。
  当前另有 `src/package_editor.*` internal copy/replace 基础，可替换一个已存在
  part 并把未替换 entries（包括 unknown entries）按原始 bytes 写到新 stored
  package；当前还支持 core/app docProps generated-small-XML 窄切片，可生成/替换
  `docProps/core.xml` 和 `docProps/app.xml`，包括两者都缺失的输入，并同步 package rels / content types；
  当前还支持 worksheet replacement 窄切片，可在替换既有 worksheet part
  时同步 calcChain remove、workbook fullCalcOnLoad、workbook rels 和 content types。
当前还支持内部 `PackageEditor::replace_worksheet_part_by_name()` 目标定位
helper：通过 `PackageReader` workbook sheet catalog resolver 找到已有 worksheet
part；该 resolver 要求 sheet relationship id 使用 officeDocument relationships XML
namespace（可用非 `r` 前缀，普通 `id` 或错误 namespace 的 `id` 会被拒绝），
并且只读取 workbook `<sheets>` 目录的直接 `<sheet>` 子元素，要求 `name` /
`sheetId` 是未命名空间的 workbook sheet 属性，忽略目录外或嵌套在
非 sheet 目录子元素下的同名标签，
再委托同一 `replace_worksheet_part()` 路径处理 calcChain/fullCalcOnLoad 与
preservation 副作用；缺失或重复 sheet name 失败不污染 EditPlan、manifest、
package-entry audit、calc policy 或输出 bytes；invalid package `officeDocument`
entrypoint 也会在 by-name Patch 前失败且不污染状态；source workbook catalog
查找还会在 by-name Patch 状态变更前拒绝 workbook `.rels` 中缺失的 sheet relationship
id，以及指向未注册 worksheet part 的 worksheet relationship，这只是目标定位校验，
不是 relationship repair、pruning 或 orphan cleanup。这不是 sheet rename/delete、
sheet catalog mutation、任意 workbook part-location 支持、随机 cell 编辑或 public API。
...
当前还支持内部 `PackageEditor::replace_worksheet_sheet_data()` helper：只替换
既有 worksheet XML 的 `<sheetData>` / `<sheetData/>`，保留同一 worksheet part
的外围 XML metadata，并复用 worksheet replacement 的 calcChain/fullCalcOnLoad
与 preservation 副作用；成功后会在内部 `EditPlan` notes 中审计保留的
worksheet-local metadata range/reference，当前覆盖 sheetPr、dimension、sheetViews、
...
output 中把这些 part 暴露为 relationship-derived copy-original entries；这只能写成
Patch preservation / audit 可见性，不能写成图片/VML/header-footer 语义编辑、
...
relationship-derived copy-original entry；这只能写成 Patch preservation / audit
可见性，不能写成打印设置语义编辑、relationship repair/pruning、orphan cleanup、
...
removed-part inbound relationship audit。这仍只能写成 Patch audit / no-pruning
可见性，不能写成图片/VML/header-footer 删除语义、relationship repair/pruning、
...
`CalcChainAction::Preserve`，且不发明 VML owner `.rels`。这仍只能写成 Patch
same-path state hygiene / audit 可见性，不能写成事务式
undo、图片/VML/header-footer 语义合并或删除、relationship repair/pruning、
orphan cleanup、content type repair、public API 或完整 object lifecycle 支持。
...
暴露为 relationship-derived copy-original entries；这只能写成 Patch
preservation / audit 可见性，不能写成 OLE / ActiveX / control 语义编辑、
relationship repair/pruning、orphan cleanup、content type repair、public API 或
完整 object preservation。
...
暴露结构化 removed-part inbound relationship audit。这仍只能写成 Patch audit /
no-pruning 可见性，不能写成 OLE / ActiveX / control 删除语义、relationship
...
`.rels`。这仍只能写成 Patch
same-path state hygiene / audit 可见性，不能写成事务式 undo、OLE / ActiveX /
control 语义合并或删除、relationship repair/pruning、orphan cleanup、content type
...
这不是 public API、随机 cell 编辑、range 修复、
dataValidations/conditionalFormatting/hyperlinks/table/drawing 语义同步、sharedStrings/styles
迁移或大文件低内存 transformer。若 replacement
`<sheetData>` 使用 shared string indexes、style id references 或公式 cell，当前也只写
audit notes 和结构化 `WorksheetPayloadDependencyAudit`，提示 caller 复核 `xl/sharedStrings.xml`、`xl/styles.xml` 和 calc metadata /
calcChain policy，不迁移 sharedStrings 索引、不合并 styles、不求值公式、不重建 calcChain。
...
XML repair。
  当前导入 existing package manifest 时会保留 source `[Content_Types].xml`
  defaults/overrides 形态，worksheet replacement 测试确认移除 calcChain override
  不会把 PNG media default 无故提升为 image part override；还覆盖缺失
  `xl/calcChain.xml` payload 但残留 calcChain content type override 或 workbook
  calcChain relationship 的 metadata-only cleanup，不记录缺失 payload 的
  removed-part audit、不创建 calcChain payload，也不是通用 metadata repair。
  当前还支持内部 `PackageEditor::request_full_calculation()` workbook-only calc
  metadata helper：只重写 `/xl/workbook.xml` 小型 metadata part 来设置
  `fullCalcOnLoad="1"`，可按 `CalcChainAction::Remove/Preserve` 清理或保留
  calcChain payload、content type、workbook relationship 和 calcChain owner `.rels`；
  Remove 路径还覆盖没有 `xl/calcChain.xml` payload 但残留 calcChain content type
  override 或 workbook relationship 的 metadata-only cleanup，不创建 payload 或
  removed-part audit；
  `CalcChainAction::Rebuild` 未实现且失败不污染状态。它不是 worksheet rewrite、
  公式求值、calcChain rebuild、public API 或通用 relationship/content-type repair。
  DEFLATE 源包回归中，与 workbook calc helper 无直接因果关系的 unknown owner
  `.rels` 只通过 `planned_output()` copy-original 可见性和输出重读验证保留，
  不写成 edit-plan package-entry side-effect audit。
  workbook-only helper 只更新 workbook 根元素的直接子级 `calcPr`，保留 `extLst`
  ...
  workbook-only helper 还有 prior ordinary workbook / calcChain replacement 顺序组合回归：
  `request_full_calculation()` 会使用已排队 workbook XML，保留其中非 calc
  workbook metadata，规范 `fullCalcOnLoad="1"` 并清理 calcChain payload/content
  ...
  copy-original audit；这仍不是 calcChain rebuild 或公式求值。普通 part replacement、
  docProps generated parts 和 worksheet replacement 会把 write-mode / dirty / generated /
  preserve-original 状态同步到内部 manifest；worksheet replacement 还会把
  workbook metadata rewrite 同步为 `LocalDomRewrite`；内部 `EditPlan` 现在还会记录
  `[Content_Types].xml`、package `_rels/.rels`、workbook `.rels` rewrite 和
  removed calcChain owner `.rels` omission，以及 preserved source-owned `.rels`
  存在时的 copy-original package-entry 审计项，供 Patch 审计；该审计也覆盖
  ...
  这些 package-entry audit 是内部结构化状态，分类为 content types、package
  relationships 和 source-owned relationships；只有 source-owned relationships
  携带 owner part，且 kind 必须匹配 `[Content_Types].xml`、package `_rels/.rels`
  或 owner-derived `.rels` entry；这不代表 public metadata editor。
  普通 `replace_part()` 显式拒绝 `[Content_Types].xml`、package `_rels/.rels` 和
  source-owned `.rels` metadata entry 作为 ordinary part replacement target；这些
  metadata entry 只能通过窄 helper / package-entry audit 路径变化，这不是完整
  ...
  当前组合回归还覆盖 docProps generated-small-XML 与 worksheet replacement 共存时的
  relationship/content-type 状态合并、calcChain removal、stale calcChain owner `.rels`
  omission、workbook metadata rewrite、unknown entry preservation、
  ...
  rejection；malformed workbook metadata /
  workbook calc metadata preflight failure 和 invalid replacement failure
  也覆盖不污染 edit plan entries / notes、aggregate `planned_output()` /
  legacy output-entry preview、manifest write-mode / copied output bytes；
  core/app docProps package relationship target 冲突失败也覆盖不污染
  edit plan entries / notes、manifest / package-entry audit / copied output；该拒绝只是 reader-backed copy 安全护栏，
  现在还覆盖 guard 失败后 queued part replacement 保留在 `EditPlan` / manifest /
  ...
  后续安全输出仍按计划省略 calcChain 并保留 unknown bytes；不是 atomic in-place editor。
  no-op `PackageEditor::save_as()` roundtrip 结构测试验证同一个 linked-object fixture
  的全部源 entries 保持 entry 顺序、stored entry method / CRC / uncompressed size
  和 bytes 一致，初始 `EditPlan` 只有 copy-original part entries，且没有 metadata
  package-entry side effect；这只是未修改包 copy baseline。
  ...
  entries 保持 entry 顺序、stored entry method / CRC / uncompressed size 和 bytes
  一致；这只是窄 part-level rewrite copy-original 证据。
  ...
  sheet catalog sync、relationship repair 或完整 worksheet editing。
  ...
  worksheet、drawing、chart 和 unknown extension entries 仍保持同一
  copy-original 基线；这不是图片解码、drawing mutation 或 existing-workbook image editing。
  ...
  styles、VBA 和 unknown extension entries 仍保持同一 copy-original 基线，
  ...
  当前结构测试还证明该窄路径会 byte-preserve worksheet `.rels`、drawing、
  ...
  `definedNames` 在 workbook metadata rewrite 路径下保留；另有 registered comments part
  小 fixture 证明 worksheet rewrite 会把 `xl/comments/comment1.xml` 和 source worksheet
  `.rels` 作为 copy-original preservation 处理，保留 comments content type override，
  并可由 `PackageReader` / `RelationshipGraph` 重读；这不是 comments 编辑、threaded
  comments、notes UI、relationship repair、orphan cleanup 或 public API；另有 threaded
  comments / persons 小 fixture 证明 worksheet rewrite 会把
  ...
  编辑、notes UI、relationship repair、orphan cleanup 或 public API；同一 threaded
  comments / persons 小 fixture 还覆盖 ordinary
  `replace_part("/xl/threadedComments/threadedComment1.xml", ...)` 和显式 removal：
  ...
  另有 pivot table /
  pivot cache 小 fixture 证明 worksheet rewrite 会把
  ...
  preservation 处理，并可由 `PackageReader` / `RelationshipGraph` 重读；这不是
  pivot table 编辑、pivot cache rebuild、relationship repair、orphan cleanup 或
  public API；该 worksheet rewrite 路径的内部 `planned_output()` 现在也暴露
  ...
  unknown entry copy-original，且确认不凭空创建 records owner `.rels`；这仍只是
  Patch audit 快照，不是 pivot cache rebuild、records refresh、relationship
  repair/pruning、orphan cleanup 或 public API；同一 pivot table / pivot cache 小 fixture 还覆盖 ordinary
  `replace_part("/xl/pivotTables/pivotTable1.xml", ...)` 和显式 removal：replacement
  只重写 pivot table XML，保留 worksheet `.rels` 中的 inbound pivotTable relationship、
  pivot table owner `.rels` / pivotCacheDefinition relationship、pivot cache definition /
  records parts、pivot cache definition owner `.rels`、workbook `<pivotCaches>`、workbook
  `.rels` pivotCacheDefinition relationship、content type overrides 和 unknown entry；
  removal 输出省略 pivot table part 及其 owner `.rels`，移除 pivot table content type
  override，但保留 worksheet `.rels` 中指向缺失 part 的 inbound relationship、workbook
  pivot cache metadata、pivot cache definition / records 链和 unknown entry。这不是 pivot
  table 语义编辑、pivot cache rebuild、cache records 刷新、relationship pruning/repair、
  ...
  或 public API；同一 fixture 还覆盖 ordinary
  ...
  或 public API；该 worksheet rewrite 路径的内部 `planned_output()` 现在也暴露
  fullCalcOnLoad / `CalcChainAction::Remove`、worksheet `StreamRewrite`、workbook
  ...
  relationship target audit；这仍只是 Patch audit 快照，不是 external links 编辑或
  relationship repair；同一 workbook external links 小 fixture 还覆盖 ordinary
  ...
  relationship pruning/repair、orphan cleanup 或 public API；另有 custom XML 小 fixture 证明 worksheet rewrite 会保留 package
  `_rels/.rels` 中的 customXml relationship、`customXml/item1.xml`、custom XML item
  owner `.rels`、`customXml/itemProps1.xml`、custom XML properties content type
  override 和 unknown entry，并可由 `PackageReader` / `RelationshipGraph` 重读；
  这不是 custom XML 编辑、schema/data binding、relationship repair、orphan cleanup
  或 public API；该 worksheet rewrite 路径的内部 `planned_output()` 现在也暴露
  fullCalcOnLoad / `CalcChainAction::Remove`、worksheet `StreamRewrite`、workbook
  ...
  preservation，且不新增 relationship target audit、不凭空创建 properties owner
  `.rels`；这仍只是 Patch audit 快照，不是 custom XML 编辑、schema/data binding 或
  relationship repair；同一 custom XML 小 fixture 还覆盖 ordinary
  ...
  `PackageReader` / `RelationshipGraph` 重读；这不是 custom XML 语义编辑、
  schema/data binding、relationship repair、content type repair、orphan cleanup 或
  public API；同一小 fixture
  还覆盖显式移除 `customXml/item1.xml`：输出省略 custom XML item 及其
  source-owned owner `.rels`、保留 package `_rels/.rels` customXml inbound
  relationship、保留 `customXml/itemProps1.xml`、custom XML properties content type
  override、默认 XML content type 和 unknown entry，且不重写 `[Content_Types].xml`；
  这不是 custom XML 删除语义、schema/data binding、relationship pruning/repair、
  content type repair、orphan cleanup 或 public API；同一小 fixture
  还覆盖同一路径先显式移除再 ordinary replace，以及先 ordinary replace 再显式移除：
  前者恢复 active custom XML item、清理 stale removed-part / removed owner `.rels`
  audit、恢复 owner `.rels` copy-original audit，且不重写 `[Content_Types].xml`；
  后者清理 active replacement、记录 removed-part / removed owner `.rels` audit、
  输出省略 custom XML item 和 owner `.rels`，并保留 package inbound relationship、
  properties part、默认 XML content type 和 unknown entry。内部 `planned_output()`
  快照还覆盖该 restore 状态：暴露 active custom XML item `LocalDomRewrite` entry、
  owner `.rels` copy-original `SourceRelationships` audit，以及 preserved package
  relationships、content types、workbook、worksheet、properties part 和 unknown entry。
  同一快照还覆盖该 final-removal 状态：暴露 omitted custom XML item、omitted
  source-owned owner `.rels`、package inbound customXml relationship audit，以及
  preserved package relationships、content types、workbook、worksheet、properties part
  和 unknown entry。这不是事务式 undo、
  custom XML 语义合并、relationship pruning/repair、content type repair、orphan cleanup
  或 public API；同一小 fixture
  ...
  再显式移除：前者清理 stale removed-part audit、恢复 active properties part、
  恢复 properties content type override/content-types copy-original audit，并继续保留
  item owner `.rels`；后者清理 active replacement、记录 removed-part audit、输出省略
  ...
  语义合并、relationship pruning/repair、content type repair、orphan cleanup 或
  public API；同一小 fixture
  ...
  properties payload，保留 removed custom XML item / removed owner `.rels` audit，
  输出继续省略 custom XML item 和 item owner `.rels`，并保留 package customXml inbound
  relationship、properties content type override、默认 XML content type 和 unknown
  ...
  item。后续 item replacement 只重写 item payload，保留 removed properties part
  audit / content-types rewrite，输出继续省略 properties part 和 properties content
  type override，并保留 item owner `.rels` 中指向缺失 properties part 的 customXmlProps
  relationship、package customXml inbound relationship、默认 XML content type 和 unknown
  entry。这不是 custom XML dependency repair、relationship pruning/repair、
  content type repair、orphan cleanup、事务式 undo 或 public API；同一小 fixture
  ...
  public API；同一小 fixture
  还覆盖 ordinary `replace_part("/xl/comments/comment1.xml", ...)`：只重写 comments
  XML，保留 worksheet `.rels` inbound comments relationship、comments content type
  override、workbook XML / workbook `.rels`、worksheet 和 unknown entry copy-original 基线，
  且不凭空创建 comments owner `.rels`；这不是 comments model mutation、threaded
  comments、notes UI、relationship repair、orphan cleanup 或 public API；内部
  `planned_output()` 快照还覆盖该 ordinary replacement 状态：暴露 active comments
  part local-DOM rewrite、preserved content types / package relationships / workbook /
  workbook `.rels` / worksheet / worksheet `.rels` / unknown entry，且不凭空创建
  comments owner `.rels`。这仍只是 Patch audit，不是 comments model mutation、notes
  UI、relationship repair、orphan cleanup 或 public API；同一小 fixture
  还覆盖显式移除 `xl/comments/comment1.xml`：输出省略 comments part、移除 comments
  content type override、保留 worksheet `.rels` inbound comments relationship，且不凭空
  创建 comments owner `.rels` omission；这不是 comments 删除语义、threaded comments、
  notes UI、relationship pruning/repair、orphan cleanup 或 public API；同一小 fixture
  还覆盖先显式移除再 ordinary `replace_part()` 的反向顺序：后续 replacement 会恢复 active
  comments part、清理 stale removed-part audit、让 `[Content_Types].xml` 回到
  source/copy-original audit、保留 inbound worksheet `.rels`，且仍不凭空创建 comments
  owner `.rels`；这不是事务式 undo、comments 语义合并、relationship repair、
  orphan cleanup 或 public API；内部 `planned_output()` 快照还覆盖该
  remove-then-replace 状态：暴露 active comments part local-DOM rewrite、content types
  copy-original audit、preserved package/workbook/worksheet `.rels` 和 unknown entry，
  清空 output-plan removed_parts / removed_package_entries 中的 stale removal 状态，
  且不凭空创建 comments owner `.rels`。这仍只是 Patch audit，不是 comments undo、
  semantic merge、relationship repair、orphan cleanup 或 public API；同一小 fixture 还覆盖先 ordinary `replace_part()`
  再显式移除的顺序：后续 removal 会清理 active comments replacement、记录
  removed-part audit、输出省略 comments part、移除 comments content type override、
  保留 inbound worksheet `.rels`，且仍不凭空创建 comments owner `.rels`；这不是
  comments 删除语义、事务式 undo、relationship pruning/repair、orphan cleanup
  或 public API；内部 `planned_output()` 快照还覆盖该 replace-then-remove
  final-removal 状态：暴露单个 omitted comments part / removed-part audit（target 为
  comments part、reason 保留 after replacement、inbound relationship 一条）、content
  types rewrite、preserved package/workbook/worksheet `.rels`，且 removed_package_entries
  为空、不凭空创建 comments owner `.rels`；这仍只是 Patch audit，不是 comments
  删除语义、事务式 undo、relationship pruning/repair、orphan cleanup 或 public API；
  同时覆盖
  ...
  `PackageEditor` edit plan。它不新增
  worksheet，不做 relationship pruning/orphan cleanup，也不对
  sharedStrings/styles/tables/drawings/defined names 做语义联动同步、索引迁移或重建，也不是
  public editing API、完整 document properties API、完整图片/图表/VBA passthrough
  或 broad preservation 证明。
  `CellEncoder` 等名称仍主要是文档设计目标；除非源码中存在，否则不是已实现 API。
- 当前内部 `EditPlan` / `DependencyAnalyzer` / `ReferencePolicy` /
  `PartRewritePlanner` 只代表 Patch 计划基础；`DependencyAnalyzer` 可沿 worksheet
  ...
  relationship target 审计 note。它会跳过 external hyperlink target，不会把它当成
  ...
  worksheet `EditPlan` 和 existing-file `PackageEditor::edit_plan()`，方便 Patch 审计但不代表 target validation 或 repair；
  ...
  entries，并可把已注册目标 part 的显式删除表达为 removed-part audit；当前
  `plan_worksheet_stream_rewrite()` 在 `CalcChainAction::Remove` 下会直接产出 stale
  calcChain removed-part audit，worksheet replacement 只消费该 planner 输出；内部 `EditPlan` 还可记录非 part package-entry rewrite/omission/preserved
  copy-original 审计项，
  ...
  Patch audit metadata，静态依赖仍只带 reason 文本；
  但不代表已有文件 reader/copy/write 管线、drawing/image/chart/table 编辑或 public editing API。
- OPC 当前已有内部 manifest / relationships / `PartIndex` / `RelationshipGraph`
  / content type registry / `EditPlan` / `DependencyAnalyzer` /
  `ReferencePolicy` / `PartRewritePlanner` 基础；Phase 5 已有 data validations（含 worksheet-only
  prompt/error attributes）、external/internal hyperlinks、two-/three-color conditional color
  scales、basic conditional data bars、basic 3Arrows conditional icon sets、streaming-only tables 和
  `WorksheetWriter::add_image()`（含 path / memory-source PNG/JPEG sources，以及
  drawing-only `ImageOptions` edit_as/name/description metadata 和图片对象
  external hyperlink metadata）
  等基础切片，但不能宣称完整 data validation UI、
  conditional formatting、图片、VBA、table、drawing 编辑或 existing-workbook 保真支持。
- 图片对象 external hyperlink metadata 只属于 `WorksheetWriter::add_image()` 的
  streaming-only new-workbook drawing object：`ImageOptions::external_hyperlink_url` /
  `external_hyperlink_tooltip` 写 drawing XML `xdr:cNvPr/a:hlinkClick`，relationship 在
  drawing `.rels` 中且 `TargetMode="External"`。它不是 worksheet `<hyperlinks>`、
  cell text / style、URL 校验、internal picture link、existing-file editing 或完整
  hyperlink UI。
- `src/package_writer.*` 是内部 package writer boundary；默认 backend 是
  ...
  `src/package_reader.*` 是内部 ZIP reader + OPC metadata ingestion 基础，
  `src/package_editor.*` 是内部 copy/replace 基础，且 `save_as()` 拒绝覆盖
  exact 或 path-equivalent 的 source package path，也会在 materialize output entries
  前拒绝 empty output path、missing or non-directory output parent path 或 existing directory output path；它们不代表 public PackageEditor、
  broad preservation pipeline 或完整 XLSX package editing。
...
- 不要把所有功能继续堆进 `src/streaming_writer.cpp` 或
  `tests/test_streaming_writer.cpp`。
- 当功能已有独立 public API、独立 XML 序列化、独立状态结构、独立 QA helper
  或大量边界用例时，优先考虑 feature-specific `.cpp`、detail helper 和独立测试文件。
- `src/streaming_writer.cpp` 保留流程编排、生命周期管理、热路径调用和必要的跨功能协调。
- `tests/test_streaming_writer.cpp` 保留 streaming 主流程、核心边界和跨功能集成；
  大量 feature 结构测试和负例应逐步拆分。
- public headers 可以继续作为稳定入口；拆分主要针对内部实现、XML writer helper、
  校验逻辑、状态转换和测试组织。
- 新增 `.cpp` 或测试文件时，同步更新 `CMakeLists.txt` / `tests/CMakeLists.txt`。
- 不要过度拆分：少量协调代码、一次性修正或边界尚不稳定的小切片可以先留在现有文件。

## 推荐流程

1. 判断任务是在改已实现代码，还是只在补架构/规划文档。
2. 如果要新增实现，把模块归入文档分层：`api`、`opc`、`xml`、
   `workbook`、`worksheet`、`edit`、`style`、`strings`。
3. 判断是否已达到拆分阈值；达到时先设计 feature-specific 文件边界，未达到时避免过度拆分。
4. 保持大数据路径以 iterator/chunk 为主，不引入完整 worksheet cell matrix。
5. 如果新增 `.cpp` 文件，必须同步更新 `CMakeLists.txt` 的 `fastxlsx` 源文件列表。
6. 如果新增测试，接入已有 `FASTXLSX_BUILD_TESTS` 路径。

## 复用入口

- `docs/ARCHITECTURE.md`：分层、数据流、DOM 边界。
- `docs/EDITING_MODEL.md`：Streaming/Patch/In-memory 三种模式、EditPlan 和 sheet 联动边界。
- `docs/TASK_PLAN.md` / `docs/NEXT_STEPS.md`：阶段边界，避免把后期功能提前当成当前目标。
- `docs/TASK_BREAKDOWN.md`：执行级子任务、输入输出、触碰文件、并行边界和验收条件。
...
- CMake 修改后按 `CMakePresets.json` 和 `docs/TESTING_WORKFLOW.md` 的构建指引配置和构建。
