...
- 当前 `ImageOptions` name/description 只是 drawing XML `xdr:cNvPr` metadata，不新增
  依赖，也不改变 `stb` 只负责图片解码、尺寸和像素读取的边界。
...
3. New-workbook 插入阶段必须把 media part、drawing XML、drawing relationships、
   ...
   当前 `ImageOptions` 只在 drawing XML 的 `xdr:cNvPr` 写 name/description metadata；
   它不修改图片文件 metadata、media filename、relationships、content types 或 cell text。
