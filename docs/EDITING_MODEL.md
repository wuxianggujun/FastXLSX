...
- 当前 `replace_worksheet_cells()` 对 source package worksheet entry 已先通过
  ...
  transformer adapter；但当前 PackageEditor worksheet root validation 和 relationship-id
  audit 仍会 materialize input，planned replacement / staged chunk input 也仍会物化
...
P8.16-P8.22 又补上 cell replacement 输出侧 file-backed chunk handoff、source-entry
...
output-pass / dependency-dimension analysis chunk transformer handoff；但 PackageEditor
worksheet root validation 和 relationship-id audit 仍未消费 streaming input，因此不能写成
...
`fastxlsx.package_editor` 验证 output-pass 与 dependency/dimension analysis chunk transformer
handoff note；完整低内存 validation/input parser 接入仍是后续任务。
