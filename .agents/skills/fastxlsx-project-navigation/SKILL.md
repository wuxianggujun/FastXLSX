...
- Streaming：`WorkbookWriter` / `WorksheetWriter` / `CellView` 用于大型有序新建；`WorkbookReader` 用于已有 worksheet row/cell、worksheet-root metadata/data validations/hyperlinks/conditional formatting、strict simple sharedStrings item、narrow rich sharedStrings run、styles/cellXfs 与 narrow font/fill component 的 forward-only bounded callback traversal。
