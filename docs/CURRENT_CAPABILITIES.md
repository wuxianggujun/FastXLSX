...
- `Cell::formula()`、`CellValue::formula()` 和 streaming `CellView::formula()` 都要求
  非空公式文本；`CellValue::error()` 接受非空 opaque Excel error token 并拒绝空
  payload。它们不做公式解析、公式求值、calcChain rebuild 或 Excel error enum
  语义校验。
...
- 当前能力：`try_cell()` / `get_cell()` / `contains_cell()` / `used_range()`、
  ...
  Default Data overwrite recovery saved handles and reopened outputs also carry
  full/range/requested sparse snapshots, duplicate requested coordinates, skipped
  rejected/gap coordinates, and empty gap row/column reads after strict-option
  rematerialization; the same saved-handle snapshot shape also covers later C3
  default Data recovery cells.
...
- Row/column shift helpers only move or delete represented sparse records. Shifted records keep
  ...
  materialized store use the same narrow structural rewrite for affected references. Formula rewrite
  preserves string literal text, structured-reference bracket content, quoted sheet-name token text,
...
- Guardrail：`WorksheetEditorOptions::max_cells` 和 `memory_budget_bytes` 约束 source materialization
  ...
  Clean budget-release, missing-erase, and blank-overwrite recovery handles and reopened
  saved outputs are covered by full/range/requested sparse snapshots, row/column snapshots,
  duplicate requested coordinates, and empty gap row/column reads.
