...
try {
    ...
    $shapeSummaries = @()
    foreach ($shape in @($worksheet.Shapes)) {
        $shapeSummaries += "$($shape.Name) alt='$($shape.AlternativeText)' topLeft='$($shape.TopLeftCell.Address($false, $false))'"
    ...
    Write-Host "OK: Shape count, custom name, default name, and AlternativeText verified"
