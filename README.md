...
- In-memory existing-workbook public API 首片：`WorksheetEditorOptions`、
  ...
  `erase_cells(span<WorksheetCellReference>)`、row/column coordinate guardrails、
...
no-op，不写 tombstone。`erase_cells(span<WorksheetCellReference>)` 是 `erase_cell()` 的
显式坐标批量版本：它预先验证所有坐标，删除已有 active sparse records，duplicate
...
`erase_cell()` / `erase_cells(span)` throws、updates `last_edit_error()`，并且不会 dirty 或 mutate sparse store；
