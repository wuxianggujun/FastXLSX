...
当前 two-/three-color conditional color scale 和 basic data bar 基础：
...
- data bar 写成 worksheet-local `<conditionalFormatting><cfRule type="dataBar">`，
  写两个 `<cfvo>` 和一个 inline ARGB `<color>`；priority 与 color scale 共享同一
  worksheet-local 调用顺序。
- 它不生成 `styles.xml`、`dxfs`、worksheet `.rels`、content type、workbook
  ...
  matrix。
- 这不是 formula/cellIs、advanced data bars、icon sets、dxf-backed styles、existing-file
...
- 对当前 conditional data bar 样例，运行
  ...
  一个 `<color rgb>`、shared priority、multi-range `sqref`、suffix 顺序、无
