...
- **Streaming**：`WorkbookWriter` / `WorksheetWriter` 面向按行创建大型新 workbook；`WorkbookReader` 面向已有 workbook 的 forward-only bounded row/cell、worksheet metadata、data-validation、hyperlink、conditional-formatting、linked-table、sharedStrings 与 styles companion traversal。
