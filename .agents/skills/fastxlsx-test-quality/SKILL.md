...
- 当前 streaming tables 测试覆盖 `xl/tables/tableN.xml`、worksheet `<tableParts>`、
  ...
  true/false/default metadata、caller-supplied `totalsRowFunction`、无公式文本 /
  `totalsRowLabel` 生成、duplicate names 和 mutation-after-close。
...
9. 如果涉及 tables，重点对比 `xl/tables/table*.xml`、worksheet `<tableParts>`、
   ...
   `totalsRowCount`、caller-supplied `totalsRowFunction`，并确认不会意外生成
