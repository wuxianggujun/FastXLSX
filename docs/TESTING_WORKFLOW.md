...
- C8 structural metadata synchronization 当前覆盖 merged cells、root auto-filter、data-validation multi-range `sqref` 与 worksheet-local hyperlink `ref`。Hyperlink T1 focused gate 为 public-state insertion/deletion、独立 structural-hyperlinks、Patch internal hyperlink 与 bounded hyperlink reader targets；覆盖四轴 shift/expand/clip/full removal、clipped single-cell preservation、internal/external target 与 display/tooltip、QName/attribute-order/quote preservation、empty container、source/same-session Patch metadata、shared/exclusive relationship、非-hyperlink `r:id` 复用拒绝、duplicate/overlap/bounds、空 CellStore、combined single rewrite/commit、staging/save failure no-pollution/retry/reopen、formula/style/unknown-part 与 untouched worksheet preservation。已有 merged/filter/validation focused gate 保持；下一 T1 子切片评估 conditional formatting，table/drawing/comment/VML 在接入前保持明确 non-sync 断言，不默认升级 production 全量 CTest。
