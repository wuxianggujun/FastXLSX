...
- **下一优先级**：在 clean revision 上为 Streaming/Patch 增加至少一个更大规模和一台不同机器的 validated bundle，确认 fused CRC 与 schema-v20/v15 多 worksheet wall/CPU/memory 分布；release claim 继续使用 warmed repeated protocol，不以 dirty candidate 替代 bundle。Patch 下一轮补 mixed-inline/formula 的 fixed-shape 12-round profile，并在第二台机器复跑 fixed-shape/fixed-total；逐 entry 数据优先解释 DEFLATE restart/write envelope，当前约 5%–9% wall、约 7%–8% CPU 开销不足以授权并发、direct-zlib 或 public backend 变更。Streaming 只评估有明确 bounded-memory、failure propagation 和 paired evidence 方案的 generation/compression overlap。没有 paired wall/CPU/memory/compatibility evidence 时，不跳过 staged CRC failure gate。
