...
- Calc metadata、sheet rename 与 internal document-properties 联动必须先在 plan/replacements/manifest/public diagnostics 副本完成，再以 noexcept commit 发布；提交前失败保留调用前状态并可重试。
...
- CTest 普通上限 60 秒；legacy public-state 7 个 shard 在拆分 10 万行 translation unit 前专用 120 秒上限。
