...
void test_public_rename_before_full_calculation_allows_later_materialized_edit()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-rename-before-full-calc-materialize-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-rename-before-full-calc-materialize-output.xlsx");

    fastxlsx::WorkbookEditor editor = fastxlsx::WorkbookEditor::open(source);
    editor.rename_sheet("Data", "RenameThenFullCalcData");
    check(!editor.last_edit_error().has_value(),
        "rename-before-full-calc setup should keep diagnostics clear");
    check(editor.pending_change_count() == 1,
        "rename-before-full-calc setup should count one catalog edit");
    check(!editor.has_worksheet("Data"),
        "rename-before-full-calc setup should remove the old planned name");
    check(editor.has_worksheet("RenameThenFullCalcData"),
        "rename-before-full-calc setup should expose the planned name");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-before-full-calc setup should not create dirty materialized names");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "rename-before-full-calc setup should expose one rename summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" &&
                    summary.planned_name == "RenameThenFullCalcData",
                "rename-before-full-calc setup should keep source/planned names");
            check(summary.renamed,
                "rename-before-full-calc setup should mark the summary as renamed");
            check(!summary.sheet_data_replaced,
                "rename-before-full-calc setup should not report replacement state");
            check(!summary.materialized_dirty,
                "rename-before-full-calc setup should not report dirty materialized state");
        }
    }

    editor.request_full_calculation();
    check(!editor.last_edit_error().has_value(),
        "full-calc after rename should keep diagnostics clear");
    check(editor.pending_change_count() == 2,
        "full-calc after rename should count metadata plus catalog edit");
    check(!editor.has_worksheet("Data"),
        "full-calc after rename should preserve removal of the old planned name");
    check(editor.has_worksheet("RenameThenFullCalcData"),
        "full-calc after rename should preserve the planned name");
    check(editor.pending_materialized_worksheet_names().empty(),
        "full-calc after rename should not create dirty materialized names");
    check(editor.pending_materialized_cell_count() == 0,
        "full-calc after rename should not create dirty materialized cells");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "full-calc after rename should not create dirty materialized memory");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "full-calc after rename should keep one worksheet summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" &&
                    summary.planned_name == "RenameThenFullCalcData",
                "full-calc after rename should preserve source/planned names");
            check(summary.renamed,
                "full-calc after rename should preserve the rename flag");
            check(!summary.materialized_dirty,
                "full-calc after rename should keep the summary clean before materialization");
        }
    }

    fastxlsx::WorksheetEditor sheet = editor.worksheet("RenameThenFullCalcData");
    const fastxlsx::CellValue original_a1 = sheet.get_cell(1, 1);
    check(original_a1.kind() == fastxlsx::CellValueKind::Text &&
            original_a1.text_value() == "placeholder-a1",
        "materialization after rename-before-full-calc should read source cells");
    check(!sheet.has_pending_changes(),
        "materialization after rename-before-full-calc should start clean");
    check(editor.pending_change_count() == 2,
        "clean materialization after rename-before-full-calc should not add a handoff");

    sheet.set_cell(1, 1, fastxlsx::CellValue::text("rename-before-full-calc-materialize"));
    check(sheet.has_pending_changes(),
        "later edit after rename-before-full-calc should dirty the materialized sheet");
    check(editor.pending_change_count() == 2,
        "dirty materialized edit after rename-before-full-calc should not flush before save_as");
    check(editor.pending_materialized_worksheet_names() ==
            std::vector<std::string>{"RenameThenFullCalcData"},
        "dirty materialized diagnostics after rename-before-full-calc should use the planned name");
    check(editor.pending_materialized_cell_count() == sheet.cell_count(),
        "dirty materialized diagnostics after rename-before-full-calc should expose cell count");
    check(editor.estimated_pending_materialized_memory_usage() ==
            sheet.estimated_memory_usage(),
        "dirty materialized diagnostics after rename-before-full-calc should expose memory");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "dirty materialized edit after rename-before-full-calc should keep one summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" &&
                    summary.planned_name == "RenameThenFullCalcData",
                "dirty materialized summary after rename-before-full-calc should keep names");
            check(summary.renamed,
                "dirty materialized summary after rename-before-full-calc should keep rename flag");
            check(summary.materialized_dirty,
                "dirty materialized summary after rename-before-full-calc should report dirty state");
            check(summary.materialized_cell_count == sheet.cell_count(),
                "dirty materialized summary after rename-before-full-calc should report cells");
            check(summary.estimated_materialized_memory_usage ==
                    sheet.estimated_memory_usage(),
                "dirty materialized summary after rename-before-full-calc should report memory");
        }
    }

    editor.save_as(output);
    check(!sheet.has_pending_changes(),
        "save_as after rename-before-full-calc materialized edit should clear dirty state");
    check(editor.pending_change_count() == 3,
        "save_as after rename-before-full-calc materialized edit should count rename, metadata, and handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "save_as after rename-before-full-calc materialized edit should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "save_as after rename-before-full-calc materialized edit should clear dirty cells");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "save_as after rename-before-full-calc materialized edit should clear dirty memory");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    check_contains(output_entries.at("xl/workbook.xml"), R"(fullCalcOnLoad="1")",
        "rename-before-full-calc materialized save should persist workbook calc metadata");
    check_contains(output_entries.at("xl/workbook.xml"), R"(name="RenameThenFullCalcData")",
        "rename-before-full-calc materialized save should persist the planned sheet name");
    check_not_contains(output_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-before-full-calc materialized save should not leak the old planned name");
    check(output_entries.find("xl/calcChain.xml") == output_entries.end(),
        "rename-before-full-calc materialized save should not invent calcChain.xml");
    check_contains(output_entries.at("xl/worksheets/sheet1.xml"),
        "rename-before-full-calc-materialize",
        "rename-before-full-calc materialized save should persist the sparse projection");
    check_not_contains(output_entries.at("xl/worksheets/sheet1.xml"), "placeholder-a1",
        "rename-before-full-calc materialized save should replace the old source cell");

    fastxlsx::WorkbookEditor reopened = fastxlsx::WorkbookEditor::open(output);
    check(reopened.has_worksheet("RenameThenFullCalcData"),
        "reopened rename-before-full-calc materialized output should expose the planned name");
    check(!reopened.has_worksheet("Data"),
        "reopened rename-before-full-calc materialized output should not expose the old name");
    fastxlsx::WorksheetEditor reopened_sheet = reopened.worksheet("RenameThenFullCalcData");
    const fastxlsx::CellValue reopened_a1 = reopened_sheet.get_cell(1, 1);
    check(reopened_a1.kind() == fastxlsx::CellValueKind::Text &&
            reopened_a1.text_value() == "rename-before-full-calc-materialize",
        "reopened rename-before-full-calc materialized output should read the saved edit");
    check(!reopened_sheet.has_pending_changes(),
        "reopened rename-before-full-calc materialized output should materialize cleanly");
}

void test_public_rename_before_full_calculation_allows_later_clean_materialization()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-rename-before-full-calc-clean-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-rename-before-full-calc-clean-output.xlsx");
    const auto source_entries = fastxlsx::test::read_zip_entries(source);

    fastxlsx::WorkbookEditor editor = fastxlsx::WorkbookEditor::open(source);
    editor.rename_sheet("Data", "RenameThenFullCalcCleanData");
    editor.request_full_calculation();
    check(!editor.last_edit_error().has_value(),
        "rename-before-full-calc clean setup should keep diagnostics clear");
    check(editor.pending_change_count() == 2,
        "rename-before-full-calc clean setup should count rename plus metadata");
    check(!editor.has_worksheet("Data"),
        "rename-before-full-calc clean setup should remove the old planned name");
    check(editor.has_worksheet("RenameThenFullCalcCleanData"),
        "rename-before-full-calc clean setup should expose the planned name");

    fastxlsx::WorksheetEditor sheet = editor.worksheet("RenameThenFullCalcCleanData");
    const fastxlsx::CellValue original_a1 = sheet.get_cell(1, 1);
    check(original_a1.kind() == fastxlsx::CellValueKind::Text &&
            original_a1.text_value() == "placeholder-a1",
        "clean materialization after rename-before-full-calc should read source cells");
    check(!sheet.has_pending_changes(),
        "clean materialization after rename-before-full-calc should keep the sheet clean");
    check(editor.pending_change_count() == 2,
        "clean materialization after rename-before-full-calc should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "clean materialization after rename-before-full-calc should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "clean materialization after rename-before-full-calc should keep dirty cells empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "clean materialization after rename-before-full-calc should keep dirty memory empty");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "clean materialization after rename-before-full-calc should keep one summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" &&
                    summary.planned_name == "RenameThenFullCalcCleanData",
                "clean materialization after rename-before-full-calc should keep names");
            check(summary.renamed,
                "clean materialization after rename-before-full-calc should keep rename flag");
            check(!summary.materialized_dirty,
                "clean materialization after rename-before-full-calc should keep summary clean");
        }
    }

    editor.save_as(output);
    check(!sheet.has_pending_changes(),
        "save_as after clean rename-before-full-calc materialization should keep the sheet clean");
    check(editor.pending_change_count() == 2,
        "save_as after clean rename-before-full-calc materialization should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "save_as after clean rename-before-full-calc materialization should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "save_as after clean rename-before-full-calc materialization should keep dirty cells empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "save_as after clean rename-before-full-calc materialization should keep dirty memory empty");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    check_contains(output_entries.at("xl/workbook.xml"), R"(fullCalcOnLoad="1")",
        "clean rename-before-full-calc save should persist workbook calc metadata");
    check_contains(output_entries.at("xl/workbook.xml"), R"(name="RenameThenFullCalcCleanData")",
        "clean rename-before-full-calc save should persist the planned sheet name");
    check_not_contains(output_entries.at("xl/workbook.xml"), R"(name="Data")",
        "clean rename-before-full-calc save should not leak the old planned name");
    check(output_entries.find("xl/calcChain.xml") == output_entries.end(),
        "clean rename-before-full-calc save should not invent calcChain.xml");
    check(output_entries.at("xl/worksheets/sheet1.xml") ==
            source_entries.at("xl/worksheets/sheet1.xml"),
        "clean rename-before-full-calc save should preserve source worksheet bytes");

    fastxlsx::WorkbookEditor reopened = fastxlsx::WorkbookEditor::open(output);
    check(reopened.has_worksheet("RenameThenFullCalcCleanData"),
        "reopened clean rename-before-full-calc output should expose the planned name");
    check(!reopened.has_worksheet("Data"),
        "reopened clean rename-before-full-calc output should not expose the old name");
    fastxlsx::WorksheetEditor reopened_sheet = reopened.worksheet("RenameThenFullCalcCleanData");
    const fastxlsx::CellValue reopened_a1 = reopened_sheet.get_cell(1, 1);
    check(reopened_a1.kind() == fastxlsx::CellValueKind::Text &&
            reopened_a1.text_value() == "placeholder-a1",
        "reopened clean rename-before-full-calc output should read the preserved source cell");
    check(!reopened_sheet.has_pending_changes(),
        "reopened clean rename-before-full-calc output should materialize cleanly");
}
...
void test_public_request_full_calculation_preserves_materialized_catalog_guards()
{
    {
        const std::filesystem::path source =
            write_two_sheet_source("fastxlsx-workbook-editor-public-full-calc-dirty-catalog-guard-source.xlsx");
        const std::filesystem::path output =
            artifact("fastxlsx-workbook-editor-public-full-calc-dirty-catalog-guard-output.xlsx");

        fastxlsx::WorkbookEditor editor = fastxlsx::WorkbookEditor::open(source);
        fastxlsx::WorksheetEditor sheet = editor.worksheet("Data");
        sheet.set_cell(1, 1,
            fastxlsx::CellValue::text("full-calc-dirty-catalog-guard"));
        editor.request_full_calculation();
        ...
        check(editor.pending_change_count() == 1,
            "full-calc dirty catalog guard setup should queue only metadata");
        check(editor.pending_materialized_worksheet_names()
                  == std::vector<std::string>{"Data"},
            "full-calc dirty catalog guard setup should keep Data dirty");

        const std::optional<std::string> guard_error =
            check_public_same_sheet_rename_then_replacement_guard_sequence(
                editor,
                "Data",
                "FullCalcBlockedRename",
                "full-calc-blocked-replacement",
                "full-calc dirty materialized catalog guard");

        check(guard_error.has_value(),
            "full-calc dirty materialized catalog guard should report the replacement guard");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before, "full-calc dirty materialized catalog guard");
        check(editor.has_worksheet("Data"),
            "full-calc dirty materialized catalog guard should keep the original planned name");
        check(!editor.has_worksheet("FullCalcBlockedRename"),
            "full-calc dirty materialized catalog guard should not expose the rejected rename");
        check(!editor.has_pending_replacement("Data") &&
                !editor.has_pending_replacement("FullCalcBlockedRename"),
            "full-calc dirty materialized catalog guard should not queue replacement diagnostics");
        check_workbook_editor_no_replacement_diagnostics(
            editor, "full-calc dirty materialized catalog guard");
        check(sheet.has_pending_changes(),
            "full-calc dirty materialized catalog guard should keep the borrowed handle dirty");
        check(sheet.cell_count() == cell_count_before,
            "full-calc dirty materialized catalog guard should preserve sparse cell count");
        check(sheet.estimated_memory_usage() == memory_before,
            "full-calc dirty materialized catalog guard should preserve sparse memory");
        check(editor.pending_materialized_worksheet_names()
                  == std::vector<std::string>{"Data"},
            "full-calc dirty materialized catalog guard should keep Data dirty");
        check(editor.pending_materialized_cell_count() == cell_count_before,
            "full-calc dirty materialized catalog guard should preserve materialized cell count");
        check(editor.estimated_pending_materialized_memory_usage() == memory_before,
            "full-calc dirty materialized catalog guard should preserve materialized memory");
        check(editor.pending_change_count() == 1,
            "full-calc dirty materialized catalog guard should preserve metadata edit count");

        {
            const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
                editor.pending_worksheet_edits();
            check(summaries.size() == 1,
                "full-calc dirty materialized catalog guard should keep one worksheet summary");
            if (summaries.size() == 1) {
                const auto& summary = summaries[0];
                check(summary.source_name == "Data" && summary.planned_name == "Data",
                    "full-calc dirty materialized catalog guard should keep Data summary names");
                check(!summary.renamed && !summary.sheet_data_replaced,
                    "full-calc dirty materialized catalog guard should not mark rejected Patch edits");
                check(summary.materialized_dirty &&
                        summary.materialized_cell_count == cell_count_before &&
                        summary.estimated_materialized_memory_usage == memory_before,
                    "full-calc dirty materialized catalog guard should keep materialized summary fields");
            }
        }
        ...
        editor.save_as(output);
        check(!sheet.has_pending_changes(),
            "safe save after full-calc dirty materialized catalog guard should clean the handle");
        check(editor.pending_change_count() == 2,
            "safe save after full-calc dirty materialized catalog guard should count metadata plus handoff");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "safe save after full-calc dirty materialized catalog guard should clear materialized diagnostics");
        check_workbook_editor_no_replacement_diagnostics(
            editor, "safe save after full-calc dirty materialized catalog guard");

        const auto output_entries = fastxlsx::test::read_zip_entries(output);
        check_contains(output_entries.at("xl/workbook.xml"), R"(fullCalcOnLoad="1")",
            "safe save after full-calc dirty catalog guard should persist calc metadata");
        check_contains(output_entries.at("xl/workbook.xml"), R"(name="Data")",
            "safe save after full-calc dirty catalog guard should preserve Data catalog name");
        check_not_contains(output_entries.at("xl/workbook.xml"), "FullCalcBlockedRename",
            "safe save after full-calc dirty catalog guard should not leak rejected rename");
        check(output_entries.find("xl/calcChain.xml") == output_entries.end(),
            "safe save after full-calc dirty catalog guard should not invent calcChain.xml");
        check_contains(output_entries.at("xl/worksheets/sheet1.xml"),
            "full-calc-dirty-catalog-guard",
            "safe save after full-calc dirty catalog guard should persist materialized cells");
        check_not_contains(output_entries.at("xl/worksheets/sheet1.xml"),
            "full-calc-blocked-replacement",
            "safe save after full-calc dirty catalog guard should not leak rejected replacement");
    }

    {
        const std::filesystem::path source =
            write_two_sheet_source("fastxlsx-workbook-editor-public-full-calc-clean-catalog-guard-source.xlsx");
        const std::filesystem::path output =
            artifact("fastxlsx-workbook-editor-public-full-calc-clean-catalog-guard-output.xlsx");
        const auto source_entries = fastxlsx::test::read_zip_entries(source);
        ...
        check(original.kind() == fastxlsx::CellValueKind::Text &&
                original.text_value() == "placeholder-a1",
            "full-calc clean catalog guard setup should materialize source cells");
        editor.request_full_calculation();
        ...
        const std::optional<std::string> guard_error =
            check_public_same_sheet_rename_then_replacement_guard_sequence(
                editor,
                "Data",
                "FullCalcCleanBlockedRename",
                "full-calc-clean-blocked-replacement",
                "full-calc clean materialized catalog guard");

        check(guard_error.has_value(),
            "full-calc clean materialized catalog guard should report the replacement guard");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before, "full-calc clean materialized catalog guard");
        check(!editor.has_worksheet("FullCalcCleanBlockedRename"),
            "full-calc clean materialized catalog guard should not expose rejected rename");
        check(!editor.has_pending_replacement("Data") &&
                !editor.has_pending_replacement("FullCalcCleanBlockedRename"),
            "full-calc clean materialized catalog guard should not queue replacement diagnostics");
        check(!sheet.has_pending_changes(),
            "full-calc clean materialized catalog guard should keep the borrowed handle clean");
        check(editor.pending_change_count() == 1,
            "full-calc clean materialized catalog guard should preserve metadata edit count");
        check(editor.pending_materialized_worksheet_names().empty(),
            "full-calc clean materialized catalog guard should keep dirty names empty");
        check(editor.pending_materialized_cell_count() == 0,
            "full-calc clean materialized catalog guard should keep dirty cells empty");
        check(editor.estimated_pending_materialized_memory_usage() == 0,
            "full-calc clean materialized catalog guard should keep dirty memory empty");
        check(editor.pending_worksheet_edits().empty(),
            "full-calc clean materialized catalog guard should keep summaries empty");
        check_workbook_editor_no_replacement_diagnostics(
            editor, "full-calc clean materialized catalog guard");

        check_public_inspection_preserves_last_edit_error(editor, guard_error);

        editor.save_as(output);
        check(!sheet.has_pending_changes(),
            "safe save after full-calc clean materialized catalog guard should keep the handle clean");
        check(editor.pending_change_count() == 1,
            "safe save after full-calc clean materialized catalog guard should not add a handoff");
        check(editor.pending_materialized_worksheet_names().empty(),
            "safe save after full-calc clean materialized catalog guard should keep dirty names empty");

        const auto output_entries = fastxlsx::test::read_zip_entries(output);
        check_contains(output_entries.at("xl/workbook.xml"), R"(fullCalcOnLoad="1")",
            "safe save after full-calc clean catalog guard should persist calc metadata");
        check_not_contains(output_entries.at("xl/workbook.xml"), "FullCalcCleanBlockedRename",
            "safe save after full-calc clean catalog guard should not leak rejected rename");
        check(output_entries.find("xl/calcChain.xml") == output_entries.end(),
            "safe save after full-calc clean catalog guard should not invent calcChain.xml");
        check(output_entries.at("xl/worksheets/sheet1.xml") ==
                source_entries.at("xl/worksheets/sheet1.xml"),
            "safe save after full-calc clean catalog guard should preserve source worksheet bytes");
        check_not_contains(output_entries.at("xl/worksheets/sheet1.xml"),
            "full-calc-clean-blocked-replacement",
            "safe save after full-calc clean catalog guard should not leak rejected replacement");
    }
}
...
void test_public_worksheet_editor_rename_back_materialized_missing_clear_noop_save()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-noop.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-saved"));
    editor.save_as(output);
    ...
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-rejected"));
    }), "rename-back missing clear setup should reject row-zero mutation");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear setup should record invalid mutation diagnostics");

    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");

    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear no-op should not add another materialized handoff");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear no-op should preserve sparse count and memory");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear no-op should keep missing targets absent");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear no-op should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear no-op should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear no-op should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear no-op should keep current summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "rename-back missing clear no-op");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before, "rename-back missing clear no-op");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-saved",
        "rename-back missing clear no-op");

    editor.save_as(clear_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear no-op save should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear no-op save should not add another materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear no-op save should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear no-op save should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear no-op save should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear no-op save should keep current summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before, "rename-back missing clear no-op save");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    check_contains(output_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear output should use the restored source name");
    check_not_contains(output_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear output should not leak the transient planned name");
    check_contains(output_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-saved",
        "rename-back missing clear output should contain the saved materialized edit");
    check_not_contains(output_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-rejected",
        "rename-back missing clear output should not leak rejected payloads");
    check_not_contains(output_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear output should not synthesize row/column missing target");
    check_not_contains(output_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear output should not synthesize A1 missing target");

    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear no-op output should match the first restored-name materialized output");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_recovery_save()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-noop.xlsx");
    const std::filesystem::path recovery_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-save.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-recovery-saved"));
    editor.save_as(output);
    ...
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-recovery-rejected"));
    }), "rename-back missing clear recovery setup should reject row-zero mutation");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear recovery setup should record invalid mutation diagnostics");

    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");

    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear recovery no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear recovery no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear recovery no-op should not add another materialized handoff");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear recovery no-op should preserve sparse count and memory");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear recovery no-op should keep missing targets absent");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear recovery no-op should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear recovery no-op should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear recovery no-op should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear recovery no-op should keep current summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "rename-back missing clear recovery no-op");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before, "rename-back missing clear recovery no-op");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-recovery-saved",
        "rename-back missing clear recovery no-op");

    editor.save_as(clear_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear recovery no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear recovery no-op save should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear recovery no-op save should not add another materialized handoff");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear recovery no-op output should match the first restored-name materialized output");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-recovered"));
    check(!editor.last_edit_error().has_value(),
        "rename-back mutation after missing clear no-op should keep diagnostics clear");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back mutation after missing clear no-op should dirty both handles");
    check(editor.pending_change_count() == 3,
        "rename-back mutation after missing clear no-op should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back mutation after missing clear no-op should dirty the restored source name");
    }
    check(editor.pending_materialized_cell_count() == reacquired.cell_count(),
        "rename-back mutation after missing clear no-op should expose current dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() ==
            reacquired.estimated_memory_usage(),
        "rename-back mutation after missing clear no-op should expose current dirty memory");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "rename-back mutation after missing clear no-op should expose one summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" && summary.planned_name == "Data",
                "rename-back mutation after missing clear no-op summary should use restored names");
            check(!summary.renamed,
                "rename-back mutation after missing clear no-op summary should not be marked renamed");
            check(!summary.sheet_data_replaced,
                "rename-back mutation after missing clear no-op summary should not invent replacement");
            check(summary.materialized_dirty,
                "rename-back mutation after missing clear no-op summary should report dirty materialized state");
            check(summary.materialized_cell_count == reacquired.cell_count(),
                "rename-back mutation after missing clear no-op summary should report current cell count");
            check(summary.estimated_materialized_memory_usage ==
                    reacquired.estimated_memory_usage(),
                "rename-back mutation after missing clear no-op summary should report current memory");
        }
    }
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back mutation after missing clear no-op should keep missing targets absent");

    editor.save_as(recovery_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear recovery save should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear recovery save should count one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear recovery save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear recovery save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear recovery save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear recovery save should clear current summaries");

    const auto recovery_entries =
        fastxlsx::test::read_zip_entries(recovery_output);
    check_contains(recovery_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear recovery output should use the restored source name");
    check_not_contains(recovery_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear recovery output should not leak the transient planned name");
    check_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-recovery-saved",
        "rename-back missing clear recovery output should keep the saved materialized edit");
    check_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-recovered",
        "rename-back missing clear recovery output should include the post-no-op mutation");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-recovery-rejected",
        "rename-back missing clear recovery output should not leak rejected payloads");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear recovery output should not synthesize row/column missing target");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear recovery output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_recovery_failed_save()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-failed-save-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-failed-save-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-failed-save-noop.xlsx");
    const std::filesystem::path recovery_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-recovery-failed-save-retry.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-failed-save-saved"));
    editor.save_as(output);
    ...
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-failed-save-rejected"));
    }), "rename-back missing clear recovery failed-save setup should reject row-zero mutation");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear recovery failed-save setup should record invalid mutation diagnostics");

    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");

    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear recovery failed-save no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear recovery failed-save no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear recovery failed-save no-op should not add another materialized handoff");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear recovery failed-save no-op should preserve sparse count and memory");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear recovery failed-save no-op should keep missing targets absent");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear recovery failed-save no-op should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear recovery failed-save no-op should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear recovery failed-save no-op should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear recovery failed-save no-op should keep current summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "rename-back missing clear recovery failed-save no-op");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before, "rename-back missing clear recovery failed-save no-op");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-failed-save-saved",
        "rename-back missing clear recovery failed-save no-op");

    editor.save_as(clear_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear recovery failed-save no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear recovery failed-save no-op save should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear recovery failed-save no-op save should not add another materialized handoff");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear recovery failed-save no-op output should match the first restored-name materialized output");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-failed-save-recovered"));
    check(!editor.last_edit_error().has_value(),
        "rename-back mutation after missing clear failed-save no-op should keep diagnostics clear");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back mutation after missing clear failed-save no-op should dirty both handles");
    check(editor.pending_change_count() == 3,
        "rename-back mutation after missing clear failed-save no-op should not count a handoff before save");

    const std::size_t dirty_cell_count = reacquired.cell_count();
    const std::size_t dirty_memory = reacquired.estimated_memory_usage();
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back mutation after missing clear failed-save no-op should dirty the restored source name");
    }
    check(editor.pending_materialized_cell_count() == dirty_cell_count,
        "rename-back mutation after missing clear failed-save no-op should expose current dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == dirty_memory,
        "rename-back mutation after missing clear failed-save no-op should expose current dirty memory");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "rename-back mutation after missing clear failed-save no-op should expose one summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" && summary.planned_name == "Data",
                "rename-back mutation after missing clear failed-save no-op summary should use restored names");
            check(!summary.renamed,
                "rename-back mutation after missing clear failed-save no-op summary should not be marked renamed");
            check(!summary.sheet_data_replaced,
                "rename-back mutation after missing clear failed-save no-op summary should not invent replacement");
            check(summary.materialized_dirty,
                "rename-back mutation after missing clear failed-save no-op summary should report dirty materialized state");
            check(summary.materialized_cell_count == dirty_cell_count,
                "rename-back mutation after missing clear failed-save no-op summary should report current cell count");
            check(summary.estimated_materialized_memory_usage == dirty_memory,
                "rename-back mutation after missing clear failed-save no-op summary should report current memory");
        }
    }
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back mutation after missing clear failed-save no-op should keep missing targets absent");

    const auto source_entries_before_rejected_save =
        fastxlsx::test::read_zip_entries(source);
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear recovery failed save_as should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear recovery failed save_as should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear recovery failed save_as should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear recovery failed save_as should not count a materialized handoff");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back missing clear recovery failed save_as should preserve dirty materialized name");
    }
    check(editor.pending_materialized_cell_count() == dirty_cell_count,
        "rename-back missing clear recovery failed save_as should preserve dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == dirty_memory,
        "rename-back missing clear recovery failed save_as should preserve dirty memory");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "rename-back missing clear recovery failed save_as should preserve one summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" && summary.planned_name == "Data",
                "rename-back missing clear recovery failed save_as summary should use restored names");
            check(!summary.renamed,
                "rename-back missing clear recovery failed save_as summary should not be marked renamed");
            check(!summary.sheet_data_replaced,
                "rename-back missing clear recovery failed save_as summary should not invent replacement");
            check(summary.materialized_dirty,
                "rename-back missing clear recovery failed save_as summary should preserve dirty flag");
            check(summary.materialized_cell_count == dirty_cell_count,
                "rename-back missing clear recovery failed save_as summary should preserve cell count");
            check(summary.estimated_materialized_memory_usage == dirty_memory,
                "rename-back missing clear recovery failed save_as summary should preserve memory");
        }
    }
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear recovery failed save_as should keep missing targets absent");
    check(fastxlsx::test::read_zip_entries(source) == source_entries_before_rejected_save,
        "rename-back missing clear recovery failed save_as should leave source package unchanged");

    editor.save_as(recovery_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear recovery failed-save retry should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear recovery failed-save retry should count one more materialized handoff");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear recovery failed-save retry should keep diagnostics clear");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear recovery failed-save retry should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear recovery failed-save retry should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear recovery failed-save retry should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear recovery failed-save retry should clear current summaries");

    const auto recovery_entries =
        fastxlsx::test::read_zip_entries(recovery_output);
    check_contains(recovery_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear recovery failed-save output should use the restored source name");
    check_not_contains(recovery_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear recovery failed-save output should not leak the transient planned name");
    check_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-failed-save-saved",
        "rename-back missing clear recovery failed-save output should keep the saved materialized edit");
    check_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-failed-save-recovered",
        "rename-back missing clear recovery failed-save output should include the post-no-op mutation");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-failed-save-rejected",
        "rename-back missing clear recovery failed-save output should not leak rejected payloads");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear recovery failed-save output should not synthesize row/column missing target");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear recovery failed-save output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_reacquire()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-reacquire-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-reacquire-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-reacquire-noop.xlsx");
    const std::filesystem::path recovery_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-reacquire-retry.xlsx");
    const std::filesystem::path followup_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-reacquire-followup.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-reacquire-saved"));
    editor.save_as(output);
    ...
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-reacquire-rejected"));
    }), "rename-back missing clear failed-save reacquire setup should reject row-zero mutation");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear failed-save reacquire setup should record invalid mutation diagnostics");

    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");

    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear failed-save reacquire no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear failed-save reacquire no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear failed-save reacquire no-op should not add another materialized handoff");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear failed-save reacquire no-op should preserve sparse count and memory");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear failed-save reacquire no-op should keep missing targets absent");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear failed-save reacquire no-op should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear failed-save reacquire no-op should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear failed-save reacquire no-op should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear failed-save reacquire no-op should keep current summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "rename-back missing clear failed-save reacquire no-op");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before, "rename-back missing clear failed-save reacquire no-op");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-reacquire-saved",
        "rename-back missing clear failed-save reacquire no-op");

    editor.save_as(clear_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear failed-save reacquire no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear failed-save reacquire no-op save should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear failed-save reacquire no-op save should not add another materialized handoff");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear failed-save reacquire no-op output should match the first restored-name materialized output");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-reacquire-recovered"));
    check(!editor.last_edit_error().has_value(),
        "rename-back mutation before failed-save reacquire should keep diagnostics clear");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back mutation before failed-save reacquire should dirty both handles");
    check(editor.pending_change_count() == 3,
        "rename-back mutation before failed-save reacquire should not count a handoff before save");

    const std::size_t dirty_cell_count = reacquired.cell_count();
    const std::size_t dirty_memory = reacquired.estimated_memory_usage();
    check(editor.pending_materialized_cell_count() == dirty_cell_count,
        "rename-back mutation before failed-save reacquire should expose current dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == dirty_memory,
        "rename-back mutation before failed-save reacquire should expose current dirty memory");
    {
        const std::vector<fastxlsx::WorkbookEditorWorksheetEditSummary> summaries =
            editor.pending_worksheet_edits();
        check(summaries.size() == 1,
            "rename-back mutation before failed-save reacquire should expose one summary");
        if (summaries.size() == 1) {
            const auto& summary = summaries[0];
            check(summary.source_name == "Data" && summary.planned_name == "Data",
                "rename-back mutation before failed-save reacquire summary should use restored names");
            check(!summary.renamed,
                "rename-back mutation before failed-save reacquire summary should not be marked renamed");
            check(!summary.sheet_data_replaced,
                "rename-back mutation before failed-save reacquire summary should not invent replacement");
            check(summary.materialized_dirty,
                "rename-back mutation before failed-save reacquire summary should report dirty materialized state");
            check(summary.materialized_cell_count == dirty_cell_count,
                "rename-back mutation before failed-save reacquire summary should report current cell count");
            check(summary.estimated_materialized_memory_usage == dirty_memory,
                "rename-back mutation before failed-save reacquire summary should report current memory");
        }
    }
    ...
    const auto source_entries_before_rejected_save =
        fastxlsx::test::read_zip_entries(source);
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear failed-save reacquire should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear failed-save reacquire should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear failed-save reacquire should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear failed-save reacquire should not count a materialized handoff");
    check(editor.pending_materialized_cell_count() == dirty_cell_count,
        "rename-back missing clear failed-save reacquire should preserve dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == dirty_memory,
        "rename-back missing clear failed-save reacquire should preserve dirty memory");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear failed-save reacquire should keep missing targets absent");
    check(fastxlsx::test::read_zip_entries(source) == source_entries_before_rejected_save,
        "rename-back missing clear failed-save reacquire should leave source package unchanged");

    editor.save_as(recovery_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear failed-save reacquire first retry should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear failed-save reacquire first retry should count one more materialized handoff");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear failed-save reacquire first retry should keep diagnostics clear");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear failed-save reacquire first retry should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear failed-save reacquire first retry should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear failed-save reacquire first retry should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear failed-save reacquire first retry should clear current summaries");

    fastxlsx::WorksheetEditor post_save = editor.worksheet("Data", options);
    check_public_two_clean_retry_saved_value(
        post_save, 1, 1, "rename-back-missing-clear-reacquire-saved",
        "rename-back missing clear failed-save post-save reacquire");
    check_public_two_clean_retry_saved_value(
        post_save, 2, 2, "rename-back-missing-clear-reacquire-recovered",
        "rename-back missing clear failed-save post-save reacquire");
    check(!post_save.has_pending_changes(),
        "rename-back missing clear failed-save post-save reacquire should stay clean");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear failed-save post-save reacquire should keep prior handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear failed-save post-save reacquire should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear failed-save post-save reacquire should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear failed-save post-save reacquire should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear failed-save post-save reacquire should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear failed-save post-save reacquire should keep summaries empty");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear failed-save post-save reacquire should keep diagnostics clear");
    check(!post_save.try_cell(5, 5).has_value() &&
            !post_save.try_cell("D4").has_value(),
        "rename-back missing clear failed-save post-save reacquire should keep missing targets absent");

    post_save.set_cell(3, 3,
        fastxlsx::CellValue::text("rename-back-missing-clear-reacquire-followup"));
    check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
            post_save.has_pending_changes(),
        "rename-back missing clear failed-save post-save mutation should dirty all handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear failed-save post-save mutation should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back missing clear failed-save post-save mutation should dirty the restored source name");
    }
    check(editor.pending_materialized_cell_count() == post_save.cell_count(),
        "rename-back missing clear failed-save post-save mutation should expose current dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() ==
            post_save.estimated_memory_usage(),
        "rename-back missing clear failed-save post-save mutation should expose current dirty memory");

    editor.save_as(followup_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !post_save.has_pending_changes(),
        "rename-back missing clear failed-save post-save follow-up save should clean all handles");
    check(editor.pending_change_count() == 5,
        "rename-back missing clear failed-save post-save follow-up save should count one more handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear failed-save post-save follow-up save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear failed-save post-save follow-up save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear failed-save post-save follow-up save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear failed-save post-save follow-up save should clear summaries");

    const auto recovery_entries =
        fastxlsx::test::read_zip_entries(recovery_output);
    check_contains(recovery_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear failed-save post-save first output should use restored source name");
    check_not_contains(recovery_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear failed-save post-save first output should not leak transient name");
    check_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-saved",
        "rename-back missing clear failed-save post-save first output should keep saved value");
    check_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-recovered",
        "rename-back missing clear failed-save post-save first output should include recovered value");
    check_not_contains(recovery_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-followup",
        "rename-back missing clear failed-save post-save first output should not include follow-up value");

    const auto followup_entries =
        fastxlsx::test::read_zip_entries(followup_output);
    check_contains(followup_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear failed-save post-save follow-up output should use restored source name");
    check_not_contains(followup_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear failed-save post-save follow-up output should not leak transient name");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-saved",
        "rename-back missing clear failed-save post-save follow-up output should keep saved value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-recovered",
        "rename-back missing clear failed-save post-save follow-up output should keep recovered value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-followup",
        "rename-back missing clear failed-save post-save follow-up output should include follow-up value");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-reacquire-rejected",
        "rename-back missing clear failed-save post-save follow-up output should not leak rejected payloads");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear failed-save post-save follow-up output should not synthesize row/column missing target");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear failed-save post-save follow-up output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_option_mismatch()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-option-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-option-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-option-noop.xlsx");
    const std::filesystem::path retry_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-option-retry.xlsx");
    const std::filesystem::path followup_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-option-followup.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-option-saved"));
    editor.save_as(output);

    fastxlsx::WorksheetEditor reacquired = editor.worksheet("Data", options);
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-option-rejected"));
    }), "rename-back missing clear option setup should reject row-zero mutation");
    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear option no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear option no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear option no-op should not add another handoff");

    editor.save_as(clear_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear option no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear option no-op save should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear option no-op save should not add another handoff");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-option-recovered"));
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear option recovery mutation should dirty both handles");
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear option failed save should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear option failed save should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear option failed save should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear option failed save should not count a materialized handoff");

    editor.save_as(retry_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear option retry save should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear option retry save should count one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear option retry save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear option retry save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear option retry save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear option retry save should clear summaries");

    const WorkbookEditorPublicCatalogSnapshot catalog_after_retry =
        workbook_editor_public_catalog_snapshot(editor);
    check(threw_fastxlsx_error([&] {
        (void)editor.try_worksheet("Data", mismatched_options);
    }), "rename-back missing clear option mismatch try_worksheet should reject mismatched options");
    check(threw_fastxlsx_error([&] {
        (void)editor.worksheet("Data", mismatched_options);
    }), "rename-back missing clear option mismatch worksheet should reject mismatched options");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear option mismatch should not update last_edit_error");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear option mismatch should keep existing handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear option mismatch should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear option mismatch should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear option mismatch should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear option mismatch should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear option mismatch should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear option mismatch");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-option-saved",
        "rename-back missing clear option mismatch");
    check_public_two_clean_retry_saved_value(
        reacquired, 2, 2, "rename-back-missing-clear-option-recovered",
        "rename-back missing clear option mismatch");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear option mismatch should keep missing targets absent");

    fastxlsx::WorksheetEditor matching = editor.worksheet("Data", options);
    check_public_two_clean_retry_saved_value(
        matching, 1, 1, "rename-back-missing-clear-option-saved",
        "rename-back missing clear option matching reacquire");
    check_public_two_clean_retry_saved_value(
        matching, 2, 2, "rename-back-missing-clear-option-recovered",
        "rename-back missing clear option matching reacquire");
    check(!matching.has_pending_changes(),
        "rename-back missing clear option matching reacquire should stay clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear option matching reacquire should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear option matching reacquire should keep dirty names empty");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear option matching reacquire should keep diagnostics clear");

    matching.set_cell(3, 3,
        fastxlsx::CellValue::text("rename-back-missing-clear-option-followup"));
    check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
            matching.has_pending_changes(),
        "rename-back missing clear option follow-up mutation should dirty all handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear option follow-up mutation should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back missing clear option follow-up mutation should dirty the restored source name");
    }
    check(editor.pending_materialized_cell_count() == matching.cell_count(),
        "rename-back missing clear option follow-up mutation should expose current dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() ==
            matching.estimated_memory_usage(),
        "rename-back missing clear option follow-up mutation should expose current dirty memory");

    editor.save_as(followup_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !matching.has_pending_changes(),
        "rename-back missing clear option follow-up save should clean all handles");
    check(editor.pending_change_count() == 5,
        "rename-back missing clear option follow-up save should count one more handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear option follow-up save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear option follow-up save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear option follow-up save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear option follow-up save should clear summaries");

    const auto retry_entries = fastxlsx::test::read_zip_entries(retry_output);
    check_contains(retry_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear option retry output should use restored source name");
    check_not_contains(retry_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear option retry output should not leak transient name");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-saved",
        "rename-back missing clear option retry output should keep saved value");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-recovered",
        "rename-back missing clear option retry output should include recovered value");
    check_not_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-followup",
        "rename-back missing clear option retry output should not include follow-up value");

    const auto followup_entries =
        fastxlsx::test::read_zip_entries(followup_output);
    check_contains(followup_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear option follow-up output should use restored source name");
    check_not_contains(followup_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear option follow-up output should not leak transient name");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-saved",
        "rename-back missing clear option follow-up output should keep saved value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-recovered",
        "rename-back missing clear option follow-up output should keep recovered value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-followup",
        "rename-back missing clear option follow-up output should include follow-up value");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-option-rejected",
        "rename-back missing clear option follow-up output should not leak rejected payloads");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear option follow-up output should not synthesize row/column missing target");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear option follow-up output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_missing_lookup()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-lookup-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-lookup-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-lookup-noop.xlsx");
    const std::filesystem::path retry_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-lookup-retry.xlsx");
    const std::filesystem::path followup_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-failed-save-lookup-followup.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-lookup-saved"));
    editor.save_as(output);

    fastxlsx::WorksheetEditor reacquired = editor.worksheet("Data", options);
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-lookup-rejected"));
    }), "rename-back missing clear lookup setup should reject row-zero mutation");
    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear lookup no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear lookup no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear lookup no-op should not add another handoff");

    editor.save_as(clear_no_op_output);
    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-lookup-recovered"));
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear lookup failed save should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear lookup failed save should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear lookup failed save should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear lookup failed save should not count a materialized handoff");

    editor.save_as(retry_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear lookup retry save should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear lookup retry save should count one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear lookup retry save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear lookup retry save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear lookup retry save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear lookup retry save should clear summaries");
    ...
    const std::optional<fastxlsx::WorksheetEditor> missing_try =
        editor.try_worksheet("Missing", options);
    check(!missing_try.has_value(),
        "rename-back missing clear retry lookup should return empty for missing try_worksheet");
    check(threw_fastxlsx_error([&] {
        (void)editor.worksheet("Missing", options);
    }), "rename-back missing clear retry lookup should throw for missing worksheet");
    const std::optional<fastxlsx::WorksheetEditor> transient_try =
        editor.try_worksheet("TransientData", options);
    check(!transient_try.has_value(),
        "rename-back missing clear retry lookup should return empty for transient try_worksheet");
    check(threw_fastxlsx_error([&] {
        (void)editor.worksheet("TransientData", options);
    }), "rename-back missing clear retry lookup should throw for transient worksheet");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear retry lookup failures should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear retry lookup failures should keep existing handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry lookup failures should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry lookup failures should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry lookup failures should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry lookup failures should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry lookup failures should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear retry lookup failures");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-lookup-saved",
        "rename-back missing clear retry lookup failures");
    check_public_two_clean_retry_saved_value(
        reacquired, 2, 2, "rename-back-missing-clear-lookup-recovered",
        "rename-back missing clear retry lookup failures");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear retry lookup failures should keep missing targets absent");

    fastxlsx::WorksheetEditor matching = editor.worksheet("Data", options);
    check_public_two_clean_retry_saved_value(
        matching, 1, 1, "rename-back-missing-clear-lookup-saved",
        "rename-back missing clear retry lookup matching reacquire");
    check_public_two_clean_retry_saved_value(
        matching, 2, 2, "rename-back-missing-clear-lookup-recovered",
        "rename-back missing clear retry lookup matching reacquire");
    check(!matching.has_pending_changes(),
        "rename-back missing clear retry lookup matching reacquire should stay clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry lookup matching reacquire should not add a handoff");

    matching.set_cell(3, 3,
        fastxlsx::CellValue::text("rename-back-missing-clear-lookup-followup"));
    check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
            matching.has_pending_changes(),
        "rename-back missing clear retry lookup follow-up mutation should dirty all handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry lookup follow-up mutation should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back missing clear retry lookup follow-up mutation should dirty the restored source name");
    }

    editor.save_as(followup_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !matching.has_pending_changes(),
        "rename-back missing clear retry lookup follow-up save should clean all handles");
    check(editor.pending_change_count() == 5,
        "rename-back missing clear retry lookup follow-up save should count one more handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry lookup follow-up save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry lookup follow-up save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry lookup follow-up save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry lookup follow-up save should clear summaries");

    const auto retry_entries = fastxlsx::test::read_zip_entries(retry_output);
    check_contains(retry_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear retry lookup retry output should use restored source name");
    check_not_contains(retry_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear retry lookup retry output should not leak transient name");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-saved",
        "rename-back missing clear retry lookup retry output should keep saved value");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-recovered",
        "rename-back missing clear retry lookup retry output should include recovered value");
    check_not_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-followup",
        "rename-back missing clear retry lookup retry output should not include follow-up value");

    const auto followup_entries =
        fastxlsx::test::read_zip_entries(followup_output);
    check_contains(followup_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear retry lookup follow-up output should use restored source name");
    check_not_contains(followup_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear retry lookup follow-up output should not leak transient name");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-saved",
        "rename-back missing clear retry lookup follow-up output should keep saved value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-recovered",
        "rename-back missing clear retry lookup follow-up output should keep recovered value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-followup",
        "rename-back missing clear retry lookup follow-up output should include follow-up value");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-lookup-rejected",
        "rename-back missing clear retry lookup follow-up output should not leak rejected payloads");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear retry lookup follow-up output should not synthesize row/column missing target");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear retry lookup follow-up output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_invalid_reads()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-read-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-read-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-read-noop.xlsx");
    const std::filesystem::path retry_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-read-retry.xlsx");
    const std::filesystem::path followup_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-read-followup.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-invalid-read-saved"));
    editor.save_as(output);

    fastxlsx::WorksheetEditor reacquired = editor.worksheet("Data", options);
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-invalid-read-rejected"));
    }), "rename-back missing clear invalid-read setup should reject row-zero mutation");
    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear invalid-read no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear invalid-read no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear invalid-read no-op should not add another handoff");
    ...
    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear invalid-read no-op output should match first restored-name output");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-invalid-read-recovered"));
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear invalid-read failed save should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear invalid-read failed save should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear invalid-read failed save should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear invalid-read failed save should not count a materialized handoff");

    editor.save_as(retry_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear invalid-read retry save should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear invalid-read retry save should count one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear invalid-read retry save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear invalid-read retry save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear invalid-read retry save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear invalid-read retry save should clear summaries");
    ...
    const std::size_t saved_memory = reacquired.estimated_memory_usage();
    check(threw_fastxlsx_error([&] { (void)reacquired.try_cell(0, 1); }),
        "rename-back missing clear retry invalid read should reject row zero");
    check(threw_fastxlsx_error([&] { (void)reacquired.get_cell(1, 0); }),
        "rename-back missing clear retry invalid read should reject column zero");
    check(threw_fastxlsx_error([&] { (void)reacquired.try_cell("a1"); }),
        "rename-back missing clear retry invalid read should reject lowercase A1 references");
    check(threw_fastxlsx_error([&] { (void)reacquired.get_cell("XFE1"); }),
        "rename-back missing clear retry invalid read should reject A1 column overflow");
    check(threw_fastxlsx_error([&] {
        (void)reacquired.sparse_cells(fastxlsx::CellRange {0, 1, 1, 1});
    }), "rename-back missing clear retry invalid read should reject row-zero ranges");
    check(threw_fastxlsx_error([&] {
        (void)reacquired.sparse_cells(fastxlsx::CellRange {2, 1, 1, 1});
    }), "rename-back missing clear retry invalid read should reject reversed ranges");
    check(threw_fastxlsx_error([&] { (void)reacquired.row_cells(0); }),
        "rename-back missing clear retry invalid read should reject row snapshot zero");
    check(threw_fastxlsx_error([&] { (void)reacquired.column_cells(0); }),
        "rename-back missing clear retry invalid read should reject column snapshot zero");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear retry invalid reads should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear retry invalid reads should keep existing handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid reads should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry invalid reads should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry invalid reads should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry invalid reads should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry invalid reads should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear retry invalid reads");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear retry invalid reads should preserve sparse diagnostics");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-invalid-read-saved",
        "rename-back missing clear retry invalid reads");
    check_public_two_clean_retry_saved_value(
        reacquired, 2, 2, "rename-back-missing-clear-invalid-read-recovered",
        "rename-back missing clear retry invalid reads");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear retry invalid reads should keep missing targets absent");

    fastxlsx::WorksheetEditor matching = editor.worksheet("Data", options);
    check_public_two_clean_retry_saved_value(
        matching, 1, 1, "rename-back-missing-clear-invalid-read-saved",
        "rename-back missing clear retry invalid-read matching reacquire");
    check_public_two_clean_retry_saved_value(
        matching, 2, 2, "rename-back-missing-clear-invalid-read-recovered",
        "rename-back missing clear retry invalid-read matching reacquire");
    check(!matching.has_pending_changes(),
        "rename-back missing clear retry invalid-read matching reacquire should stay clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid-read matching reacquire should not add a handoff");

    matching.set_cell(3, 3,
        fastxlsx::CellValue::text("rename-back-missing-clear-invalid-read-followup"));
    check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
            matching.has_pending_changes(),
        "rename-back missing clear retry invalid-read follow-up mutation should dirty all handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid-read follow-up mutation should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back missing clear retry invalid-read follow-up mutation should dirty the restored source name");
    }

    editor.save_as(followup_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !matching.has_pending_changes(),
        "rename-back missing clear retry invalid-read follow-up save should clean all handles");
    check(editor.pending_change_count() == 5,
        "rename-back missing clear retry invalid-read follow-up save should count one more handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry invalid-read follow-up save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry invalid-read follow-up save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry invalid-read follow-up save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry invalid-read follow-up save should clear summaries");

    const auto retry_entries = fastxlsx::test::read_zip_entries(retry_output);
    check_contains(retry_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear retry invalid-read retry output should use restored source name");
    check_not_contains(retry_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear retry invalid-read retry output should not leak transient name");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-saved",
        "rename-back missing clear retry invalid-read retry output should keep saved value");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-recovered",
        "rename-back missing clear retry invalid-read retry output should include recovered value");
    check_not_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-followup",
        "rename-back missing clear retry invalid-read retry output should not include follow-up value");

    const auto followup_entries =
        fastxlsx::test::read_zip_entries(followup_output);
    check_contains(followup_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear retry invalid-read follow-up output should use restored source name");
    check_not_contains(followup_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear retry invalid-read follow-up output should not leak transient name");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-saved",
        "rename-back missing clear retry invalid-read follow-up output should keep saved value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-recovered",
        "rename-back missing clear retry invalid-read follow-up output should keep recovered value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-followup",
        "rename-back missing clear retry invalid-read follow-up output should include follow-up value");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-read-rejected",
        "rename-back missing clear retry invalid-read follow-up output should not leak rejected payloads");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear retry invalid-read follow-up output should not synthesize row/column missing target");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear retry invalid-read follow-up output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_invalid_mutations()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-mutation-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-mutation-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-mutation-noop.xlsx");
    const std::filesystem::path retry_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-mutation-retry.xlsx");
    const std::filesystem::path invalid_mutation_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-mutation-retry-noop.xlsx");
    const std::filesystem::path followup_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-invalid-mutation-followup.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-invalid-mutation-saved"));
    editor.save_as(output);

    fastxlsx::WorksheetEditor reacquired = editor.worksheet("Data", options);
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-invalid-mutation-setup-rejected"));
    }), "rename-back missing clear invalid-mutation setup should reject row-zero mutation");
    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear invalid-mutation no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear invalid-mutation no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear invalid-mutation no-op should not add another handoff");
    ...
    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear invalid-mutation no-op output should match first restored-name output");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-invalid-mutation-recovered"));
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear invalid-mutation failed save should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear invalid-mutation failed save should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear invalid-mutation failed save should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear invalid-mutation failed save should not count a materialized handoff");

    editor.save_as(retry_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear invalid-mutation retry save should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear invalid-mutation retry save should count one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear invalid-mutation retry save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear invalid-mutation retry save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear invalid-mutation retry save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear invalid-mutation retry save should clear summaries");
    ...
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-invalid-mutation-rejected-row"));
    }), "rename-back missing clear retry invalid mutation should reject row-zero set");
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell("XFE1",
            fastxlsx::CellValue::text("rename-back-missing-clear-invalid-mutation-rejected-a1"));
    }), "rename-back missing clear retry invalid mutation should reject A1 column overflow");
    check(threw_fastxlsx_error([&] { reacquired.erase_cell(1, 0); }),
        "rename-back missing clear retry invalid mutation should reject column-zero erase");
    check(threw_fastxlsx_error([&] { reacquired.clear_cell_value(1048577, 1); }),
        "rename-back missing clear retry invalid mutation should reject row-overflow clear");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear retry invalid mutations should record diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear retry invalid mutations should keep existing handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid mutations should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry invalid mutations should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry invalid mutations should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry invalid mutations should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry invalid mutations should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear retry invalid mutations");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear retry invalid mutations should preserve sparse diagnostics");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-invalid-mutation-saved",
        "rename-back missing clear retry invalid mutations");
    check_public_two_clean_retry_saved_value(
        reacquired, 2, 2, "rename-back-missing-clear-invalid-mutation-recovered",
        "rename-back missing clear retry invalid mutations");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear retry invalid mutations should keep missing targets absent");

    editor.save_as(invalid_mutation_no_op_output);
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear retry invalid-mutation no-op save should preserve diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear retry invalid-mutation no-op save should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid-mutation no-op save should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry invalid-mutation no-op save should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry invalid-mutation no-op save should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry invalid-mutation no-op save should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry invalid-mutation no-op save should keep summaries empty");
    const auto invalid_mutation_no_op_entries =
        fastxlsx::test::read_zip_entries(invalid_mutation_no_op_output);
    check(invalid_mutation_no_op_entries == retry_entries,
        "rename-back missing clear retry invalid-mutation no-op output should match retry output");

    fastxlsx::WorksheetEditor matching = editor.worksheet("Data", options);
    check_public_two_clean_retry_saved_value(
        matching, 1, 1, "rename-back-missing-clear-invalid-mutation-saved",
        "rename-back missing clear retry invalid-mutation matching reacquire");
    check_public_two_clean_retry_saved_value(
        matching, 2, 2, "rename-back-missing-clear-invalid-mutation-recovered",
        "rename-back missing clear retry invalid-mutation matching reacquire");
    check(!matching.has_pending_changes(),
        "rename-back missing clear retry invalid-mutation matching reacquire should stay clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid-mutation matching reacquire should not add a handoff");

    matching.set_cell(3, 3,
        fastxlsx::CellValue::text("rename-back-missing-clear-invalid-mutation-followup"));
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear retry invalid-mutation follow-up mutation should clear diagnostics");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
            matching.has_pending_changes(),
        "rename-back missing clear retry invalid-mutation follow-up mutation should dirty all handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear retry invalid-mutation follow-up mutation should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
            "rename-back missing clear retry invalid-mutation follow-up mutation should dirty the restored source name");
    }

    editor.save_as(followup_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !matching.has_pending_changes(),
        "rename-back missing clear retry invalid-mutation follow-up save should clean all handles");
    check(editor.pending_change_count() == 5,
        "rename-back missing clear retry invalid-mutation follow-up save should count one more handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear retry invalid-mutation follow-up save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear retry invalid-mutation follow-up save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear retry invalid-mutation follow-up save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear retry invalid-mutation follow-up save should clear summaries");

    check_contains(retry_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear retry invalid-mutation retry output should use restored source name");
    check_not_contains(retry_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear retry invalid-mutation retry output should not leak transient name");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-saved",
        "rename-back missing clear retry invalid-mutation retry output should keep saved value");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-recovered",
        "rename-back missing clear retry invalid-mutation retry output should include recovered value");
    check_not_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-followup",
        "rename-back missing clear retry invalid-mutation retry output should not include follow-up value");

    const auto followup_entries =
        fastxlsx::test::read_zip_entries(followup_output);
    check_contains(followup_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear retry invalid-mutation follow-up output should use restored source name");
    check_not_contains(followup_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear retry invalid-mutation follow-up output should not leak transient name");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-saved",
        "rename-back missing clear retry invalid-mutation follow-up output should keep saved value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-recovered",
        "rename-back missing clear retry invalid-mutation follow-up output should keep recovered value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-followup",
        "rename-back missing clear retry invalid-mutation follow-up output should include follow-up value");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-invalid-mutation-rejected",
        "rename-back missing clear retry invalid-mutation follow-up output should not leak rejected payloads");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear retry invalid-mutation follow-up output should not synthesize row/column missing target");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear retry invalid-mutation follow-up output should not synthesize A1 missing target");
}

void test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_clear_cleanup()
{
    const std::filesystem::path source =
        write_two_sheet_source("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-source.xlsx");
    const std::filesystem::path output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-output.xlsx");
    const std::filesystem::path clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-noop.xlsx");
    const std::filesystem::path retry_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-retry.xlsx");
    const std::filesystem::path cleanup_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-retry-noop.xlsx");
    const std::filesystem::path cleanup_invalid_mutation_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-invalid-mutation-noop.xlsx");
    const std::filesystem::path cleanup_invalid_mutation_clear_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-invalid-mutation-clear-noop.xlsx");
    const std::filesystem::path cleanup_matching_reacquire_no_op_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-matching-reacquire-noop.xlsx");
    const std::filesystem::path followup_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-missing-clear-cleanup-followup.xlsx");
    ...
    fastxlsx::WorksheetEditor sheet = editor.worksheet("Data", options);
    sheet.set_cell(1, 1,
        fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-saved"));
    editor.save_as(output);

    fastxlsx::WorksheetEditor reacquired = editor.worksheet("Data", options);
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-setup-rejected"));
    }), "rename-back missing clear cleanup setup should reject row-zero mutation");
    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup no-op should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup no-op should keep both handles clean");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear cleanup no-op should not add another handoff");
    ...
    const auto clear_no_op_entries =
        fastxlsx::test::read_zip_entries(clear_no_op_output);
    check(clear_no_op_entries == output_entries,
        "rename-back missing clear cleanup no-op output should match first restored-name output");

    reacquired.set_cell(2, 2,
        fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-recovered"));
    check(threw_fastxlsx_error([&] { editor.save_as(source); }),
        "rename-back missing clear cleanup failed save should reject source overwrite");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup failed save should not create last_edit_error");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes(),
        "rename-back missing clear cleanup failed save should keep both handles dirty");
    check(editor.pending_change_count() == 3,
        "rename-back missing clear cleanup failed save should not count a materialized handoff");

    editor.save_as(retry_output);
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup retry save should clean both handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup retry save should count one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup retry save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup retry save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup retry save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup retry save should clear summaries");
    ...
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-rejected-row"));
    }), "rename-back missing clear cleanup retry should reject row-zero set");
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell("XFE1",
            fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-rejected-a1"));
    }), "rename-back missing clear cleanup retry should reject A1 column overflow");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup retry invalid mutations should record diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup retry invalid mutations should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup retry invalid mutations should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup retry invalid mutations should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup retry invalid mutations should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup retry invalid mutations should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup retry invalid mutations should keep summaries empty");

    reacquired.clear_cell_value(5, 5);
    reacquired.clear_cell_value("D4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup no-op after retry should clear invalid mutation diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup no-op after retry should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup no-op after retry should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup no-op after retry should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup no-op after retry should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup no-op after retry should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup no-op after retry should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear cleanup no-op after retry");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear cleanup no-op after retry should preserve sparse diagnostics");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-cleanup-saved",
        "rename-back missing clear cleanup no-op after retry");
    check_public_two_clean_retry_saved_value(
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup no-op save should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup no-op save should not add a handoff");
    const auto cleanup_no_op_entries =
        fastxlsx::test::read_zip_entries(cleanup_no_op_output);
    check(cleanup_no_op_entries == retry_entries,
        "rename-back missing clear cleanup no-op output should match retry output");

    check(threw_fastxlsx_error([&] {
        reacquired.set_cell(0, 1,
            fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-late-rejected-row"));
    }), "rename-back missing clear cleanup invalid mutations should reject row-zero set");
    check(threw_fastxlsx_error([&] {
        reacquired.set_cell("XFE1",
            fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-late-rejected-a1"));
    }), "rename-back missing clear cleanup invalid mutations should reject A1 column overflow");
    check(threw_fastxlsx_error([&] { reacquired.erase_cell(1, 0); }),
        "rename-back missing clear cleanup invalid mutations should reject column-zero erase");
    check(threw_fastxlsx_error([&] { reacquired.clear_cell_value(1048577, 1); }),
        "rename-back missing clear cleanup invalid mutations should reject row-overflow clear");
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup invalid mutations should record diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup invalid mutations should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup invalid mutations should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup invalid mutations should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup invalid mutations should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup invalid mutations should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup invalid mutations should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear cleanup invalid mutations");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear cleanup invalid mutations should preserve sparse diagnostics");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-cleanup-saved",
        "rename-back missing clear cleanup invalid mutations");
    check_public_two_clean_retry_saved_value(
        reacquired, 2, 2, "rename-back-missing-clear-cleanup-recovered",
        "rename-back missing clear cleanup invalid mutations");
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value(),
        "rename-back missing clear cleanup invalid mutations should keep missing targets absent");

    editor.save_as(cleanup_invalid_mutation_no_op_output);
    check(editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup invalid-mutation no-op save should preserve diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup invalid-mutation no-op save should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup invalid-mutation no-op save should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup invalid-mutation no-op save should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup invalid-mutation no-op save should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup invalid-mutation no-op save should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup invalid-mutation no-op save should keep summaries empty");
    const auto cleanup_invalid_mutation_no_op_entries =
        fastxlsx::test::read_zip_entries(cleanup_invalid_mutation_no_op_output);
    check(cleanup_invalid_mutation_no_op_entries == retry_entries,
        "rename-back missing clear cleanup invalid-mutation no-op output should match retry output");

    reacquired.clear_cell_value(6, 6);
    reacquired.clear_cell_value("E4");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup invalid-mutation clear no-op should clear diagnostics");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup invalid-mutation clear no-op should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup invalid-mutation clear no-op should not add a handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup invalid-mutation clear no-op should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup invalid-mutation clear no-op should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup invalid-mutation clear no-op should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup invalid-mutation clear no-op should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry,
        "rename-back missing clear cleanup invalid-mutation clear no-op");
    check(reacquired.cell_count() == saved_cell_count &&
            reacquired.estimated_memory_usage() == saved_memory,
        "rename-back missing clear cleanup invalid-mutation clear no-op should preserve sparse diagnostics");
    check_public_two_clean_retry_saved_value(
        reacquired, 1, 1, "rename-back-missing-clear-cleanup-saved",
        "rename-back missing clear cleanup invalid-mutation clear no-op");
    check_public_two_clean_retry_saved_value(
    ...
    check(!reacquired.try_cell(5, 5).has_value() &&
            !reacquired.try_cell("D4").has_value() &&
            !reacquired.try_cell(6, 6).has_value() &&
            !reacquired.try_cell("E4").has_value(),
        "rename-back missing clear cleanup invalid-mutation clear no-op should keep missing targets absent");

    editor.save_as(cleanup_invalid_mutation_clear_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup invalid-mutation clear no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup invalid-mutation clear no-op save should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup invalid-mutation clear no-op save should not add a handoff");
    const auto cleanup_invalid_mutation_clear_no_op_entries =
        fastxlsx::test::read_zip_entries(cleanup_invalid_mutation_clear_no_op_output);
    check(cleanup_invalid_mutation_clear_no_op_entries == retry_entries,
        "rename-back missing clear cleanup invalid-mutation clear no-op output should match retry output");

    fastxlsx::WorksheetEditor matching = editor.worksheet("Data", options);
    check_public_two_clean_retry_saved_value(
        matching, 1, 1, "rename-back-missing-clear-cleanup-saved",
        "rename-back missing clear cleanup matching reacquire");
    check_public_two_clean_retry_saved_value(
        matching, 2, 2, "rename-back-missing-clear-cleanup-recovered",
        "rename-back missing clear cleanup matching reacquire");
    check(!matching.has_pending_changes(),
        "rename-back missing clear cleanup matching reacquire should stay clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup matching reacquire should not add a handoff");
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup matching reacquire should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "rename-back missing clear cleanup matching reacquire should keep existing handles clean");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup matching reacquire should keep dirty names empty");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup matching reacquire should keep dirty cell count empty");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup matching reacquire should keep dirty memory empty");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup matching reacquire should keep summaries empty");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_after_retry, "rename-back missing clear cleanup matching reacquire");
    check(matching.cell_count() == saved_cell_count &&
            matching.estimated_memory_usage() == saved_memory,
        "rename-back missing clear cleanup matching reacquire should preserve sparse diagnostics");
    check(!matching.try_cell(5, 5).has_value() &&
            !matching.try_cell("D4").has_value() &&
            !matching.try_cell(6, 6).has_value() &&
            !matching.try_cell("E4").has_value(),
        "rename-back missing clear cleanup matching reacquire should keep missing targets absent");

    editor.save_as(cleanup_matching_reacquire_no_op_output);
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup matching reacquire no-op save should keep diagnostics clear");
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !matching.has_pending_changes(),
        "rename-back missing clear cleanup matching reacquire no-op save should keep handles clean");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup matching reacquire no-op save should not add a handoff");
    const auto cleanup_matching_reacquire_no_op_entries =
        fastxlsx::test::read_zip_entries(cleanup_matching_reacquire_no_op_output);
    check(cleanup_matching_reacquire_no_op_entries == retry_entries,
        "rename-back missing clear cleanup matching reacquire no-op output should match retry output");

    matching.set_cell(3, 3,
        fastxlsx::CellValue::text("rename-back-missing-clear-cleanup-followup"));
    check(!editor.last_edit_error().has_value(),
        "rename-back missing clear cleanup follow-up mutation should keep diagnostics clear");
    check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
            matching.has_pending_changes(),
        "rename-back missing clear cleanup follow-up mutation should dirty all handles");
    check(editor.pending_change_count() == 4,
        "rename-back missing clear cleanup follow-up mutation should not count a handoff before save");
    {
        const std::vector<std::string> names =
            editor.pending_materialized_worksheet_names();
        check(names.size() == 1 && names[0] == "Data",
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !matching.has_pending_changes(),
        "rename-back missing clear cleanup follow-up save should clean all handles");
    check(editor.pending_change_count() == 5,
        "rename-back missing clear cleanup follow-up save should count one more handoff");
    check(editor.pending_materialized_worksheet_names().empty(),
        "rename-back missing clear cleanup follow-up save should clear dirty names");
    check(editor.pending_materialized_cell_count() == 0,
        "rename-back missing clear cleanup follow-up save should clear dirty cell count");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "rename-back missing clear cleanup follow-up save should clear dirty memory");
    check(editor.pending_worksheet_edits().empty(),
        "rename-back missing clear cleanup follow-up save should clear summaries");

    check_contains(retry_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear cleanup retry output should use restored source name");
    check_not_contains(retry_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear cleanup retry output should not leak transient name");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-saved",
        "rename-back missing clear cleanup retry output should keep saved value");
    check_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-recovered",
        "rename-back missing clear cleanup retry output should include recovered value");
    check_not_contains(retry_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-followup",
        "rename-back missing clear cleanup retry output should not include follow-up value");

    const auto followup_entries =
        fastxlsx::test::read_zip_entries(followup_output);
    check_contains(followup_entries.at("xl/workbook.xml"), R"(name="Data")",
        "rename-back missing clear cleanup follow-up output should use restored source name");
    check_not_contains(followup_entries.at("xl/workbook.xml"), "TransientData",
        "rename-back missing clear cleanup follow-up output should not leak transient name");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-saved",
        "rename-back missing clear cleanup follow-up output should keep saved value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-recovered",
        "rename-back missing clear cleanup follow-up output should keep recovered value");
    check_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-followup",
        "rename-back missing clear cleanup follow-up output should include follow-up value");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"),
        "rename-back-missing-clear-cleanup-rejected",
        "rename-back missing clear cleanup follow-up output should not leak rejected payloads");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="E5")",
        "rename-back missing clear cleanup follow-up output should not synthesize row/column missing target");
    check_not_contains(followup_entries.at("xl/worksheets/sheet1.xml"), R"(r="D4")",
        "rename-back missing clear cleanup follow-up output should not synthesize A1 missing target");
}
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (should_run_workbook_editor_shard(shard, "public")) {
            ...
            test_public_rename_before_full_calculation_allows_later_materialized_edit();
            test_public_rename_before_full_calculation_allows_later_clean_materialization();
            ...
            test_public_worksheet_editor_rename_back_materialized_missing_clear_noop_save();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_recovery_save();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_recovery_failed_save();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_reacquire();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_option_mismatch();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_missing_lookup();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_invalid_reads();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_invalid_mutations();
            test_public_worksheet_editor_rename_back_materialized_missing_clear_failed_save_retry_clear_cleanup();
        }
