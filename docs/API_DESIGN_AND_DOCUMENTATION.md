...
- Row/column structural edit 的 merged-cell、root auto-filter 与 data-validation synchronization 必须使用 CellStore/session candidate + 当前有效 worksheet metadata plans 两阶段预构造。三类 metadata 分别 strict scan/translate/audit，再合并为一次 file-backed worksheet rewrite 和一次 `PackageEditor` commit；最后 session 与 public filter diagnostic map publish 必须 noexcept。Merged cells 只产生完整 container replacement/removal；auto-filter 只改 opening-tag `ref` value 或移除 whole element；validation 只改 opening-tag `sqref`、删除 empty child/container，并在 child removal 时归一化 `count`。Validation source/final ranges 必须受 64K aggregate guardrail 和全局 duplicate/overlap audit，rule/formula/prompt/error 保持不变。不能通过多次 public metadata API 拼接，否则后续失败会留下部分 package mutation。Metadata-only 空 CellStore 变化必须 dirty，失败 save 必须保留最终 cells/metadata retry。
