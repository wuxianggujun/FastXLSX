...
- `WorksheetEditor` row/column insert/delete 已同步 worksheet-root merged ranges：CellStore/formula candidate 与完整 `<mergeCells>` replacement/removal 分开预构造，strict scan 后只产生一次 metadata replacement，最后 noexcept 发布 session。插入支持 shift/expand，删除支持 shift/clip/full-or-single removal；metadata-only 空 store 也 dirty，schema/count/QName/overlap/bounds/staging 失败不污染 package/session 并可 retry。Auto filter、data validations 与其他 linked metadata 尚不自动同步。
