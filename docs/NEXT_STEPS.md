...
The public-edge max-coordinate projection and erase-shrinks paths now also run
clean no-op `save_as()` checks after the sparse `XFD1048576` handoff, proving
the repeated outputs are byte-stable, the source package stays unchanged, and
