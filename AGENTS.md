...
- `WorksheetEditor` row/column insert/delete 已同步 worksheet-root merged ranges 与 root auto-filter：CellStore/formula candidate、完整 `<mergeCells>` replacement/removal 和 opening-tag-only `<autoFilter ref>` replacement/removal 分开预构造，再合并为一次 file-backed worksheet rewrite 与一次 `PackageEditor` commit，最后 noexcept 发布 session 和 public filter diagnostic map。插入支持 shift/expand；删除支持 shift/clip/full removal，merged single-cell result 移除而 filter single-cell result 保留。Surviving changed filter 含 criteria/sort element child 时 fail，完整 range 删除可移除 whole element；metadata-only 空 store 也 dirty，schema/count/QName/overlap/bounds/child-semantics/staging 失败不污染 package/session 并可 retry。Data validations 与其他 linked metadata 尚不自动同步。
