...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  `WorksheetWriter`、`CellView`、`DocumentProperties`、`DataValidationRule`、`TableOptions`、
...
- OPC 当前已有内部 manifest / relationships / `PartIndex` / `RelationshipGraph`
  / content type registry 基础；Phase 5 已有 data validations、external hyperlinks、
  streaming-only tables 和 `WorksheetWriter::add_image()` 等基础切片，但不能宣称完整
