...
- `WorkbookEditor::add_note()` 是跨 worksheet XML、worksheet `.rels`、comments/VML linked parts 与 content types 的 conservative metadata edit。首次调用要求目标没有 source-owned classic/threaded comments 或任何 VML relationship，分配 paired generated parts 和关系 id，并在 schema rank 25 插入 `<legacyDrawing>`；同会话追加重新序列化 owning note list、first-use author table 与 hidden shapes。Worksheet/file replacement、relationships、manifest、EditPlan、temporary ownership 和 public `classic_note_count` 在提交前保持隔离。Source-owned comments/VML、existing-object merge/update/delete、rich text、shape customization 和 cell/formula sync 默认 fail 或不支持。
