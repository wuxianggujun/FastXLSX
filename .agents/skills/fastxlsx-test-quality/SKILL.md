...
- 当前数值边界测试覆盖：in-memory `Workbook::save()` 拒绝 numeric cell / row height
  的非有限值；streaming `WorksheetWriter::append_row()` 拒绝 numeric cell 的非有限值，
  并拒绝 row height 的 `0`、负数、`NaN`、`+Inf` 和 `-Inf`；
