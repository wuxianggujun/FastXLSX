...
- `WorkbookEditor::replace_cells(..., CellPatchMissingCellPolicy::Insert)` 是当前
  ...
  internal `WorksheetCellIndex` source-offset 索引基础，可把 source worksheet cells
  映射到 `<c>` byte ranges；它不是 public API，也不是当前 public Patch 的默认
  algorithm switch 或 source ZIP seek 能力。
