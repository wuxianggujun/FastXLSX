...
void test_public_worksheet_editor_rename_back_failed_save_as_max_coordinate_erase_shrinks_projection()
{
    ...
    const std::filesystem::path noop_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-max-coordinate-erase-noop.xlsx");
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !after_max_save.has_pending_changes(),
        "max-coordinate erase no-op save should keep recovery handles clean");
    check(editor.pending_change_count() == 5,
        "max-coordinate erase no-op save should not add another materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "max-coordinate erase no-op save should keep dirty materialized diagnostics empty");
    check(editor.pending_worksheet_edits().empty(),
        "max-coordinate erase no-op save should keep edit summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "max-coordinate erase no-op save");
    check(!editor.last_edit_error().has_value(),
        "max-coordinate erase no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop, "max-coordinate erase no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "max-coordinate erase no-op save");
    check(fastxlsx::test::read_zip_entries(noop_output) == third_entries,
        "max-coordinate erase no-op output should match the third output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "max-coordinate erase no-op save should leave the source package unchanged");

    fastxlsx::WorkbookEditor noop_editor = fastxlsx::WorkbookEditor::open(noop_output);
    fastxlsx::WorksheetEditor noop_sheet = noop_editor.worksheet("Data", options);
    check(!noop_sheet.has_pending_changes(),
        "max-coordinate erase no-op reopened sheet should start clean");
    check(noop_sheet.cell_count() == 3,
        "max-coordinate erase no-op reopened sheet should preserve sparse cell count");
    check(!noop_sheet.try_cell(max_row, max_column).has_value(),
        "max-coordinate erase no-op reopened sheet should keep the edge cell absent");
    check(noop_sheet.get_cell("A1").text_value() ==
            "rename-back-max-coordinate-erase-first",
        "max-coordinate erase no-op reopened sheet should keep the setup A1 text");
}
...
void test_public_worksheet_editor_rename_back_failed_save_as_max_coordinate_a1_mutations()
{
    ...
    const std::filesystem::path noop_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-max-coordinate-a1-noop.xlsx");
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !after_set_save.has_pending_changes(),
        "max-coordinate A1 no-op save should keep recovery handles clean");
    check(editor.pending_change_count() == 5,
        "max-coordinate A1 no-op save should not add another materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "max-coordinate A1 no-op save should keep dirty materialized diagnostics empty");
    check(editor.pending_worksheet_edits().empty(),
        "max-coordinate A1 no-op save should keep edit summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "max-coordinate A1 no-op save");
    check(!editor.last_edit_error().has_value(),
        "max-coordinate A1 no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop, "max-coordinate A1 no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "max-coordinate A1 no-op save");
    check(fastxlsx::test::read_zip_entries(noop_output) == third_entries,
        "max-coordinate A1 no-op output should match the third output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "max-coordinate A1 no-op save should leave the source package unchanged");

    fastxlsx::WorkbookEditor noop_editor = fastxlsx::WorkbookEditor::open(noop_output);
    fastxlsx::WorksheetEditor noop_sheet = noop_editor.worksheet("Data", options);
    check(!noop_sheet.has_pending_changes(),
        "max-coordinate A1 no-op reopened sheet should start clean");
    check(noop_sheet.cell_count() == 3,
        "max-coordinate A1 no-op reopened sheet should preserve sparse cell count");
    check(!noop_sheet.try_cell("XFD1048576").has_value(),
        "max-coordinate A1 no-op reopened sheet should keep the edge cell absent");
    check(noop_sheet.get_cell("A1").text_value() ==
            "rename-back-max-coordinate-a1-first",
        "max-coordinate A1 no-op reopened sheet should keep the setup A1 text");
}
...
void test_public_worksheet_editor_rename_back_failed_save_as_max_coordinate_blank_projection()
{
    ...
    const std::filesystem::path noop_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-max-coordinate-blank-noop.xlsx");
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !after_blank_save.has_pending_changes(),
        "max-coordinate blank no-op save should keep recovery handles clean");
    check(editor.pending_change_count() == 5,
        "max-coordinate blank no-op save should not add another materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "max-coordinate blank no-op save should keep dirty materialized diagnostics empty");
    check(editor.pending_worksheet_edits().empty(),
        "max-coordinate blank no-op save should keep edit summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "max-coordinate blank no-op save");
    check(!editor.last_edit_error().has_value(),
        "max-coordinate blank no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop, "max-coordinate blank no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "max-coordinate blank no-op save");
    check(fastxlsx::test::read_zip_entries(noop_output) == third_entries,
        "max-coordinate blank no-op output should match the third output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "max-coordinate blank no-op save should leave the source package unchanged");

    fastxlsx::WorkbookEditor noop_editor = fastxlsx::WorkbookEditor::open(noop_output);
    fastxlsx::WorksheetEditor noop_sheet = noop_editor.worksheet("Data", options);
    check(!noop_sheet.has_pending_changes(),
        "max-coordinate blank no-op reopened sheet should start clean");
    check(noop_sheet.cell_count() == 3,
        "max-coordinate blank no-op reopened sheet should preserve sparse cell count");
    check(!noop_sheet.try_cell("XFD1048576").has_value(),
        "max-coordinate blank no-op reopened sheet should keep the edge cell absent");
    check(noop_sheet.get_cell("A1").text_value() ==
            "rename-back-max-coordinate-blank-first",
        "max-coordinate blank no-op reopened sheet should keep the setup A1 text");
}
...
void test_public_worksheet_editor_rename_back_failed_save_as_max_coordinate_formula_projection()
{
    ...
    const std::filesystem::path noop_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-max-coordinate-formula-noop.xlsx");
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes(),
        "max-coordinate formula no-op save should keep recovery handles clean");
    check(editor.pending_change_count() == 4,
        "max-coordinate formula no-op save should not add another materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "max-coordinate formula no-op save should keep dirty materialized diagnostics empty");
    check(editor.pending_worksheet_edits().empty(),
        "max-coordinate formula no-op save should keep edit summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "max-coordinate formula no-op save");
    check(!editor.last_edit_error().has_value(),
        "max-coordinate formula no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop, "max-coordinate formula no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "max-coordinate formula no-op save");
    check(fastxlsx::test::read_zip_entries(noop_output) == second_entries,
        "max-coordinate formula no-op output should match the second output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "max-coordinate formula no-op save should leave the source package unchanged");

    fastxlsx::WorkbookEditor noop_editor = fastxlsx::WorkbookEditor::open(noop_output);
    fastxlsx::WorksheetEditor noop_sheet = noop_editor.worksheet("Data", options);
    check(!noop_sheet.has_pending_changes(),
        "max-coordinate formula no-op reopened sheet should start clean");
    check(noop_sheet.cell_count() == 4,
        "max-coordinate formula no-op reopened sheet should preserve sparse cell count");
    check(noop_sheet.get_cell(1048576, 16384).text_value() ==
            R"(SUM(A1:B1)&"<edge>")",
        "max-coordinate formula no-op reopened sheet should read the edge formula");
    check(noop_sheet.get_cell("A1").text_value() ==
            "rename-back-max-coordinate-formula-first",
        "max-coordinate formula no-op reopened sheet should keep the setup A1 text");
}
...
void test_public_worksheet_editor_rename_back_failed_save_as_max_coordinate_scalar_projection()
{
    ...
    const std::filesystem::path noop_output =
        artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-max-coordinate-scalar-noop.xlsx");
    ...
    check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
            !after_number_save.has_pending_changes(),
        "max-coordinate scalar no-op save should keep recovery handles clean");
    check(editor.pending_change_count() == 5,
        "max-coordinate scalar no-op save should not add another materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "max-coordinate scalar no-op save should keep dirty materialized diagnostics empty");
    check(editor.pending_worksheet_edits().empty(),
        "max-coordinate scalar no-op save should keep edit summaries empty");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "max-coordinate scalar no-op save");
    check(!editor.last_edit_error().has_value(),
        "max-coordinate scalar no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop, "max-coordinate scalar no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "max-coordinate scalar no-op save");
    check(fastxlsx::test::read_zip_entries(noop_output) == third_entries,
        "max-coordinate scalar no-op output should match the third output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "max-coordinate scalar no-op save should leave the source package unchanged");

    fastxlsx::WorkbookEditor noop_editor = fastxlsx::WorkbookEditor::open(noop_output);
    fastxlsx::WorksheetEditor noop_sheet = noop_editor.worksheet("Data", options);
    check(!noop_sheet.has_pending_changes(),
        "max-coordinate scalar no-op reopened sheet should start clean");
    check(noop_sheet.cell_count() == 4,
        "max-coordinate scalar no-op reopened sheet should preserve sparse cell count");
    {
        const fastxlsx::CellValue edge_cell = noop_sheet.get_cell("XFD1048576");
        check(edge_cell.kind() == fastxlsx::CellValueKind::Boolean &&
                !edge_cell.boolean_value(),
            "max-coordinate scalar no-op reopened sheet should read the edge boolean");
    }
    check(noop_sheet.get_cell("A1").text_value() ==
            "rename-back-max-coordinate-scalar-first",
        "max-coordinate scalar no-op reopened sheet should keep the setup A1 text");
}
