...
py tools\run_workbook_editor_qa.py `
  ...
  --scenario generated_in_memory_stationary_range_formula_shift_noop_save `
...
For generated in-memory clear/erase persistence smoke, run:
...
py tools\run_workbook_editor_qa.py `
  --qa-exe build\windows-nmake-release\tools\fastxlsx_workbook_editor_qa_tool.exe `
  --scenario generated_in_memory_retry_reopen_modify_noop_save `
  --work-dir build\qa\workbook-editor-in-memory-retry-reopen-modify-noop-save
```

This scenario rejects `save_as(source)`, writes a safe retry output, opens that
retry output through a fresh `WorkbookEditor`, applies second-stage `Data`
...
For the path-equivalent failed-save variant of the retry/reopen/no-op plus
third-edit path, run:
...
py tools\run_workbook_editor_qa.py `
  --qa-exe build\windows-nmake-release\tools\fastxlsx_workbook_editor_qa_tool.exe `
  --scenario generated_in_memory_retry_path_equivalent_reopen_modify_post_noop_third_save `
  --work-dir build\qa\workbook-editor-in-memory-retry-path-equivalent-reopen-post-noop-third-save
```

This mirrors the single-sheet post-noop third-save shape while using a
path-equivalent source output path for the rejected first save. It is opt-in QA
only; it does not add overwrite mode, path repair, transaction replay,
metadata repair, sharedStrings/styles migration, calcChain rebuild, or
low-memory random editing.
...
py tools\run_workbook_editor_qa.py `
  --qa-exe build\windows-nmake-release\tools\fastxlsx_workbook_editor_qa_tool.exe `
  --scenario generated_in_memory_retry_reopen_modify_post_noop_third_save `
  --work-dir build\qa\workbook-editor-in-memory-retry-reopen-modify-post-noop-third-save
```

This scenario repeats the retry/reopen/modify/no-op path, edits `Data` again
after the clean no-op save, writes a third-stage output, and requires the final
no-op `save_as()` output to be byte-identical to that third-stage save while
the original source workbook remains unchanged.
...
For the path-equivalent failed-save variant of the multi-sheet retry,
reopen/modify, no-op save, and post-no-op third-save path, run:

```powershell
cmake --build --preset windows-nmake-release --target fastxlsx_workbook_editor_qa_tool
py tools\run_workbook_editor_qa.py `
  --qa-exe build\windows-nmake-release\tools\fastxlsx_workbook_editor_qa_tool.exe `
  --scenario generated_in_memory_multi_sheet_retry_path_equivalent_reopen_modify_post_noop_third_save `
  --work-dir build\qa\workbook-editor-in-memory-multi-sheet-retry-path-equivalent-reopen-post-noop-third-save
```

This mirrors the multi-sheet retry/reopen/post-noop third-save shape while
using a path-equivalent source output path for the rejected first save. It is
opt-in QA only; it does not add overwrite mode, path repair, transaction
replay, cross-sheet dependency synchronization, metadata repair,
sharedStrings/styles migration, calcChain rebuild, or low-memory random
editing.
...
py tools\run_workbook_editor_qa.py `
  --qa-exe build\windows-nmake-release\tools\fastxlsx_workbook_editor_qa_tool.exe `
  --scenario generated_in_memory_multi_sheet_retry_reopen_modify_post_noop_third_save `
  --work-dir build\qa\workbook-editor-in-memory-multi-sheet-retry-reopen-modify-post-noop-third-save
```

This post-noop variant repeats the same generated retry/reopen/no-op path, then
edits `Data` and `Summary` again, saves a third-stage output, requires the final
no-op save to be byte-identical to that third output, and validates the final
workbook with the same ZIP/XML and `openpyxl` checks.
...
test-maintenance scaffolding only and does not change public behavior.
The broader two-clean retry family now reuses that saved materialized value
...
The materialized formula rewrite guard failure regression also reuses the
...
The materialization failure clean-state helper now reuses the same
...
The failed-save edge after invalid mutations is pinned too: if
...
formula non-goal boundaries are maintained in `docs/FORMULA_SUPPORT.md`. For the local
...
This generated case renames `Data` to `Renamed & O'Brien` with
`WorkbookEditorRenameFormulaPolicy::RewriteDefinedNamesAndMaterializedWorksheetFormulas`.
ZIP/XML and `openpyxl` verify the workbook catalog uses XML-escaped sheet-name
metadata, direct local definedNames and already-materialized worksheet formulas
use quoted formula qualifiers with doubled apostrophes, external-workbook and
3D sheet-range references remain unchanged, string literals stay literal, the
...
This generated case uses the same workbook shape but calls default
`rename_sheet("Data", "RenamedData")`. ZIP/XML and `openpyxl` verify that the
catalog rename happened while direct local definedNames, already-materialized
worksheet formulas, external-workbook references, 3D sheet-range references,
