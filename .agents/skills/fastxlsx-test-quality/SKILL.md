...
- 当前 `fastxlsx.worksheet_transformer` 还覆盖 targeted-cell Patch 的 completed-target
  ...
  source-order plan / chunk-boundary / invalid-source 边界；它还覆盖 compact
  `{row, column, range}` index 在非 row-major source XML 下的 lookup、lazy
  diagnostic `cells()` snapshot 稳定性，以及排序后拒绝非相邻 duplicate source
  cells；`fastxlsx.worksheet_transformer`
  还覆盖 transform action source offset、synthetic insert offset，以及 internal
  materialized indexed slicer 的 source-range splicing / no-partial-output failure
  边界；`fastxlsx.package_editor.core` 还覆盖 internal `PackageEntryChunk`
  byte-range emitter 的 full/empty/file-only/cross-chunk slicing 和 invalid range /
  invalid chunk 边界，以及 internal PackageEditor chunk-backed indexed strict-replace
  slicer prototype 的 staged source splicing / preplanned range overload /
  missing target / stale range / stale staged-size rejection 边界。
  `fastxlsx_bench_package_editor_cell_replacement` 还提供
  opt-in `--rewrite-strategy indexed-staged` schema-v2 benchmark/prototype 路径，
  用于验证 benchmark 生成的 staged chunks + target-only preplanned ranges。它不是
  public Patch 默认算法、PackageEditor source-entry ZIP seek、metadata repair 或 Office 兼容性证明；
  性能声明仍要靠 opt-in benchmark 结果。
...
- 当前 streaming 行上限测试用 `FASTXLSX_ENABLE_TEST_HOOKS` 下的
  ...
  benchmark。
- 当前 streaming metadata 测试覆盖公式 XML escape、row height、多个
...
当前最小 writer 质量不只是“能编译”。生成的 `.xlsx` 应检查：
...
- 当前 streaming metadata 推荐 preset 样例为
  ...
  公式计算、cached values、calcChain、styles 或完整 worksheet metadata 功能集。
...
- 当前 streaming styles number-format + wrap-text + limited horizontal/vertical alignment +
...
- 当前最小 writer smoke 样例通常位于测试工作目录；推荐 preset 路径是
...
完整图片 OpenXML 读取/插入仍是 planned / not yet public；但
...
当前最小 streaming writer benchmark 入口是 `FASTXLSX_BUILD_BENCHMARKS=ON` 下的手工工具
...
当前 PackageEditor cell replacement benchmark
`fastxlsx_bench_package_editor_cell_replacement` 使用独立
`package_editor_cell_replacement_benchmark_schema_version = "2"`，默认
`rewrite_strategy="transformer"`，可显式用
`--editor-api internal-package-editor --rewrite-strategy indexed-staged` 运行 internal
indexed staged-chunk prototype。schema-v2 会记录 `index_build_ms`、
`indexed_emit_ms`、`indexed_stage_commit_ms`、indexed matched/source counts 和 staged
worksheet output bytes；当前 indexed-staged benchmark 的 `index_build_ms` 是
target-only range scan/planning 耗时，`indexed_source_cell_count` 是扫描过的 source
cell 数，不是保存在内存中的 index entry 数。这仍是手工 opt-in benchmark，不进默认
CTest/CI，也不是 public Patch 默认算法或 source-entry ZIP seek 证明。
