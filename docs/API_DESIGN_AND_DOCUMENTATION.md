...
- Row/column structural edit 的 merged-cell 与 root auto-filter synchronization 必须使用 CellStore/session candidate + 当前有效 worksheet metadata plans 两阶段预构造。两类 metadata 分别 strict scan/translate/audit，再合并为一次 file-backed worksheet rewrite 和一次 `PackageEditor` commit；最后 session 与 public filter diagnostic map publish 必须 noexcept。Merged cells 只产生完整 container replacement/removal；auto-filter 只改 opening-tag `ref` value或移除 whole element。不能通过多次 public metadata API 拼接，否则后续失败会留下部分 package mutation。Metadata-only 空 CellStore 变化必须 dirty，失败 save 必须保留最终 cells/metadata retry。
