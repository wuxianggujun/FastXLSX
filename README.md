...
- **Patch rewrite 现状**：最新同机 warmed 2+5 event/action profile 中，5,000,000-cell numeric level 1 upsert 的 total/transform/residual median 为 8.004/5.180/4.060 秒，较紧邻同协议 bundle 分别降低 5.77%/11.53%/14.21%，process peak working set 为 8.79297 MB，owned batching buffer 仍为 256 KiB。另有 1,000,000-cell numeric、mixed inline、sharedStrings、formula + hyperlink 分布验证；这些结果只证明记录 workload 的顺序 rewrite 与有界内存，不是任意随机编辑承诺。
