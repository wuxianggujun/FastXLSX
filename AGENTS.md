...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `WorkbookWriterOptions::zip_compression_level`、
...
- `include/fastxlsx/detail/cell_store.hpp` 和 `src/cell_store.cpp` 是当前
  ...
  payload 可进入当前 bounded `sheetData` Patch helper；当前 public
  `WorkbookEditor::replace_sheet_data()` 只复用其 whole-`<sheetData>` handoff，
  不代表 `WorksheetEditor`、random cell editing、workbook-level guardrails、
  完整 in-memory save-as / Patch handoff、sharedStrings migration、styles merge、
  calcChain rebuild 或 relationship repair。
...
copy-original entries 暴露到 `EditPlan` / planned output。内部
`planned_output()` 快照现在还覆盖该状态的边界：fullCalcOnLoad /
`CalcChainAction::Remove`、worksheet / workbook `LocalDomRewrite`、content types /
package relationships / workbook relationships / worksheet relationships
copy-original、OLE / control part copy-original relationship metadata、
preserved OLE/control caller-review notes、无 relationship target audit、无 worksheet
relationship-id audit、无 removed parts / removed package entries，且不凭空创建
`xl/calcChain.xml`；这只是 Patch preservation / audit 可见性，不是 OLE /
ActiveX / control 语义编辑、calcChain rebuild、relationship repair/pruning、
orphan cleanup、content type repair、public API 或完整 object preservation。
...
- 阶段任务重排：当前默认推进线切到 `C0 -> C7`，`P*` 只作为历史索引和能力切片，
  不再作为下一步执行入口。下一轮实现或设计任务必须先按 `docs/TASK_BREAKDOWN.md`
  选择子任务编号，不再把“任务 4”或整个 Phase 4 当作一个可直接执行的大任务。
  C0 先校准文档和代码基线，C1 硬化当前 `WorkbookEditor` Patch facade，C2 补
  preservation / dependency audit，C3 决定 public editor 展开口径，C4 走小文件
  In-memory，C5 走大 worksheet 低内存 rewrite，C6 继续支撑线，C7 处理 release /
  packaging。sharedStrings、minizip、benchmark、file-backed/chunked worksheet entry 和
  streaming hot path 继续推进，但不再作为阻塞编辑能力的唯一主线。In-memory 小文件随机
  编辑是单独路径，可规划 `get_cell()` / `set_cell()` / `erase_cell()` 等便利能力，但不能
  成为大文件低内存路径，也不能把已实现的 `CellValue` value type 或当前窄
  `WorkbookEditor` Patch subset 扩大成 `WorksheetEditor`、public `PackageEditor` 或
  editor-ready 口径。
- 跨语言库参考矩阵：`docs/ARCHITECTURE.md` 和 `docs/PERFORMANCE_TARGETS.md` 记录
  跨语言 XLSX 参考边界和 benchmark 证据。新增功能时可以吸收其他库的 API 体验、写入
  性能、大文件低内存、OPC 严谨性或兼容性样例，但必须写清对应到 FastXLSX 的
  In-memory / Streaming / Patch 哪条路径，不能把所有优点堆进一个无边界的完整 workbook DOM。
...
- document properties：基础。`DocumentProperties`、`Workbook::set_document_properties()`
  ...
  custom properties content type override 和 unknown part bytes；内部
  `planned_output()` 快照也暴露 generated core/app entries、preserved custom
  properties / unknown entries、metadata rewrites，以及无 removal /
  relationship-target audit 污染。它不是 public
...
- OPC edit plan：基础，是 P4.0 之后窄 Patch MVP 的内部主线。当前是 internal manifest、relationships、
  ...
  copy-original、docProps generated-small-XML 新增/重写（含 custom properties
  preservation 场景的 output-plan 可见性）、worksheet calcChain omission
...
的 removed-part inbound audit；
  ...
  replacement reason、content types / package relationships / workbook /
  workbook `.rels` / worksheet / worksheet `.rels` / drawing / drawing `.rels` /
  chart / table / VML / percent-decoded drawing / sharedStrings /
...
- 把 `src/zip_store_writer.*`、`src/package_writer.*`、`src/package_reader.*` 或
  ...
  public package writer API 或 broad existing-file preservation。当前 package reader 只是 stored/no-compression
