...
- **Patch**：`WorkbookEditor`，面向已有 workbook 的 part-level rewrite，并支持受限的 basic classic note、writer-compatible data-validation add/remove、conditional-format add/update/remove 与 table metadata lifecycle；未修改和未知 part 默认保留。
...
int main()
{
    auto workbook = fastxlsx::WorkbookReader::open("report.xlsx");
    fastxlsx::WorksheetReadCallbacks callbacks;
    callbacks.on_cell = [](const fastxlsx::WorksheetCellView& cell) {
        // reference/text_value/formula_text 只在当前 callback 内有效。
        if (cell.has_formula) {
            // formula_text 与可选 cached value 分开投影；FastXLSX 不求值。
        }
    };
    (void)workbook.read_worksheet("Data", callbacks);
}
```

`WorkbookReader` 每次调用只顺序打开目标 package entry，不构建 worksheet DOM、dense matrix 或 `CellStore`。Worksheet traversal 支持 typed number/boolean、simple inline/text/date token、error、formula + cached scalar、sharedStrings index 和 opaque style index；独立 companion 可顺序读取 primary frozen pane、worksheet-root auto-filter、merged-cell ranges、writer-compatible worksheet data validations、worksheet-local internal/external hyperlinks、writer-compatible conditional formatting、linked table 的 range/name/basic header/table-local auto-filter、writer-compatible two-cell picture anchor 与 PNG/JPEG format/encoded size、classic comments/notes 的 cell ref/author/simple text、strict simple sharedStrings item、保留 item/run 边界的 narrow rich sharedStrings、custom number format/cellXfs，以及 writer-compatible bold/italic/direct-ARGB font 与 none/gray125/solid fill。Metadata/data-validation/hyperlink/conditional-format/table/image/comment callback 的字段均为 owning value；通用 metadata projection 不读取 table-local filter，只有显式 table traversal 会跟随 owner-local relationship 读取该边界。Image traversal 会完整流过每个 unique media entry 以验证 ZIP size/CRC/signature，但不保留 encoded bytes 或解码 pixels；comment traversal 只保留受限 author table，optional legacy VML 仅审计 relationship/part，不解析 payload。Rich-run traversal 的 text 等 borrowed 字段仍必须在 callback 返回前复制。各 traversal 不自动关联 index，也不保留完整 table/drawing/comments/registry；XML/text/nesting/count/media 上限由对应 ReaderOptions 控制。
...
int main()
{
    ...
    editor.add_note("Archive", {1, 1}, "Report Service", "Archived output");
...
- `WorkbookReader` 是 read-only forward traversal，不提供 seek、自动 sharedStrings/style resolution、worksheet mutation 或与 `WorkbookEditor` / `WorksheetEditor` 的隐式 handoff。Shared-string/style index 只做语法与关系存在性检查，不验证对应 table 的 index 上界；显式 companion traversal 也不会自动关联这些 index。`read_worksheet_metadata()` 只审计并投影 primary frozen pane、worksheet-root auto-filter 和 worksheet-root merged cells；`read_worksheet_data_validations()` 只投影 writer-compatible worksheet-root rules、`sqref`、formula1/formula2 与 prompt/error metadata；`read_worksheet_hyperlinks()` 只投影单一 A1 cell/range、worksheet-local internal `location` 或经 owner-local `.rels` 解析的 external target，并严格审计 relationship type/`TargetMode`、QName、schema order 和 duplicate/overlap；`read_worksheet_conditional_formats()` 只投影 writer-compatible two-/three-color color scale、basic data bar、basic `3Arrows` icon set、multi-range `sqref` 与 priority；`read_worksheet_tables()` 只跟随 `<tableParts>` 的标准 internal relationship，投影 table id/range/name/displayName、basic columns、writer-compatible totals function/label、table-local auto-filter boundary 与 style flags；`read_worksheet_images()` 只跟随单一 worksheet drawing 与 drawing-local image relationships，投影 writer-compatible direct two-cell picture anchor、transform extent、name/description、PNG/JPEG format 和 encoded size；`read_worksheet_comments()` 只跟随唯一 standard internal comments relationship，解析 author table、single-cell ref 与 simple text，并对 optional direct legacy VML relationship/part 做 presence audit。它不检查 hyperlink target reachability，不支持 advanced/custom conditional formatting、calculated/other totals formula、完整 table filter/sort/extension semantics、`xdr:oneCellAnchor` / `xdr:absoluteAnchor` 元素、chart/shape/crop/rotation/hyperlink、完整 drawing model、rich/threaded comments、persons 或 VML visibility/shape semantics，也不创建、修复、裁剪或修改 relationships/content types/manifest。
...
- `read_worksheet_comments()` 只读投影已有 classic comments/notes；`WorksheetWriter::add_note()` 为 new-workbook Streaming 创建 simple author/text、hidden VML shape 和必要 package relationships/content types。`WorkbookEditor::add_note()` 只允许在目标 worksheet 没有 source-owned classic comments、threaded comments 或任何 VML relationship 时直接插入；`update_note()` / `remove_note()` 仅接管 comments 与 VML bytes 均与 FastXLSX serializers exact 匹配的 source notes。最后一条删除会同步清理 `<legacyDrawing>`、两条关系、两个 part 与不再使用的 content types。Rich/threaded comments、persons、visibility/shape customization、Excel 重写或其他 unknown VML 仍明确 fail，不形成通用 comments/VML 编辑。
...
- `WorkbookEditor::add_note()` 可向没有 source-owned comments/threaded/VML 语义的 planned worksheet 追加 simple classic note；`update_note()` / `remove_note()` 可按 one-based cell 编辑 FastXLSX writer-canonical source 或同会话 generated note。Canonical source audit 同时比较 bounded reader 投影、comments serializer 与 VML serializer 的 exact payload，并在 percent-decoding、query/fragment 剥离和 part-name 归一化后拒绝其他 worksheet、part 或 package-root relationship 对 linked parts 的入边；update missing fail、相同值 clean no-op，remove missing fail，最后一条删除同步清理 worksheet/relationships/parts/content types。三者都不要求、创建或修改目标 cell，也不支持 rich/threaded comments、custom shape、unknown VML 或 linked cell/formula 同步。
- `WorkbookEditor::add_data_validation()` 可复用 Streaming 的 `DataValidationRule`，向一个或多个 range 追加 worksheet-local data validation；`remove_data_validation()` 按当前 effective worksheet 的 zero-based source index 删除一条，并与 `WorksheetDataValidationView::index` 对齐。删除前会完成 strict bounded projection；最后一条删除整个 `<dataValidations>`，否则归一化 `count`。两者支持 planned rename、同会话新增 worksheet 和同会话组合，不创建 relationship/content type，不求值、不请求重算。后续 materialized `WorksheetEditor` row/column insert/delete 会严格投影当前 effective rules，并对 multi-range `sqref` 执行 shift/expand 或 shift/clip/full removal；空 rule 与最终空 container 会删除，合法 single-cell range 保留，rule/formula/prompt/error payload 不重写。Point cell edit、copy/move、table、defined-name 与其他 linked-object mutation 不联动；失败不污染 pending/unsaved 状态并可 retry。
- `WorkbookEditor::add_conditional_color_scale()` / `add_conditional_data_bar()` / `add_conditional_icon_set()` 可向 planned worksheet 追加 writer-compatible conditional formatting；`update_conditional_format()` / `remove_conditional_format()` 按当前 effective worksheet 的 zero-based `WorksheetConditionalFormatView::index` 替换或删除一条 writer-compatible 单规则容器，same-session add/update/remove 会改变后续 effective 对象。Update 保留原 priority 与 source-order 位置，可同时替换 ranges 和 two-/three-color scale、basic data bar、basic `3Arrows` rule kind/payload；remove 不重排剩余 priority。Add 与 update 的四种规则均支持 single range、`span` 与 initializer-list multi-range overload。Update/remove 先执行 strict bounded projection，advanced/custom、`dxf`/styles、formula/cellIs 与 multiple-rule source payload 不接管并明确 fail。三类 mutation 支持 planned rename/added worksheet，只改 worksheet XML，不改 cells、relationships、content types、styles/dxf、calc metadata 或 linked objects；`pending_worksheet_edits()` 分别暴露 `conditional_format_count`、`conditional_format_update_count` 与 `conditional_format_removal_count`，失败不污染状态且可 retry。
- `WorkbookEditor::add_table()` / `update_table()` / `remove_table()` 可管理 writer-compatible existing-workbook table metadata lifecycle。Add 创建 owner-local table package graph；update 保留 table id、relationship id、part identity 与顺序；remove 删除对应 graph，最后一项同时删除 `<tableParts>`。API 支持 source/planned rename/same-session-added worksheet 与同会话组合，统一审计 workbook-wide ASCII-insensitive name、id/part uniqueness、same-sheet range overlap 和 source part exclusive ownership。Identical update 是 clean no-op，missing mutation fail；删除后的 name/range 可复用，但 id/part 在本 session 不复用。它不读取或同步 cell payload、公式/cached result、defined names 或后续 structural edits，也不是完整 table object model。
- `WorkbookEditor::set_auto_filter()` / `clear_auto_filter()` 可整体替换或删除 worksheet-root `<autoFilter>`；set 会丢弃旧 filter criteria/sort metadata，clear absent 是 clean no-op。`xl/tables/table*.xml` 中的 table-local filter、worksheet relationships、content types、calc metadata 和未知 package entries 保留。后续 materialized `WorksheetEditor` row/column insert/delete 会事务式平移、扩展、裁剪或移除 root range，single-cell final range 保持合法；surviving changed filter 含 criteria/sort element child 时明确 fail，完整 range 删除可移除整个 element。Cell copy/move 与其他 linked objects 不同步该 range。
- `WorkbookEditor::merge_cells()` / `unmerge_cells()` 可严格增删 worksheet-root `<mergeCells>` 中的 multi-cell range；merge 拒绝 duplicate/overlap，unmerge 只删除 exact range，partial overlap 拒绝而 absent disjoint 是 clean no-op。实现审计已有 container、`count`、direct child、ref 与 schema order，保留非左上角 cell payload、relationships、content types、tables、calc metadata 和未知 package entries。`WorksheetEditor` 的 row/column insert/delete 会在同一 structural metadata 事务中平移、扩展、裁剪或移除 merged ranges、root auto-filter、strict writer-compatible data-validation `sqref` 与 worksheet-local hyperlink `ref`。Hyperlink 的 internal location、external target、display/tooltip 不改；external `r:id` 只有在没有 surviving hyperlink 且证明未被其他 worksheet element 复用时才随 relationship 一起删除。Conditional formatting、tables、drawings、comments/VML 和其他 linked objects 尚不随结构编辑同步。
- `WorkbookEditor::set_freeze_panes()` / `clear_freeze_panes()` 可设置或清除 primary `workbookViewId="0"` 的 direct frozen `<pane>`；支持 missing/self-closing `sheetViews`/primary `sheetView`、单轴/双轴冻结、planned rename 和同会话新增 worksheet。实现保留 sheetView 属性、其他 workbook views、仍合法的 selection、cells、relationships、content types、tables、calc metadata 和未知 package entries；普通 split/frozenSplit、pivotSelection、失效 pane selection 与歧义 view metadata 默认 fail。
- 窄读取、table metadata lifecycle 或 preservation 证据不等于 cell-aware structural table sync，也不等于 charts、comments、VBA、pivot 或 custom XML 的完整语义编辑。
