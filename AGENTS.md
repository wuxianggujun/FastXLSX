...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `TableOptions::column_totals_functions`、`ImageFormat`、`ImageInfo`、
  `ImageEditAs`、`ImageOptions`、`read_image_info()`、
...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  hyperlinks / tables / 默认 stb image drawing XML 输出。
...
- Phase 5 worksheet metadata：基础。`WorksheetWriter::add_data_validation()`
  ...
  `totalsRowFunction` metadata 写入 table XML。它不读取已写 header 行、不推断列名、
  不计算 totals、不生成公式文本、`totalsRowLabel`、样式或 `xl/styles.xml`，
...
- 当前 `fastxlsx.streaming` tables 推荐 preset 输出样例为
  ...
  `totalsRowCount="1"`、用户声明的 `totalsRowFunction="sum"`，并确认不会生成
  公式文本、`totalsRowLabel` 或 `xl/styles.xml`。
