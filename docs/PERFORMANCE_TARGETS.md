...
- 十一个 bundle 都不是跨机器、跨数据规模或泛化“高性能/低内存”证据；每个 bundle 只支持其 manifest 中记录的同机同数据集结论。历史 schema-v4 继续作为回归基线，release 当前值以 schema-v5 bundle 为准。
...
- 1,000,000-cell level 6 upsert 中，package writer median 为 933.636 ms，single-pass transform 为 865.366 ms；DEFLATE output 是该 profile 更大的测量阶段。Event/action traffic 已单独优化，relationship scanner 也已迁移为只接收 metadata 的 16 KiB 有界批次；下一轮先用 schema-v6 bundle 验证该路径，再评估 compression backend/参数，不以扩大 In-memory guardrail 代替热路径优化，也不在无证据时引入并行压缩。
