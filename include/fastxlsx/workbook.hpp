...
/// worksheet writes depend on a DOM or full random-access cell matrix. The
/// current cell value surface has no dedicated date type; date/time values must
/// be supplied by the caller as Excel serial numbers through number(), and
/// number formatting / styles are not generated by Cell.
///
...
public:
    ...
    /// worksheet XML. This is also the current path for date/time serial
    /// values; no date-specific cell encoding or style generation is performed.
...
public:
    ...
    /// Returns the number of buffered cell values across all appended rows.
    ///
    /// Empty appended rows count as rows but contribute zero cells. This is a
    /// small in-memory diagnostic, not a streaming progress counter.
    [[nodiscard]] std::size_t cell_count() const noexcept;

    /// Estimates memory held by this worksheet's small in-memory buffer.
    ///
    /// The estimate includes the worksheet object, buffered row/cell vectors,
    /// sheet name storage, and owned string/formula payload capacity. It is a
    /// diagnostic approximation, not process RSS and not a hard memory budget.
...
public:
    ...
    /// unique. Duplicate checks are ASCII case-insensitive. Worksheets are
    /// stored in the workbook's internal vector, so a
    ...
    /// Renames a worksheet in the buffered workbook being generated.
    ///
    /// API mode: small in-memory new-workbook creation. This only mutates the
    /// workbook being generated by Workbook::save(); it does not edit an
    /// existing XLSX file and does not preserve package parts. The rename only
    /// changes the stored worksheet name that save() later writes into
    /// workbook.xml; it does not alter worksheet contents. The new name is XML
    ...
    /// the same as the current name, or if `new_name` contains invalid Excel
    /// worksheet-name characters.
    void rename_worksheet(std::string_view old_name, std::string new_name);

    /// Removes a worksheet by name from the small in-memory workbook model.
    ///
    /// API mode: small in-memory new-workbook creation. This only mutates the
    /// workbook being generated by Workbook::save(); it does not edit an
    /// existing XLSX file and does not preserve package parts. Removing a sheet
    /// erases its buffered rows and invalidates worksheet references at or after
    ...
    /// Returns the number of worksheets currently stored in the workbook.
    [[nodiscard]] std::size_t worksheet_count() const noexcept;

    /// Returns the total number of buffered cell values across all worksheets.
    ///
    /// This is a small in-memory creation diagnostic. It does not inspect an
    /// existing XLSX file and should not be used as a large-export progress API.
    [[nodiscard]] std::size_t cell_count() const noexcept;

    /// Estimates memory held by the small in-memory workbook model.
    ///
    /// The estimate includes workbook/worksheet containers, buffered rows,
    /// buffered cells, owned string/formula payload capacity, and copied
    /// document-property text capacity. It is a diagnostic approximation for
    /// choosing between Workbook and WorkbookWriter, not process RSS, not a
    /// hard guardrail, and not a guarantee for save-time package assembly peak.
    [[nodiscard]] std::size_t estimated_memory_usage() const noexcept;

    /// Returns the current worksheet names in workbook order.
    ///
    /// This is a convenience inspection helper for small new-workbook creation
    /// and does not expose any existing-file editing state.
    ...
    [[nodiscard]] Worksheet& worksheet(std::string_view name);
    ...
    /// model. Use worksheet_names()/has_worksheet()/worksheet() for smaller
    /// convenience lookups.
