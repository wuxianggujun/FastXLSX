...
- **Streaming**：`WorkbookWriter` / `WorksheetWriter` 面向按行创建大型新 workbook；`WorkbookReader` 面向已有 workbook 的 forward-only bounded row/cell traversal。
...
`WorkbookReader` 每次调用只顺序打开目标 worksheet package entry，不构建 worksheet DOM、dense matrix 或 `CellStore`。第一切片支持 typed number/boolean、simple inline/text/date token、error、formula + cached scalar、sharedStrings index 和 opaque style index；sharedStrings 不整表加载或解析，rich inline text、formula metadata 和其他不稳定投影会明确失败。回调中的字符串是 borrowed view，超出 callback 前必须复制；XML token window 与 active-cell decoded text 分别受 `WorksheetReaderOptions` 限制。
...
- `WorkbookReader` 是 read-only forward traversal，不提供 seek、sharedStrings value resolution、worksheet mutation 或与 `WorkbookEditor` / `WorksheetEditor` 的隐式 handoff。Shared-string/style index 只做语法与关系存在性检查，不验证对应 table 的 index 上界。
