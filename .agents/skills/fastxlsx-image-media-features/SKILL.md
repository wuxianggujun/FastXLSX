...
- 图片 OpenXML 插入/编辑按当前事实源分层推进；当前只完成 streaming-only
...
- FastXLSX 自己负责 OpenXML / OPC 语义：media part、drawing part、drawing
  ...
  preservation。
- 当前内部 `PackageEditor` linked-object fixture 已有 ordinary drawing replacement
  回归：只重写 `xl/drawings/drawing1.xml` 时，drawing `.rels` 作为 copy-original
  package-entry audit 记录，chart/media/unknown extension entries 保持 copy-original
  baseline。这只是 existing-package copy-original evidence，不是 drawing mutation、
  图片编辑、图表编辑或 full object preservation。
- 当前内部 `PackageEditor` linked-object fixture 还覆盖 ordinary VML drawing
  replacement：只重写 `xl/drawings/vmlDrawing1.vml` 时，worksheet `.rels` 中的
  URI-qualified `vmlDrawing` relationship、VML content type override 和其它
  linked entries 保持 copy-original baseline。这不是 VML shape editing、legacy
  drawing mutation、relationship repair 或 full VML/drawing support。
- 当前内部 `PackageEditor` linked-object fixture 还覆盖 ordinary percent-decoded
  drawing replacement：只重写 `xl/drawings/drawing space.xml` 时，worksheet
  `.rels` 中原始 `../drawings/drawing%20space.xml` relationship、drawing content type
  override 和其它 linked entries 保持 copy-original baseline。这不是
  percent-encoded target repair、relationship rewrite、drawing mutation 或 full drawing support。
- 当前内部 `PackageEditor` linked-object fixture 还覆盖 ordinary media replacement：
  只重写 `xl/media/image1.png` 时，drawing `.rels`、PNG default content type、
  workbook/worksheet/drawing/chart/unknown extension entries 保持 copy-original
  baseline。这只是 existing-package media part rewrite / relationship preservation
  evidence，不是图片解码、drawing mutation、existing-workbook image editing 或完整图片保真。
- 当前内部 `PackageEditor` linked-object fixture 还覆盖 explicit media removal：
  显式移除 default-typed `xl/media/image1.png` 时，输出省略 media entry，保留
  PNG default content type 和 drawing `.rels` inbound image relationship，不凭空创建
  media owner `.rels` omission。这只是 no-pruning / preservation evidence，
  不是 relationship repair、object deletion、existing-workbook image editing 或完整图片保真。
- 当前内部 `PackageEditor` linked-object fixture 还覆盖 media ordering：
  default-typed media 先显式移除再 ordinary replacement 会恢复 active media part、
  清理 stale removed-part audit、保留 PNG default content type 且不提升成 override；
  先 ordinary replacement 再显式移除会清理 active media replacement、记录
  removed-part audit 和 inbound drawing relationship metadata，输出省略
  `xl/media/image1.png`，保留 inbound drawing `.rels`，且不凭空创建 media owner
  `.rels`。这只是顺序互斥和 no-pruning / preservation evidence，不是事务式 undo、
  图片语义合并、relationship pruning/repair、content type repair、
  existing-workbook image editing 或完整图片保真。
...
6. 读取或编辑已有 workbook 图片时，先证明未知和未修改 media/drawing/table/chart/VBA parts
   能保留。
...
1. 依赖发现和图片元数据 / 像素 helper：验证默认 `stb` 依赖的 vcpkg 解析、
   include 路径、license、CMake package/include 行为、CI 安装成本和失败模式。当前
   已有默认 `stb` 依赖、`read_image_info()` 和 `read_image_pixels(path/span)`；
   前者只代表 PNG/JPEG 格式、尺寸和通道读取，后者只代表 caller 显式请求时的
   owned decoded pixel buffer。
2. 插入/编辑 API 设计和文档注释：先声明 Streaming、Patch 或 In-memory 模式，写清原始图片
...
3. New-workbook-only 插入：当前基础切片是
...
4. 验证闭环：当前 `fastxlsx.streaming` 已覆盖 media/drawing/rels/content
...
5. Existing-workbook 图片读取、编辑或保真：只在 package reader/writer 和保真
   fixtures 证明未修改 media/drawing/table/chart/VBA/unknown parts 保留后推进。
...
- 不要从已知 part 全量重建 package 导致已有图片、drawing、table、chart、VBA 或未知扩展丢失。
...
- 图片元数据 / 像素 helper：默认构建下 `fastxlsx.image` 读取 PNG/JPEG 文件和内存
...
- 图片插入基础：默认构建下验证 PNG/JPEG media、drawing、rels、
...
- 保真测试：编辑无关 part 后，确认未修改 media/drawing/table/chart/VBA/unknown parts 仍存在且
  relationships 有效。
