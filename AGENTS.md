...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `ThreeColorScaleRule`、`TableOptions`、`TableTotalsFunction`、`TableOptions::show_totals_row`、
...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  tables / two-/three-color conditional color scales / streaming number-format styles /
...
- 当前 `fastxlsx.streaming` conditional color scale 推荐 preset 输出样例为
  ...
  two-color、three-color 与 multi-range 样例并核对 color scale 可见性。不要把它扩展成完整
...
- 把 `WorksheetWriter::add_conditional_color_scale()` 写成完整 conditional formatting
  或 styles/dxfs 支持。当前只写 streaming-only new-workbook two-/three-color color scale 的
