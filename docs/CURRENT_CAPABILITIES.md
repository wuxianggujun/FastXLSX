...
- 最新 Patch schema-v14/v8 双顺序 A/B 使用与 schema-v13 baseline byte-identical 的三类 1,000,000-cell source。Bounded staged-file prefetch 在六组 optimized matrix 中全部生效，前台 input-read median 降低 78.75%–91.79%，completion wait 为 62–135 us；固定 buffer 总量为 2 MiB，paired process peak working set median 变化不超过 0.02734 MB。Target-entry median 在六组中的五组下降，reverse numeric 因 writer wall-time 波动而回退，因此不形成通用 target-entry/total-editor、物理 OS cache-hit、任意 workbook、Office 或全功能性能承诺。
