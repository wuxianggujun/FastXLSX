...
- `src/worksheet_image_reader.cpp`
...
- `tests/test_worksheet_image_reader.cpp`
...
`read_worksheet_hyperlinks()` 是另一条独立 bounded worksheet-root companion：按 source order 投影 owning A1 range、internal location 或 owner-local external relationship target 与 display/tooltip。Internal 不要求 `.rels`；external 必须匹配标准 hyperlink relationship type、`TargetMode="External"` 和非空 target，relationship id 保持 internal。保持 XML window、nesting、hyperlink/ref/id/target/metadata-text guardrail；审计唯一 container/direct child、QName/namespace scope、suffix schema、恰好一个 location/id、unsupported child/attribute 与 duplicate/overlap。失败前允许 partial callbacks，callback exception 原样传播且 entry 可从头 retry；禁止 target reachability、relationship repair、OPC mutation 或 Patch/In-memory handoff。
...
`read_worksheet_comments()` 是独立 bounded classic comments/notes companion：跟随 worksheet-local 唯一 standard internal comments relationship，以 fresh comments-part source 投影 owning one-based row/column、resolved author 与 simple text。保留受限 author table 和 cell-reference 去重集合；限制 XML/nesting/comment/author/per-author/aggregate-author/text/ref/authorId/shapeId/id/target，审计 target part/content type、authorId bounds、optional numeric shapeId 与 optional direct legacy VML relationship/part presence。Rich/phonetic/extension 与 worksheet-local threaded-comment relationship 明确 fail；persons part 与 VML payload/visibility/shape 不跟随、不投影。失败前允许 partial callbacks 且可从 fresh worksheet/comments entry retry；禁止完整 comments model、OPC mutation 或 Patch/In-memory handoff。
