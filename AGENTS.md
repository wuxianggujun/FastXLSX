...
- `tests/test_package_editor.cpp` 通过 CTest 注册为 `fastxlsx.package_editor`，
  ...
  replacement payload preflight，验证 missing selector、非 cell root、缺失未命名空间
  `r`、qualified-only `r` 和 selector / `r` mismatch 失败不污染 `EditPlan`、
  manifest、payload audits、calc policy 或 planned output；当前还覆盖
  ...
  handoff / staged payload foundation 回归，不是
