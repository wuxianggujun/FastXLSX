...
- Patch 已覆盖 catalog、事务式空白 worksheet add、关系闭合 worksheet remove、sheetData/cell replacement、worksheet-local internal/external hyperlink、conservative basic classic note add/update/remove、data validation、writer-compatible table add/update/remove lifecycle、worksheet-root auto-filter set/clear、merged-cell exact merge/unmerge 与 primary-view freeze-pane set/clear、窄 rename、formula audit/recalculation、core/app properties 和 media bytes replacement。Internal hyperlink 只改 worksheet XML；external hyperlink 同步 worksheet XML 与 worksheet `.rels`；classic note source edit 仅接管 writer-canonical pair；table lifecycle 复用 Streaming `TableOptions`，update 保留 package identity，remove 关闭 owner-local graph，但不检查 cell payload 或做 structural sync；其他 metadata 保持各自窄边界。当前没有 public existing-workbook worksheet clone。新增 worksheet 可同会话用 Patch 填充/rename/hyperlink/note/data validation/table/auto-filter/merged-cell/freeze-pane，但需保存重开后才能 In-memory materialize。
