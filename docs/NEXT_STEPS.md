...
`insert_columns()`, exact and path-equivalent source overwrite attempts both
preserve dirty diagnostics under `RenamedData`, keep the source workbook
unchanged under `Data`, leave the first renamed output isolated, and a safe
retry reopens only as `RenamedData` with the combined shifted coordinates.
