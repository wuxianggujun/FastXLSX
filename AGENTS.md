...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  tables / two-color conditional color scales / streaming number-format styles /
...
`CellView` 流式写入骨架已存在；公式、行高、列宽、冻结窗格、自动筛选、
  合并单元格、data validations、external/internal hyperlinks、two-color conditional
  color scales 和 streaming-only tables 是当前写入骨架能力，不等同完整 Phase 3
  或完整 Phase 5。当前可见
...
existing-file style preservation 或完整 Excel formatting parity。当前 two-color
conditional color scale 是 worksheet metadata 切片，不走 styles registry 或 `dxfs`。
...
- Phase 5 worksheet metadata：基础。`WorksheetWriter::add_data_validation()`
  ...
  two-color color scale 切片，会把 `TwoColorScaleRule` 写成 worksheet-local
  `<conditionalFormatting><cfRule type="colorScale">` XML；`ArgbColor` 序列化为
  8 位大写 ARGB，`priority` 按同一 worksheet 内调用顺序从 1 开始分配，
  多区域 overload 会把 `CellRange` 列表复制进 writer state 并写成空格分隔
  `sqref`。该切片不生成 `styles.xml`、`dxfs`、worksheet `.rels`、workbook
  relationships、content type entries、cell text 或 `<calcPr>`；不支持 dataBar、
  iconSet、cellIs/formula/top/bottom/duplicate/unique 等完整 conditional formatting
...
- 当前 `fastxlsx.streaming` tables 推荐 preset 输出样例为
  ...
  各自匹配对应 `.rels` entry。
- 当前 `fastxlsx.streaming` conditional color scale 推荐 preset 输出样例为
  ...
  基础与 multi-range 样例并核对 two-color scale 可见性。不要把它扩展成完整
  conditional formatting、dxf/styles、公式规则、dataBar/iconSet、existing-file editing
  或 Excel UI parity。
...
- 把 `WorksheetWriter::add_conditional_color_scale()` 写成完整 conditional formatting
  或 styles/dxfs 支持。当前只写 streaming-only new-workbook two-color color scale 的
  worksheet-local XML；不支持公式规则、data bars、icon sets、dxf 样式、冲突检测、
  existing-file editing 或完整 Excel UI。
