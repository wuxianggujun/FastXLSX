...
- OPC 当前已有内部 manifest / relationships / `PartIndex` / `RelationshipGraph`
  / content type registry 基础；Phase 5 仍是规划，不能宣称完整图片、VBA 或
  table 支持。
- `src/package_writer.*` 是内部 package writer boundary；当前 backend 仍是
  `src/zip_store_writer.*` bootstrap ZIP writer，只写 stored/no-compression ZIP。
