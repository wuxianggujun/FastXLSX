...
- Streaming：`WorkbookWriter` / `WorksheetWriter` / `CellView` 用于大型有序新建；`WorkbookReader` 用于已有 worksheet row/cell、sharedStrings item、styles/cellXfs 与 narrow font/fill component 的 forward-only bounded callback traversal。
...
- Public `WorkbookReader` 已覆盖 bounded row/cell callback、typed scalar、simple inline text、shared/style index、formula + cached scalar 分离，以及独立 simple sharedStrings item、styles/cellXfs 与 writer-compatible font/fill component traversal。Companions 审计 relationship target/content type，使用 callback-lifetime borrowed text 或 owning scalar、显式 guardrail 与 stored/DEFLATE entry retry；cellXfs component ids 保持 workbook-local，由 caller 显式关联。Style components 只投影 bold/italic/direct ARGB 与 none/gray125/solid fill，非默认 font/theme、其他 color/pattern/gradient 明确 fail。它不 seek、不构建完整 sharedStrings/styles table/registry、不自动解析 worksheet index、不 materialize worksheet。公式不求值、不生成 cached result、不完整重建 calcChain。
