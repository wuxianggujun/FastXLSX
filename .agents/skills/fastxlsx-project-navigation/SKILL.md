...
- Patch 已覆盖 catalog、事务式空白 worksheet add、关系闭合 worksheet remove、sheetData/cell replacement、worksheet-local internal/external hyperlink、conservative basic classic note add/update/remove、data validation、worksheet-root auto-filter set/clear、merged-cell exact merge/unmerge 与 primary-view freeze-pane set/clear、窄 rename、formula audit/recalculation、core/app properties 和 media bytes replacement。Internal hyperlink 只改 worksheet XML；external hyperlink 同步 worksheet XML 与 worksheet `.rels`；classic note add 在无 source comments/VML 时生成 linked parts，update/remove 仅接管 comments/VML serializers exact 匹配的 writer-canonical source pair；data validation、auto-filter、merged-cell 与 freeze-pane 保持各自窄边界。这些 metadata 不随 structural mutation 自动同步。当前没有 public existing-workbook worksheet clone。新增表可同会话用 Patch 填充/rename/hyperlink/note/data validation/auto-filter/merged-cell/freeze-pane，但需保存重开后才能 In-memory materialize。
...
- Tables、writer-compatible picture anchors 与 classic comments/notes 已有窄 read projection；classic note 另有 new-workbook Streaming write 和 conservative existing-workbook Patch add/update/remove。Patch source edit 仅支持 writer-canonical comments/VML pair，不接受 rich/threaded、custom shape 或 Excel/unknown VML，因此完整 existing-object table/drawing/comments semantic edit 仍未公开。Charts、VBA、pivot、external links 和 custom XML 默认只可 preserve/audit/fail；不能因读取、新建或保留测试宣称 existing-object semantic edit。
