...
description: "实现或审查 FastXLSX 样式注册表、StyleId、CellAlignment、CellStyle、WorkbookWriter::add_style()、CellView::with_style()、xl/styles.xml、number formats、wrap-text alignment、worksheet s 属性、sharedStrings + styles 共存和样式 QA。用于 P9 styles、未来 font/fill/border/full-alignment、conditional formatting 与 styles 交互，或排查 Excel 修复样式输出。"
---
...
- 当前 P9 基础切片支持 streaming-only new-workbook custom number format styles 和窄
  wrap-text alignment styles。
- `StyleId` 是 workbook-local handle；默认构造是 style `0`。非默认 id 必须来自同一个
  `WorkbookWriter::add_style()`。
- `CellAlignment` 当前只支持 `wrap_text`；`CellStyle` 当前包含 `number_format` 和可选
  `alignment`。完全空的 style 会被拒绝；`alignment` 为空或 `wrap_text=false` 不贡献
  style 属性。
- 重复完整 style 复用同一个 `StyleId`；相同 number format 在不同 style 组合中复用同一个
  custom `numFmtId`。number format 字符串只按精确文本匹配，不做 Excel 语义规范化。
...
1. 判断任务是修补现有 number format / wrap-text slice，还是新增 font/fill/border/full-alignment 等后续 slice。
...
公式、NumberFormat 和 WrapText。这些是本地 QA，不是运行时依赖，也不是默认 CI 强制项。

## 禁止事项

- 不要把 P9 写成完整 styles 或 Excel formatting parity。
- 不要声称支持 font、fill、border、full alignment、rich text、dxf-backed conditional formatting、
