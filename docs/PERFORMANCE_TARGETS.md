...
当前 benchmark executable 使用 schema v5：Streaming 分列 generation、package close、million-cells/s、body buffer peak/flush count 与 close 后 active temporary file count；Patch 分列 direct-range/single-pass transform telemetry，以及 raw compressed-copy entry names/count/bytes。历史 schema-v4 artifact 仍可汇总，但不能提供新增资源生命周期或 raw-copy 证据。
...
- 九个 bundle 都不是跨机器、跨数据规模或泛化“高性能/低内存”证据；每个 bundle 只支持其 manifest 中记录的同机同数据集结论。历史 schema-v4 继续作为回归基线，release 当前值以 schema-v5 bundle 为准。
...
- 最新 schema-v5 targeted replace total/mutation/save median 为 1844/596/1249 ms、peak 为 8.41406 MB。Single-pass targeted upsert 为 3821/2327/1494 ms、transform median 为 2132 ms、peak 为 8.55078 MB；相对同协议旧 upsert 的 total/mutation 4353/2968 ms 降低约 12.2%/21.6%。Targeted replace/upsert 仍各重写完整 logical worksheet XML。
...
- Targeted replace/upsert 仍重写约 34.9 MB logical worksheet XML；level 1/6 median save 为 221/1157 ms 与 302/1128 ms，level 1 输出约增大 9.7%。下一轮 profiling 优先拆分 XML transform、temporary IO、chunk assembly 与 DEFLATE encode，不以扩大 In-memory guardrail 代替热路径优化。
