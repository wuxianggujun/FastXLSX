...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  `WorksheetWriter`、`CellView`、`StyleId`、`CellAlignment`、`CellStyle`、`DocumentProperties`、`DataValidationRule`、
...
- 当前写出基础可配置 `docProps/core.xml` 和 `docProps/app.xml` 小型 XML part；
  ...
  public API。
- 当前已有 P9 streaming-only number-format + wrap-text alignment styles 基础切片：
  `StyleId` 是 workbook-local handle，`CellStyle` 当前支持 `number_format` 和可选
  `CellAlignment::wrap_text`，`WorkbookWriter::add_style()`
  ...
  font/fill/border/full alignment、date cell type、dxf-backed conditional formatting、
