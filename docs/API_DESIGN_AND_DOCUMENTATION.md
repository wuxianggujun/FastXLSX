...
- Mutation semantics：`set_cell(ref, value)` 覆盖该 cell 的 active value；
  `erase_cell(ref)` 删除 active record；`CellValue::blank()` 表示 caller 明确写入
  blank / clear 候选值。blank 是否写出空 `<c>`、是否保留 style、是否生成
  tombstone，要等 save-as handoff 固化后再实现。
...
- P8.509 materializes source `t="str"` cells through the same public
  ...
  formulas as `<f>` without cached values. This is not date/error cell
  materialization, formula evaluation, cached-result preservation,
...
- P8.513 pins guardrail recovery after `erase_cell()`: exact `max_cells` and
  ...
  not tombstones, style-preserving clear, row/column delete, metadata/range
  sync, workbook-level budgeting, or large-file random editing.
...
- P8.518 pins representative custom/unknown source cell type failure hygiene:
  ...
  fallback, date/error support, or metadata migration.
...
- P8.466 normalizes source explicit default style attributes: selected
  ...
  materializes canonical non-zero unsigned decimal source style ids and writes
  the same numeric ids back when the source styles part is preserved. This is
  not style validation, style id migration, style merge, stylesheet semantic
...
- Mutation semantics: `set_cell()` replaces the active value; `erase_cell()`
  removes the sparse record; `CellValue::blank()` writes an explicit blank
  replacement cell. Tombstones, style-preserving clear, row insertion/deletion,
  and metadata edits are deferred.
...
| Source dependency QA addenda P8.563 | P8.563 adds `generated_shared_formula_boundary_materialization` to the opt-in workbook-editor QA runner. The generated source drives public `WorksheetEditor::try_cell()` and dirty save through shared formula definitions/followers containing sheet-qualified A1 refs, quoted sheet names, quoted strings, bracketed external tokens, structured-reference-like text, whole-row/whole-column refs, function names, name-like tokens, and R1C1-like text. The ZIP/XML and `openpyxl` checks prove the dirty output has 4 ordinary formula elements and 0 shared formula metadata elements while preserving the documented narrow translation boundaries. | This is generated QA evidence only; it does not broaden formula parsing, evaluate formulas, preserve shared formula metadata, validate sheet/external references, build a formula dependency graph, rebuild calcChain, repair relationships, migrate sharedStrings/styles, or add default CI fixture data. |
...
| Source dependency addenda P8.567 | P8.567 treats formula text as authoritative for default/numeric, `t="str"`, and `t="b"` cached-result formula cells in the public `WorkbookEditor` source path. These cells materialize as `CellValue::formula(...)`, read-only materialization stays clean, and dirty projection writes plain `<f>...</f>` cells without preserving stale cached `<v>` values. | This is source formula import compatibility only; it does not add error/date cell import, formula evaluation, cached result generation or preservation, calcChain rebuild, shared/array formula metadata preservation, sharedStrings/styles migration, relationship repair, or a complete Excel formula parser. Inline-string and shared-string formula cell shapes remain unsupported. |
...
- **Supported source values:** blank, number, boolean, `t="str"` scalar text,
  formula text without stale cached values (including default/numeric,
  `t="str"`, and `t="b"` cached-result formula cells), plain inline strings,
  simple inline rich text flattened to plain text, and workbook-backed `t="s"`
...
- **Strict failure groups:** non-default or malformed source styles,
  unsupported cell types such as error `t="e"`, date-like `t="d"`, or
  custom/unknown `t="z"` tokens, unsupported metadata,
...
- The first mutation API must keep the current internal semantics:
  ...
  Do not add tombstones, style-preserving clear, row/column insertion, or
  metadata edits in the first slice.
...
3. Freeze style and dependency policy:
   ...
   - Caller-supplied non-default `StyleId` values still fail in
     ...
     source style ids now materialize as numeric passthrough handles and are
     written back when the source styles part is preserved. Explicit
...
- 当前 internal F2.3 smoke 还覆盖了 source-backed 路径：从 `PackageReader`
  ...
  source style ids 只做 numeric passthrough，不做 styles.xml validation /
  migration / merge。workbook-backed valid source sharedStrings indexes now
