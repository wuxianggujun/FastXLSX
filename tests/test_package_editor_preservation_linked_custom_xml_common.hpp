...
// Common helpers for PackageEditor preservation linked custom XML tests.
...
bool is_package_editor_shard(std::string_view shard)
{
    return shard == "all" || shard == "preservation-linked-custom-xml"
        || shard == "preservation-linked-custom-xml-properties"
        || shard == "preservation-linked-custom-xml-ordering";
}
