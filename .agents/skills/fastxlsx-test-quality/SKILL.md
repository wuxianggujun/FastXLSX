...
- 当前 streaming data validations 测试覆盖 `count`、`sqref`、`type`、`operator`、
  ...
  `stop` / `warning` / `information` error styles、无 `styles.xml` / content type
  side effects、multi-area `sqref` token / `count` 语义、空 range list 和 multi-range
  内非法 range 拒绝，以及 mutation-after-close。
...
- 当前 streaming tables 测试覆盖 `xl/tables/tableN.xml`、worksheet `<tableParts>`、
  ...
  ranges/options、table style flags 且不生成 `xl/styles.xml`、duplicate names 和
  mutation-after-close。
- 当前 image 测试覆盖默认无 `stb` 构建下 `read_image_info()` 的明确错误，以及
  opt-in `FASTXLSX_ENABLE_STB=ON` 构建下 PNG/JPEG 文件/内存尺寸和通道读取、
  unsupported memory/file header、empty memory buffer、empty file 和 missing file。
  当前 streaming 测试还覆盖默认无 `stb`
  时 `WorksheetWriter::add_image()` 明确失败，以及 opt-in image preset 下
  PNG/JPEG media、drawing、rels、content types、worksheet `<drawing>` 结构。当前还
  覆盖 `ImageOptions` drawing metadata：`xdr:cNvPr name` / `descr`、XML attribute
  escape、空 description 省略、默认 `Picture N` 名称，以及无额外 relationship /
  content type / media side effects。
- 当前没有 Catch2 集成。
...
- worksheet metadata：列宽、冻结窗格、自动筛选、合并单元格、data validations、
  external/internal hyperlinks 和 tables。
- dimension tracking。
- Excel 行/列边界拒绝路径；行上限测试不要真实写满 1,048,576 行进默认 CTest。
- row buffer 复用不变量。
...
5. 如果涉及 data validations，重点对比 worksheet XML 中的 `<dataValidations>`、
   ...
   `prompt`、`errorTitle`、`error` 等 prompt/error attributes。multi-area `sqref`
...
Python helper 检查 package XML、prompt/error attributes、无 `.rels` / `styles.xml`
/ content type 副作用，并创建 `openpyxl` / `XlsxWriter` 参考 workbook；它还检查
multi-range 样例的 package XML、`sqref` token、`count` 语义和 `openpyxl` 多区域读取，
并创建 `openpyxl` multi-range 参考 workbook。Excel helper 只读打开 workbook 并核对
`ValidationPrompt!A2:D2` 的 validation 属性，同时确认 multi-range 样例中 A/C/E 三段
validation areas。Excel COM 会把 custom validation 公式返回为带 `=` 的形式，结构语义
仍以拆包后的 worksheet XML 为准。
...
- 如果涉及 image metadata，还要检查 drawing XML 中 `xdr:cNvPr name` / `descr`、
  XML attribute escape、空值省略和默认名称规则。
...
image metadata 验证结果还应记录 shape count、custom/default shape names 和
`AlternativeText`；但结构语义仍以拆包后的 drawing XML 为准，不要宣称完整 UI parity。
