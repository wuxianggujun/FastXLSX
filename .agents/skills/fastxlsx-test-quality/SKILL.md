...
- 当前 `fastxlsx.worksheet_transformer` 还覆盖 targeted-cell Patch 的 completed-target
  ...
  slicer prototype 的 staged source splicing / missing target / stale range / stale
  staged-size rejection 边界。`fastxlsx_bench_package_editor_cell_replacement` 还提供
  opt-in `--rewrite-strategy indexed-staged` schema-v2 benchmark/prototype 路径，
  用于验证 benchmark 生成的 staged chunks + prebuilt index。它不是 public Patch
  默认算法、PackageEditor source-entry ZIP seek、metadata repair 或 Office 兼容性证明；
...
worksheet output bytes；这仍是手工 opt-in benchmark，不进默认 CTest/CI，也不是 public
Patch 默认算法或 source-entry ZIP seek 证明。
