...
- `WorksheetWriter::add_table()` 是 Streaming metadata API：range、table name、
  column names、style flags、`show_totals_row` 和 `column_totals_functions` 被复制进
  ...
  它不计算 totals、不生成公式文本、`totalsRowLabel`、样式、table resize 或
...
不生成 `styles.xml`、只支持 totals-row visibility metadata 和 caller-supplied
`totalsRowFunction` attributes、无 `totalsRowLabel` / 公式生成、无 table resize、无
existing-file editing，以及不代表完整 table 支持。
