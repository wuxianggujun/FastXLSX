...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  `include/fastxlsx/streaming_writer.hpp` 和 `include/fastxlsx/image.hpp`
  提供当前 public API：
...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  data validations / external hyperlinks / tables XML 输出。
...
- `stb`：Phase 5 图片读取/插入中的图片解码、尺寸和像素读取；当前只通过
  ...
  `read_image_info()` 图片元数据 helper，不属于默认构建。
- `Catch2`：单元测试。
...
验证 opt-in stb 图片元数据 helper 时，确保 `VCPKG_ROOT` 指向目标 vcpkg 根目录后运行：
