...
foundation；P8.13 已落地 internal worksheet replacement chunk handoff。后续真正 package-entry staged
...
payload preflight、chunked replacement source foundation、worksheet chunk handoff 或
...
可并行性：
- 后续 `replace_worksheet_cells()` staged output handoff 和 true file-backed worksheet
  transformer 必须串行推进。
...
- 文档明确这只是 internal worksheet replacement chunk handoff，不是 public API、
  low-memory validation/audit、cell replacement staged output、full worksheet stream writer、
