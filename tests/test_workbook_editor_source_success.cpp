...
std::string source_success_snapshot_prefix(
...
void check_source_success_row_snapshots(
    ...
    std::string_view stage)
{
    ...
    const std::string prefix = source_success_snapshot_prefix(scenario, stage);
...
void check_source_success_column_snapshots(
    fastxlsx::WorksheetEditor& sheet,
    std::span<const ReopenedSourceSuccessCell> expected_cells,
    std::string_view scenario,
    std::string_view stage)
    ...
    for (const ReopenedSourceSuccessCell& expected : expected_cells) {
        ...
        for (const ReopenedSourceSuccessCell& candidate : expected_cells) {
            ...
            check(source_success_snapshot_matches(column_cells[index], candidate),
                prefix + " column_cells should preserve row-major values");
            ++index;
        }
    }
}

void check_source_success_snapshots(
    fastxlsx::WorksheetEditor& sheet,
    ...
    std::string_view stage)
{
    ...
    check(sheet.cell_count() == expected_cells.size(),
        prefix + " should preserve the expected sparse cell count");
    ...
    if (actual_cells.size() == expected_cells.size()) {
        for (std::size_t index = 0; index < expected_cells.size(); ++index) {
            check(source_success_snapshot_matches(actual_cells[index], expected_cells[index]),
                prefix + " sparse_cells should preserve row-major values");
        }
    }

    check_source_success_row_snapshots(sheet, expected_cells, scenario, stage);
    check_source_success_column_snapshots(sheet, expected_cells, scenario, stage);

    for (const ReopenedSourceSuccessCell& expected : expected_cells) {
        const fastxlsx::CellValue actual =
            sheet.get_cell(expected.row, expected.column);
        check(source_success_values_equal(actual, expected.value),
            prefix + " should read each expected cell directly");
    }
}
    ...
    std::string_view scenario)
{
    ...
    check_source_success_snapshots(
        reopened_sheet, expected_range, expected_cells, scenario, "fresh reopen");
...
void test_public_worksheet_editor_structural_shift_source_supported_values()
{
    ...
    const auto run_scalar_shift_case =
        [&](std::string_view artifact_suffix,
            ...
            check_source_success_snapshots(
                sheet, expected_range, expected_cells, scenario, "live shifted");
            check(sheet.has_pending_changes(),
...
void test_public_worksheet_editor_structural_shift_source_scalar_string_cells()
{
    const auto write_string_shift_source =
        [](std::string_view name, std::string_view sheet_data) {
            const std::filesystem::path source = write_two_sheet_source(name);
            const std::string worksheet_xml =
                std::string(R"(<?xml version="1.0" encoding="UTF-8"?>)")
    ...
    const auto run_string_shift_case =
        [&](std::string_view artifact_suffix,
            std::string_view scenario,
            const std::filesystem::path& source,
            std::size_t source_cell_count,
            ...
            const std::string scenario_text(scenario);
            const std::filesystem::path output = artifact(
                "fastxlsx-workbook-editor-public-structural-shift-source-string-type-"
                + artifact_suffix_text + "-output.xlsx");
            const std::filesystem::path noop_output = artifact(
                "fastxlsx-workbook-editor-public-structural-shift-source-string-type-"
                + artifact_suffix_text + "-noop-output.xlsx");
            ...
            check(editor.has_pending_changes(),
                scenario_text + " should dirty WorkbookEditor");
            check(sheet.cell_count() == expected_cells.size(),
                scenario_text + " should expose the shifted t=str cell count");
            check_source_success_snapshots(
                sheet, expected_range, expected_cells, scenario, "live shifted");
            check(sheet.has_pending_changes(),
                scenario_text + " live shifted reads should keep Data dirty");
            ...
            for (std::string_view expected_xml_fragment : expected_xml_fragments) {
                check_contains(output_worksheet_xml, expected_xml_fragment,
                    scenario_text + " save should write shifted t=str cells as inline text");
            }
            for (std::string_view omitted_token : omitted_tokens) {
                check_not_contains(output_worksheet_xml, omitted_token,
                    scenario_text + " save should omit deleted source t=str cells");
            }
            check_not_contains(output_worksheet_xml, R"(t="str")",
                scenario_text + " save should not preserve source t=str tokens");
            check(output_entries.find("xl/sharedStrings.xml") == output_entries.end(),
                scenario_text + " save should not introduce sharedStrings");
            check(fastxlsx::test::read_zip_entries(source) == source_entries,
                scenario_text + " save should not mutate the source package");
            ...
            for (std::string_view expected_xml_fragment : expected_xml_fragments) {
                check_contains(reopened_worksheet_xml, expected_xml_fragment,
                    scenario_text + " fresh-reopen save should keep shifted t=str cells");
            }
            check_contains(reopened_worksheet_xml, reopened_edit_xml_fragment,
                scenario_text + " fresh-reopen save should include the later edit");
            check_not_contains(reopened_worksheet_xml, R"(t="str")",
                scenario_text + " fresh-reopen save should keep regenerated cells free of t=str");
            check(reopened_entries.find("xl/sharedStrings.xml") == reopened_entries.end(),
                scenario_text + " fresh-reopen save should still avoid sharedStrings");
            check(fastxlsx::test::read_zip_entries(noop_output) == output_entries,
                scenario_text + " fresh-reopen save should not mutate its saved input");
    ...
    constexpr std::string_view insert_source_sheet_data =
        R"(<sheetData><row r="1">)"
        R"(<c r="A1" t="str"><v>alpha &amp; &lt;one&gt;</v></c>)"
        R"(<c r="C1" t="str"><v>gamma</v></c>)"
        R"(</row><row r="2">)"
        R"(<c r="B2" t="str"><v>beta</v></c>)"
        R"(</row></sheetData>)";
    const ReopenedSourceSuccessCell insert_rows_expected[] = {
        {2, 1, fastxlsx::CellValue::text("alpha & <one>")},
        {2, 3, fastxlsx::CellValue::text("gamma")},
        {3, 2, fastxlsx::CellValue::text("beta")},
    };
    run_string_shift_case("insert-rows",
        "structural insert_rows source t=str scalar shift",
        write_string_shift_source(
            "fastxlsx-workbook-editor-public-structural-insert-rows-source-string-type-source.xlsx",
            insert_source_sheet_data),
        3,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.insert_rows(1, 1); },
        ...
        insert_rows_expected,
        {
            R"(<c r="A2" t="inlineStr"><is><t>alpha &amp; &lt;one&gt;</t></is></c>)",
            R"(<c r="C2" t="inlineStr"><is><t>gamma</t></is></c>)",
            R"(<c r="B3" t="inlineStr"><is><t>beta</t></is></c>)",
        },
        {},
        fastxlsx::CellRange {2, 1, 4, 4},
        ReopenedSourceSuccessCell {4, 4, fastxlsx::CellValue::text("later tstr & <row>")},
        R"(<c r="D4" t="inlineStr"><is><t>later tstr &amp; &lt;row&gt;</t></is></c>)");

    const ReopenedSourceSuccessCell insert_columns_expected[] = {
        {1, 2, fastxlsx::CellValue::text("alpha & <one>")},
        {1, 4, fastxlsx::CellValue::text("gamma")},
        {2, 3, fastxlsx::CellValue::text("beta")},
    };
    run_string_shift_case("insert-columns",
        "structural insert_columns source t=str scalar shift",
        write_string_shift_source(
            "fastxlsx-workbook-editor-public-structural-insert-columns-source-string-type-source.xlsx",
            insert_source_sheet_data),
        3,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.insert_columns(1, 1); },
        ...
        insert_columns_expected,
        {
            R"(<c r="B1" t="inlineStr"><is><t>alpha &amp; &lt;one&gt;</t></is></c>)",
            R"(<c r="D1" t="inlineStr"><is><t>gamma</t></is></c>)",
            R"(<c r="C2" t="inlineStr"><is><t>beta</t></is></c>)",
        },
        {},
        fastxlsx::CellRange {1, 2, 3, 5},
        ReopenedSourceSuccessCell {3, 5, fastxlsx::CellValue::text("later tstr & <column>")},
        R"(<c r="E3" t="inlineStr"><is><t>later tstr &amp; &lt;column&gt;</t></is></c>)");

    constexpr std::string_view delete_rows_source_sheet_data =
        R"(<sheetData><row r="1">)"
        R"(<c r="A1" t="str"><v>deleted-row-tstr</v></c>)"
        R"(</row><row r="2">)"
        R"(<c r="A2" t="str"><v>alpha &amp; &lt;one&gt;</v></c>)"
        R"(<c r="C2" t="str"><v>gamma</v></c>)"
        R"(</row><row r="3">)"
        R"(<c r="B3" t="str"><v>beta</v></c>)"
        R"(</row></sheetData>)";
    const ReopenedSourceSuccessCell delete_rows_expected[] = {
        {1, 1, fastxlsx::CellValue::text("alpha & <one>")},
        {1, 3, fastxlsx::CellValue::text("gamma")},
        {2, 2, fastxlsx::CellValue::text("beta")},
    };
    run_string_shift_case("delete-rows",
        "structural delete_rows source t=str scalar shift",
        write_string_shift_source(
            "fastxlsx-workbook-editor-public-structural-delete-rows-source-string-type-source.xlsx",
            delete_rows_source_sheet_data),
        4,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.delete_rows(1, 1); },
        ...
        delete_rows_expected,
        {
            R"(<c r="A1" t="inlineStr"><is><t>alpha &amp; &lt;one&gt;</t></is></c>)",
            R"(<c r="C1" t="inlineStr"><is><t>gamma</t></is></c>)",
            R"(<c r="B2" t="inlineStr"><is><t>beta</t></is></c>)",
        },
        {"deleted-row-tstr"},
        fastxlsx::CellRange {1, 1, 3, 4},
        ReopenedSourceSuccessCell {3, 4, fastxlsx::CellValue::text("later tstr & <delete-row>")},
        R"(<c r="D3" t="inlineStr"><is><t>later tstr &amp; &lt;delete-row&gt;</t></is></c>)");

    constexpr std::string_view delete_columns_source_sheet_data =
        R"(<sheetData><row r="1">)"
        R"(<c r="A1" t="str"><v>deleted-column-tstr</v></c>)"
        R"(<c r="B1" t="str"><v>alpha &amp; &lt;one&gt;</v></c>)"
        R"(<c r="D1" t="str"><v>gamma</v></c>)"
        R"(</row><row r="2">)"
        R"(<c r="C2" t="str"><v>beta</v></c>)"
        R"(</row></sheetData>)";
    const ReopenedSourceSuccessCell delete_columns_expected[] = {
        {1, 1, fastxlsx::CellValue::text("alpha & <one>")},
        {1, 3, fastxlsx::CellValue::text("gamma")},
    ...
    run_string_shift_case("delete-columns",
        "structural delete_columns source t=str scalar shift",
        write_string_shift_source(
            "fastxlsx-workbook-editor-public-structural-delete-columns-source-string-type-source.xlsx",
            delete_columns_source_sheet_data),
        4,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.delete_columns(1, 1); },
        fastxlsx::CellRange {1, 1, 2, 3},
        ...
        {"deleted-column-tstr"},
        fastxlsx::CellRange {1, 1, 3, 4},
        ReopenedSourceSuccessCell {3, 4, fastxlsx::CellValue::text("later tstr & <delete-column>")},
        R"(<c r="D3" t="inlineStr"><is><t>later tstr &amp; &lt;delete-column&gt;</t></is></c>)");
}
...
void test_public_worksheet_editor_structural_shift_source_inline_rich_text()
{
    const auto write_rich_shift_source =
        [](std::string_view name, std::string_view sheet_data) {
            const std::filesystem::path source = write_two_sheet_source(name);
            const std::string worksheet_xml =
                std::string(R"(<?xml version="1.0" encoding="UTF-8"?>)")
    ...
    const auto run_rich_shift_case =
        [&](std::string_view artifact_suffix,
            std::string_view scenario,
            const std::filesystem::path& source,
            std::size_t source_cell_count,
            ...
            const std::string scenario_text(scenario);
            const std::filesystem::path output = artifact(
                "fastxlsx-workbook-editor-public-structural-shift-source-inline-rich-"
                + artifact_suffix_text + "-output.xlsx");
            const std::filesystem::path noop_output = artifact(
                "fastxlsx-workbook-editor-public-structural-shift-source-inline-rich-"
                + artifact_suffix_text + "-noop-output.xlsx");
            ...
            const std::filesystem::path reopened_noop_output = artifact(
                ...
                + artifact_suffix_text + "-reopened-noop-output.xlsx");
            const auto source_entries = fastxlsx::test::read_zip_entries(source);
            check_contains(source_entries.at("xl/worksheets/sheet1.xml"), "<rPr>",
                scenario_text + " fixture should contain source inline rich text formatting");
            check_contains(source_entries.at("xl/worksheets/sheet1.xml"), "ignored-rich",
            ...
            check(editor.has_pending_changes(),
                scenario_text + " should dirty WorkbookEditor");
            check(sheet.cell_count() == expected_cells.size(),
                scenario_text + " should expose the shifted rich text cell count");
            check_source_success_snapshots(
                sheet, expected_range, expected_cells, scenario, "live shifted");
            check(sheet.has_pending_changes(),
                scenario_text + " live shifted reads should keep Data dirty");
            ...
            for (std::string_view expected_xml_fragment : expected_xml_fragments) {
                check_contains(output_worksheet_xml, expected_xml_fragment,
                    scenario_text + " save should write shifted rich text as plain inline text");
            }
            for (std::string_view omitted_token : omitted_tokens) {
                check_not_contains(output_worksheet_xml, omitted_token,
                    scenario_text + " save should omit deleted source rich text cells");
            }
            check_not_contains(output_worksheet_xml, "<rPr>",
                scenario_text + " save should flatten rich formatting");
            check_not_contains(output_worksheet_xml, "<rPh",
                scenario_text + " save should omit phonetic markup");
            check_not_contains(output_worksheet_xml, "ignored-rich",
                scenario_text + " save should omit ignored rich metadata text");
            check(output_entries.find("xl/sharedStrings.xml") == output_entries.end(),
                scenario_text + " save should not introduce sharedStrings");
            check(fastxlsx::test::read_zip_entries(source) == source_entries,
                scenario_text + " save should not mutate the source package");
            ...
            for (std::string_view expected_xml_fragment : expected_xml_fragments) {
                check_contains(reopened_worksheet_xml, expected_xml_fragment,
                    scenario_text + " fresh-reopen save should keep shifted rich text cells");
            }
            check_contains(reopened_worksheet_xml, reopened_edit_xml_fragment,
                scenario_text + " fresh-reopen save should include the later edit");
            check_not_contains(reopened_worksheet_xml, "<rPr>",
                scenario_text + " fresh-reopen save should keep rich formatting flattened");
            check_not_contains(reopened_worksheet_xml, "<rPh",
                scenario_text + " fresh-reopen save should keep phonetic markup omitted");
            check_not_contains(reopened_worksheet_xml, "ignored-rich",
                scenario_text + " fresh-reopen save should keep ignored metadata omitted");
            check(reopened_entries.find("xl/sharedStrings.xml") == reopened_entries.end(),
                scenario_text + " fresh-reopen save should still avoid sharedStrings");
            check(fastxlsx::test::read_zip_entries(noop_output) == output_entries,
                scenario_text + " fresh-reopen save should not mutate its saved input");
    ...
    constexpr std::string_view insert_source_sheet_data =
        R"(<sheetData><row r="1">)"
        R"(<c r="A1" t="inlineStr"><is>)"
        R"(<r><rPr><b/></rPr><t>rich-</t></r>)"
        R"(<r><t>A&amp;B</t></r>)"
        R"(<rPh sb="0" eb="1"><t>ignored-rich-phonetic</t></rPh>)"
        R"(<extLst><ext uri="{fastxlsx-test}"><t>ignored-rich-ext</t></ext></extLst>)"
        R"(</is></c>)"
        R"(<c r="C1" t="inlineStr"><is>)"
        R"(<r><t>ga</t></r><r><t>mma</t></r>)"
        R"(<rPh sb="0" eb="1"><t>ignored-rich-gamma</t></rPh>)"
        R"(</is></c>)"
        R"(</row><row r="2">)"
        R"(<c r="B2" t="inlineStr"><is><r><t>be</t></r><r><t>ta</t></r></is></c>)"
        R"(</row></sheetData>)";
    const ReopenedSourceSuccessCell insert_rows_expected[] = {
        {2, 1, fastxlsx::CellValue::text("rich-A&B")},
        {2, 3, fastxlsx::CellValue::text("gamma")},
        {3, 2, fastxlsx::CellValue::text("beta")},
    };
    run_rich_shift_case("insert-rows",
        "structural insert_rows source inline rich text shift",
        write_rich_shift_source(
            "fastxlsx-workbook-editor-public-structural-insert-rows-source-inline-rich-source.xlsx",
            insert_source_sheet_data),
        3,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.insert_rows(1, 1); },
        fastxlsx::CellRange {2, 1, 3, 3},
        insert_rows_expected,
        {
            R"(<c r="A2" t="inlineStr"><is><t>rich-A&amp;B</t></is></c>)",
            R"(<c r="C2" t="inlineStr"><is><t>gamma</t></is></c>)",
            R"(<c r="B3" t="inlineStr"><is><t>beta</t></is></c>)",
        },
        {},
        fastxlsx::CellRange {2, 1, 4, 4},
        ReopenedSourceSuccessCell {4, 4, fastxlsx::CellValue::text("later rich & <row>")},
        R"(<c r="D4" t="inlineStr"><is><t>later rich &amp; &lt;row&gt;</t></is></c>)");

    const ReopenedSourceSuccessCell insert_columns_expected[] = {
        {1, 2, fastxlsx::CellValue::text("rich-A&B")},
        {1, 4, fastxlsx::CellValue::text("gamma")},
        {2, 3, fastxlsx::CellValue::text("beta")},
    };
    run_rich_shift_case("insert-columns",
        "structural insert_columns source inline rich text shift",
        write_rich_shift_source(
            "fastxlsx-workbook-editor-public-structural-insert-columns-source-inline-rich-source.xlsx",
            insert_source_sheet_data),
        3,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.insert_columns(1, 1); },
        fastxlsx::CellRange {1, 2, 2, 4},
        insert_columns_expected,
        {
            R"(<c r="B1" t="inlineStr"><is><t>rich-A&amp;B</t></is></c>)",
            R"(<c r="D1" t="inlineStr"><is><t>gamma</t></is></c>)",
            R"(<c r="C2" t="inlineStr"><is><t>beta</t></is></c>)",
        },
        {},
        fastxlsx::CellRange {1, 2, 3, 5},
        ReopenedSourceSuccessCell {3, 5, fastxlsx::CellValue::text("later rich & <column>")},
        R"(<c r="E3" t="inlineStr"><is><t>later rich &amp; &lt;column&gt;</t></is></c>)");

    constexpr std::string_view delete_rows_source_sheet_data =
        R"(<sheetData><row r="1">)"
        R"(<c r="A1" t="inlineStr"><is><r><rPr><b/></rPr><t>deleted-rich-row</t></r></is></c>)"
        R"(</row><row r="2">)"
        R"(<c r="A2" t="inlineStr"><is>)"
        R"(<r><rPr><b/></rPr><t>rich-</t></r>)"
        R"(<r><t>A&amp;B</t></r>)"
        R"(<rPh sb="0" eb="1"><t>ignored-rich-phonetic</t></rPh>)"
        R"(<extLst><ext uri="{fastxlsx-test}"><t>ignored-rich-ext</t></ext></extLst>)"
        R"(</is></c>)"
        R"(<c r="C2" t="inlineStr"><is>)"
        R"(<r><t>ga</t></r><r><t>mma</t></r>)"
        R"(<rPh sb="0" eb="1"><t>ignored-rich-gamma</t></rPh>)"
        R"(</is></c>)"
        R"(</row><row r="3">)"
        R"(<c r="B3" t="inlineStr"><is><r><t>be</t></r><r><t>ta</t></r></is></c>)"
        R"(</row></sheetData>)";
    const ReopenedSourceSuccessCell delete_rows_expected[] = {
    ...
    run_rich_shift_case("delete-rows",
        "structural delete_rows source inline rich text shift",
        write_rich_shift_source(
            "fastxlsx-workbook-editor-public-structural-delete-rows-source-inline-rich-source.xlsx",
            delete_rows_source_sheet_data),
        4,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.delete_rows(1, 1); },
        fastxlsx::CellRange {1, 1, 2, 3},
        delete_rows_expected,
        {
            R"(<c r="A1" t="inlineStr"><is><t>rich-A&amp;B</t></is></c>)",
            R"(<c r="C1" t="inlineStr"><is><t>gamma</t></is></c>)",
            R"(<c r="B2" t="inlineStr"><is><t>beta</t></is></c>)",
        },
        {"deleted-rich-row"},
        fastxlsx::CellRange {1, 1, 3, 4},
        ReopenedSourceSuccessCell {3, 4, fastxlsx::CellValue::text("later rich & <delete-row>")},
        R"(<c r="D3" t="inlineStr"><is><t>later rich &amp; &lt;delete-row&gt;</t></is></c>)");

    constexpr std::string_view delete_columns_source_sheet_data =
        R"(<sheetData><row r="1">)"
        R"(<c r="A1" t="inlineStr"><is><r><rPr><b/></rPr><t>deleted-rich-column</t></r></is></c>)"
        R"(<c r="B1" t="inlineStr"><is>)"
        R"(<r><rPr><b/></rPr><t>rich-</t></r>)"
        R"(<r><t>A&amp;B</t></r>)"
        R"(<rPh sb="0" eb="1"><t>ignored-rich-phonetic</t></rPh>)"
        R"(<extLst><ext uri="{fastxlsx-test}"><t>ignored-rich-ext</t></ext></extLst>)"
        R"(</is></c>)"
        R"(<c r="D1" t="inlineStr"><is>)"
        R"(<r><t>ga</t></r><r><t>mma</t></r>)"
        R"(<rPh sb="0" eb="1"><t>ignored-rich-gamma</t></rPh>)"
        R"(</is></c>)"
        R"(</row><row r="2">)"
        R"(<c r="C2" t="inlineStr"><is><r><t>be</t></r><r><t>ta</t></r></is></c>)"
        R"(</row></sheetData>)";
    const ReopenedSourceSuccessCell delete_columns_expected[] = {
        {1, 1, fastxlsx::CellValue::text("rich-A&B")},
        {1, 3, fastxlsx::CellValue::text("gamma")},
    ...
    run_rich_shift_case("delete-columns",
        "structural delete_columns source inline rich text shift",
        write_rich_shift_source(
            "fastxlsx-workbook-editor-public-structural-delete-columns-source-inline-rich-source.xlsx",
            delete_columns_source_sheet_data),
        4,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.delete_columns(1, 1); },
        fastxlsx::CellRange {1, 1, 2, 3},
        ...
        {"deleted-rich-column"},
        fastxlsx::CellRange {1, 1, 3, 4},
        ReopenedSourceSuccessCell {3, 4, fastxlsx::CellValue::text("later rich & <delete-column>")},
        R"(<c r="D3" t="inlineStr"><is><t>later rich &amp; &lt;delete-column&gt;</t></is></c>)");
}
...
void test_public_worksheet_editor_structural_shift_prefixed_source_inline_strings()
{
    const auto write_prefixed_inline_shift_source =
        [](std::string_view name, std::string_view sheet_data) {
            const std::filesystem::path source = write_two_sheet_source(name);
            const std::string worksheet_xml =
                std::string(R"(<?xml version="1.0" encoding="UTF-8"?>)")
    ...
    const auto run_prefixed_inline_shift_case =
        [&](std::string_view artifact_suffix,
            std::string_view scenario,
            const std::filesystem::path& source,
            std::size_t source_cell_count,
            ...
            const std::string scenario_text(scenario);
            const std::filesystem::path output = artifact(
                "fastxlsx-workbook-editor-public-structural-shift-prefixed-inline-"
                + artifact_suffix_text + "-output.xlsx");
            const std::filesystem::path noop_output = artifact(
                "fastxlsx-workbook-editor-public-structural-shift-prefixed-inline-"
                + artifact_suffix_text + "-noop-output.xlsx");
            ...
            check_contains(source_worksheet_xml, "<x:worksheet",
                scenario_text + " fixture should use a qualified worksheet root");
            check_contains(source_worksheet_xml, "<x:is>",
                scenario_text + " fixture should use qualified inline-string wrappers");
            check_contains(source_worksheet_xml, "<x:rPr>",
                scenario_text + " fixture should include prefixed rich text formatting");
            check_contains(source_worksheet_xml, "ignored-prefixed",
            ...
            check(editor.has_pending_changes(),
                scenario_text + " should dirty WorkbookEditor");
            check(sheet.cell_count() == expected_cells.size(),
                scenario_text + " should expose the shifted prefixed inline cell count");
            check_source_success_snapshots(
                sheet, expected_range, expected_cells, scenario, "live shifted");
            check(sheet.has_pending_changes(),
                scenario_text + " live shifted reads should keep Data dirty");
            ...
            check_contains(output_worksheet_xml,
                R"(<x:worksheet xmlns:x="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:fx="urn:fastxlsx:test">)",
                scenario_text + " save should preserve the source worksheet wrapper prefix");
            check_contains(output_worksheet_xml, "<x:dimension",
                scenario_text + " save should refresh dimension using the source prefix");
            for (std::string_view expected_xml_fragment : expected_xml_fragments) {
                check_contains(output_worksheet_xml, expected_xml_fragment,
                    scenario_text + " save should write shifted prefixed values as plain cells");
            }
            for (std::string_view omitted_token : omitted_tokens) {
                check_not_contains(output_worksheet_xml, omitted_token,
                    scenario_text + " save should omit deleted prefixed source cells");
            }
            check_not_contains(output_worksheet_xml, "<x:c",
                scenario_text + " save should not preserve source cell element prefixes");
            check_not_contains(output_worksheet_xml, "<x:is",
                scenario_text + " save should not preserve source inline-string prefixes");
            check_not_contains(output_worksheet_xml, "<x:t",
                scenario_text + " save should not preserve source text element prefixes");
            check_not_contains(output_worksheet_xml, "<x:r",
                scenario_text + " save should not preserve source rich-text run prefixes");
            check_not_contains(output_worksheet_xml, "ignored-prefixed",
                scenario_text + " save should omit ignored prefixed metadata text");
            check(output_entries.find("xl/sharedStrings.xml") == output_entries.end(),
                scenario_text + " save should not introduce sharedStrings");
            check(fastxlsx::test::read_zip_entries(source) == source_entries,
                scenario_text + " save should not mutate the source package");
            ...
            for (std::string_view expected_xml_fragment : expected_xml_fragments) {
                check_contains(reopened_worksheet_xml, expected_xml_fragment,
                    scenario_text + " fresh-reopen save should keep shifted prefixed cells");
            }
            check_contains(reopened_worksheet_xml, reopened_edit_xml_fragment,
                scenario_text + " fresh-reopen save should include the later edit");
            check_not_contains(reopened_worksheet_xml, "<x:c",
                scenario_text + " fresh-reopen save should keep regenerated cells unprefixed");
            check_not_contains(reopened_worksheet_xml, "ignored-prefixed",
                scenario_text + " fresh-reopen save should keep ignored metadata omitted");
            check(reopened_entries.find("xl/sharedStrings.xml") == reopened_entries.end(),
                scenario_text + " fresh-reopen save should still avoid sharedStrings");
            check(fastxlsx::test::read_zip_entries(noop_output) == output_entries,
    ...
    constexpr std::string_view insert_source_sheet_data =
        R"(<x:sheetData><x:row r="1">)"
        R"(<x:c r="A1" t="inlineStr"><x:is><x:t>prefixed-a</x:t></x:is></x:c>)"
        R"(<x:c r="C1" t="inlineStr"><x:is><x:t xml:space="preserve"> spaced </x:t></x:is></x:c>)"
        R"(</x:row><x:row r="2">)"
        R"(<x:c r="B2" t="inlineStr"><x:is>)"
        R"(<x:r><x:rPr><x:b/></x:rPr><x:t>rich-</x:t></x:r>)"
        R"(<x:r><x:t>tail</x:t></x:r>)"
        R"(<x:rPh sb="0" eb="1"><x:t>ignored-prefixed-phonetic</x:t></x:rPh>)"
        R"(<x:extLst><x:ext uri="{fastxlsx-test}"><fx:opaque><x:t>ignored-prefixed-ext</x:t></fx:opaque></x:ext></x:extLst>)"
        R"(</x:is></x:c>)"
        R"(</x:row></x:sheetData>)";
    const ReopenedSourceSuccessCell insert_rows_expected[] = {
        {2, 1, fastxlsx::CellValue::text("prefixed-a")},
        {2, 3, fastxlsx::CellValue::text(" spaced ")},
        {3, 2, fastxlsx::CellValue::text("rich-tail")},
    };
    run_prefixed_inline_shift_case("insert-rows",
        "structural insert_rows prefixed inline source shift",
        write_prefixed_inline_shift_source(
            "fastxlsx-workbook-editor-public-structural-insert-rows-prefixed-inline-source.xlsx",
            insert_source_sheet_data),
        3,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.insert_rows(1, 1); },
        fastxlsx::CellRange {2, 1, 3, 3},
        insert_rows_expected,
        {
            R"(<c r="A2" t="inlineStr"><is><t>prefixed-a</t></is></c>)",
            R"(<c r="C2" t="inlineStr"><is><t xml:space="preserve"> spaced </t></is></c>)",
            R"(<c r="B3" t="inlineStr"><is><t>rich-tail</t></is></c>)",
        },
        {},
        fastxlsx::CellRange {2, 1, 4, 4},
        ReopenedSourceSuccessCell {4, 4, fastxlsx::CellValue::text("later prefixed & <row>")},
        R"(<c r="D4" t="inlineStr"><is><t>later prefixed &amp; &lt;row&gt;</t></is></c>)");

    const ReopenedSourceSuccessCell insert_columns_expected[] = {
        {1, 2, fastxlsx::CellValue::text("prefixed-a")},
        {1, 4, fastxlsx::CellValue::text(" spaced ")},
        {2, 3, fastxlsx::CellValue::text("rich-tail")},
    };
    run_prefixed_inline_shift_case("insert-columns",
        "structural insert_columns prefixed inline source shift",
        write_prefixed_inline_shift_source(
            "fastxlsx-workbook-editor-public-structural-insert-columns-prefixed-inline-source.xlsx",
            insert_source_sheet_data),
        3,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.insert_columns(1, 1); },
        fastxlsx::CellRange {1, 2, 2, 4},
        insert_columns_expected,
        {
            R"(<c r="B1" t="inlineStr"><is><t>prefixed-a</t></is></c>)",
            R"(<c r="D1" t="inlineStr"><is><t xml:space="preserve"> spaced </t></is></c>)",
            R"(<c r="C2" t="inlineStr"><is><t>rich-tail</t></is></c>)",
        },
        {},
        fastxlsx::CellRange {1, 2, 3, 5},
        ReopenedSourceSuccessCell {3, 5, fastxlsx::CellValue::text("later prefixed & <column>")},
        R"(<c r="E3" t="inlineStr"><is><t>later prefixed &amp; &lt;column&gt;</t></is></c>)");

    constexpr std::string_view delete_rows_source_sheet_data =
        R"(<x:sheetData><x:row r="1">)"
        R"(<x:c r="A1" t="inlineStr"><x:is><x:r><x:rPr><x:b/></x:rPr><x:t>deleted-prefixed-row</x:t></x:r></x:is></x:c>)"
        R"(</x:row><x:row r="2">)"
        R"(<x:c r="A2" t="inlineStr"><x:is><x:t>prefixed-a</x:t></x:is></x:c>)"
        R"(<x:c r="C2" t="inlineStr"><x:is><x:t xml:space="preserve"> spaced </x:t></x:is></x:c>)"
        R"(</x:row><x:row r="3">)"
        R"(<x:c r="B3" t="inlineStr"><x:is>)"
        R"(<x:r><x:rPr><x:b/></x:rPr><x:t>rich-</x:t></x:r>)"
        R"(<x:r><x:t>tail</x:t></x:r>)"
        R"(<x:rPh sb="0" eb="1"><x:t>ignored-prefixed-phonetic</x:t></x:rPh>)"
        R"(<x:extLst><x:ext uri="{fastxlsx-test}"><fx:opaque><x:t>ignored-prefixed-ext</x:t></fx:opaque></x:ext></x:extLst>)"
        ...
        R"(</x:row></x:sheetData>)";
    const ReopenedSourceSuccessCell delete_rows_expected[] = {
        {1, 1, fastxlsx::CellValue::text("prefixed-a")},
        {1, 3, fastxlsx::CellValue::text(" spaced ")},
        {2, 2, fastxlsx::CellValue::text("rich-tail")},
    };
    run_prefixed_inline_shift_case("delete-rows",
        "structural delete_rows prefixed inline source shift",
        write_prefixed_inline_shift_source(
            "fastxlsx-workbook-editor-public-structural-delete-rows-prefixed-inline-source.xlsx",
            delete_rows_source_sheet_data),
        4,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.delete_rows(1, 1); },
        fastxlsx::CellRange {1, 1, 2, 3},
        delete_rows_expected,
        {
            R"(<c r="A1" t="inlineStr"><is><t>prefixed-a</t></is></c>)",
            R"(<c r="C1" t="inlineStr"><is><t xml:space="preserve"> spaced </t></is></c>)",
            R"(<c r="B2" t="inlineStr"><is><t>rich-tail</t></is></c>)",
        },
        {"deleted-prefixed-row"},
        fastxlsx::CellRange {1, 1, 3, 4},
        ReopenedSourceSuccessCell {3, 4, fastxlsx::CellValue::text("later prefixed & <delete-row>")},
        R"(<c r="D3" t="inlineStr"><is><t>later prefixed &amp; &lt;delete-row&gt;</t></is></c>)");

    constexpr std::string_view delete_columns_source_sheet_data =
        R"(<x:sheetData><x:row r="1">)"
        R"(<x:c r="A1" t="inlineStr"><x:is><x:r><x:rPr><x:b/></x:rPr><x:t>deleted-prefixed-column</x:t></x:r></x:is></x:c>)"
        R"(<x:c r="B1" t="inlineStr"><x:is><x:t>prefixed-a</x:t></x:is></x:c>)"
        R"(<x:c r="D1" t="inlineStr"><x:is><x:t xml:space="preserve"> spaced </x:t></x:is></x:c>)"
        R"(</x:row><x:row r="2">)"
        R"(<x:c r="C2" t="inlineStr"><x:is>)"
        R"(<x:r><x:rPr><x:b/></x:rPr><x:t>rich-</x:t></x:r>)"
        R"(<x:r><x:t>tail</x:t></x:r>)"
        R"(<x:rPh sb="0" eb="1"><x:t>ignored-prefixed-phonetic</x:t></x:rPh>)"
        R"(<x:extLst><x:ext uri="{fastxlsx-test}"><fx:opaque><x:t>ignored-prefixed-ext</x:t></fx:opaque></x:ext></x:extLst>)"
    ...
    run_prefixed_inline_shift_case("delete-columns",
        "structural delete_columns prefixed inline source shift",
        write_prefixed_inline_shift_source(
            "fastxlsx-workbook-editor-public-structural-delete-columns-prefixed-inline-source.xlsx",
            delete_columns_source_sheet_data),
        4,
        [](fastxlsx::WorksheetEditor& sheet) { sheet.delete_columns(1, 1); },
        fastxlsx::CellRange {1, 1, 2, 3},
        ...
        {"deleted-prefixed-column"},
        fastxlsx::CellRange {1, 1, 3, 4},
        ReopenedSourceSuccessCell {3, 4, fastxlsx::CellValue::text("later prefixed & <delete-column>")},
        R"(<c r="D3" t="inlineStr"><is><t>later prefixed &amp; &lt;delete-column&gt;</t></is></c>)");
}
...
int main()
{
    try {
        ...
        test_public_worksheet_editor_structural_shift_source_scalar_string_cells();
        test_public_worksheet_editor_flattens_source_inline_rich_text();
        test_public_worksheet_editor_structural_shift_source_inline_rich_text();
        test_public_worksheet_editor_materializes_prefixed_source_inline_strings();
        test_public_worksheet_editor_structural_shift_prefixed_source_inline_strings();
