...
handoff first slices、planned staged chunk input chunk-source first slice，以及 queued
planned replacement string chunk-source first slice 已有测试。queued string 仍是前序
...
- `PackageReader::entry_chunk_source()` 已支持 internal pull-based package entry
  ...
  DEFLATE 路径也复用该 chunk source，但仍会 materialize 完整 payload 作为
  `std::string` 返回。
- `PackageEditor::replace_worksheet_cells()` 输出侧已有 temporary file-backed chunk；
  ...
  output pass 各自创建新的 `PackageReader::entry_chunk_source()` 并扫描 source ZIP entry。
- `PackageEditor::replace_worksheet_cells()` 不再用
  `package_editor_cell_replacement_materialized_input_byte_limit` 拒绝 source package
  worksheet XML 或 queued planned replacement string 的总大小；这些路径改由
  event-reader `max_window_bytes` 约束单个未完成 token / retained window。queued
  planned string 仍是前序 helper 已持有的完整字符串。
...
- `scan_worksheet_events_from_chunks()` 已支持 internal chunk/window 输入，事件视图只在
  callback 期间有效，并用 `WorksheetEventReaderOptions::max_window_bytes` 限制未闭合
  markup / text token 的 retained window；测试覆盖跨 token 分块、总 XML 大于窗口仍成功、
  以及单个未闭合 token 超限失败。
- `scan_worksheet_events_from_chunk_source()` 已支持 internal pull-based chunk source，
  可从文件/reader source 逐块填充输入，不需要完整 worksheet string 或预构造 chunk
  vector；测试覆盖 chunk-source 与 full-buffer token 等价。
- `scan_cell_replacement_actions_from_chunks()` /
  `emit_cell_replacement_worksheet_from_chunks()` 已复用现有 replacement preflight 和
  action model 消费 chunk event stream；测试覆盖 chunk/full 输出等价、总 XML 大于窗口仍可
  替换、missing selector summary 和 oversized window failure 传播。
...
- `PackageEditor::replace_worksheet_cells()` 的 source-entry output pass 已通过
  ...
  queued planned string input 也通过 string-view chunk-source reader 逐块喂给同一
...
- `PackageEditor::replace_worksheet_cells()` 的 source-entry dependency/dimension analysis、
  ...
  event-reader chunk-source；planned staged chunk input 和 queued planned string input
  也已走 chunk-source。
...
- `PackageEditor` 回归覆盖 queued planned replacement string 总大小超过
  `package_editor_cell_replacement_materialized_input_byte_limit` 时仍能完成 cell
  replacement、dimension refresh、calcChain cleanup、unknown bytes preservation 和
  output `PackageReader` 重读。
...
- `read_entry()` 因返回 `std::string` 仍会物化 payload；当前只完成
  `entry_chunk_source()`、`extract_entry_to_file()` 和 `read_entry()` 内部解压逻辑
  复用 DEFLATE input chunk-source 的窄切片。
...
3. 把 transformer action stream 接到 package-entry staged writer。
  ...
  P8.32 已完成 DEFLATE read-entry chunk-source materialization handoff。
...
- `fastxlsx.worksheet_event_reader` 覆盖 chunk/window scanner 与 full-buffer scanner
  token 等价、总 XML 大于 retained window 的成功路径，以及 oversized incomplete token
  的拒绝路径，并覆盖 chunk-source scanner 与 full-buffer scanner token 等价。
...
- `fastxlsx.package_editor` 覆盖 source-entry cell replacement output pass、
  ...
  queued planned string input 暴露 chunk-source adapter notes。
...
- `replace_worksheet_cells()` 的 queued/current planned worksheet XML 输入仍在读取前用
  `current_planned_part_data_size()` 做状态变更前检查。
- source package worksheet XML 的 P8.18 materialized-input guard 已由 P8.25
  source-entry file-backed chunk-source readers 取代；大 source worksheet cell
  replacement 现在应走 chunk-source 成功路径，而不是 source input over-limit 失败路径。
...
验收标准：
- `fastxlsx.package_editor` 覆盖 queued planned input over-limit no-state-pollution 路径，
  并由 P8.25 覆盖 source package worksheet XML 超过 planned materialized guard 后的
  file-backed chunk-source 成功路径。
...
输入：
- P8.6 internal worksheet event reader first implementation slice。
- P8.18 materialized input guard 暴露了现有 cell replacement validation 仍会物化输入的边界。
...
- 新增 internal `scan_worksheet_events_from_chunks()`，可消费
  `std::span<const std::string_view>` chunk 序列，不拼接完整 worksheet。
- chunk scanner 与 full-buffer scanner 复用同一 worksheet event state 语义；row/cell
  上下文在 chunk 模式下复制到短生命周期内部存储，event views 只在 callback 期间有效。
...
输出：
- 新增 internal `scan_cell_replacement_actions_from_chunks()`。
- 新增 internal `emit_cell_replacement_worksheet_from_chunks()`。
...
验收标准：
- `fastxlsx.worksheet_transformer` 覆盖 chunk emitter 与 full-buffer emitter 输出等价。
...
- P8.18 materialized input guard。

输出：
- `write_worksheet_cell_replacement_stream()` 将当前 bounded materialized `worksheet_xml`
  切成 internal `std::string_view` chunks 后调用
  `scan_cell_replacement_actions_from_chunks()`。
- event-reader window limit 复用
  `package_editor_cell_replacement_materialized_input_byte_limit`，避免 output pass 引入比
  现有 materialized input guard 更窄的 token 限制。
...
输出：
- `analyze_worksheet_cell_replacement_stream()` 将当前 bounded materialized
  `worksheet_xml` 切成 chunks 后调用 `scan_cell_replacement_actions_from_chunks()`。
...
- P8.25 scope 开始时，planned worksheet replacement input 仍是 materialized guard
  路径；P8.27 后续切片已把 queued planned string 接到 chunk-source reader。
...
- source package worksheet XML 总大小不再受
  `package_editor_cell_replacement_materialized_input_byte_limit` 拒绝；该 guard 只保留给
  P8.25 当时的 queued/planned worksheet replacement input；P8.27 后续切片已让 queued
  planned string 走 chunk-source reader。source 路径仍受 event-reader `max_window_bytes`
...
验收标准：
- `fastxlsx.worksheet_event_reader` 覆盖 chunk-source scanner 与 full-buffer scanner
  token 等价。
- `fastxlsx.worksheet_transformer` 覆盖 chunk-source emitter 与 full-buffer emitter 输出等价。
- `fastxlsx.package_editor` 覆盖 source-entry cell replacement、linked-object fixture 和
  large source worksheet 成功路径均暴露 chunk-source notes。
- `fastxlsx.package_editor` 覆盖 source worksheet XML 超过
  `package_editor_cell_replacement_materialized_input_byte_limit` 仍可成功替换目标 cell，
...
- P8.26 scope 开始时，ordinary planned worksheet replacement string input 仍保留
  materialized guard；P8.27 后续切片已把 queued planned string 接到 chunk-source reader。
...
- `replace_worksheet_cells()` 区分三种输入：source package worksheet entry、
  current planned staged chunks、ordinary planned materialized string；P8.27 后续切片把
  ordinary queued planned string 改为 string-view chunk-source input。
...
- P8.26 本片不把 ordinary planned string input 写成已 source 化；P8.27 后续切片已完成
  queued planned string chunk-source first slice。
...
`current_planned_part_data_size()` / materialized total-size guard 拒绝该输入，而是通过
...
- 当前 ordinary planned worksheet replacement string 已由 prior helper 持有完整
...
- 新增 internal `WorksheetStringChunkReader`，按 `worksheet_file_chunk_size` 从
...
验收标准：
- `fastxlsx.package_editor` 覆盖 ordinary queued planned string cell replacement
  成功路径暴露 queued-string chunk-source notes。
- `fastxlsx.package_editor` 覆盖 queued planned worksheet string 总 payload 超过
  `package_editor_cell_replacement_materialized_input_byte_limit` 时仍可完成 cell
  replacement。
...
禁止项：
- 不把 `replace_worksheet_part()` / `replace_worksheet_sheet_data()` 的 prior queued
  string production 写成低内存。
- 不把 PackageReader source extraction 写成直接 ZIP entry streaming。
- 不新增 public `PackageEditor` / `WorksheetEditor` API。
- 不修复 sharedStrings/styles、relationships、tables、drawings、definedNames、公式、
  calcChain rebuild 或 range-bearing metadata。
...
- source-entry large worksheet、planned staged chunk 和 queued planned string 大 payload
  回归继续通过。
