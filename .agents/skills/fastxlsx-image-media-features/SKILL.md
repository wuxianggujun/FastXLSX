...
3. P17.2 new-workbook-only 插入：当前基础切片是
   ...
   drawing mutation。memory-source overload 只同步复制 caller-owned bytes，不代表任意
   stream/URL/base64 图片源。当前 `ImageOptions` 增量只在该 drawing XML 中写
...
4. P17.3 验证闭环：当前 `fastxlsx.streaming` 已覆盖 media/drawing/rels/content
   ...
   openpyxl smoke、XlsxWriter reference 和 Excel COM anchors。
...
- 结构测试：检查 `xl/media/*`、`xl/drawings/drawing*.xml`、
  ...
  `xdr:cNvPr name` / `descr`。memory-source 样例还要检查 caller buffer mutation 后
...
- 当前固定本地 QA 入口是 `tools/verify_image_metadata.py` 和
  ...
  `--basic-input`、`--mixed-object-input` 和 `--memory-input`，分别检查 image metadata、
  基础图片插入、mixed-object relationship 和 memory-source image 样例；Excel helper
  接受 `-Path`、`-BasicPath`、`-MixedObjectPath` 和 `-MemoryPath`。`openpyxl` 可能跳过
