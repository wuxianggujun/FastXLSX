...
- tracked benchmark evidence 机制已建立；当前有 4 个 production Streaming bundle、11 个 production Patch bundle 与 1 个 OpenXLSX reference bundle。每个 bundle 只支持 manifest 限定的单机同数据集结论，warm-up/measured 次数以各自 run context 为准。
...
- 最新 Patch schema-v10/v4 complete-cell profile 中，1,000,000-cell numeric/mixed-inline/formula + external hyperlink level-1 targeted upsert 的 total editor median 为 1075/1205/1007 ms，较紧邻同协议 baseline 降低 51.18%/33.13%/25.85%；transform median 为 611/691/591 ms，median process peak working set 为 8.816/9.832/8.820 MB。每类 profile 至少合并 999,982 个完整 source cell、最多 18 个 fallback；代表输出通过 openpyxl 3.1.5，hyperlink target 保留。该结果不形成任意 workbook、Office 或全功能性能承诺。
