...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  `WorksheetWriter`、`CellView`、`DocumentProperties`、`DataValidationRule`、
  `DataValidationErrorStyle`、`TableOptions`、
  `ImageFormat`、`ImageInfo`、`ImageOptions`、`read_image_info()`、`WorksheetWriter::add_image()`
...
- OPC 当前已有内部 manifest / relationships / `PartIndex` / `RelationshipGraph`
  ...
  prompt/error attributes）、external/internal hyperlinks、streaming-only tables 和
  `WorksheetWriter::add_image()`（含 drawing-only `ImageOptions` name/description metadata）
  等基础切片，但不能宣称完整 data validation UI、
  图片、VBA、table、drawing 编辑或 existing-workbook 保真支持。
