...
当前 benchmark executable 使用 schema v5：Streaming 分列 generation、package close、million-cells/s、body buffer peak/flush count 与 close 后 active temporary file count；Patch 分列 direct-range/single-pass transform、output append/flush/peak buffer、relationship/temporary IO、package writer/target entry timing，以及 raw compressed-copy entry names/count/bytes。历史 schema-v4 artifact 仍可汇总，但不能提供新增资源生命周期、batching、writer decomposition 或 raw-copy 证据。
...
- 十个 bundle 都不是跨机器、跨数据规模或泛化“高性能/低内存”证据；每个 bundle 只支持其 manifest 中记录的同机同数据集结论。历史 schema-v4 继续作为回归基线，release 当前值以 schema-v5 bundle 为准。
...
- 最新 rewrite batching profile 的 1,000,000-cell level 1 upsert total/mutation/transform median 为 1205/931/767.186 ms、peak 为 8.80078 MB；相对同机同协议 raw-copy bundle 的 2054/1759/1602 ms 分别降低 41.33%/47.07%/52.11%。5,000,000-cell level 1 upsert 重写 183,697,902 logical bytes，total/transform median 为 8494/5855.255 ms、peak 为 8.80859 MB；owned batching buffer 在两个规模都不超过 262,144 bytes。该比较仍受单机调度影响，只支持记录 workload。
...
- 1,000,000-cell level 6 upsert 中，package writer median 为 933.636 ms，single-pass transform 为 865.366 ms；DEFLATE output 是该 profile 更大的测量阶段。Transform 内部的 source scan/action residual median 为 695.318 ms，relationship scan 为 151.006 ms，temporary write 为 19.042 ms。下一轮优先评估 compression backend/参数和 source event/action 热点，不以扩大 In-memory guardrail 代替热路径优化，也不在无证据时引入并行压缩。
...
- Patch upsert/fallback matrix 必须证明 single-pass mode、生效的 scanned/matched/inserted counts、非零 staged output bytes、精确 dimension、relationship policy 与保存后可重开；同时分列 append/flush/peak owned buffer、relationship/temporary IO 和 package writer target-entry telemetry，避免把 XML scan 与 DEFLATE encode 混为一个 elapsed。
