...
empty output path, and missing-parent output attempts all preserve dirty
diagnostics under `RenamedData`, keep the source workbook unchanged under
`Data`, leave the first renamed output isolated, and a safe retry reopens only
as `RenamedData` with the combined shifted coordinates.
