...
- Existing-workbook data-validation API 复用 Streaming 的 owning `DataValidationRule`。Add 必须单独声明 multi-range `sqref`、formula1/formula2 shape、prompt/error escaping与已有 container/count/schema-order guardrail；remove 必须声明当前 effective source order 的 zero-based index、与 `WorksheetDataValidationView::index` 的一致性、strict bounded projection 前置审计、final-container removal 和 count normalization。两者均需写清 planned rename/added worksheet、same-session composition、failure-before-commit/retry，以及不创建 `.rels`/content type、不求值/请求重算、不随 structural mutation 同步的边界。共享 rule 类型或 reader index 不代表共享 worksheet DOM 或完整 validation 对象模型。
...
- Row/column structural edit 的 merged-cell synchronization 必须使用 CellStore/session candidate + 当前有效 worksheet metadata replacement 两阶段预构造。全部 range 一次 strict scan/translate/audit 后只产生一个 container replacement/removal，最后 session publish 必须 noexcept；不能通过多次 public merge/unmerge 拼接，否则后续失败会留下部分 package mutation。Metadata-only 空 CellStore 变化必须 dirty，失败 save 必须保留最终 cells/metadata retry。
