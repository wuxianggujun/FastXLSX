...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  `WorksheetWriter`、`CellView`、`StyleId`、`CellAlignment`、`CellFont`、`CellFill`、`CellStyle`、`DocumentProperties`、`DataValidationRule`、
...
- 当前写出基础可配置 `docProps/core.xml` 和 `docProps/app.xml` 小型 XML part；
  ...
  public API。
- 当前已有 P9 streaming-only number-format + wrap-text alignment + bold/italic font + solid fill
  styles 基础切片：`StyleId` 是 workbook-local handle，`CellStyle` 当前支持
  `number_format`、可选 `CellAlignment::wrap_text` 和可选 `CellFont::bold` /
  `CellFont::italic`、可选 `CellFill` solid foreground ARGB，`WorkbookWriter::add_style()`
  会注册样式并让 `close()` 写 `xl/styles.xml`、workbook styles relationship 和 content
  type override，`CellView::with_style()` 让 cell 写 `s="N"`。wrap-text 只写
  `applyAlignment="1"` / `<alignment wrapText="1"/>`，不计算行高。这不是完整 styles、
