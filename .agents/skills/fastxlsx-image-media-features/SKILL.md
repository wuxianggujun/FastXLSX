...
3. P17.2 new-workbook-only 插入：当前基础切片是
   ...
   `xdr:cNvPr` 的 `name` / `descr`，不是 `oneCellAnchor` / `absoluteAnchor`
   元素支持。
4. P17.3 验证闭环：当前 `fastxlsx.streaming` 已覆盖 media/drawing/rels/content
   ...
   placement / marker-offset geometry 检查。
...
- 当前固定本地 QA 入口是 `tools/verify_image_metadata.py` 和
  ...
  `--basic-input` 和 `--mixed-object-input`，分别检查 image metadata、基础图片插入和
  mixed-object relationship 样例；Excel helper 接受 `-Path`、`-BasicPath` 和
  `-MixedObjectPath`。`openpyxl` 可能跳过 JPEG 图片读取，JPEG media/drawing 关系以
