...
`CellView` 流式写入骨架已存在；公式、行高、列宽、冻结窗格、自动筛选、
  ...
  color scales、basic data bars 和 streaming-only tables 是当前写入骨架能力，不等同完整 Phase 3
  或完整 Phase 5。当前可见
...
conditional color scale 和 basic data bar 是 worksheet metadata 切片，不走 styles registry 或 `dxfs`。
...
- Phase 5 worksheet metadata：基础。`WorksheetWriter::add_data_validation()`
  ...
  `<conditionalFormatting><cfRule type="dataBar">` XML。`ArgbColor` 序列化为
  ...
  negative bar / axis / border / gradient / `extLst`、iconSet、cellIs/formula/top/bottom/duplicate/unique
...
- conditional formatting、color scale、basic data bar、priority 和本地 QA：
...
- 当前 `fastxlsx.streaming` conditional formatting 基础切片推荐 preset 输出样例包括 color scale：
  ...
  shared priority、multi-range `sqref`、suffix 顺序和无 side effects。本地 QA 入口是
  ...
  conditional formatting、dxf/styles、公式规则、advanced data bar、iconSet、existing-file editing
  或 Excel UI parity。
...
- 把 `WorksheetWriter::add_conditional_color_scale()` / `WorksheetWriter::add_conditional_data_bar()`
  写成完整 conditional formatting 或 styles/dxfs 支持。当前只写 streaming-only new-workbook
  two-/three-color color scale 与 basic data bar 的 worksheet-local XML；不支持公式规则、
  advanced data bars、icon sets、dxf 样式、冲突检测、
  existing-file editing 或完整 Excel UI。
...
- `fastxlsx-conditional-formatting-features`：streaming conditional formatting、color scale、
  basic data bar、priority、multi-range `sqref` 和 Excel/openpyxl/XlsxWriter QA。
