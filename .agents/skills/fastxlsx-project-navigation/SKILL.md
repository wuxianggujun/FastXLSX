...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  `WorksheetWriter`、`CellView`、`StyleId`、`CellAlignment`、`CellFont`、`CellStyle`、`DocumentProperties`、`DataValidationRule`、
...
- 当前已有 P9 streaming-only number-format + wrap-text alignment + bold/italic font
  ...
  full font control、fill/border/full alignment、date cell type、dxf-backed conditional formatting、
  rich text 或 existing-file style preservation；当前 bold/italic font 只写 `<fonts>` 中的
  `<b/>` / `<i/>`、`fontId` 和 `applyFont="1"`。当前 two-/three-color color scale 和
