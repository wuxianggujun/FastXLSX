...
- 四个 bundle 都不是跨机器、跨数据规模或泛化“高性能/低内存”证据；三个矩阵只支持各自 manifest 中记录的同机同数据集结论。
...
- 优化 bundle 的 targeted upsert total/mutation median 为 4353/2968 ms、peak 为 8.01562 MB；它仍走 source-order transformer，不能用 strict replace 的 direct-range 结果替代。Targeted replace/upsert 仍各重写约 34.9 MB logical XML。
- No-op copy-original 当前只验证 CRC/logical size，并在 output 重新封装/可能重新压缩 entry；不声称 raw compressed-byte passthrough。后续优先评估 raw-copy backend 与 upsert one-pass transformation，而不是扩大 In-memory guardrail。
