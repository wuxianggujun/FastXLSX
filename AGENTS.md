...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `WorksheetWriter`、`CellView`、`StyleId`、`CellAlignment`、`CellFont`、`CellStyle`、`DataValidationRule`、
  ...
  `ImageOptions::external_hyperlink_tooltip`、`read_image_info()`、
...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  streaming number-format、wrap-text alignment 和 bold/italic font styles /
...
当前 P9 styles 基础切片已存在：`WorkbookWriter::add_style(CellStyle)`
会在 workbook state 注册 streaming-only new-workbook 样式，当前支持自定义
number format、窄 `CellAlignment::wrap_text` 和窄 `CellFont::bold` /
`CellFont::italic`。重复完整 style 复用同一个 `StyleId`；相同 number format
在不同 style 组合里复用同一个 custom `numFmtId`；相同 bold/italic font
组合在不同 style 组合里复用同一个 `fontId`。`CellView::with_style()` 只携带 workbook-local `StyleId` 小句柄，
...
cell 会写 `s="N"`，默认样式不写 `s="0"`。wrap-text alignment 只写
`cellXfs` 里的 `applyAlignment="1"` 和 `<alignment wrapText="1"/>`，不计算行高、
不代表完整 alignment。bold/italic font 只写 `<fonts>` 里的 `<b/>` / `<i/>`、
`fontId` 和 `applyFont="1"`，不代表完整 font control。该切片不支持字体颜色、
字号、字体名、下划线、填充、边框、完整 alignment、rich text、
...
- P9 styles number formats + wrap-text alignment + bold/italic fonts：基础。`StyleId` / `CellAlignment` / `CellFont` / `CellStyle` /
  `WorkbookWriter::add_style()` / `CellView::with_style()` 已支持 streaming-only
  new-workbook 自定义 number format、wrap-text alignment 和 bold/italic font 样式注册及 cell style 引用；`xl/styles.xml`
  ...
  custom `numFmtId` 复用、`fontId` 复用、`applyFont` / `<b/>` / `<i/>`、
  `applyAlignment` / `wrapText`、默认 `s="0"` 省略、
  sharedStrings + styles 共存、以及非法 foreign `StyleId`
  ...
  这不是完整 styles、full font control、fill/border/full alignment、date cell type、conditional
...
- 当前 image metadata 样例
  ...
  `a:hlinkClick`、drawing-owned hyperlink `.rels` 和 `XlsxWriter` 参考语义。
...
- 把 P9 styles 写成完整样式系统。当前只支持 workbook-local `StyleId`、自定义
  number format、窄 wrap-text alignment、窄 bold/italic font 和 `xl/styles.xml` 基础骨架；
  不支持字体颜色、字号、字体名、下划线、填充、边框、完整对齐、rich text、dxf-backed conditional formatting、date cell type、existing-file style preservation
...
- `fastxlsx-style-registry`：number format / wrap-text alignment / bold-italic font styles、
  `StyleId`、`CellAlignment`、`CellFont`、`CellStyle`、`xl/styles.xml`
  和样式 QA 验证。
