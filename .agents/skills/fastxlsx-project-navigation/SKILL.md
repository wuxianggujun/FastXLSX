...
- Streaming：`WorkbookWriter` / `WorksheetWriter` / `CellView` 用于大型有序新建；`WorkbookReader` 用于已有 worksheet 的 forward-only bounded callback traversal。
...
- Public `WorkbookReader` 已覆盖 bounded row/cell callback、typed scalar、simple inline text、shared/style index、formula + cached scalar 分离和 stored/DEFLATE entry lifecycle；不 seek、不解析完整 sharedStrings/styles、不 materialize worksheet。公式不求值、不生成 cached result、不完整重建 calcChain。
