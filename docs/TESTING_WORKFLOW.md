...
当前 benchmark executable 输出 schema v5。Streaming 必须分列 generation、package close、throughput、body buffer peak/flush count 和 close 后 active temporary file count；Patch 必须分列 direct-range/single-pass transform、parser/source-callback/coalesced/action traffic、output append/flush/peak buffer、relationship/temporary IO、package writer target-entry telemetry，以及 raw compressed-copy entry names/count/bytes。历史 schema-v4 artifact 仍可由 summarizer 读取，但新 evidence 不得删除 v5 资源生命周期字段。
