...
- 已实现 public API：`Workbook`、`Worksheet`、`Cell`、`WorkbookWriter`、
  ...
  `DataValidationErrorStyle`、`TableOptions`、`TableTotalsFunction`、
  `ImageFormat`、`ImageInfo`、`ImageEditAs`、`ImageOptions`、`read_image_info()`、
  `WorkbookWriter::add_style()`、`CellView::with_style()`、`WorksheetWriter::add_image()`
...
- 当前已有 P9a streaming-only number-format styles 基础切片：`StyleId` 是
  ...
  font/fill/border/alignment、date cell type、conditional formatting、rich text 或
