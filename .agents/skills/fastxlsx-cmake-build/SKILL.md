...
- `planned-dev`：已评估但暂不接入任何 preset；现有 tests/benchmarks 不消费 Catch2 或 Google Benchmark。只有真实 target 迁移开始后，才在独立 development preset 中添加 `find_package`、target link 与 focused build 验证。
