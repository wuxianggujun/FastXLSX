...
- 当前数值边界测试覆盖 `NaN`、`+Inf` 和 `-Inf` 非有限输入拒绝：in-memory
  `Workbook::save()` 拒绝 numeric cell / row height，streaming
  `WorksheetWriter::append_row()` 拒绝 numeric cell / row height，
  `WorksheetWriter::set_column_width()` 拒绝非有限 width，避免 worksheet XML 写出
  `nan`、`inf` 或 `-inf`。
...
- XML escape。
- 单元格引用生成。
- 数字/日期/布尔/字符串编码，包括非有限数字、row height 和 column width 的拒绝路径。
