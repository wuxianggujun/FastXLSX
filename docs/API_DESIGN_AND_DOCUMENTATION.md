...
custom number format、窄 wrap-text alignment、窄 bold/italic font 和窄 solid foreground fill slices。`StyleId` 是 workbook-local handle，默认 id `0`
...
- styles API 还需验证文档注释写清 workbook-local `StyleId`、foreign id 拒绝、
  ...
  bold/italic font 不是完整 font control、solid fill 不是完整 fill control，
