...
- 当前测试生成基础和 streaming smoke `.xlsx`，通过 backend-neutral ZIP helper
  ...
  streaming writer metadata、data validations、sharedStrings 结构、基础 docProps 输出和内部 OPC
  manifest 基础。
...
- 当前 image 测试覆盖默认无 `stb` 构建下 `read_image_info()` 的明确错误，以及
  ...
  unsupported image header、missing file。
- 当前没有 Catch2 集成。
...
当前图片 OpenXML 读取/插入仍是 Phase 5 计划。`read_image_info()` 只表示
PNG/JPEG 图片元数据 helper 已有 opt-in `stb` 路径；不要把它写成图片 OpenXML
插入或 existing-workbook 图片保真已完成。
图片功能实现后，结构测试至少检查：
