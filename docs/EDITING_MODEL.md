...
- `WorkbookEditor::add_data_validation()` 是 worksheet-local metadata edit：Streaming/Patch 共用 owning `DataValidationRule`，Patch 将一个或多个 range 序列化为单个 `sqref`，在 schema 正确位置新建或追加 `<dataValidations>`，展开自闭合容器并严格校验已有 `count` 与直接子节点一致。它不创建 `.rels`/content type，不解析或求值 formula1/formula2、不请求重算，也不与后续 cell/row/column mutation 或 linked objects 同步；staged worksheet replacement 与 public count 在提交前保持隔离，失败可 retry。
