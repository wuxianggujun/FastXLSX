...
- C8 第一批 structural metadata sync 已覆盖 worksheet-root merged cells、root auto-filter 与 strict writer-compatible data-validation multi-range `sqref`：先复制 materialized session 并完成 cell/formula candidate，再分别 strict scan 当前 effective worksheet 的三类 metadata，把 changed plans 合并为一次 file-backed worksheet rewrite 和一次 `PackageEditor` commit，最后 noexcept 发布 session 与 public filter diagnostic map。插入执行 shift/expand；删除执行 shift/clip/full removal，merged single-cell result 移除而 filter/validation single-cell result 保留，empty validation/final container 删除。Merged canonical rewrite 只接管 `count`/`ref` attributes 与 whitespace，其他 opaque content 在确实改写时 strict fail；auto-filter 只替换 opening-tag `ref` value 并保留 QName、attribute/quote、comment/PI 与其他 bytes，surviving changed filter 含 criteria/sort element child 时 fail，完整删除可移除 whole element；validation 只替换 opening-tag `sqref`、删除 empty child/container，并在 child removal 时归一化 container `count`，rule/formula/prompt/error 与其余 bytes 保留。Validation source/final ranges 受 64K aggregate guardrail 和全局 duplicate/overlap audit。Metadata-only 空 store 也 dirty，schema/count/QName/overlap/bounds/child-semantics 或 staging 失败不发布半边状态。Materialization 必须把 worksheet root prefix 保存在 session/candidate/swap state；prefixed root 的 dirty `<sheetData>` 保留该 prefix，并用 element-local default SpreadsheetML namespace 约束无前缀 row/cell，避免保存后 metadata reader QName mismatch。下一子切片为 worksheet-local hyperlinks；conditional formatting、tables、drawings、comments/VML 和 relationship graph 继续 non-sync。
...
- Structural metadata focused gate 覆盖 merged-cell、root auto-filter 与 data-validation multi-range `sqref` 的四轴 shift/expand/clip/full removal、merged single-cell removal、filter/validation single-cell preservation、empty validation/final container removal、source/same-session Patch metadata、criteria/sort child rejection、完整 child-bearing filter removal、validation count normalization、formula/prompt/error 与 prefixed QName/attribute-order/quote/child-byte preservation、global duplicate/overlap 与 64K aggregate-range rejection、组合单次 rewrite/commit、空 CellStore dirty、bounds/schema/opaque-content rejection、metadata 与 CellStore 双候选 no-pollution、package staging hook、公式/cell/style/unknown-part preservation 及 save retry/reopen；邻接运行 Patch metadata lifecycle、worksheet metadata/data-validation readers、Streaming metadata、merged-cell lifecycle 与 CellStore source-success targets，其他对象保持明确 non-sync regression。
