...
- Tables、writer-compatible picture anchors 与 classic comments/notes 已有窄 read projection；classic note 另有 new-workbook Streaming write 和 conservative existing-workbook generated-part insertion。Patch `add_note()` 不解析、合并、更新或删除 source-owned comments/VML，因此 existing-object table/drawing/comments semantic edit 仍未公开。Charts、VBA、pivot、external links 和 custom XML 默认只可 preserve/audit/fail；不能因读取、新建或保留测试宣称 existing-object semantic edit。
