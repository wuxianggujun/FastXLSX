...
- Tables、writer-compatible picture anchors 与 classic comments/notes 已有窄 read projection；classic note 另有 new-workbook-only write，但 existing-workbook table/drawing/comments semantic edit 仍未公开。Charts、VBA、pivot、external links 和 custom XML 默认只可 preserve/audit/fail；不能因读取、new-workbook 创建或保留测试宣称 existing-object semantic edit。
