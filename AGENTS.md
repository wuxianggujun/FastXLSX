...
- package writer boundary：基础。`src/package_writer.*` 已把新建 workbook
  ...
  内部 writer 会在打开输出路径前拒绝非法或重复 ZIP entry name。
...
- 把 `src/zip_store_writer.*`、`src/package_writer.*`、`src/package_reader.*` 或
  ...
  会在打开输出路径前拒绝非法或重复 ZIP entry name、需要 Zip64 的 entry count 或
  单 entry 未压缩大小，以及超过 ZIP 16-bit 字段的 entry name；仍无 Zip64、true package streaming、
