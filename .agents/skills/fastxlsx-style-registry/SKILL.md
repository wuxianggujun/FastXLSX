...
description: "实现或审查 FastXLSX 样式注册表、StyleId、CellAlignment、CellFont、CellStyle、WorkbookWriter::add_style()、CellView::with_style()、xl/styles.xml、number formats、wrap-text alignment、bold/italic font、worksheet s 属性、sharedStrings + styles 共存和样式 QA。用于 P9 styles、未来 full-font/fill/border/full-alignment、conditional formatting 与 styles 交互，或排查 Excel 修复样式输出。"
---
...
- 当前 P9 基础切片支持 streaming-only new-workbook custom number format styles、窄
  wrap-text alignment styles 和窄 bold/italic font styles。
...
- `CellAlignment` 当前只支持 `wrap_text`；`CellFont` 当前只支持 `bold` / `italic`；
  `CellStyle` 当前包含 `number_format`、可选 `alignment` 和可选 `font`。完全空的 style
  会被拒绝；`alignment` 为空或 `wrap_text=false` 不贡献 style 属性；`font` 为空或
  `bold=false && italic=false` 不贡献 style 属性。
...
1. 判断任务是修补现有 number format / wrap-text / bold-italic font slice，还是新增 full-font/fill/border/full-alignment 等后续 slice。
...
公式、NumberFormat、WrapText、Font.Bold 和 Font.Italic。这些是本地 QA，不是运行时依赖，也不是默认 CI 强制项。

## 禁止事项

- 不要把 P9 写成完整 styles 或 Excel formatting parity。
- 不要声称支持 full font control、fill、border、full alignment、rich text、dxf-backed conditional formatting、
