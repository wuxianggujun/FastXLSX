...
- `include/fastxlsx/detail/cell_store.hpp` 和 `src/cell_store.cpp` 是当前
  ...
  和单个 `CellStore` 的 `max_cells` / `memory_budget_bytes` enforcement；不代表
  public `WorkbookEditor`、`WorksheetEditor`、random cell editing、workbook-level
  guardrails、save-as / Patch handoff、
