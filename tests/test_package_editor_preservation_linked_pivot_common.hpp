...
// Common helpers for PackageEditor linked pivot preservation tests.
...
bool is_package_editor_shard(std::string_view shard)
{
    return shard == "all" || shard == "preservation-linked-pivot"
        || shard == "preservation-linked-pivot-cache-definition"
        || shard == "preservation-linked-pivot-cache-records";
}
