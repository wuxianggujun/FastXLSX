...
- `CellStore` 不承载 row/column metadata、merged cells、tables、filters、validations、conditional formatting、hyperlinks、drawings、charts、VBA、defined names、relationships 或 calcChain。Merged-cell 与 worksheet-root auto-filter structural sync 由 facade 协调独立 metadata plans，不得把 ranges 塞入 CellStore。
...
- C8 merged-cell 与 worksheet-root auto-filter structural sync 已实现：先复制 materialized session 并完成 cell/formula candidate，再分别 strict scan 当前 effective worksheet 的 `<mergeCells>` 与 root `<autoFilter>`，把 changed plans 合并为一次 file-backed worksheet rewrite 和一次 `PackageEditor` commit，最后 noexcept 发布 session 与 public filter diagnostic map。插入执行 shift/expand；删除执行 shift/clip/full removal，merged single-cell result 移除而 filter single-cell result 保留。Merged canonical rewrite 只接管 `count`/`ref` attributes 与 whitespace，其他 opaque content 在确实改写时 strict fail；auto-filter 只替换 opening-tag `ref` value 并保留 QName、attribute/quote、comment/PI 与其他 bytes，surviving changed filter 含 criteria/sort element child 时 fail，完整删除可移除 whole element。Metadata-only 空 store 也 dirty，schema/count/QName/overlap/bounds/child-semantics 或 staging 失败不发布半边状态。Materialization 必须把 worksheet root prefix 保存在 session/candidate/swap state；prefixed root 的 dirty `<sheetData>` 保留该 prefix，并用 element-local default SpreadsheetML namespace 约束无前缀 row/cell，避免保存后 metadata reader QName mismatch。下一子切片为 data validations multi-range `sqref`；hyperlinks、conditional formatting、tables、drawings、comments/VML 和 relationship graph 继续 non-sync。
...
- Structural metadata focused gate 覆盖 merged-cell 与 root auto-filter 的四轴 shift/expand/clip/full removal、merged single-cell removal、filter single-cell preservation、source/same-session filter、criteria/sort child rejection、完整 child-bearing filter removal、prefixed QName/attribute/quote/comment preservation、组合单次 rewrite/commit、空 CellStore dirty、bounds/schema/opaque-content rejection、metadata 与 CellStore 双候选 no-pollution、package staging hook、公式/cell/style/unknown-part preservation 及 save retry/reopen；data validations 接入时独立补 reader/serializer 邻接测试，其他对象保持明确 non-sync regression。
