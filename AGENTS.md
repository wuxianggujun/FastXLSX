...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `DataValidationOperator`、`DataValidationErrorStyle`、`HyperlinkOptions`、
  `ImageFormat`、`ImageInfo`、`ImageOptions`、`read_image_info()`、
...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  空行 dimension、Excel 行/列上限拒绝路径、data validations / external and internal
  hyperlinks / tables / opt-in image drawing XML 输出。
...
- `tests/test_image.cpp` 通过 CTest 注册为 `fastxlsx.image`，覆盖默认无 `stb`
  构建下的明确错误，以及 opt-in `FASTXLSX_ENABLE_STB=ON` 构建下 PNG/JPEG
  文件/内存尺寸和通道读取、unsupported memory/file header、empty memory buffer、
...
当前还支持窄 `ImageOptions` metadata：非空 `name` 会写成 drawing XML
`<xdr:cNvPr name="...">`，非空 `description` 会写成 `descr="..."`；空 `name`
保留生成的 `Picture N`，空 `description` 省略。该 metadata 只改变 drawing
non-visual picture properties，不修改图片二进制、EXIF、media filename、anchor、
relationships、content types 或 worksheet cell text。

不要把文档里的设计名当成已实现符号。`WorkbookWriter` / `WorksheetWriter` /
`CellView` 流式写入骨架已存在；公式、行高、列宽、冻结窗格、自动筛选、
合并单元格、data validations、external/internal hyperlinks 和 streaming-only tables
...
- Phase 5 worksheet metadata：基础。`WorksheetWriter::add_data_validation()`
  ...
  `show_error_message`、`error_style`、`prompt_title`、`prompt`、`error_title`
  和 `error` 会写成 `<dataValidation>` attributes；空字符串省略，false 布尔值
  省略，不生成 `.rels`、`styles.xml` 或 content type 副作用，也不代表完整 Excel
  data validation UI。当前还支持一条 data validation 规则对应多个合法 `CellRange`：
  ...
  压缩或格式转换。`ImageOptions` 只把 name/description 字符串复制进 writer state，
  并在 drawing XML 的 `xdr:cNvPr` 写 `name` / `descr` attributes；它不代表完整
  alt text/accessibility UI、图片文件 metadata、existing-workbook 图片编辑或图片保真。
...
- 当前 `fastxlsx.streaming` data validations 推荐 preset 输出样例为
  ...
  `promptTitle`、`prompt`、`errorTitle` 和 `error` 只写 `<dataValidation>`
  attributes，覆盖 attribute escape、空字符串省略、false 布尔值省略、`stop` /
  `warning` / `information` 三种 error style，并确认不生成 worksheet `.rels`、
  `xl/metadata.xml`、`xl/styles.xml`、workbook relationships、content type
  side effects 或 `<calcPr>`。本机 `py tools\verify_data_validation_prompts.py`
  已验证 FastXLSX package XML、`openpyxl 3.1.2` 读取结果，并创建
  `openpyxl` / `XlsxWriter 3.2.0` 参考 workbook；本机 Excel COM 已只读打开
  该样例并核对 `ValidationPrompt!A2:D2` 的 validation prompt/error 属性。
  Excel COM 会把 custom validation 公式 `LEN(D2)>0` 返回为 `=LEN(D2)>0`；
  公式和属性结构语义仍以拆包后的 worksheet XML 为准。
...
- 当前 opt-in image preset 还覆盖最大合法 anchor marker 结构测试，确认
  ...
  不是百万行/最大列大文件性能证明。
- 当前 opt-in image preset 还覆盖 drawing non-visual metadata：
  `build/windows-nmake-release-image/tests/fastxlsx-streaming-image-metadata.xlsx`
  会在同一 worksheet 的三个 anchors 中写自定义 `xdr:cNvPr name`、`descr`、
  XML attribute escape、空 description 省略和默认 `Picture N` 名称。该测试确认
  metadata 不新增额外 drawing part、worksheet relationships、content types 或
  media part 语义。
...
- 把 `ImageOptions::name` / `description` 写成完整图片 metadata、完整 alt text UI、
  EXIF/PNG/JPEG metadata、media filename 语义或 existing-workbook 图片编辑能力。
