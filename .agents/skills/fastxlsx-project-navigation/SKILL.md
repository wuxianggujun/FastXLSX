...
- `WorkbookEditor::replace_cells(..., CellPatchMissingCellPolicy::Insert)` 是当前
  ...
  index 拼接 existing-cell replacement payload。它们不是 public API，也不是当前 public
  Patch 的默认 algorithm switch、PackageEditor source-entry ZIP seek 或完整 indexed
  worksheet rewrite 能力。
