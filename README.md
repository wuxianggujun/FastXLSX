...
- **Patch rewrite 现状**：最新同机 warmed 2+5 upsert profile 中，1,000,000 cells、level 1 的 total/mutation/transform median 为 1.205/0.931/0.767 秒，较同协议旧 bundle 分别降低 41.33%/47.07%/52.11%，process peak working set 为 8.80078 MB。5,000,000 cells、约 183.7 MB logical worksheet rewrite 的 total/transform median 为 8.494/5.855 秒，peak 为 8.80859 MB，owned batching buffer 仍为 256 KiB；这只证明记录 workload 的顺序 rewrite 内存有界，不是任意随机编辑承诺。
- **Patch 下一瓶颈**：同一 1,000,000-cell level 6 profile 中，package writer median 为 0.934 秒，single-pass transform 为 0.865 秒，DEFLATE 输出已成为更大的测量阶段。后续应优先评估 compression backend/参数与 source scan/action residual，不扩大 In-memory guardrail，也不在无证据时引入并行压缩。
