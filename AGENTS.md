...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  tables / streaming number-format styles / 默认 stb image drawing XML 输出。
...
对齐、rich text、条件格式、date cell type、existing-file style preservation 或完整
...
- 把 P9 number-format styles 写成完整样式系统。当前只支持 workbook-local
  ...
  边框、对齐、rich text、条件格式、date cell type、existing-file style preservation
