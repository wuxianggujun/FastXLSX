...
但它仍由 stored ZIP bootstrap 支撑，不是生产 `PackageWriter`。`PackageReader`、
生产 `PackageWriter` 以及已有文件编辑仍是计划。当前已有内部
...
- package writer boundary：基础。`src/package_writer.*` 已把新建 workbook
  输出从 workbook writer 代码中隔离出来，当前 backend 仍是
  `src/zip_store_writer.*` 的 stored/no-compression bootstrap；不要据此宣称
  compression、Zip64 或 package streaming。
...
- 把 `src/zip_store_writer.*` 或 `src/package_writer.*` 当成长期生产 ZIP 后端。
  当前 package writer 只是内部边界，backend 仍是 Phase 1 bootstrap：
