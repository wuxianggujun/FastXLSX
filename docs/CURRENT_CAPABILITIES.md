...
- Guardrail：`WorksheetEditorOptions::max_cells` 和 `memory_budget_bytes` 约束 source materialization
  ...
  Clean budget-release, missing-erase, and blank-overwrite recovery handles are covered
  by full/range/requested sparse snapshots, row/column snapshots, duplicate requested
  coordinates, and empty gap row/column reads.
