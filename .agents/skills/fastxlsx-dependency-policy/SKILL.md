...
PNG/JPEG `read_image_info()` 元数据 API；这只能说明图片解码、尺寸、通道和像素读取
可用，不能代表 OpenXML media/drawing packaging、relationships、content types、anchors
或已有 workbook 图片保真已经实现。
