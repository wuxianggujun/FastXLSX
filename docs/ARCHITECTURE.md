...
- `Cell` 这类 owning public value 适合小文件便利 API 和临时输入值，不应成为
  大规模 worksheet 的统一内部存储模型；大文件写入走 `CellView` / row iterator，
  小文件随机编辑也需要独立的紧凑 cell store。
...
以及 `[Content_Types].xml` / `.rels` 第一个真实 XML 元素必须是
`Types` / `Relationships` 的 decoy-root 拒绝；reader 只 ingest root 的 direct-child
`Default` / `Override` / `Relationship` 元素，嵌套 declaration decoy 也会失败。
这只是 metadata 校验；
unknown extension owner `.rels` metadata ingestion 和 `RelationshipGraph` 挂回；当前还
有内部 workbook sheet-catalog resolver，可只读取 workbook `<sheets>` 目录的直接
`<sheet>` 子元素前，先验证 package `_rels/.rels` 中存在且仅存在一个 internal
`officeDocument` relationship；当前窄实现只接受解析到 `/xl/workbook.xml` 的 target，
并显式覆盖 `xl/./workbook.xml` 这类 dot-segment 相对 package entrypoint；
缺失、重复、external、带 query/fragment 或非固定 target 会在 lookup 阶段失败。
...
并按 sheet name 找到已有 worksheet part；缺失 namespace-valid `r:id`、
workbook `.rels` 中缺失对应 relationship id、worksheet relationship 指向未注册
worksheet part、external sheet target 或非 worksheet target 会在 by-name Patch
状态变更前的 lookup 阶段失败，重复 sheet name
lookup 会按 ambiguous 失败。这只是 Patch
目标定位基础，不是 sheet add/delete/rename、workbook DOM 或 public editing API。当前还有内部 `PackageEditor`
stored-package copy/replace 基础。`save_as()` 会拒绝写回覆盖 exact 或
path-equivalent 的 source package 路径，也会在 materialize output entries 前拒绝
空路径、缺失父目录、非目录父路径或已存在目录作为输出路径，因为当前 reader-backed copy 路径不是
atomic in-place editor；当前回归还验证这些 guard 失败后保留 queued part
replacement、structured audit snapshots、calc policy 和 removal audits 的
`EditPlan` / manifest / planned output 状态，并允许后续安全
`save_as()` 输出 queued rewrite、继续保留 untouched worksheet 和 unknown bytes；
同一 guard 现在还覆盖 queued worksheet replacement 的 `fullCalcOnLoad` /
calcChain removal / package-entry audit / planned output 状态，后续安全输出仍按计划
省略 calcChain 并保留 unknown bytes。该 editor 当前已有 worksheet replacement 窄切片，
以及 core/app document-properties generated-small-XML 窄切片；docProps 切片可为
既有 package 生成/替换 `docProps/core.xml` 和 `docProps/app.xml`，包括两者都缺失的
输入，并同步 package rels 和 content types；若此前显式移除了已有 core/app docProps
payload，后续 docProps helper 也可清理 stale removed-part / omitted-payload 状态并恢复
generated-small-XML 输出，但不恢复 docProps owner `.rels`；当前结构回归验证输出包继续
省略该 owner `.rels`，并保留 removed package-entry audit。当前还验证该 helper 在重写
package rels / content types 时保留已有 `docProps/custom.xml`、custom-properties
package relationship、custom properties content type override 和 unknown bytes；这不是
custom properties 编辑或事务式 undo；worksheet
切片可在替换既有 worksheet part 时同步
calcChain remove、workbook fullCalcOnLoad、workbook rels 和 content types，
并保留 source content type defaults/overrides 形态，避免把默认类型媒体 part
无故提升为 override；worksheet 切片还会清理缺失 `xl/calcChain.xml` payload
时残留的 calcChain content type override 或 workbook relationship metadata，
但不创建 payload、不记录缺失 payload 的 removed-part audit，也不是通用 metadata
repair；当前还存在内部 `PackageEditor::replace_worksheet_sheet_data()` helper，
只替换已有 worksheet XML 中的 `<sheetData>` / `<sheetData/>`，保留同一 worksheet
part 的外围 XML metadata，并复用 worksheet replacement 的 calcChain/fullCalcOnLoad
和 preservation 副作用；成功后会把保留的 worksheet-local metadata ranges/references
写入内部 `EditPlan` notes 供 caller review，当前覆盖 sheetPr、dimension、
sheetViews、sheetFormatPr、cols、sheetProtection、protectedRanges、autoFilter、
mergeCells、scenarios、dataConsolidate、customProperties、cellWatches、smartTags、
...
当前组合回归还验证先排队 worksheet replacement 后再执行
sheetData patch 时，helper 基于当前 planned worksheet bytes 替换，保留 queued
wrapper metadata，覆盖 queued worksheet 中普通 `<sheetData>` 和 self-closing
`<sheetData/>` 两种形态，不把 source-only worksheet metadata 复活；当前还覆盖源 worksheet
使用 self-closing `<sheetData/>` 的成功替换：输出改为普通
`<sheetData>...</sheetData>`，保留 dimension / autoFilter，沿默认 calcChain
remove / fullCalcOnLoad 路径清理 stale 计算 metadata，并保留 unknown bytes；
...
`sheetData` replacement 路径；resolver 会从 package root 解析 `_rels/.rels`
的 internal `officeDocument` target，相对、绝对和 dot-segment package target
（例如 `xl/./workbook.xml`）都会规范到 `/xl/workbook.xml`，且不会把 package root
建模成真实 `PartName`；当前 source-catalog
by-name Patch 回归还覆盖 workbook `.rels` 中的绝对 worksheet target
（例如 `/xl/worksheets/sheetN.xml`）和 dot-segment 相对 target
（例如 `./worksheets/../worksheets/sheetN.xml`）解析到已有 worksheet part；
完整 worksheet by-name replacement 与 by-name `sheetData` patch 都复用该定位，
输出继续保留 worksheet target 文本和 unknown extension bytes；当 calcChain
cleanup 需要重写 workbook `.rels` 时，这不是整份 workbook relationships
字节保真，也不是 arbitrary workbook location、relationship repair 或 pruning。缺失或重复 sheet name 失败不污染 EditPlan、manifest、
...
当前还支持内部 `PackageEditor::rename_sheet_catalog_entry()` 窄切片：只改当前
planned `/xl/workbook.xml` 中直接 `<sheets><sheet name="...">` 的 `name`
attribute，把 workbook part 标记为 `LocalDomRewrite`，并保留 worksheet parts、
workbook `.rels`、content types、calcChain 和 unknown entries。它会基于 planned
workbook catalog 解析旧名；planned workbook XML 路径的内部 `planned_output()`
...
`ReferencePolicyAction::Fail` 且 workbook 有直接 `definedNames`、或 workbook 已 planned removal 时，失败不污染
EditPlan、manifest、package-entry audit、calc policy 或输出 bytes。它不更新
definedNames、公式、tables、drawings、charts、hyperlinks、relationship targets、
sharedStrings、styles 或 calcChain，不能写成完整 sheet rename/add/delete 或 public API；
...
replacement `<sheetData>` payload 若使用
shared string indexes、style id references 或公式 cell，也只会生成 audit notes 和结构化
`WorksheetPayloadDependencyAudit`，提示 caller 复核 `xl/sharedStrings.xml`、`xl/styles.xml`、workbook calc metadata 和
calcChain policy，不迁移 sharedStrings 索引、不合并 styles、不计算公式、不重建
calcChain；当前结构测试还验证输出包中保留的 worksheet `.rels` legacyDrawing
`rId7` target `../drawings/vmlDrawing1.vml#shape1` 可由 `PackageReader` /
`RelationshipGraph` 重读，这仍是 preservation 证据，不是 VML/drawing 编辑；这不是
XML repair；invalid/malformed replacement XML、source worksheet 缺失 `sheetData`，以及 source
worksheet `<sheetData>` 起始标签存在但闭合标签损坏/缺失时，失败不污染
EditPlan、manifest、package-entry audit、calc policy 或输出 bytes；这仍不是
public API、随机 cell 编辑、range 修复、
dataValidations/conditionalFormatting/hyperlinks/table/drawing 语义同步、sharedStrings/styles
迁移或大文件低内存 transformer；当前还存在内部
`PackageEditor::request_full_calculation()` workbook calc
metadata helper，只局部重写 `/xl/workbook.xml` 小型 metadata part 来请求
`fullCalcOnLoad="1"`，并可按 `CalcChainAction::Remove` 清理 stale calcChain
payload、content type、workbook relationship 和 calcChain owner `.rels`，或按
同一 remove 路径清理只有 stale metadata、没有 `xl/calcChain.xml` payload 的残留
calcChain content type / workbook relationship，且不创建 payload 或 removed-part audit；
`CalcChainAction::Preserve` 保留这些 calcChain payload/metadata；`Rebuild`
仍未实现且失败不污染状态。这不是 worksheet rewrite、公式求值、calcChain rebuild、
public editing API 或通用 relationship/content-type repair；workbook-only calc helper
只更新 workbook 根元素的直接子级 `calcPr`，会保留
`extLst` / custom extension 内的嵌套同名 decoy；缺少直接子级时才在真实 workbook
closing tag 前插入按根前缀命名的 `calcPr`，这不是 XML schema validation 或
namespace repair；组合回归还覆盖同一 edit 内 docProps 生成与 worksheet
replacement 的 relationships/content types 状态合并、calcChain removal、stale
calcChain owner `.rels` omission、workbook metadata rewrite 和 unknown entry
preservation、exact/path-equivalent source-overwrite rejection 和
empty-output / missing-parent / non-directory-parent / existing-directory output rejection；普通
`replace_part()` 现在显式拒绝
`[Content_Types].xml`、package `_rels/.rels` 和 source-owned `.rels` 作为 ordinary
part replacement target，避免绕过窄 metadata helper 和 package-entry audit；
malformed workbook metadata / workbook calc metadata preflight failure 与
invalid replacement failure 回归覆盖 edit plan、manifest write-mode、
aggregate `planned_output()` / legacy output-entry preview 和 copied output bytes 不污染；
metadata-entry replacement failure 回归也覆盖 edit plan entries / notes、
package-entry audit、calc policy、manifest write-mode、aggregate `planned_output()` /
legacy output-entry preview 和 copied output bytes 不污染；
workbook-only `request_full_calculation()` 还覆盖 prior ordinary workbook / calcChain
replacement 顺序组合：helper 会消费已排队 workbook XML，保留其非 calc metadata，
规范 `fullCalcOnLoad="1"` 并清理 calcChain payload/content type/workbook
relationship；此前排队的 calcChain replacement 不会在输出包复活，也不会回退到
source workbook bytes；
若同一路径指定 `CalcChainAction::Preserve`，此前排队的 calcChain replacement 会保持为
active `LocalDomRewrite` 并作为最终 `xl/calcChain.xml` payload 写出，同时只记录
calcChain owner `.rels` copy-original audit；这不是 calcChain rebuild 或公式求值；
普通 part replacement、docProps generated parts 和 worksheet
replacement 会把 write-mode / dirty / generated / preserve-original 状态同步到内部
manifest；内部 `EditPlan` 还会记录 `[Content_Types].xml`、package `_rels/.rels`、
workbook `.rels` rewrite、removed calcChain owner `.rels` omission，以及窄 worksheet
replacement 中 source-owned `.rels` 存在时的 copy-original audit 这类非 part
package-entry 审计项；该 copy-original audit 也覆盖 `CalcChainAction::Preserve`
时被原样保留的 calcChain owner `.rels`，以及 workbook metadata rewrite 时未改写而
原样保留的 workbook `.rels`；
这些审计项带内部结构化 kind：content types、package relationships 或
source-owned relationships；只有 source-owned relationships 携带 owner part。
kind 与 entry path 必须匹配：content types 指向 `[Content_Types].xml`，
package relationships 指向 `_rels/.rels`，source-owned relationships 指向
owner part 推导出的 `.rels` entry。
它们只是 Patch traceability metadata，不是 relationship/content-type mutator；
重复 ordinary part replacement 回归还验证同一 part 的 replacement、edit-plan entry、
manifest write-mode 和 preserved source-owned `.rels` audit 会 upsert 到最终状态；
内部 `EditPlan` 也覆盖 part-level set/remove 互斥状态，restored active part 会清理
stale removed-part audit，已有 relationship-derived entry 改成 rewrite/generate entry
时也会清理 stale relationship metadata；package-entry set/remove 也有互斥回归，
避免同一 metadata entry 同时存在 active 和 removed 审计；
docProps generated-small-XML 被后续 ordinary replacement 覆盖时，输出 bytes、
EditPlan 和 manifest 也采用最终 ordinary replacement，同时保留 metadata helper
负责的 content types / package relationships audit；反向顺序也有窄回归覆盖：
docProps metadata helper 后调用会接管此前 ordinary replacement 的 core/app part，
输出 bytes、EditPlan 和 manifest 采用 helper 生成状态；同类回归还覆盖 docProps helper
接管此前 explicit removal 的 core/app payload，并清理 stale removed-part / omitted-payload
状态；worksheet replacement 删除
calcChain 时会压过此前 ordinary calcChain replacement，并接管此前 ordinary workbook
replacement 以写入 helper-generated fullCalcOnLoad metadata，避免旧 replacement 复活；
反向顺序也覆盖 worksheet replacement 后再 ordinary replace `xl/workbook.xml` 时，后者输出
仍保留既有 `fullCalcOnLoad` / calcChain policy，且不会把已重写的 workbook `.rels`
...
root-level ordinary owner replacement 还验证 preserved source-owned `.rels`
roundtrip 后可由 `PackageReader` / `RelationshipGraph` 挂回 owner part；reader-only
测试也直接覆盖 unknown extension owner `.rels` 的挂回；
worksheet replacement 会把 workbook metadata rewrite 同步为
`LocalDomRewrite`，供 Patch 审计；
并有 linked-object fixture 证明 worksheet
`.rels`、drawing、drawing `.rels`、media、chart、table、untouched `xl/sharedStrings.xml`、
sharedStrings owner `.rels`、untouched `xl/styles.xml`、VBA 和可达 unknown extension
part 及其 owner `.rels` 在该窄路径下 byte-preserved，
...
另有 no-op `save_as()` roundtrip 证明同一 linked-object fixture 的全部源 entries
在未修改输出中保持 entry 顺序、stored entry method / CRC / uncompressed size 和 bytes，
且初始 edit plan 没有 metadata package-entry side effect；这只是 copy baseline，不是
broad preservation pipeline；
...
ordinary single-part replacement 回归还验证目标 entry 保持源顺序位置并被重写，
其它源 entries 保持 stored entry method / CRC / uncompressed size 和 bytes；这只是窄
...
不保留源 ZIP compression method、timestamps、extra fields 或压缩字节；
...
同一路径还覆盖 ordinary workbook replace-then-remove：后续 removal 清理 active workbook
replacement、记录 removed-part 和 workbook owner `.rels` omission audit，输出省略
workbook part 及其 owner `.rels`、移除 workbook content type override，但保留 package
`_rels/.rels` 中指向缺失 workbook 的 officeDocument relationship 以及 worksheet/
drawing/table/sharedStrings/styles/VBA/calcChain/unknown downstream parts；这不是
...
该 fixture 还确认 external、URI-qualified、invalid 和 unresolved relationship target 的审计
note 与结构化 `RelationshipTargetAudit` 会传播到 existing-file `PackageEditor`
edit plan，但不创建或修复 package part；
`ReferencePolicy` 相关窄回归还覆盖 linked-object
fail、calcChain preserve / rebuild 拒绝、malformed workbook metadata / workbook calc
metadata rewrite 预检失败不污染状态并保持 aggregate `planned_output()` copy-original、
缺少 `xl/workbook.xml` 时 worksheet rewrite 失败不污染状态、已有 ordinary workbook replacement 排队后 linked-object fail
仍保留既有 replacement / manifest / source-owned `.rels` audit / 输出 bytes，
queued core/app docProps helper 后 linked-object fail 仍保留既有 metadata edit / audit / 输出 bytes，和
request-recalculation fullCalcOnLoad 输出路径；
这些 no-state-pollution 回归也覆盖 `relationship_target_audits()` 和
`worksheet_relationship_reference_audits()` 不追加 stale 结构化 relationship target /
worksheet reference audit；这只是内部 Patch audit 状态卫生，不是 target validation
或 repair。
...
只替换目标 worksheet 的 `<sheetData>`，`save_as()` 后 re-read 验证 untouched
...
保留且通过最终 worksheet 根校验；source 走 sharedStrings 与 styles 时，还验证
`xl/sharedStrings.xml`、`xl/styles.xml`、对应 content type override 和 workbook relationships byte-preserved，
并把 replacement `<sheetData>` 的 shared string index / style id references 记录为
结构化 `WorksheetPayloadDependencyAudit`。workbook metadata 请求
`fullCalcOnLoad="1"`，且不会在 source package 没有 `xl/calcChain.xml` 时创建
calcChain。这是 Patch MVP/template-fill 方向的内部架构回归，不是 public editing
API、随机 cell 编辑、sharedStrings 索引迁移、style id 迁移、styles 合并、
relationship repair、table/drawing 语义同步或大文件 worksheet transformer。
已有文件编辑用 public `PackageWriter`、
public `PackageEditor`、`WorksheetReader`、`TemplateEditor` 等仍是规划模块。
...
CellValue -> implemented owning semantic value; future editor/in-memory API value boundary
CellStore -> implemented first internal sparse-store slice; future compact/pool evolution
```

P7.3 internal storage draft：

- `CellStore` 是 worksheet-local sparse store，只索引存在或被显式编辑过的 cells，
  当前首片使用 row-major map，不创建完整 worksheet matrix。
- `CellRecord` 保存 value kind、default / workbook-local style handle 和紧凑 payload；
  number / boolean 内联保存，当前首片把 text / formula 存为 record-owned string。
- string / formula pool 是后续 compact-storage detail，不等同于 `xl/sharedStrings.xml`
...
- missing cell 表示 store 中无 record；blank / clear 候选值可用 explicit blank record
  或 tombstone 表达。当前 `erase_cell()` 只移除 sparse record，最终 erase/save-as 行为
  由后续 P7.5 定义。
...
- `docProps/core.xml` / `docProps/app.xml`：当前已有内部小型 metadata
  generated-small-XML rewrite 窄切片。
- `calcChain.xml`：修改公式或数据后可能删除、重建或标记重算。
- drawings / charts / pivot caches / VBA / unknown parts：默认保留，只有明确支持时才改写。

当前内部 `DependencyAnalyzer` 只做保守 part-level summary：它可以识别 worksheet
`.rels` 是否存在，并沿已知 internal relationship target 做保守遍历，例如
worksheet → table 和 worksheet → drawing → image/chart，把这些 part 加入依赖列表；
当前回归还覆盖递归到 drawing-owned `.rels` 后的 external、URI-qualified、invalid 和
unresolved relationship target 审计 note。external hyperlink target 不会被当成 package
part，但会留下 external target 审计 note；带 query/fragment 的
internal relationship target 会留下 URI-qualified 审计 note，并在 base target 解析到
已注册 package part 时保守纳入依赖；以 `/` 开头的 absolute internal target 会按
package part path 做 normalization；percent-encoded internal relationship target
会先解码 `%XX` 后再做 part-name normalization，已注册的 decoded target 会纳入依赖；
malformed percent escape 或解码后非法的 target 仍走 invalid-target 审计路径；逃出 package root，或解析到内部路径但当前 manifest
未注册的 internal relationship target 只留下 package structure review note，不虚构 package part。
这些 relationship target 审计 note 和结构化 `RelationshipTargetAudit` 会携带 owner part、
relationship id、relationship type、原始 target，以及可用时的 normalized base target part，
并会从 `DependencyAnalysis` 传播到 worksheet `EditPlan` 和 existing-file
...
`RelationshipTargetAudit` 做 upsert，并对完全相同的 audit note 去重；重复
worksheet rewrite 不会让同一 relationship target audit 无限累积。这只是内部
Patch audit traceability 的状态一致性，不是 relationship mutator。
完整 worksheet replacement 和 bounded `sheetData` replacement 的 payload dependency
review 还会传播为结构化 `WorksheetPayloadDependencyAudit`，按 worksheet/kind/scope/element
upsert 到 `EditPlan` 和 `PackageEditorOutputPlan`；它只标记 sharedStrings、styles、
formula、range metadata 和 relationship-bearing metadata 的复核范围，不做语义修复。
...
`PartRewritePlanner` 会把这些 dependency reason
写回 copy-original entries，并可把已注册目标 part 的显式删除表达为 removed-part
audit；当前 `plan_worksheet_stream_rewrite()` 在 `CalcChainAction::Remove` 下会直接
产出 stale calcChain removed-part audit，worksheet replacement 只消费该 planner 输出；
relationship-driven reason 会携带 relationship id、
relationship type 和 normalized target part path，其中 workbook part 会显式携带 calcPr / definedNames
review 上下文，方便审计 part-level copy/rewrite/removal 决策。这仍不是完整
drawings/tables/images/charts 编辑能力。当前 linked-object / sharedStrings / styles /
definedNames 的回归只证明未修改内容在窄路径下保留，不代表关系裁剪、
orphan cleanup、字符串索引迁移、style id 迁移或 defined-name 引用更新。
...
Patch 审计元数据。workbook/sharedStrings/styles 等静态依赖仍只带 reason 文本，
不会伪造 relationship metadata；这些字段不是 public relationship editor。
...
- In-memory 便利模型必须有明确 size / memory guardrails，且不能把 owning `Cell`
  作为所有单元格规模的默认存储策略。
