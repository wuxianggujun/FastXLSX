...
- 当前 `Cell::formula()` / `CellView::formula()` 写入 worksheet `<f>`，并让包含公式
  ...
  `calcChain.xml` 支持。
- `WorksheetWriter::add_data_validation()` 是 Streaming metadata API：规则、公式文本和
  prompt/error 文本被复制进 writer state，内存按规则数量、公式文本长度和
  prompt/error 文本长度增长；`showInputMessage`、`showErrorMessage`、`errorStyle`、
  `promptTitle`、`prompt`、`errorTitle` 和 `error` 只写为 worksheet `<dataValidation>`
  attributes，空字符串和 false flags 省略。它不解析公式、不校验单元格值、不检查重叠、
  不新增 relationships/content types/styles，也不支持 existing-file editing 或完整 Excel UI。
...
new-workbook-only、规则数量/公式文本/prompt-error 文本内存成本、公式文本和
prompt/error 文本拷贝、attribute escape、空字符串和 false flag 省略、无公式求值、
无单元格值校验、无重叠检查、无完整 Excel UI 保证，以及是否新增
relationships/content types/styles。
