...
`replace_cells(sheet_name, cells, CellPatchMissingCellPolicy::Insert)` /
...
`PackageEditor::replace_worksheet_cells_by_name()`，默认只替换 source/planned worksheet
stream 中已经存在的 `<c>` cells；显式 `CellPatchMissingCellPolicy::Insert` 复用同一
transformer path 做 bounded point upsert，可插入缺失 cells 或合成 minimal rows。
rewritten worksheet 以 file-backed chunks 进入 Patch 输出；`rename_sheet()` 复用
...
worksheet 的 bounded targeted-cell Patch path，默认 strict replacement，`Insert` policy
只做 point upsert；不 shift 行列、不迁移 sharedStrings/styles、不修复 range metadata 或
relationships。它仍**不**表示完整
...
- 当前 bounded `sheetData` local rewrite 会物化 worksheet XML，不能写成大文件低内存
  transformer。
- 当前 `replace_worksheet_cells()` 对 source package worksheet entry 已通过
  `PackageReader::entry_chunk_source()` 直接创建 pull-based ZIP-entry chunk source，再由
  ...
  `extract_entry_to_file()` 的 minizip DEFLATE 路径也复用该 chunk source 逐块写 output
  file；`read_entry()` 的 minizip DEFLATE 路径也复用该 chunk source 后再 materialize
  到返回的 `std::string`。
  P8.19 已新增
  ...
  early-abandon cleanup，P8.31 已把 DEFLATE `extract_entry_to_file()` 接到 chunk
  source，P8.32 已把 DEFLATE `read_entry()` 的解压逻辑接到同一 chunk source。
  queued string 仍由 prior planned replacement helper 持有完整
  `std::string`，`read_entry()` 仍会返回 materialized `std::string`，所以这不是 full input low-memory pipeline；
...
P8.16-P8.32 又补上 cell replacement 输出侧 file-backed chunk handoff、source-entry
...
P8.31 又把 minizip DEFLATE `extract_entry_to_file()` 接到同一 chunk-source extraction。
P8.32 又把 minizip DEFLATE `read_entry()` 的解压逻辑接到同一 chunk source，但返回值
仍是完整 `std::string`。
但 queued string 仍是 prior helper 已持有的完整字符串，且
`read_entry()` 仍会 materialize，因此不能写成
...
abandoned DEFLATE chunk-source handle cleanup、DEFLATE extract-to-file chunk-source handoff
和 DEFLATE read-entry chunk-source materialization handoff，
...
更广的 PackageReader input-state hardening、full planned-input low-memory pipeline 和
public/full transformer boundary 仍是后续任务。
...
- 当前 public `WorksheetEditor` 回归也把 clear 与 erase 的预算语义分开：erase 删除
  ...
  batch clear 保留 blank records，但释放被清空 value payload 的 estimate；这仍不是
  进程 RSS 或保存阶段峰值承诺。
...
替换整个 `<sheetData>`、替换一批已存在 cells、用 explicit enum policy 做 point upsert，
或改写 sheet catalog 名，再 `save_as()` 输出新 package。这是当前 existing-file editing public facade
