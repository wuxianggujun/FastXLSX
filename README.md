...
- **Patch 下一瓶颈**：canonical complete-cell path 完成后，记录的 optimized source scan/action median 仍约 0.272–0.510 秒，而 package target-entry 与 temporary-file timing 已可能主导 total elapsed。下一轮应在隔离负载下 profile staged temporary output → package writer handoff，并保持事务、retry、CRC 与 bounded-memory 契约；只有 size-oriented workload 在 matched level 下仍受压缩限制时，才评估其他 backend。
