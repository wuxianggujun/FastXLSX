...
- `WorksheetEditor::copy_cell_style()` / `clear_cell_style()` 提供同一 materialized worksheet 内的窄 style-only mutation：前者只在 source/target 都是 represented records 时复制 materialization 已从 source styles.xml 校验过的 workbook-local StyleId，unstyled source 会清除 target style；后者清除 represented target 的样式且对 missing/unstyled target clean no-op。两者都保留目标 CellValue、cell count 和 styles.xml bytes，不接受任意 caller StyleId，不创建、合并、验证或迁移 style table，也不是 range/conditional/theme formatting API。
