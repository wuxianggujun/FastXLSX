...
- **Patch package handoff**：Windows production minizip-ng 对至少 4 MiB 的 staged file chunk 使用两个固定 1 MiB buffer，在同步压缩当前 buffer 前发起下一次 overlapped read；小 chunk、raw compressed-copy、stored-only 和非 Windows 路径保持同步读取。最新 schema-v14/v8 双顺序 A/B 中，六组 1,000,000-cell workload 的前台 staged read median 降低 78.75%–91.79%，completion wait 为 62–135 us，paired process peak working set median 变化不超过 0.02734 MB。Target-entry median 仅在六组中的五组下降，因此不声明通用 target-entry 或 total-editor 提升，也不把 warmed protocol 写成物理 OS cache-hit 证据。
...
- **Patch 下一瓶颈**：staged-file 前台读取已不再是记录 workload 的主要 handoff 成本；下一轮应隔离 minizip DEFLATE writer 的 wall time、write/close process CPU 与调度波动，再决定是否优化调用粒度或比较 backend。事务、retry、CRC、bounded memory 与 public compression 默认值保持不变；没有 matched-level 依赖、兼容性、CPU、输出大小和内存证据前，不引入并行压缩或自定义 backend。
