...
改依赖配置前，先读取根目录 `vcpkg.json`。当前仓库已有保守 manifest：默认 `dependencies` 包含 `stb`，用于 PNG/JPEG `read_image_info()` 元数据 helper，并支撑 `WorksheetWriter::add_image()` 的 streaming-only new-workbook 图片插入基础切片；`planned-runtime` 和 `planned-dev` 继续记录 opt-in 或计划依赖。`planned-runtime` 中的 `minizip-ng[core,zlib]` 通过 `FASTXLSX_ENABLE_MINIZIP_NG` 接入 CMake。
...
- `stb`：图片解码、尺寸和像素读取。当前本机 vcpkg port 名称已验证为
  `stb`，license 为 `MIT OR CC-PDDC`。它是默认 manifest dependency，
  vcpkg usage 是 `find_package(Stb REQUIRED)` 和 `${Stb_INCLUDE_DIR}`，当前未见
  imported target；项目 CMake 会把 vcpkg installed tree 中的 `share/stb` 加入
  `CMAKE_MODULE_PATH`，从 installed include 目录预解析 `Stb_INCLUDE_DIR`，再显式使用
  `find_package(Stb MODULE REQUIRED)`。

图片依赖必须按能力边界处理：当前已默认接入 `stb` 和
PNG/JPEG `read_image_info()` 元数据 API；当前也已有
...
当前 `src/zip_store_writer.*` 是内部 bootstrap writer，只用于打通最小
...
- `zlib-ng`、`expat`、`pugixml` 已能随 planned-runtime clean install，但当前源码
  尚未链接或使用它们。
- `windows-nmake-release` 和 `windows-nmake-release-vcpkg` preset 使用 vcpkg
  toolchain；默认会安装 `stb`，但不会安装 `planned-runtime` feature。
- `windows-nmake-release-minizip` preset 会启用 `planned-runtime` 和
  `FASTXLSX_ENABLE_MINIZIP_NG=ON`。
- CI workflow 默认 job 跑 `windows-nmake-release`，覆盖默认 `stb` 路径；独立 opt-in
  vcpkg matrix 跑 `windows-nmake-release-minizip`。这只是结构/CTest 门禁；
  不代表 Excel 可视化验证已在 CI 完成，或 opt-in backend 路径已经生产化。
...
当前图片依赖事实：`stb` vcpkg port 存在，描述为 public domain header-only
libraries，license 是 `MIT OR CC-PDDC`；当前通过 vcpkg installed tree 的
`FindStb.cmake`、预解析的 `${Stb_INCLUDE_DIR}` 和
`find_package(Stb MODULE REQUIRED)` 接入，CI 安装耗时和缓存行为以真实 run 为准。
...
- 图片依赖阶段先确认默认 `vcpkg install --dry-run` 可解析。
- 修改 `planned-runtime`、minizip 或 stb 接入时，本地运行对应
  `windows-nmake-release` / `windows-nmake-release-minizip` preset，并在推送后
  核查对应 GitHub Actions job。
