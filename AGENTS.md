...
- `WorksheetEditor` row/column insert/delete 已同步 worksheet-root merged ranges、root auto-filter、strict writer-compatible data-validation multi-range `sqref` 与 strict worksheet-local hyperlink `ref`：CellStore/formula candidate 和四类 metadata replacements 分开预构造，再合并为一次 file-backed worksheet rewrite 与一次 `PackageEditor` commit，最后 noexcept 发布 session 和 public filter diagnostic map。插入支持 shift/expand；删除支持 shift/clip/full removal，merged single-cell result 移除而 filter/validation/hyperlink clipped single-cell result 保留；空 validation、fully removed hyperlink 与最终空 container 删除。Validation rule/formula/prompt/error 和 hyperlink QName、attribute order/quote、location/target/display/tooltip bytes 保留；external hyperlink `r:id` 只有在没有 surviving hyperlink 且证明未被其他 worksheet element 复用时才删除对应 owner-local relationship。Surviving changed filter 含 criteria/sort element child 时 fail；strict projection、range ownership、64K validation aggregate、schema/count/QName/bounds/child-semantics/relationship ownership/staging 失败不污染 package/session 并可 retry。Conditional formatting、tables、drawings、comments/VML 与其他 linked metadata 尚不自动同步。
...
- `WorkbookEditor` conditional-format lifecycle 覆盖 writer-compatible two-/three-color scale、basic data bar 与 basic `3Arrows` icon set。Add 从 effective max + 1 分配 priority；update/remove 按 current effective zero-based reader index 选择 strict writer-compatible 单规则 container，update 保留原 priority/source order 并替换 ranges + kind/payload，remove 不重排其余 priority。Existing add 走 bounded structural/priority audit，update/remove 走 strict bounded projection；advanced/custom、dxf/styles、formula/cellIs 与 multiple-rule source 不接管并 fail。三者支持 planned rename/added worksheet，只改 worksheet XML，不改 cells、relationships、content types、styles/dxf、calc metadata 或 linked objects；`conditional_format_count`/`conditional_format_update_count`/`conditional_format_removal_count`、pending/unsaved 与 replacement 先 staging 后 commit，失败不污染并可 retry。
- `WorkbookEditor::add_table()` / `update_table()` / `remove_table()` 只管理 writer-compatible table metadata/package graph。Effective catalog 合并 source、planned rename、同会话新增 worksheet 与 lifecycle overlay；update 保留 table id/relationship/part/order，remove 关闭 owner-local graph，最后一项删除 `<tableParts>`。Source mutation 要求 bounded projection、无 child relationships 与 normalized exclusive inbound ownership；identical update clean no-op，missing mutation fail，删除后的 name/range 可复用但 id/part 本 session 不复用。三者均不读取或同步 cell payload、formula/cached result、defined names 或 structural edits，也不是完整 table object model。
