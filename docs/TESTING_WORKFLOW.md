...
- C8 structural metadata synchronization（planned）：对 row/column insert/delete 分别验证 merged-cell、worksheet-root auto-filter 与 data-validation range 的 translate、边界/overlap/invalid-schema rejection、source order/count/schema preservation、公式与 cell/style payload 保持、unknown-part/relationship/content-type/calc metadata 无意外 side effect、failure-before-state-change、dirty/save retry 与 reopen。第一批只运行直接 In-memory structural target、相关 metadata reader/serializer target 和必要邻接测试；hyperlink/conditional-format、table/drawing/comment/VML 同步在各自切片完成前必须保持明确的 non-sync 断言，不默认升级为 production 全量 CTest。
