...
- **下一优先级**：隔离 minizip internal output buffering/backend bookkeeping、zlib compression 与 scheduler noise，必要时使用 ETW 或 sampling profiler；在同 compression level、同 staged payload、同兼容性/输出大小门禁下建立 backend 或内部 buffer 对照。保持 stage → package write → state commit、retry、CRC、bounded memory 和 public compression 默认值；没有独立依赖、CPU、内存与兼容性证据时不引入 zlib-ng、并行压缩或自定义 backend。
