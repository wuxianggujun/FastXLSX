...
- **Patch**：`WorkbookEditor`，面向已有 workbook 的 part-level rewrite，并支持受限的 basic classic note insertion；未修改和未知 part 默认保留。
...
- `read_worksheet_comments()` 只读投影已有 classic comments/notes；`WorksheetWriter::add_note()` 为 new-workbook Streaming 创建 simple author/text、hidden VML shape 和必要 package relationships/content types。`WorkbookEditor::add_note()` 只允许在目标 worksheet 没有 source-owned classic comments、threaded comments 或任何 VML relationship 时生成本 editor 会话拥有的 comments/VML parts；同会话后续调用可重新生成这些 parts。它不解析、合并、更新或删除已有 comments/VML，也不形成 rich/threaded comments、persons、visibility/shape customization 或通用 VML 编辑。
...
- `WorkbookEditor::add_note()` 可向没有 source-owned comments/threaded/VML 语义的 planned worksheet 追加 simple classic note；它按 one-based cell 保存 owning non-empty author/text，不要求或创建目标 cell，并事务式协调 worksheet `<legacyDrawing>`、worksheet `.rels`、generated comments/VML parts、content types、manifest 与 public diagnostics。重复 cell、已有 comments/threaded/VML 或无法证明的 suffix ordering 默认 fail；不做 existing-object merge/update/delete、rich text、shape customization 或 linked cell/formula 同步。
