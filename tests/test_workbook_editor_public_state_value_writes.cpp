...
void test_public_worksheet_editor_set_cell_values_accepts_default_style_id_as_style_preserving_batch()
{
    const std::filesystem::path source = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-default-style-source.xlsx");
    const std::filesystem::path output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-default-style-output.xlsx");
    const std::filesystem::path noop_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-default-style-noop-output.xlsx");
    const std::filesystem::path second_noop_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-default-style-second-noop-output.xlsx");
    ...
    fastxlsx::StyleId non_default_style;
    {
        ...
        styled_sheet.append_row({
            fastxlsx::CellView::number(1.0).with_style(non_default_style),
            fastxlsx::CellView::text("batch-value-default-tail"),
        });
        styled_sheet.append_row({fastxlsx::CellView::text("batch-value-default-a2")});
        writer.close();
    }
    const auto source_entries = fastxlsx::test::read_zip_entries(source);
    ...
    check(live_a1.kind() == fastxlsx::CellValueKind::Number &&
            ...
            live_a1.has_style() &&
            live_a1.style_id().value() == non_default_style.value(),
        "set_cell_values explicit default StyleId should preserve source style on A1");
    const fastxlsx::CellValue live_b1 = sheet.get_cell("B1");
    check(live_b1.kind() == fastxlsx::CellValueKind::Blank &&
            !live_b1.has_style(),
        "set_cell_values explicit default StyleId should keep unstyled B1 blank unstyled");
    const fastxlsx::CellValue live_a2 = sheet.get_cell("A2");
    check(live_a2.kind() == fastxlsx::CellValueKind::Text &&
            live_a2.text_value() == "batch-value-default-a2" &&
            !live_a2.has_style(),
        "set_cell_values explicit default StyleId should keep untouched A2 unstyled");
    ...
    check(live_c2.kind() == fastxlsx::CellValueKind::Formula &&
            ...
            !live_c2.has_style(),
        "set_cell_values explicit default StyleId should insert missing C2 without a style");

    const auto check_batch_value_default_a1_snapshot =
        [non_default_style](
            const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            const std::string prefix(scenario);
            check(snapshot.reference.row == 1 &&
    ...
    const auto check_batch_value_default_b1_snapshot =
        [](const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            const std::string prefix(scenario);
            check(snapshot.reference.row == 1 &&
                    snapshot.reference.column == 2 &&
    ...
    const auto check_batch_value_default_a2_snapshot =
        [](const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            const std::string prefix(scenario);
            check(snapshot.reference.row == 2 &&
                    snapshot.reference.column == 1 &&
                    snapshot.value.kind() == fastxlsx::CellValueKind::Text &&
                    snapshot.value.text_value() == "batch-value-default-a2" &&
                    !snapshot.value.has_style(),
                prefix + " should expose unstyled A2 text");
        };
    const auto check_batch_value_default_c2_snapshot =
        [](const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            const std::string prefix(scenario);
            check(snapshot.reference.row == 2 &&
                    snapshot.reference.column == 3 &&
    ...
    check(sheet.contains_cell("A1") && sheet.contains_cell("B1") &&
        ...
        "set_cell_values explicit default StyleId should keep represented cells queryable");
    check(!sheet.contains_cell("B2") && !sheet.contains_cell("D4"),
        "set_cell_values explicit default StyleId should keep unrelated missing cells absent");
    const std::vector<fastxlsx::WorksheetCellSnapshot> live_row_one =
        sheet.row_cells(1);
    check(live_row_one.size() == 2,
        "set_cell_values explicit default StyleId row_cells should expose row one");
    if (live_row_one.size() == 2) {
        check_batch_value_default_a1_snapshot(live_row_one[0],
            "set_cell_values explicit default StyleId row_cells");
        check_batch_value_default_b1_snapshot(live_row_one[1],
            "set_cell_values explicit default StyleId row_cells");
    }
    const std::vector<fastxlsx::WorksheetCellSnapshot> live_row_two =
        sheet.row_cells(2);
    check(live_row_two.size() == 2,
        "set_cell_values explicit default StyleId row_cells should expose sparse row two");
    if (live_row_two.size() == 2) {
        check_batch_value_default_a2_snapshot(live_row_two[0],
            "set_cell_values explicit default StyleId row_cells");
        check_batch_value_default_c2_snapshot(live_row_two[1],
            "set_cell_values explicit default StyleId row_cells");
    }
    const std::vector<fastxlsx::WorksheetCellSnapshot> live_column_one =
        sheet.column_cells(1);
    check(live_column_one.size() == 2,
        "set_cell_values explicit default StyleId column_cells should expose column one");
    if (live_column_one.size() == 2) {
        check_batch_value_default_a1_snapshot(live_column_one[0],
            "set_cell_values explicit default StyleId column_cells");
        check_batch_value_default_a2_snapshot(live_column_one[1],
            "set_cell_values explicit default StyleId column_cells");
    }
    const std::vector<fastxlsx::WorksheetCellSnapshot> live_column_two =
        sheet.column_cells(2);
    check(live_column_two.size() == 1,
        "set_cell_values explicit default StyleId column_cells should expose B1 only");
    if (live_column_two.size() == 1) {
        check_batch_value_default_b1_snapshot(live_column_two[0],
            "set_cell_values explicit default StyleId column_cells");
    }
    const std::vector<fastxlsx::WorksheetCellSnapshot> live_column_three =
        sheet.column_cells(3);
    check(live_column_three.size() == 1,
        "set_cell_values explicit default StyleId column_cells should expose C2 only");
    if (live_column_three.size() == 1) {
        check_batch_value_default_c2_snapshot(live_column_three[0],
            "set_cell_values explicit default StyleId column_cells");
    }
    check(sheet.row_cells(3).empty() && sheet.column_cells(4).empty(),
        "set_cell_values explicit default StyleId sparse views should keep gaps empty");
    check(sheet.cell_count() == 4,
        "set_cell_values explicit default StyleId should keep represented sparse count");
    check_cell_range_equals(sheet.used_range(), 1, 1, 2, 3,
        "set_cell_values explicit default StyleId should extend bounds to C2");
    check(sheet.has_pending_changes(),
        "set_cell_values explicit default StyleId should dirty the materialized worksheet");
    check(editor.pending_materialized_cell_count() == 4,
        "set_cell_values explicit default StyleId should expose aggregate materialized count");
    check_public_state_single_named_dirty_materialized_summary(
        editor, sheet, "Styled", 0, "set_cell_values explicit default StyleId dirty summary");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values explicit default StyleId should keep diagnostics clear");

    const auto inspect_default_style_batch_output =
        [non_default_style, check_batch_value_default_a1_snapshot,
            check_batch_value_default_b1_snapshot,
            check_batch_value_default_a2_snapshot,
            check_batch_value_default_c2_snapshot](
            fastxlsx::WorksheetEditor& reopened_sheet,
            std::string_view scenario) {
            const std::string prefix(scenario);
            ...
            const fastxlsx::CellValue reopened_a2 = reopened_sheet.get_cell("A2");
            check(reopened_a2.kind() == fastxlsx::CellValueKind::Text &&
                    reopened_a2.text_value() == "batch-value-default-a2" &&
                    !reopened_a2.has_style(),
                prefix + " reopened output should keep untouched A2 unstyled");
            ...
            check(row_one.size() == 2,
                prefix + " reopened row_cells should expose row one");
            if (row_one.size() == 2) {
                check_batch_value_default_a1_snapshot(row_one[0],
                    prefix + " reopened row_cells");
                check_batch_value_default_b1_snapshot(row_one[1],
                    prefix + " reopened row_cells");
            }
            const std::vector<fastxlsx::WorksheetCellSnapshot> row_two =
                reopened_sheet.row_cells(2);
            ...
            if (row_two.size() == 2) {
                check_batch_value_default_a2_snapshot(row_two[0],
                    prefix + " reopened row_cells");
                check_batch_value_default_c2_snapshot(row_two[1],
                    prefix + " reopened row_cells");
            }
            const std::vector<fastxlsx::WorksheetCellSnapshot> column_one =
                reopened_sheet.column_cells(1);
            check(column_one.size() == 2,
                prefix + " reopened column_cells should expose column one");
            if (column_one.size() == 2) {
                check_batch_value_default_a1_snapshot(column_one[0],
                    prefix + " reopened column_cells");
                check_batch_value_default_a2_snapshot(column_one[1],
                    prefix + " reopened column_cells");
            }
            const std::vector<fastxlsx::WorksheetCellSnapshot> column_two =
                reopened_sheet.column_cells(2);
            check(column_two.size() == 1,
                prefix + " reopened column_cells should expose B1 only");
            if (column_two.size() == 1) {
                check_batch_value_default_b1_snapshot(column_two[0],
                    prefix + " reopened column_cells");
            }
            const std::vector<fastxlsx::WorksheetCellSnapshot> column_three =
                reopened_sheet.column_cells(3);
            ...
            if (column_three.size() == 1) {
                check_batch_value_default_c2_snapshot(column_three[0],
                    prefix + " reopened column_cells");
            }
            check(reopened_sheet.row_cells(3).empty() &&
                    reopened_sheet.column_cells(4).empty(),
    ...
    editor.save_as(output);
    check(!sheet.has_pending_changes(),
        "set_cell_values explicit default StyleId save should clean the materialized worksheet");
    check(editor.pending_change_count() == 1,
        "set_cell_values explicit default StyleId save should record one materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "set_cell_values explicit default StyleId save should clear dirty diagnostics");
    check(editor.pending_worksheet_edits().empty(),
        "set_cell_values explicit default StyleId save should not leave dirty summaries");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values explicit default StyleId save should not queue diagnostics");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values explicit default StyleId save should keep diagnostics clear");
    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values explicit default StyleId save should leave the source package unchanged");
    check(output_entries.at("xl/styles.xml") == source_entries.at("xl/styles.xml"),
        "set_cell_values explicit default StyleId save should preserve source styles.xml bytes");
    const std::string worksheet_xml = output_entries.at("xl/worksheets/sheet1.xml");
    const std::string styled_a1 =
        R"(<c r="A1" s=")" + std::to_string(non_default_style.value()) + R"("><v>2.5</v></c>)";
    check_contains(worksheet_xml, R"(<dimension ref="A1:C2"/>)",
        "set_cell_values explicit default StyleId should project extended bounds");
    check_contains(worksheet_xml, styled_a1,
        "set_cell_values explicit default StyleId should persist source-styled A1");
    check_contains(worksheet_xml, R"(<c r="B1"/>)",
        "set_cell_values explicit default StyleId should persist B1 blank without style");
    check_contains(worksheet_xml, "batch-value-default-a2",
        "set_cell_values explicit default StyleId should keep untouched A2 text");
    check_contains(worksheet_xml, R"(<c r="C2"><f>A1+B1</f></c>)",
        "set_cell_values explicit default StyleId should persist C2 formula without style");
    check_not_contains(worksheet_xml, R"(<c r="B1" s=")",
        "set_cell_values explicit default StyleId should not write a style id on B1");
    check_not_contains(worksheet_xml, R"(<c r="C2" s=")",
        "set_cell_values explicit default StyleId should not write a style id on C2");
    check_not_contains(worksheet_xml, R"(s="0")",
        "set_cell_values explicit default StyleId should not write default style ids");
    check_reopened_clean_sheet_output(
        output, "Styled", "set_cell_values explicit default StyleId",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
            inspect_default_style_batch_output(
                reopened_sheet, "set_cell_values explicit default StyleId");
        });
    ...
    editor.save_as(noop_output);
    check(!sheet.has_pending_changes(),
        "set_cell_values explicit default StyleId no-op save should keep the materialized sheet clean");
    check(editor.pending_change_count() == pending_count_after_save,
        "set_cell_values explicit default StyleId no-op save should not record another handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "set_cell_values explicit default StyleId no-op save should keep dirty diagnostics clear");
    check(editor.pending_worksheet_edits().empty(),
        "set_cell_values explicit default StyleId no-op save should not leave dirty summaries");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values explicit default StyleId no-op save should not queue diagnostics");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values explicit default StyleId no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop,
        "set_cell_values explicit default StyleId no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop,
        "set_cell_values explicit default StyleId no-op save");
    const auto noop_entries = fastxlsx::test::read_zip_entries(noop_output);
    check(noop_entries == output_entries,
        "set_cell_values explicit default StyleId no-op output should match materialized output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values explicit default StyleId no-op save should leave the source package unchanged");
    check_reopened_clean_sheet_output(
        noop_output, "Styled", "set_cell_values explicit default StyleId no-op save",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
            inspect_default_style_batch_output(
                reopened_sheet, "set_cell_values explicit default StyleId no-op save");
        });
    ...
    editor.save_as(second_noop_output);
    check(!sheet.has_pending_changes(),
        "set_cell_values explicit default StyleId second no-op save should keep the materialized sheet clean");
    check(editor.pending_change_count() == pending_count_after_save,
        "set_cell_values explicit default StyleId second no-op save should not record another handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "set_cell_values explicit default StyleId second no-op save should keep dirty diagnostics clear");
    check(editor.pending_worksheet_edits().empty(),
        "set_cell_values explicit default StyleId second no-op save should not leave dirty summaries");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values explicit default StyleId second no-op save should not queue diagnostics");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values explicit default StyleId second no-op save should keep diagnostics clear");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_second_noop,
        "set_cell_values explicit default StyleId second no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_second_noop,
        "set_cell_values explicit default StyleId second no-op save");
    const auto second_noop_entries =
        fastxlsx::test::read_zip_entries(second_noop_output);
    check(second_noop_entries == noop_entries,
        "set_cell_values explicit default StyleId second no-op output should match first no-op output");
    check(fastxlsx::test::read_zip_entries(output) == output_entries,
        "set_cell_values explicit default StyleId second no-op save should leave the materialized output unchanged");
    check(fastxlsx::test::read_zip_entries(noop_output) == noop_entries,
        "set_cell_values explicit default StyleId second no-op save should leave the first no-op output unchanged");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values explicit default StyleId second no-op save should leave the source package unchanged");
    check_reopened_clean_sheet_output(
        second_noop_output, "Styled",
        "set_cell_values explicit default StyleId second no-op save",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
...
void test_public_worksheet_editor_set_cell_values_style_rejection_preserves_dirty_session()
{
    const std::filesystem::path source = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-style-rejection-dirty-source.xlsx");
    const std::filesystem::path output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-style-rejection-dirty-output.xlsx");
    const std::filesystem::path noop_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-style-rejection-dirty-noop-output.xlsx");
    const std::filesystem::path recovery_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-style-rejection-dirty-recovery-output.xlsx");
    const std::filesystem::path recovery_noop_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-style-rejection-dirty-recovery-noop-output.xlsx");
    ...
    fastxlsx::StyleId source_style;
    {
        ...
        styled_sheet.append_row({
            fastxlsx::CellView::number(1.0).with_style(source_style),
            fastxlsx::CellView::text("set-cell-values-dirty-source-b1"),
        });
        styled_sheet.append_row({
            fastxlsx::CellView::text("set-cell-values-dirty-source-a2"),
        });
        writer.close();
    }
    ...
    check_contains(source_entries.at("xl/worksheets/sheet1.xml"),
        R"(<c r="A1" s=")" + std::to_string(source_style.value()) + R"("><v>1</v></c>)",
        "set_cell_values dirty style rejection source fixture should start with styled A1");
    ...
    const auto check_source_b1 =
        [](const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            ...
            check(snapshot.reference.row == 1 &&
                    snapshot.reference.column == 2 &&
                    snapshot.value.kind() == fastxlsx::CellValueKind::Text &&
                    snapshot.value.text_value() == "set-cell-values-dirty-source-b1" &&
                    !snapshot.value.has_style(),
                prefix + " should expose unstyled source B1");
        };
    const auto check_dirty_a2 =
        [](const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            ...
            check(snapshot.reference.row == 2 &&
                    snapshot.reference.column == 1 &&
                    snapshot.value.kind() == fastxlsx::CellValueKind::Text &&
                    snapshot.value.text_value() == "set-cell-values-dirty-kept" &&
                    !snapshot.value.has_style(),
                prefix + " should expose preserved dirty A2");
        };
    const auto check_dirty_b2 =
    ...
    const auto check_recovered_a2 =
        [](const fastxlsx::WorksheetCellSnapshot& snapshot, std::string_view scenario) {
            ...
            check(snapshot.reference.row == 2 &&
                    snapshot.reference.column == 1 &&
                    snapshot.value.kind() == fastxlsx::CellValueKind::Text &&
                    snapshot.value.text_value() == "set-cell-values-dirty-recovered" &&
                    !snapshot.value.has_style(),
                prefix + " should expose recovered A2");
        };
    const auto check_recovered_b2 =
    ...
    const auto check_recovery_views =
        [&](fastxlsx::WorksheetEditor& current_sheet, std::string_view scenario) {
            ...
            check(column_three.size() == 1,
                prefix + " column_cells should expose recovered column three");
            if (column_three.size() == 1) {
                check_recovered_c2(column_three[0], prefix + " column_cells");
            }
        };

    sheet.set_cell_values({
        {fastxlsx::WorksheetCellReference {2, 1},
            fastxlsx::CellValue::text("set-cell-values-dirty-kept")
                .with_style(fastxlsx::StyleId {})},
        {fastxlsx::WorksheetCellReference {2, 2},
            fastxlsx::CellValue::formula("A1+B1").with_style(fastxlsx::StyleId {})},
    });
    check(!editor.last_edit_error().has_value(),
        "set_cell_values dirty style rejection setup should start diagnostic-clean");
    check(sheet.has_pending_changes(),
        "set_cell_values dirty style rejection setup should dirty the materialized sheet");
    check_public_state_single_named_dirty_materialized_summary(
        editor, sheet, "Styled", 0, "set_cell_values dirty style rejection setup");
    check_dirty_views(sheet, "set_cell_values dirty style rejection setup");

    bool failed = false;
    try {
        sheet.set_cell_values({
            {fastxlsx::WorksheetCellReference {1, 3},
                fastxlsx::CellValue::text("set-cell-values-dirty-rejected")
                    .with_style(source_style)},
        });
    } catch (const fastxlsx::FastXlsxError& error) {
        failed = true;
        check_contains(error.what(), "StyleId",
            "set_cell_values dirty style rejection should expose the unsupported StyleId boundary");
    }
    check(failed,
        "set_cell_values dirty style rejection should reject caller-supplied non-default StyleId values");
    check(editor.last_edit_error().has_value() &&
            editor.last_edit_error()->find("StyleId") != std::string::npos,
        "set_cell_values dirty style rejection should retain the public StyleId diagnostic");
    check(sheet.has_pending_changes(),
        "set_cell_values dirty style rejection should keep the prior dirty materialized sheet");
    check_public_state_single_named_dirty_materialized_summary(
        editor, sheet, "Styled", 0, "set_cell_values dirty style rejection",
        editor.last_edit_error());
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values dirty style rejection should not queue replacement diagnostics");
    check_dirty_views(sheet, "set_cell_values dirty style rejection live");

    const WorkbookEditorPublicCatalogSnapshot catalog_before_save =
        workbook_editor_public_catalog_snapshot(editor);
    editor.save_as(output);
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_save, "set_cell_values dirty style rejection save");
    check(!sheet.has_pending_changes(),
        "set_cell_values dirty style rejection save should clean the materialized sheet");
    check(editor.pending_change_count() == 1,
        "set_cell_values dirty style rejection save should record one materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0 &&
            editor.pending_worksheet_edits().empty(),
        "set_cell_values dirty style rejection save should clear dirty materialized diagnostics");
    check(editor.last_edit_error().has_value() &&
            editor.last_edit_error()->find("StyleId") != std::string::npos,
        "set_cell_values dirty style rejection save should preserve the rejection diagnostic");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values dirty style rejection save should not queue replacement diagnostics");
    check_dirty_views(sheet, "set_cell_values dirty style rejection saved handle");

    const auto output_entries = fastxlsx::test::read_zip_entries(output);
    check(output_entries.at("xl/styles.xml") == source_entries.at("xl/styles.xml"),
        "set_cell_values dirty style rejection save should preserve source styles.xml bytes");
    const std::string worksheet_xml = output_entries.at("xl/worksheets/sheet1.xml");
    check_contains(worksheet_xml, R"(<dimension ref="A1:B2"/>)",
        "set_cell_values dirty style rejection save should keep dirty bounds");
    check_contains(worksheet_xml, "set-cell-values-dirty-source-b1",
        "set_cell_values dirty style rejection save should preserve source B1");
    check_contains(worksheet_xml, "set-cell-values-dirty-kept",
        "set_cell_values dirty style rejection save should persist prior dirty A2");
    check_contains(worksheet_xml, R"(<c r="B2"><f>A1+B1</f></c>)",
        "set_cell_values dirty style rejection save should persist prior dirty B2 formula");
    check_not_contains(worksheet_xml, "set-cell-values-dirty-rejected",
        "set_cell_values dirty style rejection save should not leak rejected payloads");
    check_contains(worksheet_xml,
        R"(<c r="A1" s=")" + std::to_string(source_style.value()) + R"("><v>1</v></c>)",
        "set_cell_values dirty style rejection save should keep source A1 styled");
    check_not_contains(worksheet_xml, R"(<c r="A2" s=")",
        "set_cell_values dirty style rejection save should keep dirty A2 unstyled");
    check_not_contains(worksheet_xml, R"(<c r="B2" s=")",
        "set_cell_values dirty style rejection save should keep dirty B2 unstyled");
    check_not_contains(worksheet_xml, R"(s="0")",
        "set_cell_values dirty style rejection save should not write default style ids");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values dirty style rejection save should leave the source package unchanged");
    check_reopened_clean_sheet_output(
        output, "Styled", "set_cell_values dirty style rejection save",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
            check_dirty_views(reopened_sheet, "set_cell_values dirty style rejection save");
        });
    ...
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop,
        "set_cell_values dirty style rejection noop save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "set_cell_values dirty style rejection noop save");
    check(editor.pending_change_count() == pending_count_after_save,
        "set_cell_values dirty style rejection noop save should not add another handoff");
    check(!sheet.has_pending_changes(),
        "set_cell_values dirty style rejection noop save should keep the sheet clean");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0 &&
            editor.pending_worksheet_edits().empty(),
        "set_cell_values dirty style rejection noop save should keep dirty diagnostics clear");
    check(editor.last_edit_error().has_value() &&
            editor.last_edit_error()->find("StyleId") != std::string::npos,
        "set_cell_values dirty style rejection noop save should preserve the rejection diagnostic");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values dirty style rejection noop save should not queue replacement diagnostics");
    check_dirty_views(sheet, "set_cell_values dirty style rejection noop saved handle");
    const auto noop_entries = fastxlsx::test::read_zip_entries(noop_output);
    check(noop_entries == output_entries,
        "set_cell_values dirty style rejection noop output should match the materialized output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values dirty style rejection noop save should leave the source package unchanged");
    check_reopened_clean_sheet_output(
        noop_output, "Styled", "set_cell_values dirty style rejection noop save",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
            check_dirty_views(
                reopened_sheet, "set_cell_values dirty style rejection noop save");
        });

    sheet.set_cell_values({
        {fastxlsx::WorksheetCellReference {2, 1},
            fastxlsx::CellValue::text("set-cell-values-dirty-recovered")
                .with_style(fastxlsx::StyleId {})},
        {fastxlsx::WorksheetCellReference {2, 2},
            fastxlsx::CellValue::blank().with_style(fastxlsx::StyleId {})},
        {fastxlsx::WorksheetCellReference {2, 3},
    ...
    check(!editor.last_edit_error().has_value(),
        "set_cell_values dirty style rejection recovery should clear the retained StyleId diagnostic");
    check(sheet.has_pending_changes(),
        "set_cell_values dirty style rejection recovery should dirty the materialized sheet again");
    check_public_state_single_named_dirty_materialized_summary(
        editor, sheet, "Styled", pending_count_after_save,
        "set_cell_values dirty style rejection recovery");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "set_cell_values dirty style rejection recovery should not queue replacement diagnostics");
    check_recovery_views(sheet, "set_cell_values dirty style rejection recovery live");
    ...
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_recovery_save,
        "set_cell_values dirty style rejection recovery save");
    check(!sheet.has_pending_changes(),
        "set_cell_values dirty style rejection recovery save should clean the materialized sheet");
    check(editor.pending_change_count() == pending_count_after_save + 1,
        "set_cell_values dirty style rejection recovery save should record one more materialized handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0 &&
            editor.pending_worksheet_edits().empty(),
        "set_cell_values dirty style rejection recovery save should clear dirty materialized diagnostics");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values dirty style rejection recovery save should keep diagnostics clear");
    check_workbook_editor_no_replacement_diagnostics(
        editor,
        "set_cell_values dirty style rejection recovery save should not queue replacement diagnostics");
    check_recovery_views(sheet, "set_cell_values dirty style rejection recovery saved handle");

    const auto recovery_entries = fastxlsx::test::read_zip_entries(recovery_output);
    check(recovery_entries.at("xl/styles.xml") == source_entries.at("xl/styles.xml"),
        "set_cell_values dirty style rejection recovery save should preserve source styles.xml bytes");
    const std::string recovery_worksheet_xml =
        recovery_entries.at("xl/worksheets/sheet1.xml");
    check_contains(recovery_worksheet_xml, R"(<dimension ref="A1:C2"/>)",
        "set_cell_values dirty style rejection recovery save should extend bounds to C2");
    check_contains(recovery_worksheet_xml, "set-cell-values-dirty-source-b1",
        "set_cell_values dirty style rejection recovery save should preserve source B1");
    check_contains(recovery_worksheet_xml, "set-cell-values-dirty-recovered",
        "set_cell_values dirty style rejection recovery save should persist recovered A2");
    check_contains(recovery_worksheet_xml, R"(<c r="B2"/>)",
        "set_cell_values dirty style rejection recovery save should persist recovered B2 blank");
    check_contains(recovery_worksheet_xml, R"(<c r="C2" t="b"><v>1</v></c>)",
        "set_cell_values dirty style rejection recovery save should persist recovered C2 boolean");
    check_not_contains(recovery_worksheet_xml, "set-cell-values-dirty-kept",
        "set_cell_values dirty style rejection recovery save should replace prior dirty A2");
    check_not_contains(recovery_worksheet_xml, R"(<c r="B2"><f>A1+B1</f></c>)",
        "set_cell_values dirty style rejection recovery save should replace prior dirty B2 formula");
    check_not_contains(recovery_worksheet_xml, "set-cell-values-dirty-source-a2",
        "set_cell_values dirty style rejection recovery save should not revive source A2");
    check_not_contains(recovery_worksheet_xml, "set-cell-values-dirty-rejected",
        "set_cell_values dirty style rejection recovery save should not leak rejected payloads");
    check_contains(recovery_worksheet_xml,
        R"(<c r="A1" s=")" + std::to_string(source_style.value()) + R"("><v>1</v></c>)",
        "set_cell_values dirty style rejection recovery save should keep source A1 styled");
    check_not_contains(recovery_worksheet_xml, R"(<c r="A2" s=")",
        "set_cell_values dirty style rejection recovery save should keep recovered A2 unstyled");
    check_not_contains(recovery_worksheet_xml, R"(<c r="B2" s=")",
        "set_cell_values dirty style rejection recovery save should keep recovered B2 unstyled");
    check_not_contains(recovery_worksheet_xml, R"(<c r="C2" s=")",
        "set_cell_values dirty style rejection recovery save should keep recovered C2 unstyled");
    check_not_contains(recovery_worksheet_xml, R"(s="0")",
        "set_cell_values dirty style rejection recovery save should not write default style ids");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values dirty style rejection recovery save should leave the source package unchanged");
    check_reopened_clean_sheet_output(
        recovery_output, "Styled",
        "set_cell_values dirty style rejection recovery save",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
            check_recovery_views(
                reopened_sheet, "set_cell_values dirty style rejection recovery save");
        });
    ...
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_recovery_noop,
        "set_cell_values dirty style rejection recovery noop save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_recovery_noop,
        "set_cell_values dirty style rejection recovery noop save");
    check(editor.pending_change_count() == pending_count_after_recovery_save,
        "set_cell_values dirty style rejection recovery noop save should not add another handoff");
    check(!sheet.has_pending_changes(),
        "set_cell_values dirty style rejection recovery noop save should keep the sheet clean");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0 &&
            editor.pending_worksheet_edits().empty(),
        "set_cell_values dirty style rejection recovery noop save should keep dirty diagnostics clear");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values dirty style rejection recovery noop save should keep diagnostics clear");
    check_workbook_editor_no_replacement_diagnostics(
        editor,
        "set_cell_values dirty style rejection recovery noop save should not queue replacement diagnostics");
    check_recovery_views(
        sheet, "set_cell_values dirty style rejection recovery noop saved handle");
    const auto recovery_noop_entries =
        fastxlsx::test::read_zip_entries(recovery_noop_output);
    check(recovery_noop_entries == recovery_entries,
        "set_cell_values dirty style rejection recovery noop output should match the recovered output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values dirty style rejection recovery noop save should leave the source package unchanged");
    check_reopened_clean_sheet_output(
        recovery_noop_output, "Styled",
        "set_cell_values dirty style rejection recovery noop save",
        [&](fastxlsx::WorksheetEditor& reopened_sheet) {
            check_recovery_views(
                reopened_sheet, "set_cell_values dirty style rejection recovery noop save");
        });
}
...
void test_public_worksheet_editor_set_cell_values_duplicate_max_cells_accounting()
{
    ...
    check_saved_default_data_c3_snapshots(
        ...
        "set_cell_values duplicate max_cells saved handle",
        fastxlsx::CellValueKind::Text, "duplicate-max-last");
    check_workbook_editor_public_save_state_preserved(
...
void test_public_worksheet_editor_set_cell_values_validation_and_max_cells_noop_save()
{
    {
        const std::filesystem::path source = write_two_sheet_source(
            "fastxlsx-workbook-editor-public-worksheet-set-cell-values-invalid-source.xlsx");
        const std::filesystem::path output = artifact(
            "fastxlsx-workbook-editor-public-worksheet-set-cell-values-invalid-output.xlsx");
        const std::filesystem::path noop_output = artifact(
            "fastxlsx-workbook-editor-public-worksheet-set-cell-values-invalid-noop-output.xlsx");
        const auto source_entries = fastxlsx::test::read_zip_entries(source);
        ...
        try {
            sheet.set_cell_values({
                {fastxlsx::WorksheetCellReference {0, 1},
                    fastxlsx::CellValue::text("set-cell-values-invalid-row")},
                {fastxlsx::WorksheetCellReference {3, 3},
                    fastxlsx::CellValue::text("set-cell-values-invalid-unreached")},
            });
        } catch (const fastxlsx::FastXlsxError& error) {
            failed = true;
            check_contains(error.what(), "invalid",
                "set_cell_values invalid coordinate should expose an invalid reference diagnostic");
        }
        check(failed, "set_cell_values should reject invalid batch coordinates");
        check(editor.last_edit_error().has_value(),
            "set_cell_values invalid coordinate should retain last_edit_error");
        check(!sheet.has_pending_changes(),
            "set_cell_values invalid coordinate should not dirty the materialized sheet");
        check(sheet.cell_count() == 3,
            "set_cell_values invalid coordinate should preserve sparse cell count");
        check(sheet.estimated_memory_usage() == baseline_memory,
            "set_cell_values invalid coordinate should preserve sparse memory estimate");
        check(sheet.get_cell("A1").text_value() == "placeholder-a1",
            "set_cell_values invalid coordinate should preserve source A1");
        check(sheet.get_cell("B1").number_value() == 1.0,
            "set_cell_values invalid coordinate should preserve source B1");
        check(sheet.get_cell("A2").text_value() == "placeholder-a2",
            "set_cell_values invalid coordinate should preserve source A2");
        check(!sheet.try_cell("C3").has_value(),
            "set_cell_values invalid coordinate should not leave later batch targets readable");
        check_workbook_editor_public_no_pending_state(
            editor, "set_cell_values invalid coordinate");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "set_cell_values invalid coordinate should not expose dirty materialized diagnostics");
        check_workbook_editor_no_replacement_diagnostics(
            editor, "set_cell_values invalid coordinate should not queue replacement diagnostics");
        ...
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_save,
            "set_cell_values invalid coordinate save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_save, "set_cell_values invalid coordinate save");
        check_workbook_editor_public_no_pending_state(
            editor, "set_cell_values invalid coordinate save");
        check(!sheet.has_pending_changes(),
            "set_cell_values invalid coordinate save should keep the materialized sheet clean");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "set_cell_values invalid coordinate save should keep dirty diagnostics clear");
        check_workbook_editor_no_replacement_diagnostics(
            editor,
            "set_cell_values invalid coordinate save should not queue replacement diagnostics");
        const auto output_entries = fastxlsx::test::read_zip_entries(output);
        check(output_entries == source_entries,
            "set_cell_values invalid coordinate save should copy source entries");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "set_cell_values invalid coordinate save should leave the source package unchanged");
        check_reopened_default_data_sheet_output(
            output, "set_cell_values invalid coordinate save");
        ...
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_noop,
            "set_cell_values invalid coordinate noop save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_noop,
            "set_cell_values invalid coordinate noop save");
        check_workbook_editor_public_no_pending_state(
            editor, "set_cell_values invalid coordinate noop save");
        check(!sheet.has_pending_changes(),
            "set_cell_values invalid coordinate noop save should keep the materialized sheet clean");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "set_cell_values invalid coordinate noop save should keep dirty diagnostics clear");
        check_workbook_editor_no_replacement_diagnostics(
            editor,
            "set_cell_values invalid coordinate noop save should not queue replacement diagnostics");
        const auto noop_entries = fastxlsx::test::read_zip_entries(noop_output);
        check(noop_entries == source_entries,
            "set_cell_values invalid coordinate noop save should still copy source entries");
        check(noop_entries == output_entries,
            "set_cell_values invalid coordinate noop output should match the first output");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "set_cell_values invalid coordinate noop save should leave the source package unchanged");
        check_reopened_default_data_sheet_output(
            noop_output, "set_cell_values invalid coordinate noop save");
    }

    {
        const std::filesystem::path source = write_two_sheet_source(
            "fastxlsx-workbook-editor-public-worksheet-set-cell-values-max-reject-source.xlsx");
        const std::filesystem::path output = artifact(
            "fastxlsx-workbook-editor-public-worksheet-set-cell-values-max-reject-output.xlsx");
        const std::filesystem::path noop_output = artifact(
            "fastxlsx-workbook-editor-public-worksheet-set-cell-values-max-reject-noop-output.xlsx");
        const auto source_entries = fastxlsx::test::read_zip_entries(source);
        ...
        try {
            sheet.set_cell_values({
                {fastxlsx::WorksheetCellReference {1, 1},
                    fastxlsx::CellValue::text("set-cell-values-max-overwrite")},
                {fastxlsx::WorksheetCellReference {3, 3},
                    fastxlsx::CellValue::text("set-cell-values-max-rejected")},
            });
        } catch (const fastxlsx::FastXlsxError& error) {
            failed = true;
            check_contains(error.what(), "CellStore max_cells guardrail exceeded",
                "set_cell_values max_cells rejection should expose guardrail diagnostics");
        }
        check(failed, "set_cell_values should reject new sparse targets over max_cells");
        check(editor.last_edit_error().has_value(),
            "set_cell_values max_cells rejection should retain last_edit_error");
        check(!sheet.has_pending_changes(),
            "set_cell_values max_cells rejection should not dirty the materialized sheet");
        check(sheet.cell_count() == 3,
            "set_cell_values max_cells rejection should preserve sparse cell count");
        check(sheet.estimated_memory_usage() == baseline_memory,
            "set_cell_values max_cells rejection should preserve sparse memory estimate");
        check(sheet.get_cell("A1").text_value() == "placeholder-a1",
            "set_cell_values max_cells rejection should not apply earlier overwrites");
        check(sheet.get_cell("B1").number_value() == 1.0,
            "set_cell_values max_cells rejection should preserve source B1");
        check(sheet.get_cell("A2").text_value() == "placeholder-a2",
            "set_cell_values max_cells rejection should preserve source A2");
        check(!sheet.try_cell("C3").has_value(),
            "set_cell_values max_cells rejection should keep rejected target absent");
        check_workbook_editor_public_no_pending_state(
            editor, "set_cell_values max_cells rejection");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "set_cell_values max_cells rejection should not expose dirty materialized diagnostics");
        check_workbook_editor_no_replacement_diagnostics(
            editor,
            "set_cell_values max_cells rejection should not queue replacement diagnostics");
        ...
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_save,
            "set_cell_values max_cells rejection save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_save, "set_cell_values max_cells rejection save");
        check_workbook_editor_public_no_pending_state(
            editor, "set_cell_values max_cells rejection save");
        check(!sheet.has_pending_changes(),
            "set_cell_values max_cells rejection save should keep the materialized sheet clean");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "set_cell_values max_cells rejection save should keep dirty diagnostics clear");
        check_workbook_editor_no_replacement_diagnostics(
            editor,
            "set_cell_values max_cells rejection save should not queue replacement diagnostics");
        const auto output_entries = fastxlsx::test::read_zip_entries(output);
        check(output_entries == source_entries,
            "set_cell_values max_cells rejection save should copy source entries");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "set_cell_values max_cells rejection save should leave the source package unchanged");
        check_reopened_default_data_sheet_output(
            output, "set_cell_values max_cells rejection save");
        ...
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_noop,
            "set_cell_values max_cells rejection noop save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_noop,
            "set_cell_values max_cells rejection noop save");
        check_workbook_editor_public_no_pending_state(
            editor, "set_cell_values max_cells rejection noop save");
        check(!sheet.has_pending_changes(),
            "set_cell_values max_cells rejection noop save should keep the materialized sheet clean");
        check(editor.pending_materialized_worksheet_names().empty() &&
                editor.pending_materialized_cell_count() == 0 &&
                editor.estimated_pending_materialized_memory_usage() == 0,
            "set_cell_values max_cells rejection noop save should keep dirty diagnostics clear");
        check_workbook_editor_no_replacement_diagnostics(
            editor,
            "set_cell_values max_cells rejection noop save should not queue replacement diagnostics");
        const auto noop_entries = fastxlsx::test::read_zip_entries(noop_output);
        check(noop_entries == source_entries,
            "set_cell_values max_cells rejection noop save should still copy source entries");
        check(noop_entries == output_entries,
            "set_cell_values max_cells rejection noop output should match the first output");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "set_cell_values max_cells rejection noop save should leave the source package unchanged");
        check_reopened_default_data_sheet_output(
            noop_output, "set_cell_values max_cells rejection noop save");
    }
}

void test_public_worksheet_editor_set_cell_values_memory_budget_failure_preserves_state()
{
    const std::filesystem::path source = write_two_sheet_source(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-memory-source.xlsx");
    const std::filesystem::path failure_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-memory-failure-output.xlsx");
    const std::filesystem::path recovery_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-memory-recovery-output.xlsx");
    const std::filesystem::path noop_output = artifact(
        "fastxlsx-workbook-editor-public-worksheet-set-cell-values-memory-noop-output.xlsx");
    const auto source_entries = fastxlsx::test::read_zip_entries(source);
    ...
    const std::string rejected_text =
        std::string("set-cell-values-memory-rejected-") + std::string(512, 'x');

    bool failed = false;
    try {
        sheet.set_cell_values({
            {fastxlsx::WorksheetCellReference {1, 1},
                fastxlsx::CellValue::text("batch-memory-overwrite")},
            {fastxlsx::WorksheetCellReference {4, 4},
        ...
        failed = true;
        check_contains(error.what(), "CellStore memory_budget_bytes guardrail exceeded",
            "set_cell_values batch should expose memory budget diagnostics");
    }
    check(failed,
        "set_cell_values should reject a batch that exceeds memory_budget_bytes");
    check(editor.last_edit_error().has_value(),
        "failed set_cell_values memory-budget batch should update last_edit_error");
    check_contains(*editor.last_edit_error(), "CellStore memory_budget_bytes guardrail exceeded",
        "last_edit_error should retain the set_cell_values memory-budget diagnostic");
    check(!sheet.has_pending_changes(),
        "failed set_cell_values memory-budget batch should not dirty the materialized session");
    check(!editor.has_pending_changes(),
        "failed set_cell_values memory-budget batch should not dirty the editor");
    check_workbook_editor_public_no_pending_state(
        editor, "failed set_cell_values memory-budget batch");
    check(editor.pending_materialized_worksheet_names().empty(),
        "failed set_cell_values memory-budget batch should not expose dirty materialized names");
    check(editor.pending_materialized_cell_count() == 0,
        "failed set_cell_values memory-budget batch should not expose dirty materialized cells");
    check(editor.estimated_pending_materialized_memory_usage() == 0,
        "failed set_cell_values memory-budget batch should not expose dirty materialized memory");
    check_workbook_editor_no_replacement_diagnostics(
        editor, "failed set_cell_values memory-budget batch");
    check(sheet.cell_count() == baseline_count,
        "failed set_cell_values memory-budget batch should preserve sparse cell count");
    check(sheet.estimated_memory_usage() == baseline_memory,
        "failed set_cell_values memory-budget batch should preserve sparse memory estimate");
    check(sheet.get_cell("A1").text_value() == "placeholder-a1",
        "failed set_cell_values memory-budget batch should not apply earlier batch updates");
    check(!sheet.try_cell("D4").has_value(),
        "failed set_cell_values memory-budget batch should keep rejected target absent");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "failed set_cell_values memory-budget batch should leave the source package unchanged");
    ...
    check(editor.last_edit_error() == memory_error,
        "save_as after failed set_cell_values memory-budget batch should preserve diagnostics");
    const auto failure_entries = fastxlsx::test::read_zip_entries(failure_output);
    check(failure_entries == source_entries,
        "save_as after failed set_cell_values memory-budget batch should copy source entries");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "save_as after failed set_cell_values memory-budget batch should leave the source package unchanged");
    check_reopened_default_data_sheet_output(
        failure_output, "set_cell_values memory-budget failure");

    sheet.set_cell_values({
        {fastxlsx::WorksheetCellReference {1, 1},
            fastxlsx::CellValue::text("mb-a1")},
    });
    check(!editor.last_edit_error().has_value(),
        "successful set_cell_values after memory-budget failure should clear diagnostics");
    check(sheet.has_pending_changes(),
        "successful set_cell_values after memory-budget failure should dirty the materialized session");
    check(sheet.cell_count() == baseline_count,
        "successful set_cell_values recovery should preserve sparse cell count");
    check(sheet.estimated_memory_usage() <= exact_memory_budget,
        "successful set_cell_values recovery should stay within the configured memory budget");
    check(sheet.get_cell("A1").text_value() == "mb-a1",
        "successful set_cell_values recovery should apply the in-budget payload");
    check(!sheet.try_cell("D4").has_value(),
        "successful set_cell_values recovery should keep the rejected target absent");
    check_public_state_single_data_dirty_materialized_summary(
        editor, sheet, 0, "set_cell_values memory-budget recovery dirty summary");

    editor.save_as(recovery_output);
    const auto recovery_entries = fastxlsx::test::read_zip_entries(recovery_output);
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values memory-budget recovery save should leave the source package unchanged");
    const std::string worksheet_xml = recovery_entries.at("xl/worksheets/sheet1.xml");
    check_contains(worksheet_xml, "mb-a1",
        "set_cell_values memory-budget recovery save should persist the recovery value");
    check_not_contains(worksheet_xml, "batch-memory-overwrite",
        "set_cell_values memory-budget recovery save should omit failed early batch updates");
    check_not_contains(worksheet_xml, "set-cell-values-memory-rejected",
        "set_cell_values memory-budget recovery save should omit rejected batch payloads");
    check_contains(recovery_entries.at("xl/worksheets/sheet2.xml"), "keep-me",
        "set_cell_values memory-budget recovery save should preserve untouched worksheets");
    check_reopened_default_data_overwrite_output(
        recovery_output, "set_cell_values memory-budget recovery", "mb-a1");
    ...
    editor.save_as(noop_output);
    check(!sheet.has_pending_changes(),
        "set_cell_values memory-budget recovery no-op save should keep the materialized sheet clean");
    check(editor.pending_change_count() == 1,
        "set_cell_values memory-budget recovery no-op save should not record another handoff");
    check(editor.pending_materialized_worksheet_names().empty() &&
            editor.pending_materialized_cell_count() == 0 &&
            editor.estimated_pending_materialized_memory_usage() == 0,
        "set_cell_values memory-budget recovery no-op save should keep dirty diagnostics clear");
    check(editor.pending_worksheet_edits().empty(),
        "set_cell_values memory-budget recovery no-op save should not leave dirty summaries");
    check_workbook_editor_no_replacement_diagnostics(
        editor,
        "set_cell_values memory-budget recovery no-op save should not queue replacement diagnostics");
    check(!editor.last_edit_error().has_value(),
        "set_cell_values memory-budget recovery no-op save should keep diagnostics clear");
    check_saved_default_data_overwrite_snapshots(
        editor, sheet, pending_count_after_recovery_save,
        "set_cell_values memory-budget recovery saved handle", "mb-a1");
    check_workbook_editor_public_save_state_preserved(
        editor, save_state_before_noop,
        "set_cell_values memory-budget recovery no-op save");
    check_workbook_editor_public_catalog_preserved(
        editor, catalog_before_noop, "set_cell_values memory-budget recovery no-op save");
    const auto noop_entries = fastxlsx::test::read_zip_entries(noop_output);
    check(noop_entries == recovery_entries,
        "set_cell_values memory-budget recovery no-op output should match recovery output");
    check(fastxlsx::test::read_zip_entries(source) == source_entries,
        "set_cell_values memory-budget recovery no-op save should leave the source package unchanged");
    check_reopened_default_data_overwrite_output(
        noop_output, "set_cell_values memory-budget recovery no-op save", "mb-a1");
}
...
int main()
{
    try {
        ...
        test_public_worksheet_editor_set_cell_values_accepts_default_style_id_as_style_preserving_batch();
        test_public_worksheet_editor_set_cell_values_style_rejection_preserves_dirty_session();
        test_public_worksheet_editor_set_cell_values_duplicate_max_cells_accounting();
        test_public_worksheet_editor_set_cell_values_validation_and_max_cells_noop_save();
        test_public_worksheet_editor_set_cell_values_memory_budget_failure_preserves_state();
