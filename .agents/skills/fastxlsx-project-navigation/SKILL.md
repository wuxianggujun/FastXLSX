...
- `WorkbookEditor::replace_cells(..., CellPatchMissingCellPolicy::Insert)` 是当前
  ...
  index 拼接 existing-cell replacement payload；当前
  ...
  benchmark 生成的 staged chunks + prebuilt index。它们不是 public API，也不是当前
  public Patch 的默认 algorithm switch、PackageEditor source-entry ZIP seek 或完整
