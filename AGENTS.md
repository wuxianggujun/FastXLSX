...
当前还支持窄 `ImageOptions` metadata：`edit_as` 只写成 drawing XML
`xdr:twoCellAnchor editAs` attribute；非空 `name` 会写成 drawing XML
`<xdr:cNvPr name="...">`，非空 `description` 会写成 `descr="..."`；空 `name`
保留生成的 `Picture N`，空 `description` 省略。该 metadata 只改变 drawing
anchor metadata 和 non-visual picture properties，不修改图片二进制、EXIF、media
filename、anchor 坐标、relationships、content types 或 worksheet cell text。
...
- Phase 5 worksheet metadata：基础。`WorksheetWriter::add_data_validation()`
  ...
  压缩或格式转换。`ImageOptions` 只把 `edit_as` 轻量枚举和 name/description 字符串
  复制进 writer state，并在 drawing XML 写 `xdr:twoCellAnchor editAs` 和 `xdr:cNvPr`
  `name` / `descr` attributes；它不代表完整 alt text/accessibility UI、图片文件
  metadata、existing-workbook 图片编辑或图片保真。
...
- 当前 opt-in image preset 还覆盖最大合法 anchor marker 结构测试，确认
  ...
  不是百万行/最大列大文件性能证明。
- 当前 opt-in image preset 还覆盖 drawing anchor metadata 和 non-visual metadata：
...
- 把 `ImageOptions::edit_as` 写成 `oneCellAnchor` / `absoluteAnchor` 元素支持、
  row/column resize 几何计算、跨办公软件 UI 保证、drawing mutation 或 existing-workbook
  图片编辑；或把 `ImageOptions::name` / `description` 写成完整图片 metadata、
  完整 alt text UI、EXIF/PNG/JPEG metadata、media filename 语义。
