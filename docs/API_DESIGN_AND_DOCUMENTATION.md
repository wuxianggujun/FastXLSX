...
- `WorksheetEditor`：当前 public 首片提供 `name()`、`try_cell(row, column)`、
  ...
  row/column structural edit。移动的 formula cell 只用窄 A1-style translator
  平移 relative references；仍留在原坐标但引用受影响 row/column 的 materialized
  formula cell 会用窄 structural rewriter 更新引用，保留 surviving references 的
  `$` 标记并把被删除的引用写成 `#REF!`。这仍不同步 tables、autoFilter、
  mergeCells、data validations、conditional formatting、hyperlinks、
  drawings/charts/VBA、defined names、relationships、sharedStrings/styles 或
  calcChain。
...
- `save_as(output_path)`：输出到新路径；不承诺原地 atomic overwrite，也不绕过现有
  ...
  diagnostics；因此 `has_pending_changes()` 可能在 materialized diagnostics 已经清空后
  仍因 retained staged Patch handoffs 返回 true。调用方可把同一 planned state
  另存到第二个输出路径；若 queued edit
...
| In-memory shift addenda P8.830 | Current public `WorksheetEditor` row/column shift helpers (`insert_rows()`, `delete_rows()`, `insert_columns()`, `delete_columns()`) operate only on represented sparse cells in an already materialized small-file session. They move/delete sparse records, preserve source-backed `StyleId` handles on moved cells, use the narrow A1-style translator for moved formula cells, rewrite supported references in stationary materialized formula cells when the structural edit affects those references, emit `#REF!` for references invalidated by deleted/out-of-bounds rows or columns, refresh sparse dimensions on dirty `save_as()`, and retain clean saved sessions across matching reacquire, failed save retry, option mismatch, missing-sheet queries, invalid reads/mutations, and snapshot reads under the current planned sheet name. | This is not complete Excel structural editing. The helpers do not materialize large worksheets on demand, scan or rewrite non-materialized worksheet XML, evaluate formulas, repair calcChain, migrate sharedStrings/styles, resize tables/filters/validations/conditional-formatting ranges, synchronize defined names/drawings/charts/hyperlinks, prune relationships, or provide low-memory random editing. |
| In-memory shift addenda P8.1245 | Public-state coverage now verifies stationary source formula cells are structurally rewritten even when no sparse cell coordinate moves. Insert/delete row operations on a clean materialized sheet can rewrite a `C1` formula from `A3+B1` to `A4+B1` or `#REF!+B1`; insert/delete column operations can rewrite `D1+B1` to `E1+B1` or `#REF!+B1`. The formula-only change dirties the materialized session, flushes through `save_as()`, preserves untouched worksheets, and fresh reopen reads the rewritten formula while keeping dirty diagnostics empty. | This is a narrow already-materialized formula text rewrite. It does not evaluate formulas, preserve cached formula results, implement complete Excel range-shrink semantics, inspect non-materialized worksheets, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, or provide low-memory random editing. |
| In-memory shift addenda P8.1246 | Public-state coverage now verifies stationary source-formula structural rewrites remain stable across a clean follow-up no-op save. After insert/delete row/column rewrites only the formula text, the first `save_as()` materializes the rewritten formula and clears dirty materialized diagnostics; the second `save_as()` snapshots and preserves public catalog/save-state, keeps worksheet edit summaries empty, emits byte-equivalent package entries, and reopens with the same formula and clean diagnostics. | This is no-op save-state coverage for the existing already-materialized stationary formula rewrite path only. It does not add formula evaluation, cached formula result preservation, complete Excel range-shrink semantics, non-materialized worksheet formula scans, worksheet metadata range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1247 | Public-state coverage now verifies stationary source-formula structural rewrites survive a rejected exact-source `save_as()` before the first successful materialized flush. The failed save preserves the dirty materialized worksheet name, sparse count, memory estimate, rewritten formula text, and source package bytes; the later safe save and follow-up no-op save still project the rewritten formula and reopen cleanly. | This is failed-save retry hygiene for the existing already-materialized stationary formula rewrite path only. It does not add overwrite mode, transaction replay, rollback, formula evaluation, non-materialized worksheet formula scans, metadata range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1248 | Public-state coverage now verifies stationary source formulas structurally rewrite supported cell-range and whole-axis references, not only scalar A1 references. A row insertion can rewrite `SUM(A3:B3)+3:3` to `SUM(A4:B4)+4:4`, and a column insertion can rewrite `SUM(D1:E1)+D:E` to `SUM(E1:F1)+E:F`; both cases preserve dirty diagnostics through a rejected exact-source save, then safe-save/no-op-save/reopen with byte-stable package output and clean public state. | This is public evidence for the existing narrow A1-style structural formula rewriter on already-materialized formula cells only. It does not add complete Excel range-shrink semantics, formula evaluation, non-materialized worksheet scans, metadata range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1249 | Public-state coverage now verifies `WorkbookEditor::formula_reference_audits()` observes stationary formula rewrites after structural shifts, even when the formula cell itself does not move. A dirty materialized `C1` formula can rewrite from `Data!A3+Data!B1` to `Data!A4+Data!B1`; the audit call reports both rewritten/stable references at `C1` and preserves pending materialized diagnostics, edit summaries, catalog, and last-error state. | This is read-only audit visibility for the existing already-materialized stationary formula rewrite path only. It does not add formula evaluation, non-materialized worksheet formula scans, formula dependency repair, metadata range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1250 | Public-state coverage now verifies delete-side stationary formula rewrites keep formula-reference audits lexical around `#REF!` output. A dirty materialized `C1` formula can rewrite from `Data!A3+Data!B1` to `Data!#REF!+Data!B1`; `formula_reference_audits()` skips `Data!#REF!`, reports only the surviving `Data!B1` reference, and preserves public diagnostics. | This is read-only audit visibility for existing delete-side stationary formula rewrite output only. It does not repair `#REF!`, evaluate formulas, inspect non-materialized worksheets, synchronize metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1251 | Public-state coverage now verifies `WorkbookEditor::source_formula_reference_audits()` keeps scanning the original source worksheet XML while a stationary formula-only materialized rewrite is dirty. After `insert_rows(3, 1)` rewrites materialized `C1` from `Data!A3+Data!B1` to `Data!A4+Data!B1`, the source audit still reports original `Data!A3` / `Data!B1`, does not report `Data!A4`, and preserves pending diagnostics. | This is read-only source-scan boundary coverage only. It does not merge source/materialized formula views, mutate the source package, scan non-materialized worksheets through the materialized store, evaluate formulas, synchronize metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1252 | Public-state coverage now verifies the same source-scan boundary on delete-side stationary formula rewrites. After `delete_rows(3, 1)` rewrites materialized `C1` from `Data!A3+Data!B1` to `Data!#REF!+Data!B1`, `source_formula_reference_audits()` still reports original `Data!A3` / `Data!B1`, does not report `Data!#REF!`, and preserves pending diagnostics. | This is read-only source-scan boundary coverage for delete-side materialized `#REF!` output only. It does not repair `#REF!`, merge source/materialized formula views, mutate the source package, evaluate formulas, synchronize metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1253 | Public-state coverage now verifies the saved-output boundary for stationary formula rewrites. After `insert_rows(3, 1)` rewrites materialized `C1` from `Data!A3+Data!B1` to `Data!A4+Data!B1` and `save_as(output)` flushes it, a fresh `WorkbookEditor::open(output)` has clean diagnostics, `source_formula_reference_audits()` scans the saved worksheet XML and reports `Data!A4` / `Data!B1`, and `formula_reference_audits()` reports the same references after clean materialization of `Data`. | This is saved-output audit evidence for the existing in-memory flush path only. It does not reconstruct edit history, evaluate formulas, repair formulas or worksheet metadata, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1254 | Public-state coverage now verifies the delete-side saved-output boundary for stationary formula rewrites. After `delete_rows(3, 1)` rewrites materialized `C1` from `Data!A3+Data!B1` to `Data!#REF!+Data!B1` and saves it, a fresh `WorkbookEditor::open(output)` stays clean, `source_formula_reference_audits()` scans the saved XML and reports only the surviving `Data!B1` reference, and `formula_reference_audits()` reports the same surviving reference after clean materialization while skipping `Data!#REF!`. | This is saved-output audit evidence for delete-side lexical `#REF!` output only. It does not repair `#REF!`, evaluate formulas, reconstruct edit history, synchronize metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1255 | Public-state coverage now verifies the same saved-output boundary for column insert stationary formula rewrites. After `insert_columns(4, 1)` rewrites materialized `C1` from `Data!D1+Data!B1` to `Data!E1+Data!B1`, a fresh `WorkbookEditor::open(output)` stays clean, `source_formula_reference_audits()` scans the saved worksheet XML and reports `Data!E1` / `Data!B1`, and `formula_reference_audits()` reports the same saved references after clean materialization. | This is saved-output audit evidence for the existing column-side stationary formula rewrite path only. It does not evaluate formulas, reconstruct edit history, inspect unqualified references, synchronize metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1256 | Public-state coverage now verifies the delete-column saved-output boundary for stationary formula rewrites. After `delete_columns(4, 1)` rewrites materialized `C1` from `Data!D1+Data!B1` to `Data!#REF!+Data!B1`, a fresh `WorkbookEditor::open(output)` reports only the surviving `Data!B1` reference through both source and materialized formula audits, while skipping persisted `Data!#REF!` and no longer reporting original `Data!D1`. | This is saved-output audit evidence for delete-column lexical `#REF!` output only. It does not repair `#REF!`, evaluate formulas, reconstruct edit history, synchronize metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1257 | Public-state coverage now verifies saved-output formula audits for stationary range and whole-row rewrites. After `insert_rows(3, 1)` rewrites materialized `C1` from `SUM(Data!A3:B3)+Data!3:3` to `SUM(Data!A4:B4)+Data!4:4`, a fresh `WorkbookEditor::open(output)` reports the saved `Data!A4:B4` range and `Data!4:4` whole-row tokens through both source and materialized formula audits, without reporting the original `Data!A3:B3` / `Data!3:3` tokens. | This is saved-output audit evidence for supported already-materialized range/whole-axis formula rewrites only. It does not add complete Excel range-shrink semantics, evaluate formulas, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1258 | Public-state coverage now verifies the matching column-range saved-output formula audit boundary. After `insert_columns(4, 1)` rewrites materialized `C1` from `SUM(Data!D1:E1)+Data!D:E` to `SUM(Data!E1:F1)+Data!E:F`, a fresh `WorkbookEditor::open(output)` reports the saved `Data!E1:F1` range and `Data!E:F` whole-column tokens through both source and materialized formula audits, without reporting the original `Data!D1:E1` / `Data!D:E` tokens. | This is saved-output audit evidence for supported already-materialized column-range / whole-column formula rewrites only. It does not add complete Excel range-shrink semantics, evaluate formulas, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1259 | Public-state coverage now verifies saved-output formula audits skip persisted delete-row `#REF!` tokens while keeping surviving whole-axis and shifted references. After `delete_rows(1, 1)` rewrites materialized `C4` from `Data!A1+Data!A:A+Data!1:1+Data!B4` to saved `C3` formula `Data!#REF!+Data!A:A+Data!#REF!+Data!B3`, a fresh `WorkbookEditor::open(output)` reports only `Data!A:A` and `Data!B3` through both source and materialized formula audits. | This is saved-output audit evidence for delete-row lexical `#REF!` output on already-materialized formula cells only. It does not repair `#REF!`, evaluate formulas, add complete range-shrink semantics, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1260 | Public-state coverage now verifies the matching delete-column `#REF!` saved-output formula audit boundary. After `delete_columns(1, 1)` rewrites materialized `D1` from `Data!A1+Data!A:A+Data!1:1+Data!D2` to saved `C1` formula `Data!#REF!+Data!#REF!+Data!1:1+Data!C2`, a fresh `WorkbookEditor::open(output)` reports only `Data!1:1` and `Data!C2` through both source and materialized formula audits. | This is saved-output audit evidence for delete-column lexical `#REF!` output on already-materialized formula cells only. It does not repair `#REF!`, evaluate formulas, add complete range-shrink semantics, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1261 | Public-state coverage now verifies `WorkbookEditor::source_formula_reference_audits()` keeps scanning the original source worksheet XML while dirty range / whole-axis stationary formulas have already been rewritten in the materialized session. After row insertion rewrites `C1` from `SUM(Data!A3:B3)+Data!3:3` to `SUM(Data!A4:B4)+Data!4:4`, and column insertion rewrites `SUM(Data!D1:E1)+Data!D:E` to `SUM(Data!E1:F1)+Data!E:F`, the source audit still reports only the original `Data!A3:B3` / `Data!3:3` and `Data!D1:E1` / `Data!D:E` tokens. | This is read-only dirty source-scan boundary coverage for supported range / whole-axis formula rewrites only. It does not merge source/materialized audit views, mutate source worksheet XML, evaluate formulas, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1262 | Public-state coverage now verifies the same dirty source-scan boundary for delete-row/delete-column formula rewrites that produce lexical `#REF!` tokens. Source workbooks with formulas at `C4` / `D1` are structurally deleted into materialized `C3` / `C1` formulas containing `Data!#REF!`; while dirty, `source_formula_reference_audits()` still reports the original source formula-cell coordinates and original tokens such as `Data!A1`, `Data!A:A`, `Data!1:1`, `Data!B4`, and `Data!D2`, not the materialized `#REF!` or shifted `Data!B3` / `Data!C2` tokens. | This is read-only source XML audit boundary coverage for delete-side materialized formula rewrites only. It does not repair `#REF!`, merge source/materialized audit views, mutate source worksheet XML, evaluate formulas, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1263 | Public-state coverage now verifies materialized-only dirty formulas remain invisible to `WorkbookEditor::source_formula_reference_audits()` until saved. A source workbook with no formula cells gets a dirty `C2` formula `Data!A1+Data!B1` through `WorksheetEditor::set_cell()`; the source audit stays empty and preserves dirty diagnostics, while `formula_reference_audits()` reports the in-memory `Data!A1` / `Data!B1` references from the materialized sparse store. | This is read-only source-vs-materialized audit isolation for unsaved in-memory formula cells only. It does not merge audit views, mutate source worksheet XML, save dirty formulas implicitly, evaluate formulas, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1264 | Public-state coverage now verifies the matching saved-output boundary for materialized-only formulas. After the dirty `C2` formula `Data!A1+Data!B1` is flushed through `save_as(output)`, a fresh `WorkbookEditor::open(output)` has clean diagnostics and `source_formula_reference_audits()` reports the saved `Data!A1` / `Data!B1` tokens from worksheet XML; clean materialization of `Data` then lets `formula_reference_audits()` report the same tokens without dirtying the reopened editor. | This is saved-output audit evidence for the existing in-memory flush path only. It does not evaluate formulas, reconstruct edit history, implicitly save dirty sessions, repair metadata, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1265 | Public-state coverage now verifies rejected exact-source saves preserve the materialized-only formula audit split. After dirty `C2` formula `Data!A1+Data!B1` is added and `save_as(source)` is rejected, source package bytes stay unchanged, `source_formula_reference_audits()` remains empty, `formula_reference_audits()` still reports the dirty `Data!A1` / `Data!B1` tokens, and a later safe retry saves an output whose fresh source audit reports the persisted formula references. | This is failed-save retry boundary evidence for the existing in-memory flush path only. It does not allow source overwrites, merge audit views, evaluate formulas, repair metadata, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory shift addenda P8.1266 | Public-state coverage now verifies the materialized-only formula failed-save recovery remains stable across a follow-up no-op save. After rejected `save_as(source)` and safe `save_as(output)` write dirty `C2` formula `Data!A1+Data!B1`, the no-op `save_as(noop_output)` preserves public save-state diagnostics, emits byte-equivalent package entries, and fresh reopen of the no-op output reports the persisted formula references. | This is no-op retry stability evidence for the existing in-memory flush path only. It does not add source overwrite support, clean-session commits, formula evaluation, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1267 | Public-state coverage now verifies same-editor post-save audit routing for materialized-only formulas. After `save_as(output)` writes dirty `C2` formula `Data!A1+Data!B1`, the same editor's `source_formula_reference_audits()` still scans the original source XML and remains empty, while `formula_reference_audits()` reports the clean materialized `Data!A1` / `Data!B1` references without re-dirtying the session. | This is audit routing evidence for source-vs-materialized views only. It does not retarget source audits to saved output inside the same editor, evaluate formulas, reconstruct edit history, repair metadata, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1268 | The opt-in workbook-editor QA runner now has `generated_in_memory_insert_formula`, which creates a generated workbook, opens it through public `WorkbookEditor`, performs `WorksheetEditor::insert_rows(2, 1)`, writes a new materialized row with text/number/formula cells, verifies the source-backed formula row shifts from `B2*2` to `B3*2`, then saves and validates the output with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file in-memory row-shift path only. It does not add new production behavior, evaluate formulas, preserve cached values, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1269 | The opt-in workbook-editor QA runner now also has `generated_in_memory_delete_column_formula`, which creates a generated workbook, opens it through public `WorkbookEditor`, performs `WorksheetEditor::delete_columns(1, 1)`, verifies original `B1` shifts to `A1`, original formula cell `C1` shifts to `B1` with `B1+D1` translated to `A1+C1`, original `D1` shifts to `C1`, then saves and validates the output with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file in-memory column-delete path only. It does not add new production behavior, evaluate formulas, preserve cached values, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1270 | The opt-in workbook-editor QA runner now also covers `generated_in_memory_insert_column_formula` and `generated_in_memory_delete_row_formula`. The insert-column case performs `WorksheetEditor::insert_columns(2, 1)`, writes inserted `B1`, verifies source `B1` shifts to `C1` and formula `C1` shifts to `D1` with `B1*2` translated to `C1*2`. The delete-row case performs `WorksheetEditor::delete_rows(1, 1)`, verifies source `A2` / `A3` shift to `A1` / `A2`, and formula `B2` shifts to `B1` with `A2+A3` translated to `A1+A2`. Both save and validate with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This completes generated QA evidence for the current small-file row/column shift formula directions only. It does not add new production behavior, evaluate formulas, preserve cached values, synchronize worksheet metadata ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1271 | The opt-in workbook-editor QA runner now also has `generated_in_memory_clear_erase`, which creates a generated workbook, opens it through public `WorkbookEditor`, clears source-backed formula cell `B1` to an explicit blank with `WorksheetEditor::clear_cell_value()`, erases source-backed text cell `C1` with `WorksheetEditor::erase_cell()`, writes new materialized text cell `D1`, then saves and validates that `B1` remains represented as blank while `C1` is absent using ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file in-memory clear/erase persistence semantics only. It does not add new production behavior, formula evaluation, cached value preservation, tombstones, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1272 | The opt-in workbook-editor QA runner now also has `generated_in_memory_append_row_formula`, which creates a generated workbook, opens it through public `WorkbookEditor`, appends a new materialized row with text/number/formula cells using `WorksheetEditor::append_row()`, verifies the appended formula through the public read model, then saves and validates source rows, appended row, formula text, and untouched sheet with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file in-memory append-row persistence path only. It does not add new production behavior, formula evaluation, cached value preservation, append-table/metadata synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1273 | The opt-in workbook-editor QA runner now also has `generated_in_memory_overwrite_formula_text`, which creates a generated workbook, opens it through public `WorkbookEditor`, overwrites source-backed `Data!A1` text, `Data!B1` number, and `Data!C1` formula cells using `WorksheetEditor::set_cell()`, verifies the overwritten formula through the public read model, then saves and validates old payload removal, new values, formula text, preserved `Data!A2`, and untouched `Notes` with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file in-memory source-cell overwrite persistence path only. It does not add new production behavior, formula evaluation, cached value preservation, source package overwrite, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1291 | The opt-in workbook-editor QA runner now also has `generated_in_memory_retry_noop_save`, which repeats the generated single-worksheet text/number/formula overwrite shape, observes the rejected `save_as(source)` path, verifies the source workbook still contains the old payload, writes a safe retry output, then performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for failed-save retry no-op stability on the existing single-worksheet small-file materialized path only. It does not add overwrite mode, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1294 | The opt-in workbook-editor QA runner now also has `generated_in_memory_retry_path_equivalent_noop_save`, which repeats the generated single-worksheet text/number/formula overwrite shape, observes a rejected path-equivalent source-overwrite `save_as()` path, verifies the source workbook still contains the old payload, writes a safe retry output, then performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for path-equivalent failed-save retry/no-op stability on the existing single-worksheet small-file materialized path only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1292 | The opt-in workbook-editor QA runner now also has `generated_in_memory_retry_reopen_modify_noop_save`, which rejects the generated single-worksheet source-overwrite save, writes a safe retry output, reopens that retry output through a fresh `WorkbookEditor`, applies second-stage `Data` edits, saves a final workbook, then performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for failed-save retry followed by reopen/edit/no-op stability on the existing single-worksheet small-file materialized path only. It does not add overwrite mode, rollback, transaction replay, production save behavior, commit/close semantics, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1296 | The opt-in workbook-editor QA runner now also has `generated_in_memory_retry_path_equivalent_reopen_modify_noop_save`, which rejects a path-equivalent source-overwrite save, writes a safe retry output, reopens that retry output through a fresh `WorkbookEditor`, applies second-stage `Data` edits, and requires the final no-op output to be byte-identical. | This is generated QA evidence for path-equivalent failed-save retry followed by reopen/edit/no-op stability on the existing single-worksheet small-file materialized path only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, commit/close semantics, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1298 | The opt-in workbook-editor QA runner now also has `generated_in_memory_retry_path_equivalent_reopen_modify_post_noop_third_save`, which repeats the path-equivalent failed-save retry, fresh reopen/edit/save, and clean no-op path, then edits `Data` again after that no-op save, writes a third-stage output, and requires the final no-op output to be byte-identical. | This is generated QA evidence for same-editor single-worksheet usability after a path-equivalent failed-save retry, fresh reopen/edit, and clean no-op save only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, commit/close semantics, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1293 | The opt-in workbook-editor QA runner now also has `generated_in_memory_retry_reopen_modify_post_noop_third_save`, which repeats the generated single-worksheet failed-save retry, fresh reopen/edit/save, and clean no-op path, then edits `Data` again after that no-op save, writes a third-stage output, and performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for same-editor single-worksheet usability after a failed-save retry, fresh reopen/edit, and clean no-op save only. It does not add overwrite mode, rollback, transaction replay, production save behavior, commit/close semantics, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1274 | The opt-in workbook-editor QA runner now also has `generated_in_memory_reopen_modify_save`, which creates a generated workbook, performs a first `WorkbookEditor` in-memory edit and `save_as()` to an intermediate output, reopens that intermediate workbook through a fresh `WorkbookEditor`, applies additional `WorksheetEditor::set_cell()` edits, then saves a final output and validates first-stage edits, second-stage edits, formula text, preserved rows, and untouched `Notes` with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file edit-save-reopen-edit-save loop only. It does not add new production behavior, in-place source overwrite, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1275 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_save`, which creates a generated workbook, opens it through public `WorkbookEditor`, materializes both `Data` and `Summary`, edits source-backed text/number/formula cells, appends a new formula row on `Data`, then saves once and validates both dirty worksheets plus untouched `Notes` with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing small-file multi-worksheet dirty-session save path only. It does not add new production behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1289 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_noop_save`, which repeats the generated multi-worksheet dirty-session save path and then performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for multi-worksheet clean no-op save stability only. It does not add production save behavior, commit/close semantics, in-place overwrite, rollback, transaction replay, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory public-state addenda P8.1276 | Public-state coverage now verifies multi-worksheet materialized save/no-op stability. After one editor dirties both `Data` and `Untouched`, the first `save_as()` cleans both original handles, matching reacquired handles stay clean and read the saved sparse state, pending materialized names/counts/memory and summaries are empty, and a later no-op `save_as()` preserves catalog/save diagnostics while producing byte-identical output. | This is saved-session hygiene evidence for existing multi-worksheet materialized stores only. It does not add new production behavior, session cloning, source reload, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1277 | Public-state coverage now verifies multi-worksheet materialized failed-save retry/no-op stability. After one editor dirties both `Data` and `Untouched`, a rejected source-overwrite `save_as(source)` preserves both dirty handles, aggregate materialized names/counts/memory, pending handoff count, and source package bytes; a later safe retry saves both sheets, matching reacquired handles read the saved state, and a final no-op `save_as()` preserves catalog/save diagnostics while producing byte-identical output. | This is failed-save retry hygiene evidence for existing multi-worksheet materialized stores only. It does not add overwrite mode, rollback, transaction replay, new production save behavior, session cloning, source reload, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1278 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_save`, which creates the same generated `Data` / `Summary` / `Notes` source shape as the multi-sheet save scenario, dirties both materialized worksheets, observes the rejected `save_as(source)` path, verifies the source still contains the old `Data` and `Summary` payloads, then validates the safe retry output with ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM sidecar. | This is generated QA evidence for the existing multi-worksheet failed-save retry path only. It does not add overwrite mode, rollback, transaction replay, new production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1290 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_noop_save`, which repeats the generated multi-worksheet rejected `save_as(source)` retry path, writes a safe retry output, then performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for failed-save retry no-op stability on the existing small-file materialized path only. It does not add overwrite mode, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1295 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_path_equivalent_noop_save`, which repeats the generated multi-worksheet rejected-save retry/no-op shape with a path-equivalent source-overwrite `save_as()` path before writing the safe retry output and requiring the follow-up no-op output to be byte-identical. | This is generated QA evidence for path-equivalent failed-save retry/no-op stability on the existing small-file multi-worksheet materialized path only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1279 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_reopen_modify_save`, which first drives the generated multi-worksheet rejected-save retry path, writes an intermediate safe retry output, reopens that output through a fresh `WorkbookEditor`, applies second-stage `Data` and `Summary` materialized edits, and validates the final output while also checking the original source workbook retained its old payloads. | This is generated QA evidence for the existing failed-save retry followed by reopen/edit/save path only. It does not add overwrite mode, rollback, transaction replay, new production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1280 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_reopen_modify_noop_save`, which repeats the generated multi-worksheet rejected-save retry and fresh reopen/edit/save path, then performs one more no-op `save_as()` and requires the second-stage output and no-op output to be byte-identical before validating the final workbook. | This is generated QA evidence for post-reopen no-op save stability on the existing small-file materialized path only. It does not add overwrite mode, rollback, transaction replay, new production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1297 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_path_equivalent_reopen_modify_noop_save`, which repeats the generated multi-worksheet rejected-save retry/reopen/no-op shape with a path-equivalent source-overwrite `save_as()` path before writing the safe retry output, reopening it through a fresh editor, applying second-stage `Data` / `Summary` edits, and requiring the final no-op output to be byte-identical. | This is generated QA evidence for path-equivalent failed-save retry followed by reopen/edit/no-op stability on the existing small-file multi-worksheet materialized path only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1299 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_path_equivalent_reopen_modify_post_noop_third_save`, which repeats the generated multi-worksheet path-equivalent rejected-save retry/reopen/no-op shape, then edits both `Data` and `Summary` again after that clean no-op save, writes a third-stage output, and requires the final no-op output to be byte-identical. | This is generated QA evidence for same-editor multi-worksheet usability after a path-equivalent failed-save retry, fresh reopen/edit, and clean no-op save only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1287 | The opt-in workbook-editor QA runner now also has `generated_in_memory_multi_sheet_retry_reopen_modify_post_noop_third_save`, which repeats the generated retry/reopen/modify/no-op path, edits both `Data` and `Summary` again after that clean no-op save, saves a third-stage output, and requires the final no-op `save_as()` to be byte-identical before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for same-editor multi-worksheet usability after a clean no-op save only. It does not add commit/close semantics, overwrite mode, rollback, transaction replay, new production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory public-state addenda P8.1281 | Public-state coverage now verifies the same multi-worksheet failed-save retry can be saved to a safe output, reopened through a fresh `WorkbookEditor`, edited again on both materialized sheets, saved, and then no-op saved with byte-identical package entries. The test also checks the original source and intermediate retry output remain unchanged after later saves. | This is default regression evidence for existing small-file materialized session hygiene only. It does not add overwrite mode, rollback, transaction replay, new production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1300 | Public-state coverage now also verifies the multi-worksheet retry/reopen/no-op/post-noop path rejects a path-equivalent source-overwrite `save_as()` before the safe retry. After the ordinary exact-source rejection, the path-equivalent rejection must keep both `Data` and `Untouched` materialized handles dirty, preserve source package bytes, and still allow the existing safe retry, fresh reopen/edit, no-op save, post-noop third save, and final no-op checks to pass. | This is default regression evidence for path-equivalent failed-save hygiene on the existing small-file multi-worksheet materialized path only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1303 | The same multi-worksheet retry/reopen rejected-save regression now pins full dirty materialized diagnostics for exact and path-equivalent source-overwrite failures: both dirty worksheet names, aggregate materialized cell count, and aggregate materialized memory estimate remain aligned with the dirty `Data` and `Untouched` sessions until the safe retry. | This is diagnostics contract coverage only. It does not change save behavior, add overwrite mode, introduce path repair or rollback, alter memory accounting, evaluate formulas, repair metadata/relationships, rebuild calcChain, migrate sharedStrings/styles, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1304 | Public-state coverage now pins fresh-reopen second-stage dirty memory diagnostics for the single-worksheet and multi-worksheet retry/reopen paths. After reopening the safe retry output and applying later `WorksheetEditor` edits, `estimated_pending_materialized_memory_usage()` must match the dirty materialized session memory estimate, or the aggregate estimate for both dirty sheets, before the next safe save. | This is diagnostics contract coverage only. It does not change save behavior, add memory budgets, redesign accounting, add transaction replay, evaluate formulas, repair metadata/relationships, rebuild calcChain, migrate sharedStrings/styles, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1305 | Public-state coverage now also pins post-noop third-stage dirty memory diagnostics for the single-worksheet and multi-worksheet retry/reopen/no-op paths. After a clean no-op `save_as()`, later `WorksheetEditor` edits must make `estimated_pending_materialized_memory_usage()` match the dirty `Data` session estimate, or the aggregate `Data` plus `Untouched` estimate, before the third-stage save. | This is diagnostics contract coverage only. It does not change save behavior, add memory budgets, redesign accounting, add transaction replay, evaluate formulas, repair metadata/relationships, rebuild calcChain, migrate sharedStrings/styles, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1306 | Public-state coverage now pins the matching `pending_worksheet_edits()` summaries for post-noop third-stage dirty edits. The single-worksheet path must expose one dirty `Data` materialized summary, while the multi-worksheet path must expose source-order dirty `Data` and `Untouched` summaries with materialized cell counts and memory estimates aligned with each active `WorksheetEditor` session. | This is summary diagnostics contract coverage only. It does not change summary generation, save behavior, memory accounting, formula evaluation, metadata/relationship repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1307 | Public-state coverage now pins `pending_change_count()` during post-noop third-stage dirty edits. The single-worksheet path must keep the prior saved handoff count at one, and the multi-worksheet path must keep the prior saved handoff count at two, until the next `save_as()` flushes the dirty materialized sessions. | This is pending-count lifecycle coverage only. It does not change save behavior, commit semantics, summary generation, rollback, transaction replay, metadata repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1308 | Public-state coverage now also pins `pending_change_count()` during fresh-reopen second-stage dirty edits. A freshly reopened single-worksheet or multi-worksheet editor must keep the count at zero until `save_as()` converts the dirty materialized sessions into staged handoffs. | This is pending-count lifecycle coverage only. It does not change save behavior, commit semantics, summary generation, rollback, transaction replay, metadata repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1309 | Public-state coverage now also pins `pending_worksheet_edits()` summaries during fresh-reopen second-stage dirty edits. The single-worksheet path exposes one dirty `Data` materialized summary, while the multi-worksheet path exposes source-order dirty `Data` and `Untouched` summaries with per-sheet materialized cell counts and memory estimates before `save_as()`. | This is summary diagnostics contract coverage only. It does not change summary generation, save behavior, commit semantics, memory accounting, formula evaluation, metadata/relationship repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1310 | Public-state coverage now pins fresh-reopen clean diagnostics before any second-stage edit. A reopened single-worksheet or multi-worksheet editor must expose zero pending change count, no dirty materialized worksheet names/cells/memory, and no pending edit summaries immediately after opening the previously saved output. | This is clean-state diagnostics contract coverage only. It does not change open/save behavior, commit semantics, summary generation, rollback, transaction replay, metadata repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1311 | Public-state coverage now pins dirty summary cleanup across the fresh-reopen no-op save. After the second-stage materialized sessions have already been saved, a following no-op `save_as()` must keep `pending_worksheet_edits()` empty for both the single-worksheet and multi-worksheet paths while retaining the staged handoff count. | This is no-op diagnostics contract coverage only. It does not change save behavior, commit semantics, staged handoff retention, summary generation, rollback, transaction replay, metadata repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1312 | Public-state coverage now pins pre-save dirty diagnostics for the row/column shift reacquire/no-op paths. Before the first `save_as()`, insert/delete row/column shifts must expose `Data` as the only dirty materialized worksheet, report matching cell count and memory estimates, keep `pending_change_count()` at zero, and publish one dirty materialized summary without replacement flags. | This is pre-save diagnostics contract coverage only. It does not change shift behavior, save behavior, summary generation, commit semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1313 | Public-state coverage now extends that same pre-save dirty diagnostic contract to the row/column shift saved-session rejection paths. Before their first `save_as()`, option-mismatch, missing-query, invalid-read, invalid-mutation, and invalid-shift regressions must expose the dirty `Data` materialized session, matching cell/memory estimates, zero staged handoff count, and one materialized summary without replacement flags. | This is rejection-path diagnostics contract coverage only. It does not change validation behavior, shift behavior, save/retry behavior, summary generation, commit semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1314 | Public-state coverage now extends the same pre-save dirty diagnostic contract to row/column shift failed-save retry paths. Before the initial successful materialized handoff, source-overwrite, path-equivalent source overwrite, empty-output, missing-parent, non-directory-parent, existing-directory, and after-retry no-op paths must expose the dirty `Data` materialized session with matching count/memory and one materialized summary. | This is failed-save-path diagnostics coverage only. It does not change overwrite policy, path equivalence policy, filesystem error handling, save/retry behavior, shift behavior, summary generation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1315 | Public-state coverage now pins invalid-to-valid row/column shift recovery pre-save diagnostics. After rejected row/column shifts and a later valid recovery shift, both clean-start and already-dirty sessions must clear the prior diagnostic, keep `pending_change_count()` at zero, expose `Data` as the only dirty materialized worksheet, and publish one materialized summary matching the active session count/memory. | This is recovery diagnostics coverage only. It does not change validation behavior, diagnostic clearing behavior, shift behavior, save behavior, memory accounting, summary generation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1316 | Public-state coverage now pins formula row/column shift pre-save dirty summaries. Rich insert-row/insert-column formula translation and delete-row/delete-column `#REF!` formula translation must expose the dirty `Data` materialized session, zero staged handoff count, matching count/memory estimates, and one materialized summary before `save_as()` persists the translated formula text. | This is formula-shift diagnostics coverage only. It does not change formula translation, formula evaluation, cached values, save behavior, memory accounting, summary generation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1317 | Public-state coverage now pins clean row/column shift no-op and guard-failure summaries. Zero-count shifts, nonzero out-of-range no-op shifts, validation failures, and formula-translation memory-budget failures must leave the editor with zero pending handoffs, empty pending summaries, empty replacement diagnostics, and no dirty materialized worksheet names/count/memory. | This is clean-state diagnostics coverage only. It does not change shift behavior, validation behavior, memory guard behavior, diagnostic text, save behavior, summary generation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1318 | Public-state coverage now pins dirty row/column overflow rejection summaries. When `insert_rows()` or `insert_columns()` would move an already represented edge cell past Excel limits, the rejected shift must preserve the prior dirty `Data` materialized session, the shift diagnostic, zero staged handoff count, matching materialized count/memory, empty replacement diagnostics, and one dirty materialized summary before the later safe save. | This is dirty overflow rejection diagnostics coverage only. It does not change overflow validation, shift behavior, rollback behavior, save behavior, summary generation, memory accounting, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1319 | Public-state coverage now pins guardrail failure/no-op replacement-summary hygiene. Source-load `max_cells` and memory-budget materialization failures must remain fully clean with empty pending summaries and replacement diagnostics; mutation-side memory-budget and `max_cells` failures must preserve their diagnostics while keeping pending summaries and replacement diagnostics empty; materialized recovery no-op saves must keep replacement diagnostics empty. | This is guardrail diagnostics hygiene only. It does not change guardrail enforcement, diagnostic text, recovery behavior, save behavior, summary generation, memory accounting, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1320 | Public-state coverage now pins guardrail recovery dirty summaries. After source-load memory-budget recovery or mutation-side memory-budget / `max_cells` recovery accepts an in-budget overwrite, the editor must clear diagnostics, keep staged handoff count at zero, expose `Data` as the dirty materialized worksheet, report matching count/memory, leave replacement diagnostics empty, and publish one dirty materialized summary before save. | This is recovery dirty-diagnostics coverage only. It does not change guardrail policy, recovery behavior, save behavior, summary generation, memory accounting, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1321 | Public-state coverage now pins stationary formula structural rewrites for absolute and mixed `$` markers. Row/column insertions must move affected `$A$3`, `C$3`, `$D1`, and sheet-qualified equivalents while preserving marker text; row/column deletions must emit `#REF!` for deleted absolute references and shift surviving mixed references, then save, no-op save, and reopen with the same formula text. | This is formula structural-rewrite regression coverage for already-materialized stationary formula cells only. It does not change moved-formula translation, evaluate formulas, preserve cached results, scan non-materialized worksheet XML, synchronize metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1322 | The opt-in workbook-editor QA runner now also has `generated_in_memory_stationary_formula_shift`, which opens a generated existing workbook, applies insert/delete row and column shifts to a materialized `Data` sheet, verifies stationary `C1` rewrites mixed and absolute row/column references plus deleted `#REF!` tokens, saves the output, and validates it through ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM smoke. | This is generated QA evidence for the existing already-materialized stationary formula rewrite lane only. It does not change production formula rewrite semantics, evaluate formulas, preserve cached results, scan non-materialized worksheet XML, synchronize metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1323 | The opt-in workbook-editor QA runner now also has `generated_in_memory_stationary_range_formula_shift`, which opens a generated existing workbook, applies row and column insertion to a materialized `Data` sheet, verifies stationary `C1` rewrites cell ranges plus whole-row and whole-column references, saves the output, and validates it through ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM smoke. | This is generated QA evidence for the existing supported range and whole-axis stationary formula rewrite lane only. It does not change production formula rewrite semantics, add delete-side range shrink behavior, evaluate formulas, preserve cached results, scan non-materialized worksheet XML, synchronize metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1324 | The opt-in workbook-editor QA runner now also has `generated_in_memory_stationary_formula_shift_noop_save` and `generated_in_memory_stationary_range_formula_shift_noop_save`, which repeat the generated stationary formula and range formula rewrite scenarios, save once, then require a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing stationary formula rewrite lane only. It does not change production formula rewrite semantics, evaluate formulas, preserve cached results, scan non-materialized worksheet XML, synchronize metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1325 | The opt-in workbook-editor QA runner now also has `generated_in_memory_insert_formula_noop_save`, `generated_in_memory_delete_column_formula_noop_save`, `generated_in_memory_insert_column_formula_noop_save`, and `generated_in_memory_delete_row_formula_noop_save`, which repeat the generated moving-formula row/column shift scenarios, save once, then require a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after existing moving-formula row/column shift scenarios only. It does not change production formula translation semantics, evaluate formulas, preserve cached results, scan non-materialized worksheet XML, synchronize metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1326 | The opt-in workbook-editor QA runner now also has `generated_in_memory_clear_erase_noop_save` and `generated_in_memory_append_row_formula_noop_save`, which repeat the generated clear/erase and append-row formula persistence scenarios, save once, then require a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after existing clear/erase and append-row materialized edits only. It does not change production clear/erase or append-row semantics, add tombstones, formula evaluation, cached results, metadata/range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1327 | The opt-in workbook-editor QA runner now also has `generated_in_memory_overwrite_formula_text_noop_save`, which repeats the generated source-backed text/number/formula overwrite persistence scenario, save once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing overwrite materialized edit path only. It does not change production overwrite semantics, add source overwrite mode, formula evaluation, cached results, metadata/range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1328 | The opt-in workbook-editor QA runner now also has `generated_rename_materialized_noop_save`, which repeats the generated rename/materialized persistence scenario, save once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing rename plus materialized edit path only. It does not change production rename or materialized edit semantics, add formula qualifier repair, metadata/range synchronization, calcChain rebuild, sharedStrings/styles migration, relationship repair, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1329 | The opt-in workbook-editor QA runner now also has `generated_shared_formula_materialization_noop_save` and `generated_shared_formula_office_like_materialization_noop_save`, which repeat the generated shared-formula materialization and Office-like shared-formula scenarios, save once, then require a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after existing shared-formula materialized edit paths only. It does not change formula materialization semantics, preserve shared formula metadata, evaluate formulas, preserve cached formula results, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1330 | The opt-in workbook-editor QA runner now also has no-op save variants for all generated formula rename lanes: `generated_formula_rename_rewrite_noop_save`, `generated_formula_rename_escaped_sheet_name_noop_save`, `generated_formula_rename_chain_rewrite_noop_save`, `generated_formula_rename_defined_names_only_noop_save`, and `generated_formula_rename_default_audit_noop_save`. Each repeats the existing rename policy scenario, saves once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after existing formula rename and materialized formula outputs only. It does not change rename policy semantics, broaden default formula rewrite, evaluate formulas, preserve cached formula results, scan non-materialized worksheet XML, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1331 | The opt-in workbook-editor QA runner now also has `generated_public_e2e_noop_save`, which repeats the generated public E2E combination of `rename_sheet()`, `WorksheetEditor` materialized edits, `replace_sheet_data()`, and `replace_image()`, saves once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing public combined edit smoke only. It does not add transaction history, in-place overwrite, source mutation, metadata/range synchronization, semantic image editing, relationship repair/pruning, formula evaluation, calcChain rebuild, sharedStrings/styles migration, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1332 | The opt-in workbook-editor QA runner now also has `generated_shared_formula_boundary_materialization_noop_save`, which repeats the generated shared-formula parser-boundary materialization scenario, saves once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML and `openpyxl` validation. The optional Excel COM helper keeps skipping this synthetic parser-boundary lane for both variants. | This is generated QA evidence for post-flush no-op save stability after the existing shared-formula boundary materialization path only. It does not broaden formula parsing, evaluate formulas, preserve shared formula metadata, generate cached results, validate sheet or external references, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1333 | The opt-in workbook-editor QA runner now also has `generated_style_passthrough_noop_save`, which repeats the generated existing-style passthrough scenario, saves once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing same-workbook style id passthrough path only. It does not add style migration, style merge, stylesheet semantic editing, styles.xml repair, caller-supplied arbitrary non-default style writes, relationship repair, default CTest/CI expansion, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1334 | The opt-in workbook-editor QA runner now also has `generated_image_replace_noop_save`, which repeats the generated existing-file image replacement scenario, saves once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, media-byte comparison, drawing relationship, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing public image replacement path only. It does not add semantic image editing, drawing mutation, media transformation, relationship repair/pruning, metadata/range synchronization, default CTest/CI expansion, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1335 | The opt-in workbook-editor QA runner now also has `generated_source_formula_audit_noop_save`, which repeats the generated source-worksheet formula audit scenario, saves the catalog rename output once, then requires a follow-up clean `save_as()` output to be byte-identical before the same ZIP/XML, source formula audit report, `openpyxl`, and optional Excel COM validation. | This is generated QA evidence for post-flush no-op save stability after the existing audit-only source formula scan and catalog rename path only. It does not rewrite formulas, repair stale source qualifiers, evaluate formulas, preserve cached results, rebuild calcChain, migrate sharedStrings/styles, repair relationships, expand default CTest/CI, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory QA addenda P8.1336 | The opt-in workbook-editor QA runner now also has `generated_non_default_style_rejection_noop_save`, which repeats the generated caller-supplied non-default `StyleId` rejection scenario, saves the copy-original recovery output once, then requires a follow-up clean `save_as()` output to be byte-identical before ZIP entry byte comparison, `openpyxl`, and optional Excel COM validation. | This is generated QA evidence for no-op save stability after the existing rejection-only style guardrail path only. It does not add caller-supplied non-default style writes, style migration, style merge, styles.xml repair, relationship repair, default CTest/CI expansion, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1337 | Public-state coverage now pins no-op save stability after caller-supplied non-default `StyleId` rejection through `WorksheetEditor::set_row_values()` and `set_column_values()`. Each rejected value-only row/column call keeps the materialized session clean, preserves the public failure diagnostic across a copy-original `save_as()` and a follow-up no-op `save_as()`, keeps materialized/replacement diagnostics empty, emits source-identical package entries, and reopens with the source `Data` sheet unchanged. | This is default regression evidence for rejection-only style guardrail save hygiene. It does not add caller-supplied non-default style writes, style migration, style merge, styles.xml repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1338 | Public-state coverage now pins the same save/no-op stability after caller-supplied non-default `StyleId` rejection through the full-cell `WorksheetEditor::set_row()` and `set_column()` paths. The rejected row/column calls retain the failure diagnostic, keep the materialized session and editor pending state clean, save source-identical recovery output, and keep a follow-up no-op output byte-equivalent while reopened `Data` remains unchanged. | This is default regression evidence for full-cell row/column style guardrail save hygiene only. It does not add caller-supplied non-default style writes, style migration, style merge, styles.xml repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1339 | Public-state coverage now extends the same copy-original and no-op save stability to sparse batch initializer-list style rejection through `WorksheetEditor::set_cells()` and `set_cell_values()`. Each rejected batch keeps the source-backed `Data` cells unchanged, preserves the public `last_edit_error()` diagnostic across both saves, leaves materialized/replacement diagnostics empty, emits source-identical package entries, and reopens with the source sheet unchanged. | This is default regression evidence for sparse batch rejection-only style guardrail save hygiene. It does not add caller-supplied non-default style writes, style migration, style merge, styles.xml repair, dense range editing, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1340 | Public-state coverage now pins copy-original and no-op save stability after caller-supplied non-default `StyleId` rejection through the single-cell value-only `WorksheetEditor::set_cell_value()` row/column and strict A1 overloads. Each rejected edit preserves source-backed `Data` cells, keeps the materialized session and editor pending state clean, retains the public diagnostic across both saves, emits source-identical package entries, and reopens unchanged. | This is default regression evidence for single-cell value-only rejection hygiene only. It does not add caller-supplied non-default style writes, style migration, style merge, styles.xml repair, dense range editing, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1341 | Public-state coverage now pins copy-original and no-op save stability after caller-supplied non-default `StyleId` rejection through `WorksheetEditor::append_row()`. The rejected append keeps the sparse count and source-backed `Data` cells unchanged, leaves `A3` absent, preserves the public diagnostic across both saves, emits source-identical package entries, and reopens unchanged. | This is default regression evidence for append-row rejection hygiene only. It does not add caller-supplied non-default style writes, row insertion, row metadata creation, style migration, style merge, styles.xml repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1342 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::append_row()` rejects more than 16,384 values. The failed append keeps the clean source-backed materialized session unchanged, leaves the would-be appended row absent, retains the public width diagnostic across both saves, emits source-identical package entries, and reopens unchanged. | This is default regression evidence for append-row validation failure save hygiene only. It does not add dense append, row insertion, coordinate clamping, row metadata creation, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1343 | Public-state coverage now pins dirty-session save/no-op stability after `WorksheetEditor::append_row()` rejects appending past row 1,048,576. The failed append preserves the pre-existing dirty `XFD1048576` edit, retains the row-limit diagnostic, saves the expanded sparse projection, omits the rejected payload, reopens with `A1:XFD1048576`, and keeps the follow-up no-op output byte-identical. | This is default regression evidence for append-row row-limit failure save hygiene only. It does not add dense append, row insertion, coordinate clamping, row metadata creation, rollback machinery, metadata/range synchronization, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1344 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::append_row()` rejects a new sparse record under an exact `max_cells` materialized-session budget. The failed append keeps the clean source-backed session unchanged, leaves `A3` absent, retains the public `max_cells` diagnostic across both saves, emits source-identical package entries, and reopens unchanged. | This is default regression evidence for append-row `max_cells` rejection save hygiene only. It does not add budget auto-sizing, rollback machinery, row insertion, row metadata creation, dense append, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1345 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::set_row()` rejects more than 16,384 values or row zero. Both rejected full-row calls keep the clean source-backed `Data` session unchanged, retain their public validation diagnostic across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for `set_row()` validation failure save hygiene only. It does not add coordinate clamping, dense row replacement, row insertion, row metadata creation, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1346 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::set_column()` rejects column zero. The rejected full-column call keeps the clean source-backed `Data` session unchanged, retains its public validation diagnostic across both saves, emits source-identical package entries, keeps materialized/replacement diagnostics empty, and reopens unchanged. | This is default regression evidence for `set_column()` validation failure save hygiene only. It does not add coordinate clamping, dense column replacement, column insertion, column metadata creation, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1347 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::set_row_values()` rejects row zero or a new row-prefix sparse record under an exact `max_cells` materialized-session budget. Both rejected value-prefix calls keep the clean source-backed `Data` session unchanged, retain public diagnostics across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for `set_row_values()` validation and `max_cells` rejection save hygiene only. It does not add coordinate clamping, dense row writes, row insertion, row metadata creation, budget auto-sizing, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1348 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::set_column_values()` rejects column zero or a new column-prefix sparse record under an exact `max_cells` materialized-session budget. Both rejected value-prefix calls keep the clean source-backed `Data` session unchanged, retain public diagnostics across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for `set_column_values()` validation and `max_cells` rejection save hygiene only. It does not add coordinate clamping, dense column writes, column insertion, column metadata creation, budget auto-sizing, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1349 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::clear_row()` rejects row zero and `clear_rows()` rejects a reversed row range. The rejected clear calls keep the clean source-backed `Data` session unchanged, retain public diagnostics across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for row clear validation failure save hygiene only. It does not add coordinate clamping, row metadata creation, dense row materialization, range repair, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1350 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::clear_column()` rejects column zero and `clear_columns()` rejects a reversed column range. The rejected clear calls keep the clean source-backed `Data` session unchanged, retain public diagnostics across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for column clear validation failure save hygiene only. It does not add coordinate clamping, column metadata creation, dense column materialization, range repair, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1351 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::erase_row()` rejects row zero and `erase_rows()` rejects a reversed row range. The rejected erase calls keep the clean source-backed `Data` session unchanged, retain public diagnostics across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for row erase validation failure save hygiene only. It does not add coordinate clamping, row metadata deletion semantics, dense row materialization, range repair, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1352 | Public-state coverage now pins copy-original and no-op save stability after `WorksheetEditor::erase_column()` rejects column zero and `erase_columns()` rejects a reversed column range. The rejected erase calls keep the clean source-backed `Data` session unchanged, retain public diagnostics across both saves, emit source-identical package entries, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for column erase validation failure save hygiene only. It does not add coordinate clamping, column metadata deletion semantics, dense column materialization, range repair, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1353 | Public-state coverage now pins copy-original and follow-up no-op save stability after valid missing `WorksheetEditor::erase_row()` and `erase_column()` calls clear a prior edit diagnostic without dirtying the clean source-backed `Data` session. Both missing erase no-ops keep source cells unchanged, emit source-identical package entries across both saves, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for missing row/column erase clean-save hygiene only. It does not add erase tombstones, missing-cell synthesis, row/column metadata deletion semantics, dense materialization, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1354 | Public-state coverage now pins clean no-op save stability after empty whole-store `WorksheetEditor::clear_cell_values()` and `erase_cells()` calls clear prior edit diagnostics on an already empty saved materialized `Data` session. The empty calls keep the saved empty sparse projection clean, preserve public save state/catalog diagnostics, emit byte-stable package entries, and reopen empty. | This is default regression evidence for empty whole-store clear/erase clean-save hygiene only. It does not add tombstones, dense worksheet deletion, missing-cell synthesis, session cloning, source reload, metadata/range repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1355 | Public-state coverage now pins copy-original and follow-up no-op save stability after valid missing `WorksheetEditor::clear_row()` and `clear_column()` calls clear a prior edit diagnostic without dirtying the clean source-backed `Data` session. Both missing clear no-ops keep source cells unchanged, emit source-identical package entries across both saves, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for missing row/column clear clean-save hygiene only. It does not add dense row/column materialization, missing-cell synthesis, row/column metadata creation, coordinate inference, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1356 | Public-state coverage now pins copy-original and follow-up no-op save stability after zero-count row/column shifts clear a prior edit diagnostic without dirtying the clean source-backed `Data` session. `insert_rows(..., 0)`, `delete_rows(..., 0)`, `insert_columns(..., 0)`, and `delete_columns(..., 0)` preserve source cells, emit source-identical package entries across both saves, keep materialized/replacement diagnostics empty, and reopen unchanged. | This is default regression evidence for zero-count shift clean-save hygiene only. It does not add coordinate clamping, structural metadata movement, row/column dimension repair, formula translation beyond existing represented-cell shift behavior, rollback machinery, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1357 | Public-state coverage now pins diagnostics-specific follow-up no-op save stability for pending materialized worksheet-name and aggregate diagnostics after a two-sheet materialized flush. The tests save dirty `Data` and `Untouched` sessions, verify names/counts/memory clear, preserve public save state/catalog snapshots across a clean no-op `save_as()`, keep materialized/replacement diagnostics empty, emit byte-stable package entries, and reopen both sheets unchanged. | This is default regression evidence for public diagnostics save hygiene only. It does not change retained materialized handoff semantics, add commit/close semantics, rollback, transaction history, metadata/range repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1358 | Public-state coverage now pins follow-up no-op save stability after dirty materialized ownership moves through `WorkbookEditor` move construction and move assignment. The move-assigned editor saves the assigned dirty `Data` session once, drops the discarded target dirty session, preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps materialized/replacement diagnostics empty, emits byte-stable package entries, and reopens both sheets unchanged. | This is default regression evidence for materialized session move-ownership save hygiene only. It does not add transaction transfer semantics, rollback, moved-handle guarantees beyond existing public diagnostics, metadata/range repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1359 | Public-state coverage now pins follow-up no-op save stability for dirty materialized edit summaries after `WorkbookEditor` move construction and move assignment. The summary move-owner test saves the assigned dirty `Data` summary once, verifies the summary clears, preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps materialized/replacement diagnostics empty, emits byte-stable package entries, and reopens both sheets unchanged. | This is default regression evidence for materialized summary move-ownership save hygiene only. It does not add transaction transfer semantics, rollback, moved-handle guarantees beyond existing public diagnostics, metadata/range repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1360 | Public-state coverage now pins follow-up no-op save stability when a dirty materialized summary is flushed beside a retained replacement summary. The mixed-summary test saves dirty `Data` materialized edits and a queued `Untouched` replacement, verifies the materialized summary clears while the replacement summary remains, preserves public save state/catalog snapshots across a clean no-op `save_as()`, emits byte-stable package entries, and reopens both sheets unchanged. | This is default regression evidence for mixed materialized/replacement summary save hygiene only. It does not add commit/close semantics, rollback, transaction replay, metadata/range repair, replacement cleanup, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1361 | Public-state coverage now pins follow-up no-op save stability for move-owned aggregate materialized diagnostics. The aggregate move-owner test preserves moved cell-count/memory diagnostics through move construction and assignment, saves the assigned dirty `Data` session, verifies the discarded target dirty payload does not leak, clears aggregate materialized diagnostics, preserves public save state/catalog snapshots across a clean no-op `save_as()`, emits byte-stable package entries, and reopens both sheets unchanged. | This is default regression evidence for aggregate diagnostics move-ownership save hygiene only. It does not add transaction transfer semantics, rollback, moved-handle guarantees beyond existing public diagnostics, metadata/range repair, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1362 | Public-state coverage now pins follow-up no-op save stability after same-editor saved materialized formula audit inspections. The materialized-only formula audit test saves `Data!A1+Data!B1` into `C2`, verifies same-editor source/materialized formula audits, preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps materialized/replacement diagnostics and dirty summaries empty, emits byte-stable package entries, and reopens the saved formula. | This is default regression evidence for saved-session audit inspection save hygiene only. It does not retarget same-editor source audits to saved output, evaluate formulas, preserve cached formula results, repair metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1363 | Public-state coverage now pins fresh-reopen formula audit no-op save stability. The saved-reopen materialized-only formula audit test opens the saved output, verifies both source and materialized formula audits, then preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps pending changes, materialized/replacement diagnostics, and dirty summaries empty, emits byte-stable package entries, and reopens the saved formula. | This is default regression evidence for clean reopened-session audit inspection save hygiene only. It does not add formula evaluation, cached formula result preservation, audit history transfer, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1364 | Public-state coverage now pins fresh-reopen no-op save stability after a saved stationary formula row-insert rewrite is audited. The stationary formula test saves `Data!A3+Data!B1` rewritten to `Data!A4+Data!B1`, reopens the output, verifies source/materialized audits report only the saved references, then preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps clean diagnostics empty, emits byte-stable package entries, and reopens the translated formula. | This is default regression evidence for clean reopened-session translated-formula audit save hygiene only. It does not broaden formula translation, evaluate formulas, preserve cached formula results, repair `#REF!`, repair metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1365 | Public-state coverage now pins fresh-reopen no-op save stability after a saved stationary formula delete-row rewrite is audited. The delete-row test saves `Data!A3+Data!B1` rewritten to `Data!#REF!+Data!B1`, reopens the output, verifies source/materialized audits skip `Data!#REF!` and keep only `Data!B1`, then preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps clean diagnostics empty, emits byte-stable package entries, and reopens the `#REF!` formula. | This is default regression evidence for clean reopened-session delete-side formula audit save hygiene only. It does not repair `#REF!`, evaluate formulas, preserve cached formula results, broaden formula translation, repair metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1366 | Public-state coverage now pins fresh-reopen no-op save stability after a saved stationary formula column-insert rewrite is audited. The column test saves `Data!D1+Data!B1` rewritten to `Data!E1+Data!B1`, reopens the output, verifies source/materialized audits report only the saved references, then preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps clean diagnostics empty, emits byte-stable package entries, and reopens the translated formula. | This is default regression evidence for clean reopened-session column-translated formula audit save hygiene only. It does not broaden formula translation, evaluate formulas, preserve cached formula results, repair `#REF!`, repair metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1367 | Public-state coverage now pins fresh-reopen no-op save stability after a saved stationary formula delete-column rewrite is audited. The delete-column test saves `Data!D1+Data!B1` rewritten to `Data!#REF!+Data!B1`, reopens the output, verifies source/materialized audits skip `Data!#REF!` and keep only `Data!B1`, then preserves public save state/catalog snapshots across a clean no-op `save_as()`, keeps clean diagnostics empty, emits byte-stable package entries, and reopens the `#REF!` formula. | This is default regression evidence for clean reopened-session delete-column formula audit save hygiene only. It does not repair `#REF!`, evaluate formulas, preserve cached formula results, broaden formula translation, repair metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1368 | Public-state coverage now pins fresh-reopen no-op save stability after saved stationary formula row-range and column-range rewrites are audited. The range tests save `SUM(Data!A3:B3)+Data!3:3` rewritten to `SUM(Data!A4:B4)+Data!4:4` and `SUM(Data!D1:E1)+Data!D:E` rewritten to `SUM(Data!E1:F1)+Data!E:F`, reopen each output, verify source/materialized audits report only saved range references, then preserve public save state/catalog snapshots across clean no-op `save_as()` calls, keep clean diagnostics empty, emit byte-stable package entries, and reopen the translated formulas. | This is default regression evidence for clean reopened-session range-formula audit save hygiene only. It does not broaden formula translation, evaluate formulas, preserve cached formula results, repair range metadata, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1369 | Public-state coverage now pins fresh-reopen no-op save stability after saved mixed `#REF!` formula rewrites are audited. The delete-row ref-formula test saves `Data!#REF!+Data!A:A+Data!#REF!+Data!B3`, and the delete-column ref-formula test saves `Data!#REF!+Data!#REF!+Data!1:1+Data!C2`; both reopen output, verify source/materialized audits skip persisted `Data!#REF!` tokens while reporting surviving saved references, then preserve public save state/catalog snapshots across clean no-op `save_as()` calls, keep clean diagnostics empty, emit byte-stable package entries, and reopen the saved formulas. | This is default regression evidence for clean reopened-session mixed `#REF!` formula audit save hygiene only. It does not repair `#REF!`, evaluate formulas, preserve cached formula results, implement complete range-shrink semantics, repair metadata/ranges, rebuild calcChain, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1370 | Public-state coverage now widens the materialized-only formula failed-save no-op retry check. After rejected `save_as(source)` and safe `save_as(output)` write `Data!A1+Data!B1` into `C2`, the follow-up no-op `save_as(noop_output)` now also snapshots and preserves workbook catalog/save-state views, keeps pending materialized diagnostics, replacement diagnostics, dirty summaries, and `last_edit_error()` clean, emits byte-stable package entries, and reopens the saved formula references. | This is default regression evidence for failed-save safe-retry no-op state hygiene only. It does not add source overwrite support, in-place save, rollback, transaction replay, formula evaluation, cached formula result preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1371 | Public-state coverage now reopens the no-op output from the same-handle post-save edit path. After one `WorksheetEditor` handle saves `A1`, is reused to save `B1`, and a clean no-op `save_as(noop_output)` emits byte-stable package entries, the test opens the no-op output through a fresh `WorkbookEditor` and verifies the saved sparse count, bounds, `A1`, `B1`, and source-backed `A2` read cleanly. | This is default regression evidence for same-handle saved-session no-op output readability only. It does not add commit/close semantics, in-place overwrite, rollback, transaction replay, new handle lifetime guarantees beyond the existing borrowed-handle path, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1372 | Public-state coverage now reopens the second no-op output from the A1 overload edit path. After `try_cell("A1")`, `get_cell("B1")`, `set_cell("D4")`, and `erase_cell("A2")` save once, two clean no-op `save_as()` calls keep byte-stable entries; the second no-op output is now opened through a fresh `WorkbookEditor` and verifies sparse count, bounds, source-backed `A1` / `B1`, erased `A2`, and inserted `D4`. | This is default regression evidence for A1 overload no-op output readability only. It does not change A1 parsing, add new overloads, commit/close semantics, in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1373 | Public-state coverage now reopens the second no-op output from the explicit blank `get_cell()` path. After a missing `get_cell(4,4)` read leaves state unchanged, `set_cell(4,4, blank())` saves `D4` as an explicit blank, and two clean no-op saves keep byte-stable entries, the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, blank bounds, source-backed `A1` / `B1` / `A2`, explicit blank `D4`, and missing `E5`. | This is default regression evidence for explicit-blank no-op output readability only. It does not change missing-cell exceptions, blank serialization, commit/close semantics, in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1374 | Public-state coverage now reopens the second no-op output from the `sparse_cells()` owning snapshot dirty-projection path. After the snapshot captures source-backed records, an explicit blank, and an inserted cell, a later A1 edit, erase, materialized save, and two byte-stable clean no-op saves, the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, dirty bounds, post-snapshot `A1`, source-backed `B1`, erased `A2`, explicit blank `B3`, and inserted `D4`. | This is default regression evidence for `sparse_cells()` snapshot no-op output readability only. It does not add borrowed snapshot views, dense worksheet snapshots, commit/close semantics, in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1375 | Public-state coverage now reopens the second no-op output from the bounded `sparse_cells(CellRange)` owning snapshot dirty-projection path. After the range snapshot captures only active records in `B1:C3`, later edits update `B1`, preserve outside-range `D4`, erase `A2`, save once, and perform two byte-stable clean no-op saves, the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, dirty bounds, source-backed `A1`, post-snapshot `B1`, erased `A2`, explicit blank `B3`, in-range `C3`, and outside-range `D4`. | This is default regression evidence for bounded `sparse_cells(CellRange)` no-op output readability only. It does not add dense range reads, missing-cell synthesis, borrowed snapshot views, streaming sparse iterators, range repair, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1376 | Public-state coverage now reopens the second no-op output from the strict A1 `sparse_cells("B1:C3")` owning snapshot dirty-projection path. After the A1 range snapshot captures only active records in the requested range, later edits update `B1`, preserve outside-range `D4`, erase `A2`, save once, and perform two byte-stable clean no-op saves, the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, dirty bounds, source-backed `A1`, post-snapshot `B1`, erased `A2`, explicit blank `B3`, in-range `C3`, and outside-range `D4`. | This is default regression evidence for strict A1 `sparse_cells()` range no-op output readability only. It does not add relaxed A1 parsing, lowercase/absolute/sheet-qualified references, dense range reads, missing-cell synthesis, borrowed snapshot views, streaming sparse iterators, range repair, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1377 | Public-state coverage now reopens the second no-op output from the `used_range()` dirty/empty inspection path. After diagnostic-preserving `used_range()` reads, a whole active sparse range erase, materialized save, and two byte-stable clean no-op saves, the second no-op output is opened through a fresh `WorkbookEditor` to verify the saved sheet remains empty, exposes no `used_range()`, and keeps erased `A1`, `B1`, and `A2` absent. | This is default regression evidence for `used_range()` no-op output readability only. It does not change used-range calculation, diagnostic preservation semantics, missing-cell behavior, blank serialization, commit/close semantics, in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1378 | Public-state coverage now reopens the second no-op output from the `contains_cell()` dirty inspection path. After diagnostic-preserving valid and invalid `contains_cell()` reads, dirty inserted/blank/source-backed cells are saved once and survive two byte-stable clean no-op saves; the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, dirty bounds, represented `A1` / `B3` / `D4`, erased `A2` absence, source-backed `A1` / `B1` values, explicit blank `B3`, and inserted `D4`. | This is default regression evidence for `contains_cell()` no-op output readability only. It does not change coordinate validation, A1 parsing, diagnostic preservation semantics, missing-cell behavior, blank serialization, commit/close semantics, in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1379 | Public-state coverage now reopens the second no-op output from the `row_cells()` / `column_cells()` owning snapshot path. After row/column snapshots prove owning behavior, later edits update `A1`, preserve source-backed `B1` / `A2`, add explicit blank `C1`, edited `A3`, and outside `D4`, save once, and perform two byte-stable clean no-op saves, the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, dirty bounds, row-major `row_cells(1)`, column-major `column_cells(1)`, and outside `D4`. | This is default regression evidence for row/column snapshot no-op output readability only. It does not add dense row/column reads, missing-cell synthesis, borrowed snapshot views, streaming sparse iterators, row/column metadata repair, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1380 | Public-state coverage now reopens the second no-op output from the strict A1 range clear/erase mutation path. After `clear_cell_values("B1:C3")`, `erase_cells("A1:A2")`, and a missing-only erase save the sparse projection, two byte-stable clean no-op saves are followed by a fresh `WorkbookEditor` reopen of the second no-op output, verifying sparse count, bounds, blanked `B1` / `C3`, erased `A1` / `A2`, missing `B2`, and outside `D4` text. | This is default regression evidence for strict A1 range mutation no-op output readability only. It does not add dense range writes, missing-cell synthesis, tombstones, relaxed A1 parsing, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1381 | Public-state coverage now reopens the second no-op output from the single-cell `erase_cell()` dirty-flush path. After `erase_cell(2, 1)` removes source-backed `A2`, the saved shrunk projection and two byte-stable clean no-op saves are followed by a fresh `WorkbookEditor` reopen of the second no-op output, verifying sparse count, shrunk bounds, source-backed `A1` / `B1`, and erased `A2` absence. | This is default regression evidence for single-cell erase no-op output readability only. It does not add tombstones, missing-cell synthesis, dense deletion, worksheet metadata repair, source reload, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1382 | Public-state coverage now reopens the second no-op output from the range `erase_cells(CellRange)` first-flush path. After erasing all represented cells, saving the empty sparse projection, and emitting two byte-stable clean no-op saves before saved-session reacquire, the fresh reopen of `first_second_noop_output` verifies zero sparse records, no used range, and erased `A1` / `B1` / `A2` absence. | This is default regression evidence for range erase first-flush no-op output readability only. It does not add dense range semantics, tombstones, missing-cell synthesis, metadata/range repair, source reload, undo/redo, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1383 | Public-state coverage now reopens the no-op output from the range-erase saved-session reacquire path. After the first empty projection is reacquired cleanly, a later `C3` mutation is saved and a clean no-op output is byte-stable against that second save; the no-op output is now opened through a fresh `WorkbookEditor` to verify one sparse `C3` text cell, no `A1` / `B1` / `A2`, and `C3:C3` bounds. | This is default regression evidence for range-erase reacquire no-op output readability only. It does not add dense range semantics, source reload, undo/redo, commit/close semantics, in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1384 | Public-state coverage now reopens the second no-op output from the row/column overload invalid-coordinate recovery path. After rejected row/column reads and mutations leave the sparse state intact, a valid `A1` recovery edit saves once and two clean no-op saves remain byte-stable; the second no-op output is opened through a fresh `WorkbookEditor` to verify sparse count, `A1:B2` bounds, recovered `A1`, source-backed `B1` / `A2`, and missing rejected `C1`. | This is default regression evidence for row/column overload recovery no-op output readability only. It does not change coordinate validation, add dense row/column reads, synthesize missing cells, alter saved-session handoff semantics, add in-place overwrite, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1385 | Public-state coverage now reopens the second no-op output from the coordinate-batch `sparse_cells(span<WorksheetCellReference>)` owning snapshot path. After batch snapshots preserve caller order, duplicates, skipped missing coordinates, explicit blanks, and later edits, the saved sparse projection and two byte-stable clean no-op saves are followed by a fresh `WorkbookEditor` reopen verifying sparse count, `A1:D4` bounds, post-snapshot `A1`, source-backed `B1`, erased `A2`, explicit blank `B3`, and inserted `D4`. | This is default regression evidence for coordinate-batch sparse snapshot no-op output readability only. It does not add dense batch reads, missing-cell synthesis, duplicate coalescing, borrowed snapshot views, streaming sparse iterators, coordinate repair, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1386 | Public-state coverage now reopens the no-op output from the renamed full-calculation formula-audit saved-session invalid-mutation recovery path. After a saved/reacquired shifted formula session rejects invalid mutations, recovers with `C5`, saves the recovery output, and emits a byte-stable clean no-op output, the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted styled formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit saved-session no-op output readability only. It does not change rename semantics, formula translation, invalid-mutation behavior, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1387 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit materialized-state path. After `rename_sheet("Data", "RenamedData")`, `request_full_calculation()`, and a dirty shifted styled formula session save the first materialized output, the byte-stable no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted `D3` styled formula audit/readback, source references `Data!A2` / `Data!B2`, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit materialized no-op output readability only. It does not change rename semantics, formula translation, formula audit policy, formula evaluation, cached values, explicit formula rewrite policy, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1388 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit invalid diagnostic recovery path. After rejected invalid formula mutations leave the shifted styled `D3` formula state intact, a valid `C5` recovery edit saves the output and a byte-stable no-op save is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit invalid diagnostic recovery no-op output readability only. It does not change rename semantics, formula translation, invalid-mutation diagnostics, formula audit policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1389 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit saved-reacquire invalid-read recovery path. After invalid read attempts leave the saved planned-name session clean, a valid `C5` recovery edit re-dirties the shared session, saves the recovered output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit saved-reacquire invalid-read recovery no-op output readability only. It does not change read validation, saved-session reacquire policy, rename semantics, formula translation, formula audit policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1390 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit saved-reacquire invalid-shift recovery path. After rejected row/column shift preflights leave the saved planned-name session clean except for diagnostics, a valid `C5` recovery edit clears diagnostics, saves the recovered output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit saved-reacquire invalid-shift recovery no-op output readability only. It does not change shift validation, saved-session reacquire policy, rename semantics, formula translation, formula audit policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1391 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit saved-reacquire missing-query recovery path. After missing and old-source sheet lookups leave the saved planned-name session clean, a valid `C5` recovery edit re-dirties the shared session, saves the recovered output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit saved-reacquire missing-query recovery no-op output readability only. It does not change sheet lookup policy, saved-session reacquire policy, rename semantics, source-name fallback behavior, formula translation, formula audit policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1392 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit saved-reacquire option-mismatch recovery path. After mismatched `WorksheetEditorOptions` lookups leave the saved planned-name session clean, a valid `C5` recovery edit re-dirties the shared session, saves the recovered output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit saved-reacquire option-mismatch recovery no-op output readability only. It does not change option-matching policy, saved-session reacquire policy, rename semantics, formula translation, formula audit policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1393 | Public-state coverage now reopens the clean no-op output from the renamed full-calculation formula-audit saved-reacquire same-sheet guard recovery path. After rejected same-sheet rename lookups leave the saved planned-name session clean, a valid `C5` recovery edit re-dirties the shared session, saves the recovered output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted formula audit/readback, recovered `C5` text, `fullCalcOnLoad`, and absent calcChain. | This is default regression evidence for renamed formula-audit saved-reacquire same-sheet guard recovery no-op output readability only. It does not change same-sheet guard policy, saved-session reacquire policy, rename semantics, formula translation, formula audit policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1394 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula saved-reacquire shift path. After `rename_sheet("Data", "RenamedData")`, a saved/reacquired shared session applies row and column shifts, saves the translated styled `E4` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula saved-reacquire shift no-op output readability only. It does not change rename semantics, row/column shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1395 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula failed-save retry path. After an exact source-overwrite `save_as()` rejection preserves the dirty renamed styled-formula shift session, a safe retry saves the translated styled `D4` formula output and a byte-stable no-op save follows; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, row-shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula failed-save retry no-op output readability only. It does not change failed-save policy, source-overwrite protection, rename semantics, row shift semantics, formula translation, style preservation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1396 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula option-mismatch path. After mismatched `WorksheetEditorOptions` access is rejected without dirtying the saved renamed session, a matching reacquire applies a later column shift, saves the translated styled `E4` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula option-mismatch no-op output readability only. It does not change option-matching policy, rename semantics, row/column shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1397 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula invalid-mutation recovery path. After invalid cell mutation attempts are rejected without dirtying the saved renamed session, a valid later column shift clears diagnostics, saves the translated styled `E4` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula invalid-mutation recovery no-op output readability only. It does not change mutation validation, diagnostic policy, rename semantics, row/column shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1398 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula missing-query recovery path. After missing and old-source sheet lookups are rejected without dirtying the saved renamed session, a matching reacquire applies a later column shift, saves the translated styled `E4` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula missing-query recovery no-op output readability only. It does not change sheet lookup policy, source-name fallback behavior, rename semantics, row/column shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1399 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula invalid-read recovery path. After invalid scalar, A1, range, batch, row/column, and valid-missing reads are rejected without dirtying the saved renamed session, a later column shift saves the translated styled `E4` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula invalid-read recovery no-op output readability only. It does not change read validation, diagnostic policy, rename semantics, row/column shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1400 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula snapshot-read path. After full/range/row/column/batch snapshot reads remain owning and read-only, a later column shift saves the translated styled `E4` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, clean diagnostics, shifted sparse bounds, snapshot row/column readback, translated styled formula, and absent old coordinates. | This is default regression evidence for renamed styled-formula snapshot-read no-op output readability only. It does not change snapshot lifetime, read validation, diagnostic policy, rename semantics, row/column shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1401 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column failed-save retry path. After exact source-overwrite rejection preserves the dirty delete-column session, the safe retry saves the translated styled `C2` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify the renamed sheet, clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old/deleted coordinates. | This is default regression evidence for renamed styled-formula delete-column failed-save retry no-op output readability only. It does not change failed-save policy, source-overwrite protection, rename semantics, delete-column semantics, formula translation, style preservation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1402 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column option-mismatch path. After mismatched options are rejected without dirtying the saved delete-column session, a matching reacquire applies a later row shift, saves the translated styled `C3` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-column option-mismatch no-op output readability only. It does not change option-matching policy, rename semantics, delete-column or row-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1403 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column invalid-mutation recovery path. After invalid mutations are rejected without dirtying the saved delete-column session, a later valid row shift clears diagnostics, saves the translated styled `C3` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-column invalid-mutation recovery no-op output readability only. It does not change mutation validation, diagnostic policy, rename semantics, delete-column or row-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1404 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column missing-query recovery path. After missing and old-source sheet lookups are rejected without dirtying the saved delete-column session, a matching reacquire applies a later row shift, saves the translated styled `C3` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-column missing-query recovery no-op output readability only. It does not change sheet lookup policy, source-name fallback behavior, rename semantics, delete-column or row-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1405 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column invalid-read path. After invalid scalar, A1, range, batch, row/column, and valid-missing reads are rejected without dirtying the saved delete-column session, a later row shift saves the translated styled `C3` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-column invalid-read no-op output readability only. It does not change read validation, diagnostic policy, rename semantics, delete-column or row-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1406 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column snapshot-read path. After full/range/row/column/batch snapshot reads remain owning and read-only, a later row shift saves the translated styled `C3` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, row/column snapshot readback, translated styled formula, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-column snapshot-read no-op output readability only. It does not change snapshot lifetime, read validation, diagnostic policy, rename semantics, delete-column or row-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1407 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-column reacquire path. After a matching planned-name reacquire reuses the clean saved delete-column session, a later row shift saves the translated styled `C3` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-column reacquire no-op output readability only. It does not change saved-session lookup, rename semantics, delete-column or row-shift semantics, formula translation, style preservation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1408 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row failed-save retry path. After exact source-overwrite rejection preserves the dirty delete-row session, the safe retry saves the translated styled `D1` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed rows, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row failed-save retry no-op output readability only. It does not change failed-save policy, source-overwrite protection, rename semantics, delete-row semantics, formula translation, style preservation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1409 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row option-mismatch path. After mismatched options are rejected without dirtying the saved delete-row session, a matching reacquire applies a later column shift, saves the translated styled `E1` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row option-mismatch no-op output readability only. It does not change option-matching policy, rename semantics, delete-row or column-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1410 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row invalid-mutation recovery path. After invalid mutations are rejected without dirtying the saved delete-row session, a later valid column shift clears diagnostics, saves the translated styled `E1` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row invalid-mutation recovery no-op output readability only. It does not change mutation validation, diagnostic policy, rename semantics, delete-row or column-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1411 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row missing-query recovery path. After missing and old-source sheet lookups are rejected without dirtying the saved delete-row session, a matching reacquire applies a later column shift, saves the translated styled `E1` formula output, and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row missing-query recovery no-op output readability only. It does not change sheet lookup policy, source-name fallback behavior, rename semantics, delete-row or column-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1412 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row invalid-read path. After invalid scalar, A1, range, batch, row/column, and valid-missing reads are rejected without dirtying the saved delete-row session, a later column shift saves the translated styled `E1` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row invalid-read no-op output readability only. It does not change read validation, diagnostic policy, rename semantics, delete-row or column-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1413 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row snapshot-read path. After full/range/row/column/batch snapshot reads remain owning and read-only, a later column shift saves the translated styled `E1` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, row/column snapshot readback, translated styled formula, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row snapshot-read no-op output readability only. It does not change snapshot lifetime, read validation, diagnostic policy, rename semantics, delete-row or column-shift semantics, formula translation, style preservation, saved-session reacquire policy, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1414 | Public-state coverage now reopens the clean no-op output from the renamed styled-formula delete-row reacquire path. After a matching planned-name reacquire reuses the clean saved delete-row session, a later column shift saves the translated styled `E1` formula output and emits a byte-stable no-op save; the no-op output is opened through a fresh `WorkbookEditor` to verify clean diagnostics, shifted sparse bounds, translated styled formula readback, shifted source-backed cells, and absent old coordinates. | This is default regression evidence for renamed styled-formula delete-row reacquire no-op output readability only. It does not change saved-session lookup, rename semantics, delete-row or column-shift semantics, formula translation, style preservation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1415 | Public-state coverage now reopens the clean no-op output from the non-styled renamed planned-session row/column-shift paths: reacquire reuse, failed-save retry, option mismatch, missing query, invalid reads, and invalid mutations. Each path saves the combined shifted `RenamedData` output, emits a byte-stable no-op save, and opens that no-op output through a fresh `WorkbookEditor` to verify clean diagnostics, sparse count `3`, bounds `A1:C3`, shifted `C1`/`A3` values, and absent old `B1`/`A2` coordinates. | This is default regression evidence for non-styled renamed planned-session no-op output readability only. It does not change rename semantics, row/column shift semantics, failed-save policy, option matching, sheet lookup policy, read or mutation validation, saved-session reacquire policy, formula translation, style preservation, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1416 | Public-state coverage now fresh-reopens the prior clean no-op outputs after later third-stage saves in the single-sheet and multi-sheet materialized reopen/modify/no-op loops. These checks prove the earlier byte-stable no-op files remain readable after subsequent edits and saves, covering `Data` and `Untouched` sheets in both workflows. | This is default regression evidence for old no-op output readability after later saves only. It does not change save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1417 | Public-state coverage now continues the shifted saved/reacquired no-op path with a later materialized `C3` edit. The regression proves the clean no-op save leaves the shifted session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one shifted materialized session only. It does not change save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1418 | Public-state coverage now continues the delete-column saved/reacquired no-op path with a later materialized `D2` edit. The regression proves the clean no-op save leaves the delete-column shifted and formula-translated session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A1`, translated `B1`, shifted `C2`, and new `D2` values while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one delete-column materialized session only. It does not change save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, delete-column semantics, formula translation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1419 | Public-state coverage now continues the delete-row saved/reacquired no-op path with a later materialized `D3` edit. The regression proves the clean no-op save leaves the delete-row shifted and formula-translated session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A1`, shifted `B3`, translated `C3`, and new `D3` values while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one delete-row materialized session only. It does not change save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, delete-row semantics, formula translation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1420 | Public-state coverage now continues the insert-column saved/reacquired no-op path with a later materialized `F3` edit. The regression proves the clean no-op save leaves the insert-column shifted and formula-translated session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with source `A1`, shifted `D1`, translated `E2`, shifted `E3`, and new `F3` values while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one insert-column materialized session only. It does not change save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, insert-column semantics, formula translation, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1421 | Public-state coverage now continues the `try_worksheet()` saved/reacquired no-op path with a later materialized `C3` edit. The regression proves the clean no-op save leaves the row-shifted optional reacquired session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one optional-reacquired materialized session only. It does not change save behavior, `try_worksheet()` lookup policy, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1422 | Public-state coverage now continues the saved-session option-mismatch no-op path with a later matching reacquire and materialized `C3` edit. The regression proves rejected mismatched options and the clean no-op save leave the row-shifted session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one option-mismatch materialized session only. It does not change save behavior, option matching policy, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1423 | Public-state coverage now continues the saved-session missing-query no-op path with a later matching reacquire and materialized `C3` edit. The regression proves rejected missing-sheet lookups and the clean no-op save leave the row-shifted session reusable, the post-noop edit re-dirties the shared handles with materialized names/count/memory and summary diagnostics aligned to the active `Data` session, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while the earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics on one missing-query materialized session only. It does not change save behavior, missing-sheet lookup policy, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, cached values, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1424 | Public-state coverage now continues the saved-session invalid-read no-op path with a later materialized `C3` edit. The regression proves rejected invalid scalar/A1/range/batch/row/column and valid-missing reads plus a clean no-op save leave the row-shifted session reusable, the post-noop edit re-dirties both shared handles with materialized diagnostics aligned to `Data`, the next save advances the retained handoff count, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after invalid read attempts only. It does not change read validation, diagnostic policy, save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1425 | Public-state coverage now continues the saved-session invalid-mutation no-op path with a later materialized `C3` edit. The regression proves rejected invalid set/erase attempts keep the row-shifted session clean, the no-op save preserves the invalid mutation diagnostic, the later valid post-noop edit clears that diagnostic while re-dirtying both shared handles with aligned materialized summaries, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while earlier first/no-op outputs and rejected payload absence remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after invalid mutation attempts only. It does not change mutation validation, diagnostic clearing policy, save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1426 | Public-state coverage now continues the saved-session invalid-shift no-op path with a later materialized `C3` edit. The regression proves rejected invalid insert/delete row/column shifts keep the row-shifted session clean, the no-op save preserves the invalid shift diagnostic, the later valid post-noop edit clears that diagnostic while re-dirtying both shared handles with aligned materialized summaries, and the new output fresh-reopens with shifted `A3` plus the new `C3` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after invalid shift attempts only. It does not change shift validation, diagnostic clearing policy, save behavior, staged handoff retention, commit/close semantics, overwrite policy, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1427 | Public-state coverage now continues the failed-save retry no-op path with a later materialized `C3` edit. The regression proves a rejected source-overwrite `save_as()` preserves the dirty shifted session and source bytes, the safe retry plus clean no-op save leave the saved session reusable, the post-noop edit re-dirties both shared handles while retaining the prior two staged handoffs, and the next save advances the handoff count to three and fresh-reopens with shifted `C1`, shifted `A3`, and the new `C3` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after failed-save retry only. It does not change overwrite policy, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1428 | Public-state coverage now continues the path-equivalent failed-save retry no-op path with a later materialized `C3` edit. The regression proves a rejected path-equivalent source-overwrite `save_as()` preserves the dirty shifted session and source bytes, the safe retry plus clean no-op save leave the reacquired saved session reusable, the post-noop edit re-dirties all shared handles while retaining the prior two staged handoffs, and the next save advances the handoff count to three and fresh-reopens with shifted `C1`, shifted `A3`, and the new `C3` value while source, first, and prior no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after path-equivalent failed-save retry only. It does not change path equivalence policy, overwrite policy, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1429 | Public-state coverage now continues the empty-output failed-save retry no-op path with a later materialized `C3` edit. The regression proves a rejected empty-path `save_as()` preserves the dirty shifted session, source bytes, and absence of a rejected output, the safe retry plus clean no-op save leave the reacquired saved session reusable, the post-noop edit re-dirties all shared handles while retaining the prior two staged handoffs, and the next save advances the handoff count to three and fresh-reopens with shifted `C1`, shifted `A3`, and the new `C3` value while source, first, and prior no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after empty-output failed-save retry only. It does not change empty-path validation, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1430 | Public-state coverage now continues the missing-parent failed-save retry no-op path with a later materialized `C3` edit. The regression proves a rejected output path whose parent directory is missing preserves the dirty shifted session, source bytes, and absence of the rejected output, the safe retry plus clean no-op save leave the reacquired saved session reusable, the post-noop edit re-dirties all shared handles while retaining the prior two staged handoffs, and the next save advances the handoff count to three and fresh-reopens with shifted `C1`, shifted `A3`, and the new `C3` value while source, first, and prior no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after missing-parent failed-save retry only. It does not change filesystem validation, directory creation policy, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1431 | Public-state coverage now continues the non-directory-parent failed-save retry no-op path with a later materialized `C3` edit. The regression proves a rejected output path under a regular-file parent preserves the dirty shifted session, source bytes, and parent file contents, the safe retry plus clean no-op save leave the reacquired saved session reusable, the post-noop edit re-dirties all shared handles while retaining the prior two staged handoffs, and the next save advances the handoff count to three and fresh-reopens with shifted `C1`, shifted `A3`, and the new `C3` value while source, first, and prior no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after non-directory-parent failed-save retry only. It does not change filesystem validation, parent-file handling, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1432 | Public-state coverage now continues the existing-directory failed-save retry no-op path with a later materialized `C3` edit. The regression proves a rejected output path that is already a directory preserves the dirty shifted session, source bytes, and rejected directory, the safe retry plus clean no-op save leave the reacquired saved session reusable, the post-noop edit re-dirties all shared handles while retaining the prior two staged handoffs, and the next save advances the handoff count to three and fresh-reopens with shifted `C1`, shifted `A3`, and the new `C3` value while source, first, and prior no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after existing-directory failed-save retry only. It does not change filesystem validation, directory-as-output handling, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1433 | Public-state coverage now continues the clean invalid-to-valid row-shift recovery no-op path with a later materialized `C3` edit. The regression proves rejected invalid row shifts leave the session clean, the later valid row insertion clears diagnostics and saves a shifted output, the clean no-op save leaves that saved session reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, and the next save advances the handoff count to two and fresh-reopens with shifted `A3` plus the new `C3` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after clean invalid-to-valid row-shift recovery only. It does not change shift validation, diagnostic clearing policy, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1434 | Public-state coverage now continues the clean invalid-to-valid column-shift recovery no-op path with a later materialized `D2` edit. The regression proves rejected invalid column shifts leave the session clean, the later valid column insertion clears diagnostics and saves a shifted output, the clean no-op save leaves that saved session reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, and the next save advances the handoff count to two and fresh-reopens with shifted `C1`, source `A2`, and the new `D2` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after clean invalid-to-valid column-shift recovery only. It does not change shift validation, diagnostic clearing policy, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1435 | Public-state coverage now continues the dirty invalid-to-valid row-shift recovery no-op path with a later materialized `C3` edit. The regression proves rejected invalid row shifts preserve an already dirty materialized session, the later valid row insertion clears diagnostics while shifting both source-backed and dirty cells, the clean no-op save leaves that saved session reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, and the next save advances the handoff count to two and fresh-reopens with shifted `A3`, shifted dirty `B5`, and the new `C3` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after dirty invalid-to-valid row-shift recovery only. It does not change shift validation, diagnostic clearing policy, dirty-session preservation, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1436 | Public-state coverage now continues the dirty invalid-to-valid column-shift recovery no-op path with a later materialized `F2` edit. The regression proves rejected invalid column shifts preserve an already dirty materialized session, the later valid column insertion clears diagnostics while shifting both source-backed and dirty cells, the clean no-op save leaves that saved session reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, and the next save advances the handoff count to two and fresh-reopens with shifted `C1`, shifted dirty `E2`, and the new `F2` value while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop usability and diagnostics after dirty invalid-to-valid column-shift recovery only. It does not change shift validation, diagnostic clearing policy, dirty-session preservation, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1437 | Public-state coverage now continues the row-shift cross-handle no-op path with later materialized edits on both `Data` and `Untouched`. The regression proves the clean no-op save leaves both saved materialized handles reusable, post-noop edits re-dirty both sheets in catalog order while retaining the two prior staged handoffs, the next save advances the handoff count to four, and fresh reopen reads shifted `Data` source/dirty cells plus new `C5` and preserved `Untouched` source/dirty cells plus new `C3` while earlier first/no-op outputs remain unchanged. | This is default regression evidence for same-editor cross-handle reuse after row-shift save/no-op only. It does not change cross-sheet dependency synchronization, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1438 | Public-state coverage now continues the column-shift cross-handle no-op path with later materialized edits on both `Data` and `Untouched`. The regression proves the clean no-op save leaves both saved materialized handles reusable, post-noop edits re-dirty both sheets in catalog order while retaining the two prior staged handoffs, the next save advances the handoff count to four, and fresh reopen reads shifted `Data` source/dirty cells plus new `F2` and preserved `Untouched` source/dirty cells plus new `D2` while earlier first/no-op outputs remain unchanged. | This is default regression evidence for same-editor cross-handle reuse after column-shift save/no-op only. It does not change cross-sheet dependency synchronization, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1439 | Public-state coverage now continues the delete-rows cross-handle no-op path with later materialized edits on both `Data` and `Untouched`. The regression proves the clean no-op save leaves both saved materialized handles reusable after source rows were removed from `Data`, post-noop edits re-dirty both sheets in catalog order while retaining the two prior staged handoffs, the next save advances the handoff count to four, and fresh reopen reads shifted `Data` survivors plus new `C3` and preserved `Untouched` source/dirty cells plus new `C3` while earlier first/no-op outputs remain unchanged. | This is default regression evidence for same-editor cross-handle reuse after delete-rows save/no-op only. It does not change cross-sheet dependency synchronization, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1440 | Public-state coverage now continues the delete-columns cross-handle no-op path with later materialized edits on both `Data` and `Untouched`. The regression proves the clean no-op save leaves both saved materialized handles reusable after source columns were removed from `Data`, post-noop edits re-dirty both sheets in catalog order while retaining the two prior staged handoffs, the next save advances the handoff count to four, and fresh reopen reads shifted `Data` survivors plus new `D1` and preserved `Untouched` source/dirty cells plus new `D1` while earlier first/no-op outputs remain unchanged. | This is default regression evidence for same-editor cross-handle reuse after delete-columns save/no-op only. It does not change cross-sheet dependency synchronization, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, formula evaluation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1441 | Public-state coverage now continues the `insert_rows()` rich formula reference-shape no-op path with a later materialized formula edit at `D3`. The regression proves the clean no-op save leaves the saved formula sheet reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, the translated `C3` formula remains unchanged, the next save advances the handoff count to two, and fresh reopen reads both the translated rich formula and the new `D3` formula while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop formula-sheet usability after row-shift formula translation only. It does not change formula translation, formula evaluation, cached value preservation, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1442 | Public-state coverage now continues the `insert_columns()` rich formula reference-shape no-op path with a later materialized formula edit at `F2`. The regression proves the clean no-op save leaves the saved formula sheet reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, the translated `E2` formula remains unchanged, the next save advances the handoff count to two, and fresh reopen reads both the translated rich formula and the new `F2` formula while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop formula-sheet usability after column-shift formula translation only. It does not change formula translation, formula evaluation, cached value preservation, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1443 | Public-state coverage now continues the `delete_rows()` `#REF!` formula no-op path with a later materialized formula edit at `D3`. The regression proves the clean no-op save leaves the saved formula sheet reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, the translated `C3` `#REF!` formula remains unchanged, the next save advances the handoff count to two, and fresh reopen reads both the translated `#REF!` formula and the new `D3` formula while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop formula-sheet usability after row-deletion `#REF!` formula translation only. It does not change formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1444 | Public-state coverage now continues the `delete_columns()` `#REF!` formula no-op path with a later materialized formula edit at `D1`. The regression proves the clean no-op save leaves the saved formula sheet reusable, the post-noop edit re-dirties the materialized worksheet while retaining the prior staged handoff, the translated `C1` `#REF!` formula remains unchanged, the next save advances the handoff count to two, and fresh reopen reads both the translated `#REF!` formula and the new `D1` formula while earlier first/no-op outputs remain unchanged. | This is default regression evidence for post-noop formula-sheet usability after column-deletion `#REF!` formula translation only. It does not change formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1445 | Public-state coverage now adds a `delete_rows()` rich formula reference-shape path with initial save, no-op byte-stability, and a later materialized formula edit at `D3`. The regression proves moved formulas preserve the narrow translator behavior for relative references, absolute anchors, ranges, sheet/external qualifiers, whole-column/whole-row references, quoted strings, structured-reference skips, function-name skips, and `#REF!` emissions after a row deletion; the clean no-op save leaves the saved formula sheet reusable, and the post-noop save keeps earlier outputs unchanged. | This is default regression evidence for row-deletion rich formula-shape save/no-op/post-noop usability only. It does not change formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1446 | Public-state coverage now adds the matching `delete_columns()` rich formula reference-shape path with initial save, no-op byte-stability, and a later materialized formula edit at `D2`. The regression proves moved formulas preserve the narrow translator behavior for relative references, absolute anchors, ranges, sheet/external qualifiers, whole-column/whole-row references, quoted strings, structured-reference skips, function-name skips, and `#REF!` emissions after a column deletion; the clean no-op save leaves the saved formula sheet reusable, and the post-noop save keeps earlier outputs unchanged. | This is default regression evidence for column-deletion rich formula-shape save/no-op/post-noop usability only. It does not change formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1447 | Public-state coverage now runs a second clean no-op `save_as()` after the delete-side rich formula post-noop edits. The regression proves the row-deletion `C3`/`D3` and column-deletion `C2`/`D2` formula outputs keep clean materialized diagnostics, preserve the retained handoff count, emit byte-stable package entries, leave the prior post-noop outputs unchanged, and fresh-reopen with the translated formula, later formula edit, shifted source cell, and absent old coordinate still intact. | This is default regression evidence for repeated clean no-op save readability after delete-side rich formula post-noop edits only. It does not change formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1448 | Public-state coverage now runs the same second clean no-op `save_as()` after the delete-side `#REF!` formula post-noop edits. The regression proves row-deletion `C3`/`D3` and column-deletion `C1`/`D1` formula outputs keep clean materialized diagnostics, preserve the retained handoff count, emit byte-stable package entries, leave the prior post-noop outputs unchanged, and fresh-reopen with translated `#REF!` formulas, later formula edits, shifted source cells, and absent old coordinates still intact. | This is default regression evidence for repeated clean no-op save readability after delete-side `#REF!` formula post-noop edits only. It does not change formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1449 | Public-state coverage now runs the matching second clean no-op `save_as()` after the insert-side rich formula post-noop edits. The regression proves the row-insertion `C3`/`D3` and column-insertion `E2`/`F2` formula outputs keep clean materialized diagnostics, preserve the retained handoff count, emit byte-stable package entries, leave the prior post-noop outputs unchanged, and fresh-reopen with the translated formula, later formula edit, shifted source cell, and absent old coordinate still intact. | This is default regression evidence for repeated clean no-op save readability after insert-side rich formula post-noop edits only. It does not change formula translation, formula evaluation, cached value preservation, insert semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1450 | Public-state coverage now runs a second clean no-op `save_as()` after the row-shift cross-handle post-noop edits. The regression proves both saved `Data` and `Untouched` materialized handles remain clean, the retained handoff count stays stable, dirty diagnostics stay clear, package entries are byte-stable, the prior post-noop output is unchanged, and fresh reopen reads both sheets' post-noop edits with shifted Data coordinates and untouched-sheet coordinates intact. | This is default regression evidence for repeated clean no-op save readability after row-shift cross-handle post-noop edits only. It does not change multi-handle save behavior, insert semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1451 | Public-state coverage now runs a second clean no-op `save_as()` after the column-shift cross-handle post-noop edits. The regression proves both saved `Data` and `Untouched` materialized handles remain clean, the retained handoff count stays stable, dirty diagnostics stay clear, package entries are byte-stable, the prior post-noop output is unchanged, and fresh reopen reads both sheets' post-noop edits with shifted Data columns and untouched-sheet coordinates intact. | This is default regression evidence for repeated clean no-op save readability after column-shift cross-handle post-noop edits only. It does not change multi-handle save behavior, insert semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1452 | Public-state coverage now runs a second clean no-op `save_as()` after the delete-rows cross-handle post-noop edits. The regression proves both saved `Data` and `Untouched` materialized handles remain clean, the retained handoff count stays stable, dirty diagnostics stay clear, package entries are byte-stable, the prior post-noop output is unchanged, and fresh reopen reads both sheets' post-noop edits with deleted-row Data coordinates and untouched-sheet coordinates intact. | This is default regression evidence for repeated clean no-op save readability after delete-rows cross-handle post-noop edits only. It does not change multi-handle save behavior, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1453 | Public-state coverage now runs a second clean no-op `save_as()` after the delete-columns cross-handle post-noop edits. The regression proves both saved `Data` and `Untouched` materialized handles remain clean, the retained handoff count stays stable, dirty diagnostics stay clear, package entries are byte-stable, the prior post-noop output is unchanged, and fresh reopen reads both sheets' post-noop edits with deleted-column Data coordinates and untouched-sheet coordinates intact. | This is default regression evidence for repeated clean no-op save readability after delete-columns cross-handle post-noop edits only. It does not change multi-handle save behavior, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, row/column shift semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1454 | Public-state coverage now continues the `delete_rows()` styled source formula no-op path with a later materialized `E2` text edit and a second clean no-op save. The regression proves the shifted source-backed formula keeps its non-default `StyleId` and translated `#REF!+#REF!` formula text after the clean no-op, the later edit re-dirties only the saved materialized sheet while retaining the prior handoff, the post-noop save advances the handoff count, earlier outputs stay unchanged, and the final no-op output remains byte-stable and fresh-reopenable. | This is default regression evidence for saved-session usability after row-deletion styled source formula shifts only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1455 | Public-state coverage now continues the `delete_columns()` styled source formula no-op path with a later materialized `D1` text edit and a second clean no-op save. The regression proves the shifted source-backed formula keeps its non-default `StyleId` and translated `#REF!+A1` formula text after the clean no-op, the later edit re-dirties only the saved materialized sheet while retaining the prior handoff, the post-noop save advances the handoff count, earlier outputs stay unchanged, and the final no-op output remains byte-stable and fresh-reopenable. | This is default regression evidence for saved-session usability after column-deletion styled source formula shifts only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, delete semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1456 | Public-state coverage now continues the `insert_rows()` styled source formula no-op path with a later materialized `E5` text edit and a second clean no-op save. The regression proves the shifted source-backed formula keeps its non-default `StyleId` and translated `A3+B3` formula text after the clean no-op, the later edit re-dirties only the saved materialized sheet while retaining the prior handoff, the post-noop save advances the handoff count, earlier outputs stay unchanged, and the final no-op output remains byte-stable and fresh-reopenable. | This is default regression evidence for saved-session usability after row-insertion styled source formula shifts only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, insert semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1457 | Public-state coverage now adds the matching `insert_columns()` styled source formula no-op path with a later materialized `G2` text edit and a second clean no-op save. The regression proves the source-backed styled formula shifts to `F2`, keeps its non-default `StyleId`, translates to `C1+D1`, remains reusable after a clean no-op save, and fresh-reopens with the post-noop edit while earlier outputs stay unchanged and the final no-op output remains byte-stable. | This is default regression evidence for saved-session usability after column-insertion styled source formula shifts only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, insert semantics, save behavior, staged handoff retention, commit/close semantics, rollback, transaction replay, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1458 | Public-state coverage now pins the dirty `insert_columns()` styled source formula path when workbook full-calculation metadata is queued after the materialized shift. The regression proves the shifted source-backed formula reaches `F2`, keeps its non-default `StyleId`, translates to `C1+D1`, keeps aggregate materialized diagnostics stable across `request_full_calculation()`, saves with `fullCalcOnLoad="1"` and no invented `xl/calcChain.xml`, and then performs a byte-stable clean no-op save. | This is default regression evidence for full-calculation metadata layered on a column-insertion styled formula shift only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, insert semantics, full-calculation semantics, save behavior, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1459 | Public-state coverage now pins the reverse ordering for row-insertion styled source formulas: `request_full_calculation()` is queued before `worksheet()` materialization, and a later `insert_rows()` shift still keeps the source-backed formula at `D4` with non-default `StyleId` and translated `A3+B3`. The regression proves clean materialization contributes no dirty materialized diagnostics, the later row shift reports aligned dirty diagnostics, and `save_as()` writes `fullCalcOnLoad="1"` without inventing `xl/calcChain.xml`. | This is default structural regression evidence for full-calculation metadata queued before a row-insertion styled formula shift only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, insert semantics, full-calculation semantics, save behavior, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1460 | Public-state coverage now pins workbook full-calculation metadata queued after a delete-column styled source formula shift. The regression first materializes `Data`, runs `delete_columns(1, 1)`, verifies the source-backed styled formula moves to `C2` with non-default `StyleId` and translated `#REF!+A1`, then queues `request_full_calculation()` and saves with `fullCalcOnLoad="1"`, shifted `A1:C2` cells, and no invented `xl/calcChain.xml`. | This is default structural regression evidence for full-calculation metadata layered on an existing column-deletion styled formula shift only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, delete semantics, full-calculation semantics, save behavior, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1461 | Public-state coverage now pins the reverse ordering for row-deletion styled source formulas: `request_full_calculation()` is queued before `worksheet()` materialization, and a later `delete_rows(1, 1)` shift still keeps the source-backed formula at `D1` with non-default `StyleId` and translated `#REF!+#REF!`. The regression proves clean materialization contributes no dirty materialized diagnostics, the later row delete reports aligned dirty diagnostics, and `save_as()` writes `fullCalcOnLoad="1"` with shifted `A1:D2` cells and no invented `xl/calcChain.xml`. | This is default structural regression evidence for full-calculation metadata queued before a row-deletion styled formula shift only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, delete semantics, full-calculation semantics, save behavior, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1462 | Public-state coverage now pins the reverse ordering for column-insertion styled source formulas: `request_full_calculation()` is queued before `worksheet()` materialization, and a later `insert_columns(2, 2)` shift still keeps the source-backed formula at `F2` with non-default `StyleId` and translated `C1+D1`. The regression proves clean materialization contributes no dirty materialized diagnostics, the later column insert reports aligned dirty diagnostics, and `save_as()` writes `fullCalcOnLoad="1"` with shifted `A1:F3` cells and no invented `xl/calcChain.xml`. | This is default structural regression evidence for full-calculation metadata queued before a column-insertion styled formula shift only. It does not change style preservation, style migration/merge, formula translation, formula evaluation, cached value preservation, insert semantics, full-calculation semantics, save behavior, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1463 | Public-state coverage now pins failed-save retry hygiene for that reverse column-insertion styled source formula path. The regression queues `request_full_calculation()` before materializing `Data`, runs `insert_columns(2, 2)`, verifies the shifted styled formula remains at `F2` as `C1+D1`, rejects exact source overwrite, and proves the dirty materialized diagnostics, queued full-calculation metadata, in-memory shifted cells, and source package bytes remain unchanged before a safe retry saves `fullCalcOnLoad="1"`, shifted `A1:F3` cells, and no invented `xl/calcChain.xml`. | This is default retry-state regression evidence for full-calculation metadata plus one styled column-insertion materialized session only. It does not add in-place overwrite, rollback transactions, production save behavior changes, style preservation semantic changes, style migration/merge, formula translation changes, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1464 | Public-state coverage now mirrors that failed-save retry hygiene for the reverse row-deletion styled source formula path. The regression queues `request_full_calculation()` before materializing `Data`, runs `delete_rows(1, 1)`, verifies the shifted styled formula remains at `D1` as `#REF!+#REF!`, rejects exact source overwrite, and proves the dirty materialized diagnostics, queued full-calculation metadata, in-memory shifted rows, and source package bytes remain unchanged before a safe retry saves `fullCalcOnLoad="1"`, shifted `A1:D2` cells, and no invented `xl/calcChain.xml`. | This is default retry-state regression evidence for full-calculation metadata plus one styled row-deletion materialized session only. It does not add in-place overwrite, rollback transactions, production save behavior changes, style preservation semantic changes, style migration/merge, formula translation changes, formula evaluation, cached value preservation, delete semantics changes, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1465 | Public-state coverage now adds the matching failed-save retry hygiene for the reverse column-deletion styled source formula path. The regression queues `request_full_calculation()` before materializing `Data`, runs `delete_columns(1, 1)`, verifies the shifted styled formula remains at `C2` as `#REF!+A1`, rejects exact source overwrite, and proves the dirty materialized diagnostics, queued full-calculation metadata, in-memory shifted columns, and source package bytes remain unchanged before a safe retry saves `fullCalcOnLoad="1"`, shifted `A1:C2` cells, and no invented `xl/calcChain.xml`. | This is default retry-state regression evidence for full-calculation metadata plus one styled column-deletion materialized session only. It does not add in-place overwrite, rollback transactions, production save behavior changes, style preservation semantic changes, style migration/merge, formula translation changes, formula evaluation, cached value preservation, delete semantics changes, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1466 | Public-state coverage now completes the same failed-save retry hygiene for the reverse row-insertion styled source formula path. The regression queues `request_full_calculation()` before materializing `Data`, runs `insert_rows(2, 2)`, verifies the shifted styled formula remains at `D4` as `A3+B3`, rejects exact source overwrite, and proves the dirty materialized diagnostics, queued full-calculation metadata, in-memory shifted rows, and source package bytes remain unchanged before a safe retry saves `fullCalcOnLoad="1"`, shifted `A1:D5` cells, and no invented `xl/calcChain.xml`. | This is default retry-state regression evidence for full-calculation metadata plus one styled row-insertion materialized session only. It does not add in-place overwrite, rollback transactions, production save behavior changes, style preservation semantic changes, style migration/merge, formula translation changes, formula evaluation, cached value preservation, insert semantics changes, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1467 | Public-state coverage now pins failed-save retry hygiene for the after-shift column-insertion styled source formula path. The regression materializes `Data`, runs `insert_columns(2, 2)`, queues `request_full_calculation()`, verifies the shifted styled formula remains at `F2` as `C1+D1`, rejects exact source overwrite, and proves the dirty materialized diagnostics, queued full-calculation metadata, in-memory shifted cells, and source package bytes remain unchanged before a safe retry saves `fullCalcOnLoad="1"`, shifted `A1:F3` cells, and no invented `xl/calcChain.xml`. | This is default retry-state regression evidence for full-calculation metadata plus one styled column-insertion materialized session only. It does not add in-place overwrite, rollback transactions, production save behavior changes, style preservation semantic changes, style migration/merge, formula translation changes, formula evaluation, cached value preservation, insert semantics changes, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1282 | Public-state coverage now explicitly distinguishes retained staged Patch handoffs from dirty materialized sessions after successful `save_as()`. The multi-sheet retry and retry-reopen no-op paths assert `has_pending_changes()` remains true from the retained handoff state while `pending_materialized_worksheet_names()`, `pending_materialized_cell_count()`, `estimated_pending_materialized_memory_usage()`, and dirty summaries stay clean. | This is diagnostics contract coverage only. It does not change save/commit behavior, add in-place overwrite, rollback, transaction replay, production save behavior, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory QA addenda P8.1283 | The opt-in workbook-editor QA runner now also has `generated_in_memory_reopen_modify_noop_save`, which repeats the generated single-worksheet edit/save/reopen/edit/save path and then performs one more no-op `save_as()` requiring byte-identical output before ZIP/XML, `openpyxl`, optional XlsxWriter, and optional Excel COM validation of the final workbook. | This is generated QA evidence for single-sheet post-reopen no-op save stability only. It does not add production save behavior, commit/close semantics, in-place overwrite, rollback, transaction replay, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory QA addenda P8.1288 | The opt-in workbook-editor QA runner now also has `generated_in_memory_reopen_modify_post_noop_third_save`, which repeats the generated single-worksheet edit/save/reopen/edit/save/no-op path, edits `Data` again after that clean no-op save, saves a third-stage output, and requires the final no-op `save_as()` to be byte-identical before ZIP/XML, `openpyxl`, optional XlsxWriter reference, and optional Excel COM validation. | This is generated QA evidence for same-editor single-worksheet usability after a clean no-op save only. It does not add commit/close semantics, overwrite mode, rollback, transaction replay, new production save behavior, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, default CTest/CI expansion, or low-memory random editing. |
| In-memory public-state addenda P8.1284 | Public-state coverage now pins the single-sheet edit/save/reopen/edit/save/no-op loop by default. The regression writes a first materialized `Data` edit, saves an intermediate output, reopens it through a fresh `WorkbookEditor`, applies second-stage `Data` edits, saves again, then verifies the final no-op `save_as()` is byte-equivalent while the original source and intermediate output remain unchanged. | This is default regression evidence for existing small-file single-sheet materialized save/reopen hygiene only. It does not add production save behavior, commit/close semantics, in-place overwrite, rollback, transaction replay, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1301 | Public-state coverage now also verifies the single-worksheet retry/reopen/no-op/post-noop path rejects both exact and path-equivalent source-overwrite `save_as()` attempts before the first safe output. Both failures must keep the dirty `Data` handle and dirty diagnostics intact, preserve source package bytes, and still allow the existing safe retry, fresh reopen/edit, no-op save, post-noop third save, and final no-op checks to pass. | This is default regression evidence for exact/path-equivalent failed-save hygiene on the existing small-file single-worksheet materialized path only. It does not add overwrite mode, path repair, rollback, transaction replay, production save behavior, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1302 | The same single-worksheet rejected-save regression now pins the full dirty materialized diagnostics for both exact and path-equivalent source-overwrite failures: `pending_materialized_worksheet_names()`, `pending_materialized_cell_count()`, and `estimated_pending_materialized_memory_usage()` stay aligned with the dirty `Data` session until the later safe retry. | This is diagnostics contract coverage only. It does not change save behavior, add overwrite mode, introduce rollback/transaction replay, alter memory accounting, migrate styles/sharedStrings, repair metadata/relationships, rebuild calcChain, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory public-state addenda P8.1285 | The same single-sheet public-state regression now continues after the final no-op save: it reacquires `Data`, applies a third materialized edit, saves a third output, verifies earlier second/no-op outputs stay unchanged, and then requires a third no-op `save_as()` to be byte-equivalent to that third output. | This pins same-editor usability after a clean no-op save only. It does not add commit/close semantics, transaction replay, overwrite mode, rollback, formula evaluation, cached value preservation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory public-state addenda P8.1286 | The multi-sheet retry/reopen/no-op public-state regression now also continues after the clean no-op save: it edits both `Data` and `Untouched` again, verifies all shared handles dirty together, saves a third output while leaving the retry/second/no-op outputs unchanged, and then requires a third no-op `save_as()` to be byte-equivalent to that third output. | This is default regression evidence for same-editor multi-worksheet materialized usability after no-op save only. It does not add commit/close semantics, transaction replay, overwrite mode, rollback, formula evaluation, cached value preservation, cross-sheet dependency synchronization, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| Examples addenda P13.13 | The opt-in examples lane now includes `examples/workbook_editor_in_memory.cpp` and `fastxlsx_workbook_editor_in_memory_example`. The example seeds a small workbook with `Workbook`, opens it through `WorkbookEditor`, materializes `Data` with `WorksheetEditor`, performs scalar/formula cell updates plus row insertion, and writes a separate edited output with `save_as()`. | This is compileable public API example coverage only. It does not add new editing semantics, enable in-place source overwrite, repair metadata, rebuild calcChain, migrate sharedStrings/styles, repair relationships, make examples part of default CTest, or provide low-memory random editing. |
| In-memory shift addenda P8.831 | Public-state coverage now verifies `WorkbookEditor::formula_reference_audits()` observes the current materialized store after renamed row/column shifts. A moved formula such as `Data!A1+Data!B1` is audited at its shifted formula-cell coordinate with shifted sheet-qualified tokens (`Data!A3` / `Data!B3` after row insertion and `Data!B1` / `Data!C1` after column insertion), while the audit call preserves pending materialized diagnostics. | This is read-only audit evidence only. It does not rewrite formulas by default after `rename_sheet()`, scan non-materialized worksheets, repair workbook metadata, rebuild calcChain, migrate sharedStrings/styles, or implement a formula dependency graph. |
...
| In-memory guardrail addenda P8.1158 | Public-state coverage now also verifies empty-vector `set_row()` and `set_column()` clears of represented source-backed rows/columns remain stable across clean no-op saves after their materialized flushes. Both existing paths clear a represented row or column, save once, then snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, cleared cell absence, preserved non-target records, and reopened bounds remain unchanged. | This is public save-state coverage for existing row/column clear no-op saves only. It does not add row/column metadata synchronization, dense row/column editing, tombstones, transaction history, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1159 | Public-state coverage now also verifies `set_row_values()` and `set_column_values()` exact `max_cells` and exact `memory_budget_bytes` recovery paths remain stable across clean no-op saves after their recovery materialized flushes. The existing value-prefix paths reject over-budget payloads, recover with smaller in-budget prefix writes that preserve source-backed tails/non-target cells, save once, then snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, rejected payload absence, preserved tails/non-target cells, and recovery prefix cells remain unchanged. | This is public save-state coverage for existing value-prefix guardrail recovery no-op saves only. It does not add value-prefix transactions, dense row/column writes, guardrail auto-sizing, process-RSS accounting, row/column metadata synchronization, style migration, sharedStrings migration, formula repair, calcChain rebuild, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1160 | Public-state coverage now also verifies styled source-backed `set_row_values()` and `set_column_values()` value-only updates remain stable across clean no-op saves after their materialized flushes. Both existing styled-source paths preserve source non-default `StyleId`s on overwritten prefix cells, keep tail records, save once, then snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, preserved style ids, preserved tails, and reopened bounds remain unchanged. | This is public save-state coverage for existing styled value-prefix no-op saves only. It does not add caller-supplied non-default style writes, style migration/merge, styles.xml repair, sharedStrings migration, formula repair, metadata/range repair, calcChain rebuild, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1161 | Public-state coverage now verifies styled source-backed `clear_row()` blank projection remains stable across a clean no-op save after its materialized flush. The existing styled row-clear path preserves the source non-default `StyleId` on the blank `A1` record, keeps the unstyled `B1` blank unstyled, saves once, then snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, preserved blank style ids, unstyled blank preservation, and reopened bounds remain unchanged. | This is public save-state coverage for the existing styled `clear_row()` no-op save only. It does not add `clear_column()` styled-source coverage, row/column metadata synchronization, style migration/merge, styles.xml repair, sharedStrings migration, formula repair, metadata/range repair, calcChain rebuild, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1162 | Public-state coverage now verifies styled source-backed `clear_column()` blank projection remains stable across a clean no-op save after its materialized flush. The new styled column-clear path preserves the source non-default `StyleId` on the blank `A1` record, keeps the unstyled `A2` blank unstyled, preserves the non-target `B1` text cell, saves once, then snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, preserved blank style ids, unstyled blank preservation, non-target column preservation, and reopened bounds remain unchanged. | This is public save-state coverage for styled `clear_column()` only. It does not add styled `clear_columns()` range coverage, row/column metadata synchronization, style migration/merge, styles.xml repair, sharedStrings migration, formula repair, metadata/range repair, calcChain rebuild, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1163 | Public-state coverage now verifies styled source-backed `clear_columns()` range blank projection remains stable across a clean no-op save after its materialized flush. The new styled range-clear path clears columns A:B, preserves the source non-default `StyleId` on the blank `A1` record, keeps the unstyled `B1` blank unstyled, preserves the non-target `C1` text cell, saves once, then snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, preserved blank style ids, unstyled blank preservation, non-target column preservation, and reopened bounds remain unchanged. | This is public save-state coverage for styled `clear_columns()` only. It does not add broader range metadata synchronization, style migration/merge, styles.xml repair, sharedStrings migration, formula repair, metadata/range repair, calcChain rebuild, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1164 | Public-state coverage now verifies styled source-backed `clear_rows()` range blank projection remains stable across a clean no-op save after its materialized flush. The new styled row-range path clears rows 1:2, preserves the source non-default `StyleId` on the blank `A1` record, keeps unstyled target blanks in `B1` and `A2`, preserves the non-target `A3` text cell, saves once, then snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, preserved blank style ids, unstyled blank preservation, non-target row preservation, and reopened bounds remain unchanged. | This is public save-state coverage for styled `clear_rows()` only. It does not add broader row metadata synchronization, style migration/merge, styles.xml repair, sharedStrings migration, formula repair, metadata/range repair, calcChain rebuild, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1165 | Public-state coverage now verifies basic source-backed `clear_row()` and `clear_column()` blank projection remain stable across clean no-op saves after their materialized flushes. Both existing single row/column clear paths keep represented source-backed records as explicit blanks, preserve non-target records, save once, then snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, explicit blank preservation, non-target preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing single-row/single-column clear no-op saves only. It does not add row/column metadata synchronization, dense row/column editing, tombstones, transaction history, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory guardrail addenda P8.1166 | Public-state coverage now verifies basic source-backed `clear_rows()` and `clear_columns()` range blank projection remain stable across clean no-op saves after their materialized flushes. Both existing range clear paths keep represented source-backed target records as explicit blanks, preserve non-target edits outside the cleared row/column ranges, save once, then snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, explicit blank preservation, non-target preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing row-range/column-range clear no-op saves only. It does not add range metadata synchronization, dense row/column editing, tombstones, transaction history, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory large-file random editing. |
| In-memory shift addenda P8.1167 | Public-state coverage now verifies the basic source-backed `insert_rows()` and `delete_rows()` sparse row-shift saves remain stable across clean no-op saves after their materialized flushes. The existing row insert path shifts source-backed text/number/filler cells, a styled formula cell with translated references, and a dirty trailing cell; the row delete path removes deleted-row records, shifts later source/dirty/formula records upward, and translates moved formula references. Both now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, shifted/removed coordinate preservation, formula text/style preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing row-side sparse shift no-op saves only. It does not add dense row operations, row metadata synchronization, table/filter/range/drawing repair, formula evaluation or broad formula repair beyond moved-cell translation, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1168 | Public-state coverage now verifies the basic source-backed `insert_columns()` and `delete_columns()` sparse column-shift saves remain stable across clean no-op saves after their materialized flushes. The existing column insert path shifts source-backed numeric cells and dirty formula/text cells right with formula translation; the column delete path removes deleted-column records, shifts later source/dirty/formula records left, and translates moved formula references. Both now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, shifted/removed coordinate preservation, formula text preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing column-side sparse shift no-op saves only. It does not add dense column operations, column metadata synchronization, table/filter/range/drawing repair, formula evaluation or broad formula repair beyond moved-cell translation, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1169 | Public-state coverage now verifies delete-side styled source formula shifts remain stable across clean no-op saves after their materialized flushes. The existing `delete_rows()` styled formula path preserves the moved formula cell's non-default `StyleId` while translating deleted row references to `#REF!+#REF!`; the existing `delete_columns()` styled formula path preserves the moved formula style while translating deleted column references to `#REF!+A1`. Both now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, moved style/formula preservation, removed-coordinate absence, untouched worksheet preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing styled moved-formula delete shifts only. It does not add style migration/merge, styles.xml repair, formula evaluation or broad formula repair beyond moved-cell translation, metadata/range repair, calcChain rebuild, sharedStrings migration, relationship repair, Patch/materialized composition, or low-memory random editing. |
| In-memory operation-mixing addenda P8.1170 | Public-state coverage now verifies `request_full_calculation()` combined with row-side sparse shifts remains stable across clean no-op saves after successful materialized flushes. The existing full-calc `insert_rows()` path persists `fullCalcOnLoad`, omits invented `calcChain.xml`, projects shifted source/dirty trailing rows, and preserves translated styled formula output; the existing full-calc `delete_rows()` path persists `fullCalcOnLoad`, omits invented `calcChain.xml`, shifts surviving rows upward, and preserves the moved styled formula with deleted references translated to `#REF!+#REF!`. Both now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, fullCalcOnLoad persistence, shifted/removed coordinate preservation, formula text/style preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing fullCalcOnLoad metadata edit plus row-side sparse shift no-op saves only. It does not evaluate formulas, rebuild calcChain, add calcChain payload generation, perform metadata/range repair, migrate sharedStrings/styles, repair relationships, compose Patch and materialized edits beyond the existing full-calculation metadata helper, or add low-memory random editing. |
| In-memory operation-mixing addenda P8.1171 | Public-state coverage now verifies `request_full_calculation()` combined with column-side sparse shifts remains stable across clean no-op saves after successful materialized flushes. The existing full-calc-before-`insert_columns()` path persists `fullCalcOnLoad`, omits invented `calcChain.xml`, shifts source-backed numbers and dirty formula/text cells right, and preserves translated formula output; the existing full-calc-before-`delete_columns()` path persists `fullCalcOnLoad`, omits invented `calcChain.xml`, shifts surviving columns left, and preserves the moved styled formula with deleted column references translated to `#REF!+A1`. Both now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, fullCalcOnLoad persistence, shifted/removed coordinate preservation, formula text/style preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing fullCalcOnLoad metadata edit plus column-side sparse shift no-op saves only. It does not evaluate formulas, rebuild calcChain, add calcChain payload generation, perform metadata/range repair, migrate sharedStrings/styles, repair relationships, compose Patch and materialized edits beyond the existing full-calculation metadata helper, or add low-memory random editing. |
| In-memory operation-mixing addenda P8.1172 | Public-state coverage now verifies formula-audit queries combined with full-calculation row-shift saves remain stable across clean no-op saves after successful materialized flushes. The existing materialized formula-audit path verifies shifted qualified references from the dirty in-memory formula text before save; the existing source formula-audit path verifies source-side qualified references remain reported from the source scan while the materialized save writes the shifted formula. Both now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, fullCalcOnLoad persistence, shifted formula text/style preservation, removed-coordinate absence, and reopened bounds remain unchanged. | This is public save-state coverage for existing audit query state hygiene around fullCalcOnLoad plus row-side sparse formula shifts only. It does not add formula evaluation, broad formula repair, calcChain rebuild, calcChain payload generation, metadata/range repair, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory random editing. |
| In-memory operation-mixing addenda P8.1173 | Public-state coverage now verifies renamed-sheet source formula-audit queries combined with full-calculation row-shift saves remain stable across a clean no-op save after the successful materialized flush. The existing path renames `Data` to `RenamedData`, requests full calculation, materializes the planned sheet, shifts rows, verifies source-side qualified formula references from the source scan, then saves workbook rename metadata, `fullCalcOnLoad`, and shifted formula output. It now snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, planned sheet name persistence, fullCalcOnLoad persistence, shifted formula text/style preservation, and reopened formula-audit output remain unchanged. | This is public save-state coverage for existing rename plus fullCalcOnLoad source-audit state hygiene only. It does not rewrite formula sheet qualifiers, evaluate formulas, repair renamed formula references, rebuild calcChain, perform metadata/range repair, migrate sharedStrings/styles, repair relationships, compose broader Patch/materialized edits, or add low-memory random editing. |
| In-memory operation-mixing addenda P8.1174 | Public-state coverage now verifies the renamed full-calculation formula-audit failed-save retry path remains stable across a clean no-op save after the successful retry flush. The existing path rejects exact source overwrite, preserves dirty rename/fullCalc/materialized state, verifies both materialized and source formula audits, confirms the source package bytes stay unchanged, then safely saves the renamed shifted output. It now snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, source package byte preservation, output-entry stability, planned sheet name persistence, fullCalcOnLoad persistence, and reopened formula-audit output remain unchanged. | This is public save-state coverage for failed-save retry hygiene around existing rename plus fullCalcOnLoad formula-audit state only. It does not add atomic in-place editing, rollback transactions, formula evaluation, formula qualifier repair, calcChain rebuild, metadata/range repair, sharedStrings/styles migration, relationship repair, or low-memory random editing. |
| In-memory operation-mixing addenda P8.1175 | Public-state coverage now verifies renamed full-calculation formula-audit query/read failure paths remain stable across clean no-op saves after successful materialized flushes. The existing option-mismatch, missing-query, and invalid-read paths reject access/read failures, preserve dirty rename/fullCalc/materialized state, verify materialized and source formula audits, then save the renamed shifted output. They now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, planned sheet name persistence, fullCalcOnLoad persistence, shifted formula text/style preservation, and reopened formula-audit output remain unchanged. | This is public save-state coverage for existing query/read failure hygiene around rename plus fullCalcOnLoad formula-audit state only. It does not add formula evaluation, formula qualifier repair, calcChain rebuild, metadata/range repair, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory operation-mixing addenda P8.1176 | Public-state coverage now verifies the renamed full-calculation formula-audit invalid-mutation path remains stable across a clean no-op save after the successful materialized flush. The existing path rejects invalid `set_cell()` / `erase_cell()` calls, preserves the invalid-reference diagnostic, verifies materialized and source formula audits, then saves the renamed shifted output without leaking rejected formula payloads. It now snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, preserved post-save `last_edit_error()`, clean materialized diagnostics, output-entry stability, planned sheet name persistence, fullCalcOnLoad persistence, shifted formula text/style preservation, and reopened formula-audit output remain unchanged. | This is public save-state coverage for existing invalid-mutation diagnostic hygiene around rename plus fullCalcOnLoad formula-audit state only. It does not add coordinate repair or clamping, rejected-payload staging, rollback, formula evaluation, formula qualifier repair, calcChain rebuild, metadata/range repair, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory operation-mixing addenda P8.1177 | Public-state coverage now verifies the renamed full-calculation formula-audit invalid-shift path remains stable across a clean no-op save after the successful materialized flush. The existing path rejects row/column shift preflight failures, preserves the invalid-shift diagnostic, verifies materialized and source formula audits, then saves the renamed shifted output. It now snapshots catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, preserved post-save `last_edit_error()`, clean materialized diagnostics, output-entry stability, planned sheet name persistence, fullCalcOnLoad persistence, shifted formula text/style preservation, and reopened formula-audit output remain unchanged. | This is public save-state coverage for existing invalid-shift diagnostic hygiene around rename plus fullCalcOnLoad formula-audit state only. It does not relax row/column shift validation, add coordinate repair or clamping, rollback, formula evaluation, formula qualifier repair, calcChain rebuild, metadata/range repair, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1178 | Public-state coverage now verifies basic shift-after-rename saves remain stable across clean no-op saves after successful materialized flushes. The existing planned-name row shift, styled row-formula shift, and styled column-formula shift paths rename `Data` to `RenamedData`, materialize through the planned name, shift sparse source/formula records, preserve formula text/style as appropriate, then save the renamed output. They now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, planned sheet name persistence, shifted sparse bounds, shifted source cells, and styled formula readback remain unchanged. | This is public save-state coverage for existing shift-after-rename materialized saves only. It does not add sheet rename formula qualifier repair, formula evaluation, broad formula repair beyond moved-cell translation, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1179 | Public-state coverage now verifies row and column formula-audit shift-after-rename saves remain stable across clean no-op saves after successful materialized flushes. The existing row and column audit paths rename `Data` to `RenamedData`, shift formula cells through the planned name, reject save/output/query/read/mutation/shift preflights, verify materialized and source formula audits, then save the shifted output. They now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, preserved post-save `last_edit_error()`, clean materialized diagnostics, source package preservation, output-entry stability, planned sheet name persistence, shifted formula text/style preservation, and reopened formula-audit output remain unchanged. | This is public save-state coverage for existing formula-audit shift-after-rename hygiene only. It does not add sheet rename formula qualifier repair, formula evaluation, formula dependency rewrite, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1180 | Public-state coverage now verifies delete-side formula shift-after-rename saves remain stable across clean no-op saves after successful materialized flushes. The delete-row/delete-column formula-audit path verifies translated `#REF!` tokens are skipped from public formula-audit references while surviving qualified references remain visible, and the basic delete-column/delete-row formula paths verify styled `#REF!` formula output and shifted source records. These paths now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clean materialized diagnostics, output-entry stability, planned sheet name persistence, `#REF!` formula text/style preservation, shifted source cells, and reopened delete-audit/readback output remain unchanged. | This is public save-state coverage for existing delete-side formula shift-after-rename behavior only. It does not add formula evaluation, formula qualifier repair, broader formula dependency rewrite, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory erase addenda P8.1181 | Public-state coverage now verifies single sparse row/column erases remain stable across clean no-op saves after successful materialized flushes. The existing `erase_row(1)` and `erase_column(1)` paths remove represented target-row/target-column source-backed records, preserve non-target cells and untouched worksheets, shrink projected dimensions, then save once. They now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, erased-cell absence, non-target source cell preservation, and reopened bounds remain unchanged. | This is public save-state coverage for existing single row/column sparse erase saves only. It does not add row/column metadata synchronization, dense row/column editing, tombstones, formula repair, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory random editing. |
| In-memory erase addenda P8.1182 | Public-state coverage now verifies inclusive sparse row/column range erases remain stable across clean no-op saves after successful materialized flushes. The existing `erase_rows(1, 2)` and `erase_columns(1, 2)` paths remove represented source-backed records across the target range, preserve non-target dirty sparse cells, shrink projected dimensions, and save once. They now snapshot catalog/save-state before `save_as(noop_output)` verifies pending counts, replacement diagnostics, clear `last_edit_error()`, clean materialized diagnostics, output-entry stability, erased range absence, preserved non-target dirty cell readback, and reopened bounds remain unchanged. | This is public save-state coverage for existing inclusive row/column range sparse erase saves only. It does not add row/column metadata synchronization, dense row/column deletion, tombstones, formula repair, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, Patch/materialized composition, or low-memory random editing. |
| In-memory operation-mixing addenda P8.1183 | Public-state helper coverage now verifies rename-backed clean no-op saves also preserve the `pending_worksheet_edits()` summary vector. The shared renamed full-calculation formula-audit helper and the shared renamed clean-shift helper snapshot summaries before `save_as(noop_output)` and compare them afterwards while keeping the existing catalog/save-state, replacement diagnostics, materialized diagnostics, output-entry equality, and reopened readback checks. | This is summary-state hygiene for existing renamed materialized no-op save helpers only. It does not add sheet rename formula qualifier repair, formula evaluation, new summary semantics, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1184 | The public save-state snapshot helper now includes `pending_worksheet_edits()` and compares it through `workbook_editor_edit_summaries_equal()`. Existing no-op save regressions that call `check_workbook_editor_public_save_state_preserved()` now also prove their public worksheet edit summaries survive unchanged alongside pending counts, replacement diagnostics, and `last_edit_error()`. | This is test-helper coverage for public summary stability only. It does not add new summary fields, change production summary semantics, alter save behavior, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1185 | The same public save-state snapshot helper now also includes `has_pending_changes()` and the materialized aggregate diagnostics: `pending_materialized_worksheet_names()`, `pending_materialized_cell_count()`, and `estimated_pending_materialized_memory_usage()`. Existing save-state preservation regressions now prove clean no-op saves and no-op failure-recovery saves keep the public pending-change boolean and materialized diagnostics stable alongside summaries, replacement diagnostics, and `last_edit_error()`. | This is test-helper coverage for public save-state stability only. It does not add new public state APIs, change production dirty-session semantics, alter save behavior, metadata/range repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1186 | Basic saved-session reacquire no-op saves now also snapshot and preserve the public workbook catalog. The existing handle-reuse, `worksheet()` reacquire, `try_worksheet()` reacquire, `delete_columns()`, `delete_rows()`, and `insert_columns()` no-op save paths check `source_worksheet_names()`, `worksheet_names()`, and `worksheet_catalog()` through `check_workbook_editor_public_catalog_preserved()` alongside the save-state snapshot, output-entry stability, and reopened sparse-shift readback. | This is catalog-state coverage for existing saved-session sparse-shift no-op saves only. It does not add new shift semantics, sheet catalog mutation, sheet rename/delete, relationship repair, metadata/range repair, formula repair beyond existing moved-cell text translation, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1187 | Failed-save retry saved-session reacquire no-op saves now also snapshot and preserve the public workbook catalog. The existing source-overwrite, path-equivalent source-overwrite, empty output path, missing parent, non-directory parent, and existing-directory save rejection paths perform a safe retry, reacquire the saved session, then check `source_worksheet_names()`, `worksheet_names()`, and `worksheet_catalog()` through `check_workbook_editor_public_catalog_preserved()` across the final clean no-op `save_as()` alongside save-state snapshots, output-entry stability, and reopened sparse-shift readback. | This is catalog-state coverage for existing failed-save retry sparse-shift no-op saves only. It does not add overwrite mode, path repair, rollback, transaction replay, new shift semantics, sheet catalog mutation, metadata/range repair, formula repair beyond existing moved-cell text translation, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1188 | Renamed full-calculation formula-audit saved-session reacquire no-op saves now also snapshot and preserve the public workbook catalog. The existing invalid mutation, invalid read, invalid shift, missing-query, option-mismatch, and same-sheet guard paths save the renamed shifted formula session, reacquire it cleanly, run their rejected preflight/guard checks, then check `source_worksheet_names()`, `worksheet_names()`, and `worksheet_catalog()` through `check_workbook_editor_public_catalog_preserved()` across the second clean `save_as()` alongside save-state snapshots, edit-summary stability, and byte-stable output. | This is catalog-state coverage for existing renamed formula-audit saved-reacquire no-op saves only. It does not add formula qualifier repair, formula evaluation, dependency rewrite, sheet rename/delete, metadata/range repair, calcChain rebuild beyond the existing fullCalcOnLoad request, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1189 | Early shift-after-rename regressions now also pin dirty materialized memory diagnostics. The planned-name row insert, styled row-formula insert, styled column-formula insert, styled delete-column `#REF!`, and styled delete-row `#REF!` paths compare `estimated_pending_materialized_memory_usage()` with the active `WorksheetEditor::estimated_memory_usage()` while dirty, and verify it returns to zero after the materialized `save_as()` flush. | This is public diagnostic coverage for existing renamed sparse-shift sessions only. It does not change memory accounting, add workbook-level guardrails, alter shift semantics, repair formulas or metadata ranges, rebuild calcChain beyond existing behavior, migrate sharedStrings/styles, repair relationships, broaden Patch/materialized composition, or provide low-memory random editing. |
| In-memory guardrail addenda P8.1190 | The materialized aggregate diagnostics regression now also pins replacement memory isolation. A replacement-only editor queues one whole-`sheetData` replacement cell, reports non-zero `estimated_pending_replacement_memory_usage()`, and still reports zero `pending_materialized_cell_count()` / `estimated_pending_materialized_memory_usage()`. | This is public diagnostic separation coverage only. It does not change replacement memory accounting, add new guardrails, compose Patch and materialized edits on the same sheet, migrate sharedStrings/styles, repair metadata or relationships, rebuild calcChain, or provide low-memory random editing. |
| In-memory guardrail addenda P8.1191 | Pending materialized summary move coverage now also preserves memory estimates. `test_public_workbook_editor_pending_materialized_summaries_move_with_owner()` snapshots the dirty `WorksheetEditor::estimated_memory_usage()` before moving the `WorkbookEditor`, then verifies `WorkbookEditorWorksheetEditSummary::estimated_materialized_memory_usage` survives both move construction and move assignment alongside the materialized cell count. | This is public summary move-state coverage only. It does not change move semantics, memory accounting, ownership lifetime, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1192 | Full-calculation formula-audit dirty summaries now preserve materialized memory estimates. The source formula-audit setup paths and the renamed failed-save dirty-state helper snapshot `WorksheetEditor::estimated_memory_usage()` after shifting the styled formula fixture, then require both aggregate `estimated_pending_materialized_memory_usage()` and `WorkbookEditorWorksheetEditSummary::estimated_materialized_memory_usage` to match that value. | This is public diagnostic coverage for existing formula-audit materialized sessions only. It does not change formula translation, formula audit semantics, full-calculation metadata handling, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1193 | Renamed full-calculation formula-audit preserve-state helpers now also pin dirty summary memory. Option mismatch, missing-query, invalid-read, invalid-mutation, invalid-shift, and invalid-diagnostic-recovery paths already preserved aggregate materialized count and memory while rejected operations ran; their dirty-summary checks now require `WorkbookEditorWorksheetEditSummary::estimated_materialized_memory_usage` to match the same shifted or recovered session estimate. | This is public preserve-state diagnostic coverage only. It does not change option matching, missing-sheet behavior, invalid-operation diagnostics, formula audits, full-calculation metadata, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1194 | Renamed full-calculation formula-audit saved-reacquire paths now pin dirty summary memory after post-save mutations. The clean reacquired session writes `C5`, snapshots the reacquired `WorksheetEditor::estimated_memory_usage()`, and verifies both aggregate dirty memory and `WorkbookEditorWorksheetEditSummary::estimated_materialized_memory_usage` match that value through the normal second save and the rejected-source-overwrite retry path. | This is saved-session reacquire diagnostic coverage only. It does not change saved-session ownership, retry semantics, formula audits, full-calculation metadata, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1195 | Early shift-after-rename dirty summaries now preserve memory estimates. The planned-name insert path, saved-session reacquire path, and option-mismatch recovery path snapshot the shifted `WorksheetEditor::estimated_memory_usage()` and verify both aggregate `estimated_pending_materialized_memory_usage()` and `WorkbookEditorWorksheetEditSummary::estimated_materialized_memory_usage` match the active shared session. | This is public shift diagnostic coverage only. It does not change rename semantics, shift semantics, saved-session reacquire, option matching, memory accounting, guardrails, formula translation, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1196 | Base sparse row/column shift tests now pin aggregate dirty materialized memory. Direct `insert_rows()`, `delete_rows()`, `insert_columns()`, and `delete_columns()` shift paths snapshot `WorksheetEditor::estimated_memory_usage()` after source-backed and dirty sparse cells move or disappear, then verify `pending_materialized_worksheet_names()`, `pending_materialized_cell_count()`, and `estimated_pending_materialized_memory_usage()` match the active dirty `Data` session before save. | This is public aggregate diagnostic coverage only. It does not change row/column shift semantics, formula translation, source-backed style preservation, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory mutation addenda P8.1197 | Base append/set row/column mutation tests now pin aggregate dirty materialized memory. Direct `append_row()`, `set_row()`, and `set_column()` paths snapshot `WorksheetEditor::estimated_memory_usage()` after their sparse store updates, then verify the dirty materialized worksheet name, aggregate materialized cell count, and aggregate materialized memory before save. | This is public aggregate diagnostic coverage only. It does not change append/set semantics, sparse storage, memory accounting, guardrails, row/column metadata, formula evaluation, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory mutation addenda P8.1198 | Base row/column clear tests now pin aggregate dirty materialized memory. Direct `clear_row()` and `clear_column()` paths snapshot `WorksheetEditor::estimated_memory_usage()` after converting represented cells to explicit blanks, then verify the dirty materialized worksheet name, aggregate blank-record cell count, and aggregate materialized memory before save. | This is public aggregate diagnostic coverage only. It does not change clear semantics, explicit-blank projection, sparse storage, memory accounting, guardrails, row/column metadata, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory mutation addenda P8.1199 | Base row/column erase tests now pin aggregate dirty materialized memory. Direct `erase_row()` and `erase_column()` paths snapshot `WorksheetEditor::estimated_memory_usage()` after removing represented sparse records from the target row or column, then verify the dirty materialized worksheet name, remaining aggregate materialized cell count, and aggregate materialized memory before save. | This is public aggregate diagnostic coverage only. It does not change erase semantics, sparse deletion, memory accounting, guardrails, row/column metadata, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory mutation addenda P8.1200 | Whole-store clear/erase reacquire tests now pin aggregate dirty materialized memory. `clear_cell_values()` snapshots explicit-blank worksheet memory before the first save, while post-save reacquire edits after whole-store clear and whole-store erase snapshot the reused session memory before verifying aggregate dirty materialized memory. | This is public aggregate diagnostic coverage only. It does not change whole-store clear/erase semantics, saved-session reuse, explicit-blank projection, sparse deletion, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1201 | Direct styled-formula delete-shift tests now pin aggregate dirty materialized memory. The delete-row and delete-column source-backed styled formula paths snapshot shifted worksheet memory after formula references become `#REF!` or move left/up, then verify dirty materialized worksheet name, shifted sparse count, and aggregate materialized memory before save. | This is public aggregate diagnostic coverage only. It does not change delete shift semantics, formula translation, `StyleId` preservation, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1202 | Full-calculation insert-row setup diagnostics now pin aggregate dirty materialized memory before workbook metadata is queued. The test snapshots shifted `WorksheetEditor::estimated_memory_usage()` immediately after `insert_rows()` and verifies `estimated_pending_materialized_memory_usage()` before `request_full_calculation()` adds the workbook-level edit. | This is public setup diagnostic coverage only. It does not change full-calculation metadata handling, row shift semantics, formula translation, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1203 | Cross-handle row/column shift and delete diagnostics now pin aggregate dirty materialized memory across two active `WorksheetEditor` handles. The tests snapshot each handle's `estimated_memory_usage()`, verify `WorkbookEditor::estimated_pending_materialized_memory_usage()` before and after mutating only `Data`, and confirm `save_as()` clears the aggregate memory. | This is public cross-handle diagnostic coverage only. It does not change shared-session ownership, row/column shift or delete semantics, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1204 | Invalid-to-valid row/column shift recovery diagnostics now pin aggregate dirty materialized memory. Clean recovery paths verify invalid shifts keep aggregate memory at zero before the valid shift dirties the handle, while dirty recovery paths snapshot memory before invalid shifts, verify failures preserve it, and verify the later valid shift reports the shifted worksheet memory. | This is public recovery diagnostic coverage only. It does not change invalid range validation, row/column shift semantics, diagnostic clearing, save/retry behavior, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1205 | Formula-translation row/column shift diagnostics now pin dirty materialized counts and memory before save. Rich formula reference-shape insert tests and out-of-bounds `#REF!` delete tests snapshot shifted `WorksheetEditor::estimated_memory_usage()` and verify `WorkbookEditor` aggregate dirty count/memory before saving the translated formulas. | This is public formula-shift diagnostic coverage only. It does not change formula translation, formula evaluation, cached values, row/column shift semantics, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1206 | Shift no-op, validation-failure, and overflow-failure diagnostics now pin aggregate materialized memory. Zero-count and out-of-range no-op/validation paths verify aggregate memory remains zero, while row/column overflow failures snapshot pre-existing dirty worksheet memory and verify failed shifts preserve aggregate dirty count/memory. | This is public guard/overflow diagnostic coverage only. It does not change validation rules, overflow rejection, row/column shift semantics, dirty-state preservation, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory shift addenda P8.1207 | Shift formula memory-budget guard failure diagnostics now pin aggregate materialized memory. The failing `insert_rows()` formula translation path verifies that a rejected memory-budget mutation leaves the editor clean, exposes no dirty materialized cells, and exposes no dirty materialized memory while preserving the sparse sheet memory estimate. | This is public guard failure diagnostic coverage only. It does not change formula translation, memory-budget enforcement, rollback behavior, sparse storage, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1208 | Source-load `max_cells` guard failure diagnostics now pin aggregate materialized state. The public options guard test verifies that a rejected `WorksheetEditor` materialization leaves no dirty materialized worksheet names, no dirty materialized cells, and no dirty materialized memory before the editor recovers through an existing sheetData replacement path. | This is public source-load diagnostic coverage only. It does not change `max_cells` enforcement, source materialization, replacement semantics, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1209 | Mutation guard recovery diagnostics now pin aggregate dirty materialized memory after successful overwrite recovery. The memory-budget and `max_cells` mutation recovery tests snapshot the active `WorksheetEditor::estimated_memory_usage()` after the valid overwrite and verify `estimated_pending_materialized_memory_usage()` reports that recovered dirty session before save. | This is public recovery diagnostic coverage only. It does not change mutation rollback, overwrite semantics, memory-budget or `max_cells` enforcement, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1210 | Source-load memory-budget recovery diagnostics now pin dirty materialized memory before save. After a rejected tight-budget source materialization, the default-options recovery path mutates the recovered session and verifies `pending_materialized_cell_count()` and `estimated_pending_materialized_memory_usage()` match that active `WorksheetEditor` before flushing. | This is public source-load recovery diagnostic coverage only. It does not change memory-budget enforcement, source materialization, recovery session ownership, memory accounting, guardrails, replacement semantics, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1211 | Explicit-blank guard recovery diagnostics now pin aggregate dirty materialized memory. The blank insertion guard tests verify that overwriting existing `A1` with an explicit blank dirties the editor, preserves the sparse count, and reports aggregate materialized memory matching the active `WorksheetEditor` under both `max_cells` and memory-budget options. | This is public blank-overwrite diagnostic coverage only. It does not change explicit blank serialization, blank insertion guard behavior, overwrite semantics, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1212 | Last-edit-error replacement recovery diagnostics now pin aggregate dirty materialized memory. After invalid-reference, memory-budget, and invalid-coordinate diagnostics replace each other, the successful in-budget overwrite snapshots `WorksheetEditor::estimated_memory_usage()` and verifies `estimated_pending_materialized_memory_usage()` reports that recovered dirty session before save. | This is public diagnostic-recovery coverage only. It does not change `last_edit_error()` semantics, mutation rollback, memory-budget enforcement, overwrite semantics, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory guardrail addenda P8.1213 | Mixed diagnostic recovery replacement isolation now pins aggregate materialized diagnostics. After replacement, rename, and materialized mutation diagnostics replace each other, the successful other-sheet `replace_sheet_data()` recovery verifies the clean materialized `Data` handle still contributes zero dirty materialized cells and zero dirty materialized memory. | This is public Patch/materialized diagnostic isolation coverage only. It does not add same-sheet Patch/materialized composition, change replacement summaries, mutate the clean materialized session, change memory accounting, repair metadata/relationships, rebuild calcChain, migrate sharedStrings/styles, or provide low-memory random editing. |
| In-memory guardrail addenda P8.1214 | Erase budget-release reinsertion diagnostics now pin aggregate dirty materialized memory. The erase-before-insert guardrail test verifies the memory-budget erase phase dirties the editor with count/memory matching the reduced sparse session, and both `max_cells` and memory-budget replacement insertions report aggregate materialized memory matching the active `WorksheetEditor`. | This is public guardrail recovery diagnostic coverage only. It does not change erase semantics, insertion semantics, guardrail budgeting, memory accounting, metadata/range repair, relationship repair, calcChain rebuild, sharedStrings/styles migration, broader Patch/materialized composition, or low-memory random editing. |
| In-memory formula-audit addenda P8.1215 | Full-calculation shifted formula-audit setup diagnostics now pin dirty materialized memory in both aggregate state and edit summaries. The direct shifted formula audit test snapshots the active `WorksheetEditor::estimated_memory_usage()` after `insert_rows()` and verifies `estimated_pending_materialized_memory_usage()` plus the dirty `WorkbookEditorWorksheetEditSummary` memory estimate before formula audits run. | This is public formula-audit diagnostic coverage only. It does not change formula translation, formula audit tokenization, fullCalcOnLoad metadata, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory formula-audit addenda P8.1216 | Saved-reacquire renamed formula-audit recovery diagnostics now pin dirty materialized memory in both aggregate state and edit summaries. The invalid-mutation, invalid-read, invalid-shift, missing-query, option-mismatch, and same-sheet guard recovery branches snapshot the recovered `WorksheetEditor::estimated_memory_usage()` and verify the dirty `RenamedData` summary reports matching materialized count and memory before the second save. | This is public saved-session formula-audit recovery coverage only. It does not change formula translation, recovery mutation semantics, rename semantics, formula audit tokenization, fullCalcOnLoad metadata, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory full-calculation addenda P8.1217 | Full-calculation-before-shift setup diagnostics now pin zero dirty materialized memory. The insert-columns and delete-columns full-calculation tests verify that `request_full_calculation()` alone leaves dirty materialized names, cell count, and memory empty, and that clean materialization under the queued workbook metadata edit keeps those dirty diagnostics clear before the later shift. | This is public metadata-plus-clean-materialization diagnostic coverage only. It does not change fullCalcOnLoad metadata, source materialization, row/column shift semantics, memory accounting, guardrails, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory formula-audit addenda P8.1218 | Renamed row/column formula-audit preflight diagnostics now pin dirty materialized memory preservation. The row and column shifted formula-audit tests snapshot `estimated_pending_materialized_memory_usage()` before formula audits and verify materialization guard failures, recovery materialization, invalid mutations, and invalid shifts preserve the same dirty materialized count and memory before save. | This is public formula-audit preflight diagnostic coverage only. It does not change formula translation, formula audit tokenization, save preflight behavior, materialization guardrails, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1219 | Renamed shift reopened-output diagnostics now pin clean materialized state. The direct `insert_rows()` after rename regression now verifies that reopening the saved package exposes no pending changes, no dirty materialized names, zero dirty materialized cells, and zero dirty materialized memory before reading shifted cells. | This is public reopened-output diagnostic coverage only. It does not change rename semantics, row shift semantics, saved-session lifecycle, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1220 | Styled formula rename-shift diagnostics now pin materialized memory through reopened and reacquired paths. The styled column-formula reopened output verifies clean materialized names/count/memory, while the saved/reacquired styled session test verifies first-save cleanup, later shared column-shift dirty memory, and second-save cleanup. | This is public styled formula rename-shift diagnostic coverage only. It does not change style preservation, formula translation, rename semantics, row/column shift semantics, saved-session lifecycle, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1221 | Styled formula failed-save and option-mismatch diagnostics now pin clean and dirty materialized memory. The failed-save retry path verifies safe-save cleanup clears dirty memory, while the option-mismatch saved-session path verifies first-save cleanup, rejected option access cleanliness, later dirty column-shift memory, second-save cleanup, and reopened clean diagnostics. | This is public styled-session diagnostic coverage only. It does not change failed-save policy, option matching, style preservation, formula translation, rename semantics, shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1222 | Styled formula invalid-mutation and missing-query diagnostics now pin clean and dirty materialized memory. Both saved-session recovery paths verify first-save cleanup, rejected operation cleanliness, later shared column-shift dirty memory, second-save cleanup, and reopened clean materialized diagnostics. | This is public styled-session rejection/recovery diagnostic coverage only. It does not change invalid-reference validation, missing-sheet lookup, style preservation, formula translation, rename semantics, shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1223 | Styled formula invalid-read and snapshot-read diagnostics now pin clean and dirty materialized memory. Both saved-session read-only paths verify first-save cleanup, rejected/strict read cleanliness, later shared column-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled-session read-only diagnostic coverage only. It does not change read validation, snapshot APIs, style preservation, formula translation, rename semantics, shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1224 | Styled formula delete-column failed-save and option-mismatch diagnostics now pin clean and dirty materialized memory. The failed-save retry path verifies safe-save cleanup and reopened output are materialized-clean, while the saved-session option-mismatch path verifies first-save cleanup, rejected option access cleanliness, later shared row-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean diagnostics. | This is public styled delete-column diagnostic coverage only. It does not change failed-save policy, option matching, deleted-reference formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1225 | Styled formula delete-column invalid-mutation and missing-query diagnostics now pin clean and dirty materialized memory. Both saved-session recovery paths verify first-save cleanup, rejected operation cleanliness, later shared row-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-column rejection/recovery diagnostic coverage only. It does not change invalid-reference validation, missing-sheet lookup, deleted-reference formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1226 | Styled formula delete-column invalid-read and snapshot-read diagnostics now pin clean and dirty materialized memory. Both saved-session read-only paths verify first-save cleanup, rejected/strict read cleanliness or snapshot ownership, later shared row-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-column read-only diagnostic coverage only. It does not change read validation, snapshot APIs, deleted-reference formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1227 | Styled formula delete-column reacquire diagnostics now pin clean and dirty materialized memory. The saved-session reacquire path verifies first-save cleanup, clean `try_worksheet()` reacquire diagnostics, later shared row-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-column reacquire diagnostic coverage only. It does not change saved-session lookup, deleted-reference formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1228 | Styled formula delete-row direct and failed-save diagnostics now pin clean materialized memory. The direct delete-row regression verifies reopened output has empty materialized diagnostics, while the failed-save retry path verifies safe-save cleanup and reopened output clear dirty materialized names, count, and memory. | This is public styled delete-row clean-boundary diagnostic coverage only. It does not change failed-save policy, deleted-row formula translation, style preservation, rename semantics, row shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1229 | Styled formula delete-row option-mismatch diagnostics now pin clean and dirty materialized memory. The saved-session option-mismatch path verifies first-save cleanup, rejected option access cleanliness, later shared column-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-row option-mismatch diagnostic coverage only. It does not change option matching, deleted-row formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1230 | Styled formula delete-row invalid-mutation and missing-query diagnostics now pin clean and dirty materialized memory. Both saved-session recovery paths verify first-save cleanup, rejected operation cleanliness, later shared column-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-row rejection/recovery diagnostic coverage only. It does not change invalid-reference validation, missing-sheet lookup, deleted-row formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1231 | Styled formula delete-row invalid-read and snapshot-read diagnostics now pin clean and dirty materialized memory. Both saved-session read-only paths verify first-save cleanup, rejected/strict read cleanliness or snapshot ownership, later shared column-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-row read-only diagnostic coverage only. It does not change read validation, snapshot APIs, deleted-row formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1232 | Styled formula delete-row reacquire diagnostics now pin clean and dirty materialized memory. The saved-session reacquire path verifies first-save cleanup, clean `try_worksheet()` reacquire diagnostics, later shared column-shift dirty memory, second-save cleanup, no-op save stability, and reopened clean materialized diagnostics. | This is public styled delete-row reacquire diagnostic coverage only. It does not change saved-session lookup, deleted-row formula translation, style preservation, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1233 | Non-styled saved-session reacquire diagnostics now pin clean materialized memory after first and second saves, while retaining the existing dirty shared-session memory check for the later column shift and reopened clean diagnostics. | This is public rename-shift reacquire diagnostic coverage only. It does not change saved-session lookup, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1234 | Non-styled failed-save retry diagnostics now pin clean materialized memory after successful saves/reopens and dirty materialized memory after the post-retry delete-row mutation. | This is public rename-shift failed-save retry diagnostic coverage only. It does not change failed-save policy, saved-session lookup, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1235 | Non-styled option-mismatch diagnostics now pin clean materialized memory after first save, rejected mismatched options, second save, no-op save, and reopened output, while retaining the existing dirty shared-session memory check for the later column shift. | This is public rename-shift option-mismatch diagnostic coverage only. It does not change option matching, saved-session lookup, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1236 | Non-styled missing-query diagnostics now pin clean materialized memory after first save, missing/old-name lookups, second save, no-op save, and reopened output, while adding the later column shift aggregate dirty memory check. | This is public rename-shift missing-query diagnostic coverage only. It does not change missing-sheet lookup, saved-session lookup, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory rename-shift addenda P8.1237 | Non-styled invalid-read and invalid-mutation diagnostics now pin clean materialized memory after first save, rejected read/mutation paths, second save, no-op save, and reopened output, while adding recovery column-shift aggregate dirty memory checks. | This is public rename-shift read/mutation rejection diagnostic coverage only. It does not change read validation, mutation validation, saved-session lookup, rename semantics, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1238 | Shift handle reuse after `save_as()` now pins clean aggregate materialized names/count/memory after both successful saves before the no-op save stability check. | This is public handle-reuse diagnostic coverage only. It does not change handle lifetime, saved-session reuse, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1239 | Basic shift reacquire after `save_as()` now pins clean materialized memory after first and second saves and aggregate dirty memory after the shared recovery column shift. | This is public saved-session reacquire diagnostic coverage only. It does not change saved-session lookup, handle lifetime, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1240 | `try_worksheet()` shift reacquire after `save_as()` now pins clean materialized memory after first and second saves and aggregate dirty memory after the shared recovery column shift. | This is public optional saved-session reacquire diagnostic coverage only. It does not change `try_worksheet()` lookup, handle lifetime, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1241 | Saved-session option-mismatch, missing-query, invalid-read, and invalid-mutation reacquire diagnostics now pin clean materialized memory after first/rejected/second saves and aggregate dirty memory after their recovery column shifts. | This is public saved-session rejection diagnostic coverage only. It does not change option matching, missing-sheet lookup, read/mutation validation, saved-session lookup, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1242 | Saved-session failed-save retry diagnostics now pin clean materialized memory after the initial save and safe retry, while retaining existing dirty-memory preservation checks around the rejected source-overwrite save. | This is public saved-session failed-save diagnostic coverage only. It does not change failed-save policy, saved-session lookup, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1243 | Saved-session failed-save retry reacquire diagnostics now pin clean materialized memory after first/safe/third saves and clean reacquire, plus dirty materialized memory after the post-retry delete-row mutation. | This is public saved-session failed-save retry reacquire diagnostic coverage only. It does not change failed-save policy, saved-session lookup, row/delete shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
| In-memory saved-session addenda P8.1244 | Saved-session rejected-output failed-save variants now pin clean materialized memory after their initial saves and safe retries across path-equivalent source, empty output, missing parent, file parent, and directory output cases. | This is public failed-save output-path diagnostic coverage only. It does not change output-path validation, failed-save policy, saved-session lookup, row/column shift semantics, memory accounting, metadata repair, calcChain rebuild, sharedStrings/styles migration, relationship repair, broader Patch/materialized composition, or low-memory random editing. |
