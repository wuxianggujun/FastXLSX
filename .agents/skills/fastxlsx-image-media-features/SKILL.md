...
`FASTXLSX_ENABLE_STB=ON` 时通过 `stb` 读取格式、尺寸和通道。当前还已有
...
当前还已有窄 `ImageOptions` metadata：非空 `name` / `description` 只写入 drawing
XML 的 `xdr:cNvPr name` / `descr` attributes；空 `name` 保留生成的 `Picture N`，
空 `description` 省略。它不是图片文件 metadata、EXIF、media filename、cell text、
existing drawing mutation 或完整 alt text/accessibility UI。
...
- `ImageOptions` 只把 name/description 字符串复制进 worksheet image state，并在
  drawing XML 输出阶段做 XML attribute escape；它不新增 relationship/content type/media
  副作用。
...
3. P17.2 new-workbook-only 插入：当前基础切片是
   ...
   drawing mutation。当前 `ImageOptions` 增量只在该 drawing XML 中写 `xdr:cNvPr`
   的 `name` / `descr`。
4. P17.3 验证闭环：当前 `fastxlsx.streaming` 已覆盖 media/drawing/rels/content
   types/anchor 结构测试；本机 Excel COM 已打开
   `build/windows-nmake-release-image/tests/fastxlsx-streaming-images.xlsx`，确认
   ...
   检查和 `tools/verify_image_metadata_excel.ps1` 的 Excel COM shape metadata 检查。
...
- 不要把 `ImageOptions::name` / `description` 写成完整图片 metadata、EXIF/PNG/JPEG
  metadata、完整 alt text/accessibility UI、media filename 语义或 existing-workbook 图片编辑。
...
- P17b 图片插入阶段：默认无 `stb` 时 `fastxlsx.streaming` 验证 `add_image()` 明确失败；
  ...
  图片 metadata 结构测试还应覆盖 `xdr:cNvPr name` / `descr`、XML attribute escape、
...
- 结构测试：检查 `xl/media/*`、`xl/drawings/drawing*.xml`、
  ...
  drawing reference；如果涉及 `ImageOptions`，还要检查 `xdr:cNvPr name` / `descr`。
