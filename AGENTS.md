...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  bold/italic font 和 solid fill styles /
...
`VerticalAlignment`、窄 `CellFont::bold` / `CellFont::italic` 和窄
...
bold/italic font 只写 `<fonts>` 里的 `<b/>` / `<i/>`、
...
- P9 styles number formats + limited alignment + bold/italic fonts + solid fills：基础。`StyleId` / `CellAlignment` / `HorizontalAlignment` /
  ...
  bold/italic font 和 solid foreground fill 样式注册及 cell style 引用；`xl/styles.xml`
...
- 当前 `fastxlsx.streaming` styles 推荐 preset 输出样例包括 number-format：
  ...
  wrap-text、horizontal/vertical alignment、bold/italic font 和 solid fill 语义、
  可用时创建 `XlsxWriter` 参考 workbook；Excel helper 用本机 Excel
  COM 只读打开五个样例，核对 NumberFormat、WrapText、HorizontalAlignment、
  VerticalAlignment、Font.Bold、Font.Italic、Interior.Pattern、
...
- 把 P9 styles 写成完整样式系统。当前只支持 workbook-local `StyleId`、自定义
  ...
  窄 bold/italic font、窄 solid foreground fill
