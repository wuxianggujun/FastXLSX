...
- 七个 bundle 都不是跨机器、跨数据规模或泛化“高性能/低内存”证据；每个 bundle 只支持其 manifest 中记录的同机同数据集结论。历史 schema-v4 继续作为回归基线，release 当前值以 schema-v5 bundle 为准。
...
- 在上述单机、1,000,000-cell、DEFLATE level 6 范围内，Streaming 创建路径已具备稳定的 file-backed worksheet footprint；除 unique sharedStrings 外，各 case 的 process peak working set median 为 6.87–6.93 MB，body buffer peak 小于 256 KiB，成功 close 后 active worksheet temporary files 为零。
- Numeric/mixed-inline 的 generation median 为 138/175 ms，package-close median 为 1379/1048 ms；当前这两个 workload 的主导成本是 ZIP/DEFLATE package close，下一轮不应继续微调 cell XML encoding 作为首要方向。
