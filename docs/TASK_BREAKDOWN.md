...
chunk/window input first slice、transformer chunk-event adapter first slice 已有测试，
PackageEditor 仍未接入完整低内存输入。
...
- PackageEditor 尚未把 chunk transformer 接到 staged package-entry writer；真正 rewrite
...
3. 把 transformer action stream 接到 package-entry staged writer。
   ...
   输入接线和 staged writer no-state-pollution。
...
- P8.19 event reader chunk/window input 和 P8.20 transformer chunk-event adapter 已完成；
  PackageEditor chunk transformer input 接线设计可并行只读调研。
...
- 文档明确该 guard 只是 bounded materialized validation input，不是 complete
  PackageReader input streaming、PackageEditor consumption of the chunk transformer、
...
- 当前 transformer / PackageEditor 仍接受完整 `std::string_view`。
...
可并行性：
- 后续 P8.20 transformer chunk event adapter 已完成；PackageEditor chunk transformer input
  接线设计可并行只读调研。
