...
- 当前 image 测试覆盖默认无 `stb` 构建下 `read_image_info()` 的明确错误，以及
  ...
  覆盖 `ImageOptions` drawing metadata：`xdr:twoCellAnchor editAs` 的 `oneCell` /
  `absolute` / 默认 `twoCell`、`xdr:cNvPr name` / `descr`、XML attribute escape、
  空 description 省略、默认 `Picture N` 名称，以及无额外 relationship /
...
`AlternativeText` 和 `Placement`；但结构语义仍以拆包后的 drawing XML 为准，不要
宣称完整 UI parity、row/column resize 几何计算或 `oneCellAnchor` / `absoluteAnchor`
元素支持。
