...
handles, restored-name diagnostics, row/column owning snapshot views, and saved
XML projection. The retry/guard shard also pins that shift no-ops clear stale
diagnostics without dirtying reacquired sessions, while invalid shift ranges
preserve the saved sparse store until a later valid shift clears the diagnostic
and flushes. Public-state
