...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `WorksheetWriter`、`CellView`、`StyleId`、`CellAlignment`、`CellStyle`、`DataValidationRule`、
...
- `tests/test_streaming_writer.cpp` 通过 CTest 注册为 `fastxlsx.streaming`，覆盖
  ...
  streaming number-format and wrap-text alignment styles /
...
number format 和窄 `CellAlignment::wrap_text`。重复完整 style 复用同一个
`StyleId`；相同 number format 在不同 style 组合里复用同一个 custom
`numFmtId`。`CellView::with_style()` 只携带 workbook-local `StyleId` 小句柄，
...
不代表完整 alignment。该切片不支持字体、填充、边框、完整 alignment、rich text、
...
- P9 styles number formats + wrap-text alignment：基础。`StyleId` / `CellAlignment` / `CellStyle` /
  `WorkbookWriter::add_style()` / `CellView::with_style()` 已支持 streaming-only
  new-workbook 自定义 number format 和 wrap-text alignment 样式注册及 cell style 引用；`xl/styles.xml`
  ...
  custom `numFmtId` 复用、`applyAlignment` / `wrapText`、默认 `s="0"` 省略、
  ...
  这不是完整 styles、font/fill/border/full alignment、date cell type、conditional
...
- 当前 `fastxlsx.streaming` styles 推荐 preset 输出样例包括 number-format：
  ...
  worksheet `s="N"` 引用、默认 `s="0"` 省略、`applyAlignment="1"` /
  ...
  alignment 语义、可用时创建 `XlsxWriter` 参考 workbook；Excel helper 用本机 Excel
  COM 只读打开三个样例，核对 NumberFormat、WrapText、值和公式。不要把这扩展成完整
...
- 把 P9 styles 写成完整样式系统。当前只支持 workbook-local `StyleId`、自定义
  number format、窄 wrap-text alignment 和 `xl/styles.xml` 基础骨架；不支持字体、填充、
  边框、完整对齐、rich text、dxf-backed conditional formatting、date cell type、existing-file style preservation
...
- `fastxlsx-style-registry`：number format / wrap-text alignment styles、`StyleId`、`CellStyle`、`xl/styles.xml`
  和样式 QA 验证。
