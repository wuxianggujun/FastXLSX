...
description: "实现或审查 FastXLSX streaming conditional formatting。用于 two-/three-color color scales、basic data bars、priority 共享、multi-range sqref、本地 Excel/openpyxl/XlsxWriter 验证，以及判断 icon sets、advanced data bars、dxf/styles、formula/cellIs 是否越界。"
---
...
- 修改 `WorksheetWriter::add_conditional_color_scale()` 或
  `WorksheetWriter::add_conditional_data_bar()`。
- 添加新的 conditional formatting 规则类型，例如 icon set、formula/cellIs、
...
- Color scale 与 data bar 共享同一 worksheet-local priority 序列；失败调用不能消耗
...
- 不要把 color scale 或 basic data bar 写成完整 conditional formatting。
...
- 不要声称支持 formula/cellIs、icon sets、top/bottom、duplicate/unique、dxf-backed styles、
...
- 失败路径覆盖 invalid range、empty range list、non-finite endpoint、非法 endpoint shape、
  failed call state hygiene 和 mutation-after-close。
