#include "zip_store_writer.hpp"

#include <array>
#include <fstream>
#include <limits>

#include <fastxlsx/workbook.hpp>

namespace fastxlsx::detail {
namespace {

struct CentralDirectoryRecord {
    std::string name;
    std::uint32_t crc = 0;
    std::uint32_t size = 0;
    std::uint32_t local_header_offset = 0;
};

void append_u16(std::string& output, std::uint16_t value)
{
    output.push_back(static_cast<char>(value & 0xffu));
    output.push_back(static_cast<char>((value >> 8u) & 0xffu));
}

void append_u32(std::string& output, std::uint32_t value)
{
    output.push_back(static_cast<char>(value & 0xffu));
    output.push_back(static_cast<char>((value >> 8u) & 0xffu));
    output.push_back(static_cast<char>((value >> 16u) & 0xffu));
    output.push_back(static_cast<char>((value >> 24u) & 0xffu));
}

std::uint16_t checked_u16(std::size_t value, const char* field)
{
    if (value > std::numeric_limits<std::uint16_t>::max()) {
        throw FastXlsxError(std::string("ZIP field exceeds 16-bit limit: ") + field);
    }
    return static_cast<std::uint16_t>(value);
}

std::uint32_t checked_u32(std::size_t value, const char* field)
{
    if (value > std::numeric_limits<std::uint32_t>::max()) {
        throw FastXlsxError(std::string("ZIP field exceeds 32-bit limit: ") + field);
    }
    return static_cast<std::uint32_t>(value);
}

} // namespace

std::uint32_t crc32(std::string_view data)
{
    static constexpr std::uint32_t polynomial = 0xedb88320u;
    static std::array<std::uint32_t, 256> table = [] {
        std::array<std::uint32_t, 256> values {};
        for (std::uint32_t i = 0; i < values.size(); ++i) {
            std::uint32_t crc = i;
            for (int bit = 0; bit < 8; ++bit) {
                crc = (crc & 1u) != 0u ? (crc >> 1u) ^ polynomial : crc >> 1u;
            }
            values[i] = crc;
        }
        return values;
    }();

    std::uint32_t crc = 0xffffffffu;
    for (const unsigned char byte : data) {
        crc = (crc >> 8u) ^ table[(crc ^ byte) & 0xffu];
    }
    return crc ^ 0xffffffffu;
}

void write_stored_zip(const std::filesystem::path& path, const std::vector<ZipEntry>& entries)
{
    if (entries.empty()) {
        throw FastXlsxError("cannot write an empty ZIP package");
    }

    std::string output;
    std::vector<CentralDirectoryRecord> central_directory;
    central_directory.reserve(entries.size());

    for (const ZipEntry& entry : entries) {
        if (entry.name.empty()) {
            throw FastXlsxError("ZIP entry name cannot be empty");
        }

        const auto local_header_offset = checked_u32(output.size(), "local header offset");
        const auto name_size = checked_u16(entry.name.size(), "file name length");
        const auto data_size = checked_u32(entry.data.size(), "entry data size");
        const std::uint32_t crc = crc32(entry.data);

        append_u32(output, 0x04034b50u);
        append_u16(output, 20); // version needed to extract
        append_u16(output, 0); // flags
        append_u16(output, 0); // stored, no compression
        append_u16(output, 0); // file mod time
        append_u16(output, 0); // file mod date
        append_u32(output, crc);
        append_u32(output, data_size);
        append_u32(output, data_size);
        append_u16(output, name_size);
        append_u16(output, 0); // extra field length
        output += entry.name;
        output += entry.data;

        central_directory.push_back({entry.name, crc, data_size, local_header_offset});
    }

    const auto central_directory_offset = checked_u32(output.size(), "central directory offset");

    for (const CentralDirectoryRecord& record : central_directory) {
        const auto name_size = checked_u16(record.name.size(), "central file name length");

        append_u32(output, 0x02014b50u);
        append_u16(output, 20); // version made by
        append_u16(output, 20); // version needed to extract
        append_u16(output, 0); // flags
        append_u16(output, 0); // stored, no compression
        append_u16(output, 0); // file mod time
        append_u16(output, 0); // file mod date
        append_u32(output, record.crc);
        append_u32(output, record.size);
        append_u32(output, record.size);
        append_u16(output, name_size);
        append_u16(output, 0); // extra field length
        append_u16(output, 0); // file comment length
        append_u16(output, 0); // disk number start
        append_u16(output, 0); // internal file attributes
        append_u32(output, 0); // external file attributes
        append_u32(output, record.local_header_offset);
        output += record.name;
    }

    const auto central_directory_size =
        checked_u32(output.size() - central_directory_offset, "central directory size");
    const auto entry_count = checked_u16(central_directory.size(), "central directory entry count");

    append_u32(output, 0x06054b50u);
    append_u16(output, 0); // disk number
    append_u16(output, 0); // central directory disk
    append_u16(output, entry_count);
    append_u16(output, entry_count);
    append_u32(output, central_directory_size);
    append_u32(output, central_directory_offset);
    append_u16(output, 0); // comment length

    std::ofstream stream(path, std::ios::binary);
    if (!stream) {
        throw FastXlsxError("failed to open XLSX path for writing");
    }

    stream.write(output.data(), static_cast<std::streamsize>(output.size()));
    if (!stream) {
        throw FastXlsxError("failed to write XLSX package");
    }
}

} // namespace fastxlsx::detail
