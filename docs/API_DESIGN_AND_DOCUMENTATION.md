...
- Mutation semantics：`set_cell(ref, value)` 覆盖该 cell 的 active value；
  ...
  missing target 是 successful no-op；`erase_cell(ref)` 删除 active record；
...
- Mutation semantics: `set_cell()` replaces the active value; `erase_cell()`
  ...
  replacement cell. `clear_cell_value()` is the later style-preserving explicit
  blank API. Tombstones, row insertion/deletion, and metadata edits are still
  deferred.
...
- The first mutation API must keep the current internal semantics:
  ...
  slice; style-preserving clear is now the later `clear_cell_value()` API and
  still writes explicit blanks rather than tombstones.
