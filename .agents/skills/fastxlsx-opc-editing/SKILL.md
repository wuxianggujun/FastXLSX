...
- Windows production minizip-ng 对至少 4 MiB staged file chunk 使用两个固定 1 MiB buffer 做 bounded overlapped read-ahead；先发起下一块读取，再同步写当前块。小 chunk、raw compressed-copy、stored-only 与非 Windows 路径保持同步读取；本地 RAII 必须取消并等待 pending IO，不得留下后台 worker。性能报告需分列 activation、prefetched chunks/bytes、2 MiB peak buffer、foreground read/wait，并且 warmed run 不能冒充物理 OS cache-hit 证据。
