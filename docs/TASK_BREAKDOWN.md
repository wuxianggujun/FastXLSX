...
- **下一优先级**：以吞吐配置 level 1 继续压缩 single-pass transform/parser residual，优先从 source callback、action dispatch、coordinate/value parsing 与 file-range emission 的 CPU profile 选择真实性能瓶颈。只有 size-oriented workload 明确要求更高 level 且 minizip-ng 仍不满足时，才建立 matched-level backend comparison；不在无独立依赖、兼容性、CPU、输出大小和内存证据时引入 zlib-ng、并行压缩或自定义 backend。
