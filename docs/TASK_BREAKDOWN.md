...
- **下一优先级**：建立隔离的 minizip DEFLATE writer profile，拆分 write/close wall time、process CPU、调用粒度和调度波动，并在必要时用 ETW 区分 file-cache/IO wait；先解释 reverse numeric 噪声，再决定是否优化 writer handoff 或建立 matched-level backend comparison。保持 stage → package write → state commit、retry、CRC、bounded memory 和 public compression 默认值；不在无独立依赖、兼容性、CPU、输出大小和内存证据时引入 zlib-ng、并行压缩或自定义 backend。
