...
- `tests/test_worksheet_transformer.cpp` 通过 CTest 注册为
  ...
  duplicate selector、empty payload preflight，以及
  ...
  dimension recalculation、dependency repair、`PackageEditor` / `EditPlan` commit
  或低内存大文件编辑完成。
...
- `tests/test_package_editor.cpp` 通过 CTest 注册为 `fastxlsx.package_editor`，
  ...
  worksheet replacement 路径处理 fullCalcOnLoad / calcChain cleanup / audit，并验证
  missing selector 失败不污染状态。这只是 internal Patch MVP / handoff 回归，不是
  public `PackageEditor`、随机 cell 编辑、低内存 package-entry staged stream writer、
  dimension recalculation、sharedStrings 索引迁移、style id 迁移、styles 合并、
  relationship repair、table/drawing 语义同步或大文件 streaming worksheet transformer。
