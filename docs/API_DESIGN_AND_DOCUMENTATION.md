...
`CellPatchMissingCellPolicy::{Fail,Insert}`、兼容 wrapper `replace_or_insert_cells()`、
`save_as()`，以及首个 small-file
...
| Existing workbook Patch facade | 已公开窄切片 | `WorkbookEditor::open()`, source/planned catalog inspection, `CellPatchMissingCellPolicy`, `replace_sheet_data()`, `replace_cells()`, compatibility `replace_or_insert_cells()`, `replace_image()`, `rename_sheet()`, `save_as()`, coarse diagnostics | 不公开 `PackageEditor` / `EditPlan`；`replace_cells()` 默认只替换已存在 cells，显式 `CellPatchMissingCellPolicy::Insert` 只做 point upsert，能插入缺失 cells / 合成 minimal rows，但不 shift 行列、不 resize tables/filters/drawings/defined names；不做 relationship repair、sharedStrings/style migration、drawing/image semantic editing |
...
- `replace_cells(name, span<WorksheetCellUpdate>)`：按 current planned sheet name
  ...
  都必须 fail-before-public-diagnostic-update。`replace_or_insert_cells()` 只是
  Insert policy 的 compatibility wrapper，新代码优先使用 direct enum overload。
...
- `WorksheetEditor`：当前 public 首片提供 `name()`、`try_cell(row, column)`、
  ...
  iterator，不修改 dirty state，也不更新 `last_edit_error()`。
...
`replace_cells()` / `CellPatchMissingCellPolicy` / compatibility `replace_or_insert_cells()` /
`rename_sheet()` / `save_as()`。`replace_cells()` 复用 internal worksheet transformer 的
