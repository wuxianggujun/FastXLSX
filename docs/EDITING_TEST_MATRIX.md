...
| Generated chained formula rename QA | `py tools\run_workbook_editor_qa.py --scenario generated_formula_rename_chain_rewrite --excel-verify` | The chained local QA runner verifies `Data -> TemporaryData -> FinalData`: after the first catalog-only rename and second explicit rewrite policy, both original source-name references and current planned-name references are rewritten in direct definedNames and already-materialized worksheet formulas while external-workbook / 3D / string-literal / non-materialized formulas and calcChain absence stay unchanged. | Semantic formula interpretation, default formula rewrite, formula evaluation, cached result generation, calcChain rebuild, non-materialized worksheet rewrite, relationship repair, runtime Python dependency, or default CI fixture expansion. |
...
| `WorksheetEditor` materialized edits | Public small-file materialization tests | A selected worksheet can be materialized, inspected, edited sparsely, shifted by represented sparse row/column insert/delete helpers, and auto-flushed through `WorkbookEditor::save_as()`. Default style normalization, source sharedStrings materialization, and source-failure hygiene are covered in representative cases. | Complete Excel row/column structural edits, row/column metadata edits, table/range metadata recalculation, dense range editing, workbook-level guardrails, sharedStrings/style writeback migration, or large-file random editing. |
