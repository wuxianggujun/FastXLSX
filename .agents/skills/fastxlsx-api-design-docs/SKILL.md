...
`ColorScalePoint`、`TwoColorScaleRule`、`ImageEditAs`、`ImageAnchorOffset`、
`ImageOptions`、`WorkbookWriter::add_style()`、`CellView::with_style()`、
`WorksheetWriter::add_conditional_color_scale()` 和 `FastXlsxError`。
...
是流式写入骨架；data validation、external/internal hyperlink、two-color conditional
color scale 和 table API 是 worksheet metadata 基础切片，不等同完整 Phase 3 或完整
Phase 5。
...
- 当前 `WorksheetWriter` 骨架覆盖公式、行高、列宽、冻结窗格、自动筛选、
  合并单元格、data validations 和 two-color conditional color scales 的写入 XML；
  这些不代表完整 Phase 3 或
  Phase 5 功能集。
...
Excel formatting parity。当前 two-color color scale 是 worksheet metadata，不代表
...
`ColorScaleValueType` token、finite endpoint 边界、priority 规则、multi-range `sqref`
行为、是否新增 relationships/content types/styles/calc metadata、无公式求值、无冲突
检测、无 existing-file editing 和无完整 Excel UI。当前
`WorksheetWriter::add_conditional_color_scale()` 只覆盖 two-color color scale；不要写成
formula/cellIs、data bars、icon sets、dxf-backed styles 或完整 conditional formatting。
