...
- Patch conditional-format add/update/remove：add 覆盖 two-/three-color scale、basic data bar、basic `3Arrows` icon set、三类 add API 的 single/`span`/initializer-list multi-range、existing/same-session priority continuation、planned rename/added worksheet、`conditional_format_count`、schema-safe append 和 source order；update 另覆盖四种 rule 的 single/`span`/initializer-list、原 priority/source-order 保留、ranges + kind/payload replacement、current-effective index、prefixed QName 与 `conditional_format_update_count`；remove 覆盖 effective-index final removal 与 `conditional_format_removal_count`。另测 invalid rule/range/index、root/QName/nesting/schema/direct-child/priority failure-before-state-change，以及 advanced/custom/dxf/formula/cellIs/multiple-rule source payload exact preservation；同时核对 cells、relationships、content types、styles/dxf、`calcPr`/`calcChain` 与 unknown parts，覆盖 failure hook、failed save retry/reopen。T1 focused gate 只运行直接 conditional-format target 与共享不变量所需邻接 target；focused 未暴露跨模块风险时不跑 production 全量 CTest。
...
- C8 structural metadata synchronization 第一批覆盖 merged cells、root auto-filter 与 data-validation multi-range `sqref`。Focused gate 为 public-state insertion/deletion targets，并按共享边界追加 Patch metadata lifecycle、worksheet metadata/data-validation readers、Streaming metadata、merged-cell lifecycle 与 CellStore source-success 邻接 target；共同覆盖四轴 shift/expand/clip/full removal、merged single-cell removal、filter/validation single-cell preservation、empty validation/final container removal、count normalization、source/same-session Patch add/remove、formula/prompt/error XML escaping、prefixed QName/attribute-order/quote/child-byte preservation、global duplicate/overlap 与 64K aggregate-range rejection、filter criteria/sort child rejection、组合单次 rewrite/commit、空 CellStore metadata-only dirty、Excel-bound/schema/opaque-content no-pollution、PackageEditor staging-hook retry、formula/style/unknown-part preservation与 failed-save retry/reopen。下一 T1 子切片接 worksheet-local hyperlinks；conditional-format、table/drawing/comment/VML 在接入前保持明确 non-sync 断言，不默认升级 production 全量 CTest。
