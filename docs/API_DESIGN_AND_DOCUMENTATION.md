...
// worksheet 的 <sheetData>，定点替换已存在 cells，或定点 upsert 缺失 cells/rows，
// 再输出到新路径。
...
随机写 cell     不支持                  不作为大文件路径承诺      replace_cells(existing cells) / replace_or_insert_cells(point upsert) / WorksheetEditor::set_cell
...
`replace_or_insert_cells()`、`save_as()`，以及首个 small-file
...
| Existing workbook Patch facade | 已公开窄切片 | `WorkbookEditor::open()`, source/planned catalog inspection, `replace_sheet_data()`, `replace_cells()`, `replace_or_insert_cells()`, `replace_image()`, `rename_sheet()`, `save_as()`, coarse diagnostics | 不公开 `PackageEditor` / `EditPlan`；`replace_cells()` 只替换已存在 cells；`replace_or_insert_cells()` 只做 point upsert，能插入缺失 cells / 合成 minimal rows，但不 shift 行列、不 resize tables/filters/drawings/defined names；不做 relationship repair、sharedStrings/style migration、drawing/image semantic editing |
