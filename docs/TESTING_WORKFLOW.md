...
- C8 structural metadata synchronization：merged-cell 与 root auto-filter 子切片已完成。Focused gate 为 public-state insertion/deletion targets，并按共享边界追加 Patch auto-filter、worksheet metadata reader、Streaming metadata、merged-cell lifecycle 与 CellStore source-success 邻接 target；覆盖四轴 shift/expand/clip/full removal、merged single-cell removal、filter single-cell preservation、source/same-session filter、criteria/sort child rejection、完整 child-bearing filter removal、prefixed QName/attribute/quote/comment preservation、组合单次 rewrite/commit、空 CellStore metadata-only dirty、Excel-bound/schema/opaque-content no-pollution、PackageEditor staging-hook retry、formula/style/unknown-part preservation 与 failed-save retry/reopen。Data validations 作为下一独立 T1 切片补 multi-range `sqref` reader/serializer gate；hyperlink/conditional-format、table/drawing/comment/VML 在接入前保持明确 non-sync 断言，不默认升级 production 全量 CTest。
