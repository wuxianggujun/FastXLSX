...
- C8 structural metadata synchronization：merged-cell 子切片已完成，focused gate 为 public-state insertion/deletion targets、共享 planner 的 merged-cell lifecycle 与 CellStore source-success 邻接 target；覆盖四轴 before/after/intersection translate、完整删除/single-cell 退化/container cleanup、source order/count、prefixed root + dirty sheetData/row/cell namespace、空 CellStore metadata-only dirty、Excel-bound overflow、invalid-schema/opaque-content no-pollution、PackageEditor staging-hook retry、formula/style/unknown-part preservation与 failed-save retry/reopen。Auto filter 与 data validation 仍按后续独立 T1 切片补对应 reader/serializer 邻接测试；hyperlink/conditional-format、table/drawing/comment/VML 在接入前保持明确 non-sync 断言，不默认升级 production 全量 CTest。
