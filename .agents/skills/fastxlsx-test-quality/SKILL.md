...
Benchmark 不进默认 CTest。Release 只引用 `benchmarks/evidence/` 中通过 `validate_benchmark_evidence.py` 的 bundle；当前 2 个 production Streaming bundle 中，重复策略矩阵只支持 manifest 限定的同机同数据集比较，不支持泛化性能结论，Patch 仍需独立 evidence。
