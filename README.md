...
- **Patch 下一瓶颈**：staged-file 前台读取与 package-writer 输入调用粒度已完成证据化优化，剩余主要成本是同步 minizip DEFLATE writer。下一轮应区分 minizip internal output buffer/backend bookkeeping、zlib compression 与 scheduler noise，再决定是否做 matched-level backend comparison；事务、retry、CRC、bounded memory 与 public compression 默认值保持不变，没有独立兼容性、CPU、输出大小和内存证据前不引入并行压缩或自定义 backend。
