...
- `WorkbookEditor::replace_cells(..., CellPatchMissingCellPolicy::Insert)` 是当前
  ...
  按 source ranges 拼接 strict existing-cell replacements。它不是 public API，也不是
  当前 public Patch 的默认 algorithm switch、PackageEditor source-entry range reader
  或 source ZIP seek 能力。
