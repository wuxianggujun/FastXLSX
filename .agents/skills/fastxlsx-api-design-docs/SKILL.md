...
`ColorScalePoint`、`TwoColorScaleRule`、`ThreeColorScaleRule`、`ImageEditAs`、`ImageAnchorOffset`、
...
- 当前 `WorksheetWriter` 骨架覆盖公式、行高、列宽、冻结窗格、自动筛选、
  合并单元格、data validations 和 two-/three-color conditional color scales 的写入 XML；
  这些不代表完整 Phase 3 或
  Phase 5 功能集。
...
Excel formatting parity。当前 two-/three-color color scale 是 worksheet metadata，不代表
...
`WorksheetWriter::add_conditional_color_scale()` 只覆盖 two-/three-color color scale；不要写成
