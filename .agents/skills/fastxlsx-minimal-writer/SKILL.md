...
description: "实现或审查 FastXLSX Phase 1 最小可写 XLSX。用于 workbook、worksheet、content types、relationships、基础 sheetData、数字/字符串/布尔单元格、ZIP package 输出、默认 stored bootstrap 与 opt-in minizip-ng/zlib DEFLATE backend、小型 XML part 生成、本机 Excel 可视化验证和参考文件拆包 XML 对比。"
---
...
- `src/package_writer.*` 只是内部 package writer 边界。默认 backend 是
  `src/zip_store_writer.*` Phase 1 bootstrap；opt-in backend 是 minizip-ng/DEFLATE。
