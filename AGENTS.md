...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp`、
  ...
  `WorksheetWriter`、`CellView`、`StyleId`、`CellAlignment`、`CellFont`、`CellFill`、`CellStyle`、`DataValidationRule`、
...
number format、窄 `CellAlignment::wrap_text`、窄 `CellFont::bold` /
`CellFont::italic` 和窄 `CellFill` solid foreground ARGB fill。重复完整 style 复用同一个 `StyleId`；相同 number format
...
`fontId` 和 `applyFont="1"`，不代表完整 font control。solid fill 只写 `<fills>` 中的
...
该切片不支持字体颜色、字号、字体名、下划线、边框、完整 alignment、rich text、
...
- P9 styles number formats + wrap-text alignment + bold/italic fonts + solid fills：基础。`StyleId` / `CellAlignment` / `CellFont` / `CellFill` / `CellStyle` /
  `WorkbookWriter::add_style()` / `CellView::with_style()` 已支持 streaming-only
  new-workbook 自定义 number format、wrap-text alignment、bold/italic font 和 solid foreground fill 样式注册及 cell style 引用；`xl/styles.xml`
  ...
  `applyFill` / solid `<patternFill>` / `fgColor rgb`、`applyAlignment` / `wrapText`、默认 `s="0"` 省略、
...
- 当前 `fastxlsx.streaming` styles 推荐 preset 输出样例包括 number-format：
  ...
  number format + bold 复用同一个 `fontId`、number format + fill / font + fill 复用同一个 `fillId`、
  ...
  alignment、bold/italic font 和 solid fill 语义、可用时创建 `XlsxWriter` 参考 workbook；Excel helper 用本机 Excel
  COM 只读打开五个样例，核对 NumberFormat、WrapText、Font.Bold、Font.Italic、Interior.Pattern、
...
- 把 P9 styles 写成完整样式系统。当前只支持 workbook-local `StyleId`、自定义
  number format、窄 wrap-text alignment、窄 bold/italic font、窄 solid foreground fill
  和 `xl/styles.xml` 基础骨架；不支持字体颜色、字号、字体名、下划线、
...
- `fastxlsx-style-registry`：number format / wrap-text alignment / bold-italic font / solid fill styles、
  `StyleId`、`CellAlignment`、`CellFont`、`CellFill`、`CellStyle`、`xl/styles.xml`
  和样式 QA 验证。
