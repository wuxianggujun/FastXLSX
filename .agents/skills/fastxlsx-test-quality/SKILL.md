...
- 当前 streaming data validations 测试覆盖 `count`、`sqref`、`type`、`operator`、
  ...
  side effects 和 mutation-after-close。
...
5. 如果涉及 data validations，重点对比 worksheet XML 中的 `<dataValidations>`、
   ...
   `prompt`、`errorTitle`、`error` 等 prompt/error attributes。
...
/ content type 副作用，并创建 `openpyxl` / `XlsxWriter` 参考 workbook；Excel helper
只读打开 workbook 并核对 `ValidationPrompt!A2:D2` 的 validation 属性。Excel COM
会把 custom validation 公式返回为带 `=` 的形式，结构语义仍以拆包后的 worksheet XML 为准。
