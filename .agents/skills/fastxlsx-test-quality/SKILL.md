...
- 当前 streaming conditional color scale 测试覆盖 two-color
  `<conditionalFormatting>` XML、`cfvo` min/max 与 numeric/percentile endpoints、
...
- worksheet metadata：列宽、冻结窗格、自动筛选、合并单元格、data validations、
  external/internal hyperlinks、two-color conditional color scales 和 tables。
- dimension tracking。
...
和 multi-range 条件格式；可用时会用 `XlsxWriter` 创建参考 workbook。Excel helper
只读打开 basic 和 multi-range workbook，核对 one two-color scale 规则和多区域
AppliesTo。它们是本地 QA/排障工具，不接入默认 CTest/CI，也不是运行时依赖。
