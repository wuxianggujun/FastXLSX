...
- **Patch rewrite 现状**：schema-v11/v5 pass-through profile 已把 1,000,000-cell numeric/mixed-inline/formula + external hyperlink level-1 upsert 的 transform action/output append 降低 99.82%。最新 schema-v12/v6 mixed-inline 同机同刻 A/B 进一步把 total/transform/source scan median 从 1.525 秒/0.789 秒/740,456 us 降至 1.306 秒/0.594 秒/572,988 us，分别降低 14.36%/24.71%/22.62%；333,327 个 simple inline payload 全部命中 canonical literal path，6 个跨窗口 candidate 保留原 parser，median process peak working set 只增加约 0.004 MB。该结论只覆盖记录的顺序 upsert workload，不是任意随机编辑承诺。
...
- **Patch 下一瓶颈**：canonical inline-string path 完成后，记录的 optimized mixed-inline source scan/action median 仍约 0.573 秒，而 transform action/output append 已降到约 2,100 次。下一轮应 profile generic worksheet tag 与 cell-boundary scanning，temporary output IO 为次级；只有 size-oriented workload 在 matched level 下仍受压缩限制时，才评估其他 backend。
