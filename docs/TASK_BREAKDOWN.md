...
chunk/window input first slice 已有测试，transformer / PackageEditor 仍未接入完整低内存输入。
...
- Transformer action stream 仍接受完整 worksheet `std::string_view`，尚未消费 chunk/window
  event stream；真正 rewrite validation 仍不是端到端低内存。
...
2. 让 event reader 消费 streaming chunks 或 bounded windows，而不是完整 worksheet string。
   当前 first slice 已完成；下一步是 transformer action stream 消费该输入。
...
可并行性：
- 后续 P8.19 event reader chunk/window input 已完成；transformer chunk-event adapter
  设计可并行只读调研。
...
- 文档明确该 guard 只是 bounded materialized validation input，不是 complete
  PackageReader input streaming、transformer consumption of event-reader chunk/window input、
  low-memory worksheet transformer、relationship repair、sharedStrings/style migration
  或 public API。
...
可并行性：
- 后续 transformer chunk event adapter 设计可并行只读调研。
...
- 文档明确该 slice 不是 transformer chunk input、PackageEditor 低内存 validation、
  PackageReader input streaming、relationship repair、sharedStrings/style migration 或 public API。
