...
void test_materialized_flush_uses_shared_strings_when_relationship_target_normalizes_to_part()
{
    MaterializedFlushTwoSheetSourcePackage source =
        write_two_sheet_materialized_flush_source_package(
            "fastxlsx-workbook-editor-materialized-flush-two-sheet-shared-normalized-target-source.xlsx",
            true);
    std::map<std::string, std::string> entries =
        read_stored_package_entries(source.path);
    ...
    check(workbook_relationships != entries.end(),
        "normalized sharedStrings target source should contain workbook relationships");
    const std::string package_target = R"(Target="sharedStrings.xml")";
    const std::string normalized_target = R"(Target="worksheets/../sharedStrings.xml")";
    const std::size_t target_position =
        workbook_relationships->second.find(package_target);
    check(target_position != std::string::npos,
        "normalized sharedStrings target source should contain a sharedStrings relationship target");
    workbook_relationships->second.replace(
        target_position, package_target.size(), normalized_target);
    fastxlsx::test::write_stored_zip_entries(source.path, entries);
    const std::map<std::string, std::string> source_entries =
        read_stored_package_entries(source.path);
    ...
    check(result.flushed_worksheet_count == 2,
        "normalized sharedStrings target flush should report both worksheets");
    check(!data.dirty() && !other.dirty(),
        "normalized sharedStrings target flush should clear both sessions");
    check(registry.dirty_session_count() == 0,
        "normalized sharedStrings target flush should clear dirty diagnostics");

    const std::filesystem::path output = fastxlsx::test::artifact_path(
        "fastxlsx-workbook-editor-materialized-flush-two-sheet-shared-normalized-target-output.xlsx");
    const std::filesystem::path noop_output = fastxlsx::test::artifact_path(
        "fastxlsx-workbook-editor-materialized-flush-two-sheet-shared-normalized-target-noop-output.xlsx");
    editor.save_as(output);
    ...
    check(data_worksheet.find(R"(<dimension ref="A1:B1"/>)") != std::string::npos,
        "normalized sharedStrings target flush should update Data dimensions");
    check(data_worksheet.find(R"(<c r="A1" t="s"><v>0</v></c>)")
            != std::string::npos,
        "normalized sharedStrings target flush should reuse the source string index");
    check(data_worksheet.find(R"(<c r="B1" t="s"><v>1</v></c>)")
            != std::string::npos,
        "normalized sharedStrings target flush should use the first appended string index");
    check(data_worksheet.find("inlineStr") == std::string::npos,
        "normalized sharedStrings target flush should not inline Data text");

    check(other_worksheet.find(R"(<dimension ref="A1:C3"/>)") != std::string::npos,
        "normalized sharedStrings target flush should update Other dimensions");
    check(other_worksheet.find(R"(<c r="A1" t="s"><v>1</v></c>)")
            != std::string::npos,
        "normalized sharedStrings target flush should reuse appended indexes across sheets");
    check(other_worksheet.find(R"(<c r="B2" t="s"><v>2</v></c>)")
            != std::string::npos,
        "normalized sharedStrings target flush should append later sheet-only text");
    check(other_worksheet.find(R"(<c r="C3"><v>33</v></c>)") != std::string::npos,
        "normalized sharedStrings target flush should keep non-text cells value-only");
    check(other_worksheet.find("inlineStr") == std::string::npos,
        "normalized sharedStrings target flush should not inline Other text");

    check(shared_strings.find(R"(count="4")") != std::string::npos &&
            shared_strings.find(R"(uniqueCount="3")") != std::string::npos,
        "normalized sharedStrings target flush should count cross-sheet references and unique strings");
    check(shared_strings.find(
              R"(<si><t>existing</t></si><si><t>cross-sheet-new</t></si><si><t>other-only</t></si>)")
            != std::string::npos,
        "normalized sharedStrings target flush should append each cross-sheet text once");
    check(output_workbook_relationships == source_entries.at("xl/_rels/workbook.xml.rels"),
        "normalized sharedStrings target flush should not repair workbook relationships");
    check_reopened_multi_session_appended_shared_strings_output(output);
    ...
    check_materialized_flush_noop_save_is_stable(
        ...
        output,
        noop_output,
        "normalized sharedStrings target flush no-op save should keep output byte-stable",
        "normalized sharedStrings target flush no-op save should keep registry clean");
    check_reopened_multi_session_appended_shared_strings_output(noop_output);
    check(!data.dirty() && !other.dirty(),
        "normalized sharedStrings target flush no-op save should keep both sessions clean");
    check(read_stored_package_entries(source.path) == source_entries,
