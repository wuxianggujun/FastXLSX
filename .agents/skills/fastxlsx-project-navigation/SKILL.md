...
- 参考库范围不限 C++。`docs/TECHNICAL_COMPARISON.md` 维护跨语言 XLSX 参考矩阵；
  参考其他库时必须说明吸收的是 In-memory UX、Streaming writer、Patch editor、
  OPC package 语义、兼容性 fixture 还是 benchmark 经验。
- 当前阶段计划的默认推进线是先完成 `P4.0 API surface unification`，统一
  public facade、命名、`CellView` / `Cell` / future `CellValue` 边界和
  internal/public 分界，再推进窄 Patch MVP、preservation fixture、dependency
  policy 和后续 In-memory 子任务。Phase 2 writer/backend/sharedStrings/benchmark
  工作作为并行支撑线继续硬化。不要把性能硬化解释成“编辑能力以后再说”，也不要把
  Patch 做成 streaming writer 的事后补丁。
