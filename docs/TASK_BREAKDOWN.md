...
- **下一功能主线**：推进 existing-workbook external hyperlink 编辑（worksheet `.rels` relationship mutation）和 data-validation 编辑。每项先定义 preserve/audit/fail/edit、worksheet `.rels`、content types、row-order、formula/structure side effects 和失败恢复，不把 internal hyperlink 的 relationship-free 边界扩大成通用对象编辑。
