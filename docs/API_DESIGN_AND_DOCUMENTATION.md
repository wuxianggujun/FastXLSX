...
- **Supported source values:** blank, number, boolean, `t="str"` scalar text,
  formula text without stale cached values (including `t="str"` formula cells),
  plain inline strings, simple inline rich text flattened to plain text, and
  workbook-backed `t="s"` shared-string indexes resolved through the existing
