...
- `WorksheetEditorOptions` 默认 `RejectKnownLosses`；rich/phonetic/extension、formula metadata、cached result 等已知损失必须失败。只有显式 `AllowLossyProjection` 才能拍平，且 policy 是 session identity 的一部分。
...
- Patch：failure-before-state-change、retry、reopen、unknown part、relationship/content type/calc side effects。
- In-memory：strict/lossy、policy mismatch、guardrail、no-state-pollution、dirty flush、move/handle lifecycle。
