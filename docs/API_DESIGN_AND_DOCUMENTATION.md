...
custom number format、窄 wrap-text alignment 和窄 bold/italic font slices。`StyleId` 是 workbook-local handle，默认 id `0`
...
不贡献 style 属性。`CellFont::bold` / `CellFont::italic` 是当前唯一 font 子能力，false
flags 或空 optional 不贡献 style 属性。`WorkbookWriter::add_style()` 应拒绝完全空的
...
alignment。bold/italic font 只写 `<fonts>` 中的 `<b/>` / `<i/>`、`fontId` 和
`applyFont="1"`，不代表完整 font control。当前不支持 font color、size、family、
underline、fill/border/full alignment、rich text、dxf-backed
...
- styles API 还需验证文档注释写清 workbook-local `StyleId`、foreign id 拒绝、
  ...
  bold/italic font 不是完整 font control，
