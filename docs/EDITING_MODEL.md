...
- 当前 `replace_worksheet_cells()` 对 source package worksheet entry 已通过
  ...
  直接从 source ZIP payload 分块读取并做 incremental CRC；DEFLATE entry 仍暂复用
  materialized `read_entry()` fallback。P8.19 已新增
  ...
  source entry 接到 direct ZIP-entry chunk source。queued string 仍由 prior planned
  replacement helper 持有完整 `std::string`，DEFLATE source 仍会 materialize，所以这不是
  full input low-memory pipeline；event-reader `max_window_bytes` 仍约束单个未完成 token /
  retained window。
...
P8.16-P8.28 又补上 cell replacement 输出侧 file-backed chunk handoff、source-entry
...
但 queued string 仍是 prior helper 已持有的完整字符串，且 DEFLATE input 仍是
materialized fallback，因此不能写成完整 low-memory large worksheet transformer。
本轮 C5 验收只覆盖这些基础片：`fastxlsx.package_reader` 验证 source-entry
`extract_entry_to_file()`、direct stored entry chunk source 和 CRC failure，
...
DEFLATE PackageReader input streaming 仍是后续任务。
