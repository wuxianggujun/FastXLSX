...
- `request_full_calculation()`、`rename_sheet()`、internal document-properties rewrite 与 internal part removal 的跨 plan/replacements/omitted entries/manifest/public diagnostics 变更先在副本完成，再以 noexcept swap 提交；staging 失败不能留下部分 mutation，既有 patch 必须保持可保存、可重试。
