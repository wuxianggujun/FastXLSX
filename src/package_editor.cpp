...
constexpr std::string_view worksheet_cell_replacement_analysis_input_context =
    ...
    PartName part_name, std::vector<PackageEntryChunk> chunks, std::string reason)
{
    ...
    upsert_part_replacement_chunks(replacements_, part_name, std::move(chunks),
        PartWriteMode::StreamRewrite, reason);
    manifest_.set_part_write_mode(part_name, PartWriteMode::StreamRewrite);
    edit_plan_.set_part(part_name, PartWriteMode::StreamRewrite, reason);
    restore_active_part_entry_state_after_replacement(edit_plan_, entry_replacements_,
        omitted_entries_, reader_, manifest_, part_name);
}
