...
明细的唯一权威来源，内容从 [编辑模型](EDITING_MODEL.md) 和
[任务计划](TASK_PLAN.md) 中逐字迁移而来，便于这两份设计文档只保留契约、
边界和能力摘要。
...
同一 background picture 路径还覆盖 repeated ordinary replacement 状态卫生：后续
replacement 会覆盖 stale picture bytes、manifest write-mode 和 edit-plan reason，
保留 PNG default content type，不把 `xl/media/background.png` 提升为 override，
不改写 worksheet `.rels`、不留下 removed-part / removed package-entry audit，
且不凭空创建 `xl/media/_rels/background.png.rels`；输出包可由 `PackageReader`
重读，并保持 sibling header/footer VML bytes。该回归只证明同一路径最终状态
upsert 与 no-pruning，可见性不是图片语义合并、relationship repair、content type
repair、existing-file image editing 或 public API；
...
同一 worksheet-owned object fixture 还覆盖 repeated ordinary replacement 状态卫生：
重复替换 `xl/embeddings/oleObject1.bin` 和 `xl/ctrlProps/control1.xml` 时，最后一次
replacement 会覆盖 stale payload、manifest write-mode 和 edit-plan reason，保留
各自 content type override、保留 worksheet `.rels` inbound relationship 和 sibling
对象 bytes，不留下 removed-part / removed package-entry audit，也不凭空创建
`xl/embeddings/_rels/oleObject1.bin.rels` 或
`xl/ctrlProps/_rels/control1.xml.rels`；输出包可由 `PackageReader` 重读。这仍只是
same-path final-state hygiene，不是 OLE / ActiveX / control 语义编辑、relationship
repair/pruning、content type repair、orphan cleanup、事务式 undo 或 public API；
...
`PackageReader::extract_entry_to_file()` 抽取到 file-backed source，再进入当前
event reader validation。planned worksheet replacement input 仍会物化 current planned
worksheet XML，DEFLATE extraction 暂不代表低内存 compressed input。`save_as()` 后输出
可由 `PackageReader` 重读，并验证 dimension refresh、calcChain cleanup、workbook
...
styles migration、table/drawing sync、event reader chunk/window input、PackageReader
完整输入侧 streaming，或完整 low-memory large-file editing。
...
同一路径现在还覆盖 repeated ordinary threaded comments replacement：连续两次
`replace_part("/xl/threadedComments/threadedComment1.xml", ...)` 只保留最终 threaded
comments XML、最终 write mode / reason、threaded comments content type override、
worksheet inbound threadedComment relationship、legacy comments、persons、workbook /
worksheet `.rels` 和 unknown entry，并清理 stale reason、removed-part audit 与 owner
`.rels` omission/audit，且不凭空创建 threaded comments owner `.rels`。这仍只是 Patch
audit，不是 threaded comments model mutation、persons/schema repair、notes UI、
relationship repair、content type repair、orphan cleanup 或 public API；
...
同一路径现在还覆盖 repeated ordinary persons replacement：连续两次
`replace_part("/xl/persons/person.xml", ...)` 只保留最终 persons XML、最终 write mode /
reason、persons content type override、workbook inbound persons relationship、threaded
comments、legacy comments、workbook / worksheet `.rels` 和 unknown entry，并清理 stale
reason、removed-part audit 与 owner `.rels` omission/audit，且不凭空创建 persons owner
`.rels`。这仍只是 Patch audit，不是 persons/schema repair、threaded comments model
mutation、notes UI、relationship repair、content type repair、orphan cleanup 或 public API。
...
同一路径现在还覆盖 repeated ordinary pivot table replacement：连续两次
`replace_part("/xl/pivotTables/pivotTable1.xml", ...)` 只保留最终 pivot table payload、
最终 write mode / reason、pivot table owner `.rels` copy-original audit、
content type override、workbook / worksheet relationships、pivot cache definition /
records 链和 unknown entry，并清理 stale reason、removed-part audit 与 removed owner
`.rels` omission。这不是 pivot table 语义合并、pivot cache rebuild、relationship
...
同一路径现在还覆盖 repeated ordinary pivot cache definition replacement：连续两次
`replace_part("/xl/pivotCache/pivotCacheDefinition1.xml", ...)` 只保留最终 cache
definition payload、最终 write mode / reason、owner `.rels` copy-original audit、
content type override、workbook / pivot table inbound relationships、pivot table、
cache records、worksheet 和 unknown entry，并清理 stale reason、removed-part audit
与 removed owner `.rels` omission。这不是 pivot cache 语义合并、records refresh、
relationship pruning/repair、content type repair、orphan cleanup 或 public API。
...
relationship pruning/repair、content type repair、orphan cleanup 或 public API。
同一路径现在还覆盖 repeated ordinary pivot cache records replacement：连续两次
`replace_part("/xl/pivotCache/pivotCacheRecords1.xml", ...)` 只保留最终 records
payload、最终 write mode / reason、records content type override、cache definition
owner `.rels` 中的 inbound relationship、pivot cache definition、pivot table、
workbook、worksheet 和 unknown entry，并清理 stale reason、removed-part audit，且不
凭空创建 records owner `.rels` audit / omission。这不是 pivot cache records 语义合并、
records refresh、relationship pruning/repair、content type repair、orphan cleanup 或
public API。
内部 `planned_output()` 快照还覆盖该 remove-then-replace restore 状态：暴露
...
content type repair、orphan cleanup 或 public API。
同一路径还覆盖 repeated ordinary externalLink replacement：连续两次
`replace_part("/xl/externalLinks/externalLink1.xml", ...)` 只保留最终
externalLink XML、最终 write mode / reason、owner `.rels` copy-original audit、
source content types / workbook `.rels` / worksheet / unknown entry，并清理 stale
reason、removed-part audit 和 removed owner `.rels` omission。这不是 external links
语义合并、外部数据刷新、relationship pruning/repair、content type repair、
orphan cleanup 或 public API。
内部 `planned_output()` 快照还覆盖该 remove-then-replace restore 状态：
...
同一路径现在还覆盖 repeated ordinary custom XML item replacement：连续两次
`replace_part("/customXml/item1.xml", ...)` 只保留最终 custom XML payload、最终
write mode / reason、owner `.rels` copy-original audit、package relationships、
content types、workbook、worksheet、properties part 和 unknown entry，并清理 stale
reason、removed-part audit 和 removed owner `.rels` omission。这不是 custom XML
语义合并、schema/data binding、relationship pruning/repair、content type repair、
orphan cleanup 或 public API。
...
content type repair、orphan cleanup 或 public API。
同一路径现在还覆盖 repeated ordinary custom XML properties replacement：连续两次
`replace_part("/customXml/itemProps1.xml", ...)` 只保留最终 properties payload、
最终 write mode / reason、properties content type override、custom XML item 与 item
owner `.rels`、package relationships、content types、workbook、worksheet 和 unknown
entry，并清理 stale reason、removed-part audit，且不凭空创建 properties owner
`.rels` audit / omission。这不是 custom XML properties 语义合并、schema/data
binding、relationship pruning/repair、content type repair、orphan cleanup 或 public API。
...
同一路径现在还覆盖 repeated ordinary comments replacement：连续两次
`replace_part("/xl/comments/comment1.xml", ...)` 只保留最终 comments XML、最终
write mode / reason、comments content type override、worksheet inbound comments
relationship、workbook / worksheet / unknown entry copy-original，并清理 stale reason、
removed-part audit 和 owner `.rels` omission/audit，且不凭空创建 comments owner
`.rels`。这仍只是 Patch audit，不是 comments 语义合并、notes UI、relationship
repair、content type repair、orphan cleanup 或 public API。
...
records owner `.rels` 快照、externalLink repeated replacement 的 active final
externalLink local-DOM rewrite / owner `.rels` copy-original / preserved content
types、workbook `.rels`、worksheet 与 unknown entry / empty removed_parts 与
removed_package_entries 快照、custom XML repeated replacement 的 active final item
local-DOM rewrite / owner `.rels` copy-original / preserved package relationships、
content types、properties part、workbook、worksheet 与 unknown entry / empty
removed_parts 与 removed_package_entries 快照、custom XML properties repeated
replacement 的 active final properties local-DOM rewrite / properties content type
override / preserved item owner `.rels` / no invented properties owner `.rels` /
empty removed_parts 与 removed_package_entries 快照、comments ordinary replacement 的 active comments part
...
当前 C2 charts / tables / drawings / media linked-part preservation 批次已完成覆盖核对：
ordinary drawing replacement、drawing explicit removal、drawing remove/replacement
ordering、media replace/remove/ordering、table replace/remove/ordering、chart
replace/remove/ordering，以及 percent-decoded drawing replace/remove/ordering 均有
planned_output 快照、content type side effect、no invented owner `.rels`、inbound
relationship audit 和 output `PackageReader` 重读证据。这仍只是 preservation /
audit 证据，不是 drawing、image、table 或 chart 语义编辑。
...
基线；内部 output-plan 快照现在也覆盖该 ordinary replacement 状态，暴露 active
media `StreamRewrite`、replacement reason、content types / package relationships /
workbook / workbook `.rels` / worksheet / worksheet `.rels` / drawing / drawing
`.rels` / chart / table / VML / percent-decoded drawing / sharedStrings /
sharedStrings owner `.rels` / styles / VBA / calcChain / unknown extension /
unknown owner `.rels` entries 的 copy-original 决策、drawing `.rels` 与 unknown
owner `.rels` 的 source-owned metadata context、空 removed/audit 污染，以及不凭空
创建 media owner `.rels`；这不是图片解码、drawing mutation、public output planner
或 existing-workbook image editing。
...
copy-original 基线；内部 output-plan 快照现在也覆盖该 ordinary replacement 状态，
暴露 active table `LocalDomRewrite`、content types / worksheet `.rels`
copy-original、preserved worksheet/drawing/media/unknown entries、无 removed/audit
污染，以及不凭空创建 table owner `.rels`；这不是 table resize、calculated columns、totals generation
...
语义编辑。
同一路径现在还覆盖 repeated ordinary sharedStrings replacement：连续两次
`replace_part("/xl/sharedStrings.xml", ...)` 只保留最后一次 XML payload / reason /
`StreamRewrite` write mode，`EditPlan`、manifest 和 `planned_output()` 不留下 stale
removed-part audit、stale owner `.rels` omission 或 dependency audit；`save_as()` 后
`PackageReader` 可重读最终 sharedStrings XML，并继续保留 source-owned
`xl/_rels/sharedStrings.xml.rels`、workbook inbound sharedStrings relationship、
content types/workbook/package relationships 和未触碰 linked entries。
这仍只是 ordinary part replacement 的状态卫生回归，不是 sharedStrings index
migration、字符串表重建、worksheet cell 引用同步、relationship repair 或
transactional undo。
内部 `planned_output()` 快照还覆盖该普通 replacement 状态：暴露 active
...
或完整样式编辑。
同一路径现在还覆盖 repeated ordinary styles replacement：连续两次
`replace_part("/xl/styles.xml", ...)` 只保留最后一次 styles XML payload / reason /
`LocalDomRewrite` write mode，`EditPlan`、manifest 和 `planned_output()` 不留下 stale
removed-part audit、stale reason、owner `.rels` omission/audit 或 dependency audit；
`save_as()` 后 `PackageReader` 可重读最终 styles XML，并继续保留 workbook inbound
styles relationship、styles content type override、sharedStrings / owner `.rels`、
content types/workbook/package relationships 和未触碰 linked entries。
这仍只是 ordinary part replacement 的状态卫生回归，不是 style id migration、
样式合并、cell `s` 引用同步、relationship repair 或 transactional undo。
内部 `planned_output()` 快照还覆盖该普通 replacement 状态：暴露 active
...
preservation。
同一路径现在还覆盖 repeated ordinary VBA project replacement：连续两次
`replace_part("/xl/vbaProject.bin", ...)` 只保留最后一次 replacement bytes /
reason / `StreamRewrite` write mode，`EditPlan`、manifest 和内部
`planned_output()` 不留下 stale removed-part、stale reason、owner `.rels`
omission/audit 或 dependency audit；`save_as()` 后 `PackageReader` 可重读最终
VBA bytes，并继续保留 workbook inbound VBA relationship、VBA content type
override、content types/package/workbook relationships 和未触碰 linked entries。
这仍只是 ordinary part replacement 的状态卫生回归，不是 macro generation、
VBA semantic editing、signature preservation、relationship repair 或 transactional
undo。
