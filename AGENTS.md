...
- `include/fastxlsx/fastxlsx.hpp`、`include/fastxlsx/workbook.hpp` 和
  ...
  `CellView`、`DataValidationRule`、`DataValidationType`、
  `DataValidationOperator` 和 `FastXlsxError`。
...
合并单元格和 data validations 是当前写入骨架能力，不等同完整 Phase 3
或完整 Phase 5。当前可见
...
- Phase 5 worksheet metadata：基础。`WorksheetWriter::add_data_validation()`
  ...
  XLSX，也不新增 relationships 或 content types。hyperlinks 需要 worksheet
  `.rels` 和 relationship graph 后再实现，不能因为 `RelationshipSet` 可序列化
  external target 就宣称支持。
