...
- **Strict failure groups:** non-default or malformed source styles,
  ...
  unsupported value-wrapper shapes, non-whitespace direct row text outside
  cells, non-whitespace direct cell text outside `<v>` / `<t>` / `<f>` wrappers,
  cell-internal comments / processing
