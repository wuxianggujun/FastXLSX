...
- **Patch**：`WorkbookEditor`，面向已有 workbook 的 part-level rewrite，并支持受限的 basic classic note、writer-compatible data-validation add/remove、conditional-format add/remove 与 table metadata lifecycle；未修改和未知 part 默认保留。
...
- `WorkbookEditor::add_conditional_color_scale()` / `add_conditional_data_bar()` / `add_conditional_icon_set()` 可向 planned worksheet 追加 writer-compatible conditional formatting；`remove_conditional_format()` 可按当前 effective worksheet 的 zero-based `WorksheetConditionalFormatView::index` 删除一条 writer-compatible 单规则容器，same-session additions/removals 会改变后续 index，最后一条删除整个 `<conditionalFormatting>`，其余 priority 不重排。Color scale 支持 two-/three-color，三类 API 均支持 single range、`span` 与 initializer-list multi-range overload。Patch add/remove 均先做 bounded structural audit；remove 额外执行 strict projection，advanced/custom、`dxf`/styles、formula/cellIs 与 multiple-rule source payload 不接管并明确 fail。Add/remove 支持 planned rename/added worksheet，只改 worksheet XML，不改 cells、relationships、content types、styles/dxf、calc metadata 或 linked objects；`pending_worksheet_edits()` 分别暴露 `conditional_format_count` 与 `conditional_format_removal_count`，失败不污染状态且可 retry。
