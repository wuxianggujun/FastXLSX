...
- `WorksheetEditor` row/column insert/delete 已同步 worksheet-root merged ranges、root auto-filter 与 strict writer-compatible data-validation multi-range `sqref`：CellStore/formula candidate、完整 `<mergeCells>` replacement/removal、opening-tag-only `<autoFilter ref>` replacement/removal 和 validation child/container replacements 分开预构造，再合并为一次 file-backed worksheet rewrite 与一次 `PackageEditor` commit，最后 noexcept 发布 session 和 public filter diagnostic map。插入支持 shift/expand；删除支持 shift/clip/full removal，merged single-cell result 移除而 filter/validation single-cell result 保留；空 validation 与最终空 container 删除，surviving rule/formula/prompt/error bytes 保留。Surviving changed filter 含 criteria/sort element child 时 fail；validation strict projection、64K aggregate range、duplicate/overlap、schema/count/QName/bounds/child-semantics/staging 失败不污染 package/session 并可 retry。Hyperlinks、conditional formatting、tables、drawings、comments/VML 与其他 linked metadata 尚不自动同步。
