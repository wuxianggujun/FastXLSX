...
- `src/opc.cpp`、`src/package_writer.cpp`、`src/workbook.cpp`、
...
不能写成生产策略完成。当前默认写出基础 `docProps/core.xml` 和
`docProps/app.xml` 小型 XML part；这只是静态文档属性元数据，不是完整
document properties public API。`PackageReader`、`CellEncoder` 等名称仍主要
...
默认无依赖构建仍由 stored ZIP bootstrap 支撑；启用
`FASTXLSX_ENABLE_MINIZIP_NG` 时会使用 minizip-ng DEFLATE backend。它仍不是
已有文件编辑用 public `PackageWriter`。`PackageReader`、已有文件编辑和
unknown part preservation 仍是计划。当前已有内部
...
- package writer boundary：基础。`src/package_writer.*` 已把新建 workbook
  输出从 workbook writer 代码中隔离出来；默认 backend 是
  `src/zip_store_writer.*` 的 stored/no-compression bootstrap，opt-in backend 是
  `minizip-ng[core,zlib]` DEFLATE。不要据此宣称 Zip64、true package streaming、
  existing-file editing 或大文件性能。
- OPC edit plan：基础。当前是 internal manifest、relationships、content types、
  ...
  和 preservation 测试闭环。
- Phase 5 worksheet metadata：计划。data validations 可作为 streaming-only
  新建文件切片推进；hyperlinks 需要 worksheet `.rels` 和 relationship graph
  后再实现，不能因为 `RelationshipSet` 可序列化 external target 就宣称支持。
