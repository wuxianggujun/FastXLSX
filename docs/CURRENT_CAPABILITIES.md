...
- `WorksheetEditor::set_cells()`、`set_row()`、`set_column()` 与对应 value-only batch/prefix writes 先完成 coordinate/style validation 与 duplicate later-wins/style-policy 归并，再按最终受影响 sparse records 判断 dirty；全部最终记录与 active state 相同则在 CellStore guardrail 前 clean no-op，不发布中间 duplicate value、临时 erase 或虚假 materialized handoff。实际变化的候选仍须整体通过 count/memory guardrail 才发布。
