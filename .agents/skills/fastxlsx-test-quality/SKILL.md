...
- 当前 P9 number-format + wrap-text alignment styles 推荐 preset 样例为
  ...
  `XlsxWriter` 检查和 Excel COM 只读 NumberFormat / WrapText 检查，不接入默认
  CTest/CI，也不是运行时依赖。
...
`--memory-input` / `-MemoryPath`；memory-source helper 检查 media bytes、package XML、
