...
- **下一功能主线**：先完成 `remove_worksheet()` 契约与测试设计，再实现最小事务切片。必须明确 last-visible-sheet、active/selected sheet、defined names、formula references、materialized handles、owned relationships 和 orphan parts 策略；unsupported linked semantics 默认 fail，不静默删除或遗留错误引用。完成 worksheet lifecycle 后，再推进 existing-workbook hyperlink/data-validation 等 metadata 窄编辑。
