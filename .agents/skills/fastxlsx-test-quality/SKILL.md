...
- 当前 streaming data validations 测试覆盖 `count`、`sqref`、`type`、`operator`、
  ...
  worksheet 不声明 `xmlns:r`、`formula2` XML text escape 和 mutation-after-close。
- 当前 streaming external hyperlinks 测试覆盖 worksheet `<hyperlinks>`、worksheet
  ...
  type override absence、invalid cell、empty target 和 mutation-after-close。
- 当前 streaming internal hyperlinks 测试覆盖 worksheet `<hyperlink location="...">`、
  ...
  消耗、不污染 workbook relationships 或 content types、invalid cell、empty location 和
...
8. 如果涉及 tables，重点对比 `xl/tables/table*.xml`、worksheet `<tableParts>`、
   `xl/worksheets/_rels/sheet*.xml.rels`、relationship `Type` / `Target`、
   table content type override、table `id` / `name` / `displayName` / `ref`、
   `autoFilter`、`tableColumns` 和 `tableStyleInfo`。
9. 不要求 byte-level 完全一致，重点比较 OpenXML 语义、关系、content type、
