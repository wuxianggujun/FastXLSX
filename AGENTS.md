...
- `WorkbookEditor` conditional-format add 只追加 writer-compatible two-/three-color scale、basic data bar 与 basic `3Arrows` icon set。Existing source 只做 bounded structural/priority audit；advanced/custom、dxf/styles、formula/cellIs 与 multiple-rule payload 原样保留且不投影/接管，root/QName/nesting/schema/direct-child 或 priority 异常 fail。新 priority 从 effective max + 1 分配；它支持 planned rename/added worksheet，只改 worksheet XML，不改 cells、relationships、content types、styles/dxf、calc metadata 或 linked objects；`conditional_format_count`、pending/unsaved 与 replacement 先 staging 后 commit，失败不污染并可 retry。
