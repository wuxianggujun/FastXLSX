...
source XLSX
  ...
  -> worksheet: bounded WorksheetEventReader -> active conditional-format projection
  -> worksheet: bounded WorksheetEventReader + owner .rels -> table relationship ids
     -> linked table part: bounded XML scanner -> active basic table projection
  -> worksheet: bounded WorksheetEventReader + owner .rels -> drawing relationship
     -> drawing part: bounded XML scanner + drawing .rels -> active picture projection
     -> unique media entry: complete bounded chunk drain -> ZIP/signature audit
  -> worksheet: bounded WorksheetEventReader + owner .rels -> comments relationship
     -> comments part: bounded XML scanner -> retained authors + active simple note
     -> optional legacy VML relationship/part presence audit (payload opaque)
...
`WorkbookReader` 只在 `open()` 保留小型 package/workbook catalog；每次 traversal 独占一个 stored/DEFLATE entry source，完成或异常退出后立即释放。Worksheet projector 只保留当前 row/cell；metadata projector 只保留 bounded element/view-id stack 与受 count guardrail 限制的 merged ranges，用于结束前的 overlap audit；data-validation projector 只保留当前 owning rule/ranges、formula/prompt/error text 与 bounded element stack；hyperlink projector 只保留当前 owning record、受 count guardrail 限制的 ranges、relationship namespace scope，并按需查询当前 worksheet 的小型 relationship set；conditional-format projector 只保留当前 owning rule/ranges、bounded element stack 和受 guardrail 限制的 counters；table projector 先保留受 count/id guardrail 限制的 worksheet table relationship ids，再逐 part 只保留当前 owning table/header columns、bounded element stack 与受限去重集合；image projector 只保留 worksheet drawing id、当前 owning picture/anchor、bounded stack 和按 unique media part 缓存的 format/encoded-size metadata，media bytes 通过 chunk source 完整 drain 后立即释放；comment projector 只保留受 count/byte guardrail 限制的 author table、cell-reference 去重集合与当前 owning simple note，legacy VML payload 不进入内存；strict sharedStrings projector 只保留当前 item；run projector 只保留当前 item/run text、format 与 bounded element stack；cell-format projector 只保留当前 custom format/cellXfs record、bounded nesting stack 与 bounded `numFmtId` 去重集合；style-component projector 只保留当前 font/fill value 与 bounded nesting stack。各自 XML/text/nesting/count/media 上限由 public options 控制。`read_worksheet()` 的 sharedStrings/style 仍只暴露 workbook-local index；十一个 companion 都不自动做 index/object handoff。十二条读取路径都不加载完整 worksheet/sharedStrings/styles/table/drawing/comments 对象、不构建 DOM/dense matrix/CellStore，也不进入 Patch plan 或 In-memory session。
...
source XLSX
  ...
  -> linked metadata edit: bounded worksheet rewrite + staged .rels/parts/content types
...
Classic-note Patch insertion 是 conservative linked metadata transaction：目标 worksheet 必须没有 source-owned classic/threaded comments 和任何 VML relationship；两次 bounded worksheet scan 只插入或验证本会话生成的 `<legacyDrawing>`。Worksheet replacement、owner `.rels`、generated comments/VML replacements、content types、manifest、EditPlan、临时文件所有权和 public note diagnostics 先在副本中完成，再通过 noexcept commit 发布。同会话追加只重建本会话拥有的 linked parts；未知/source-owned VML 不解析、不合并。
