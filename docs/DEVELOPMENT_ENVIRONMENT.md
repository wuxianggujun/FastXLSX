...
注意：当前 `vcpkg.json` 默认不拉取 FastXLSX 依赖，CMake 也没有
`find_package` 生产依赖。vcpkg preset 只是 toolchain 和 manifest 入口，
不代表生产 ZIP、XML parser 或测试框架已经接入。首次运行 vcpkg preset 时，
vcpkg 仍可能初始化自己的工具缓存，例如下载解压工具；这不等同于接入项目依赖。
