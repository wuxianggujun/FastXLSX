...
- Patch 已覆盖 catalog、事务式空白 worksheet add、关系闭合 worksheet remove、sheetData/cell replacement、worksheet-local internal/external hyperlink、data validation、worksheet-root auto-filter set/clear 与 merged-cell exact merge/unmerge、窄 rename、formula audit/recalculation、core/app properties 和 media bytes replacement。Internal hyperlink 只改 worksheet XML，不创建 `.rels`；external hyperlink 同步 worksheet XML 与 worksheet `.rels`；data validation 只改 worksheet-local container；auto-filter 只整体 replace/clear root element，table-local filter 保留；merged-cell 只严格增删 `<mergeCells>` 中的 multi-cell range，保留全部 cell payload、relationships、content types、tables 与 calc metadata。后三者都不随 structural mutation 自动同步。当前没有 public existing-workbook worksheet clone。新增表可同会话用 Patch 填充/rename/hyperlink/data validation/auto-filter/merged-cell，但需保存重开后才能 In-memory materialize。
