#!/usr/bin/env python3
"""Local QA for FastXLSX conditional-formatting data bars.
...
def verify_no_package_side_effects(path: Path, worksheet_rels_allowed: bool = False) -> dict[str, Any]:
    ...
    require("styles" not in workbook_rels, "data bar should not add styles relationship")
    require("conditionalFormatting" not in workbook_rels,
            "data bar should not add workbook relationship")

    workbook_xml = read_zip_text(path, "xl/workbook.xml")
    require("<calcPr" not in workbook_xml, "data bar should not request recalculation")
...
def verify_basic_package(path: Path) -> dict[str, Any]:
    ...
    require("xmlns:r=" not in worksheet_xml,
            "data-bar-only worksheet should not declare relationship namespace")
    ...
    require(fragment in worksheet_xml, "basic data bar XML fragment mismatch")
    ...
    require(worksheet_xml.count("<dataBar>") == 1, "basic dataBar count mismatch")
    require(worksheet_xml.count("<cfvo ") == 2, "basic cfvo count mismatch")
    require(worksheet_xml.count("<color rgb=") == 1, "basic color count mismatch")
...
def verify_metadata_order_package(path: Path) -> dict[str, Any]:
    ...
    require(expected_order in worksheet_xml,
            "data bar should be between mergeCells and dataValidations")
    require(
        '<hyperlinks><hyperlink ref="B2" r:id="rId1"/></hyperlinks>'
        '<tableParts count="1"><tablePart r:id="rId2"/></tableParts>' in worksheet_xml,
        "data bar should not consume worksheet-local relationship ids",
    )
...
def verify_multi_range_package(path: Path) -> dict[str, Any]:
    ...
    worksheet_xml = read_zip_text(path, "xl/worksheets/sheet1.xml")
    require("xmlns:r=" not in worksheet_xml,
            "multi-range data bar should not declare relationship namespace")
    ...
    require(fragment in worksheet_xml, "multi-range data bar XML fragment mismatch")
    require(worksheet_xml.count("<conditionalFormatting ") == 1,
...
def verify_priorities_package(path: Path) -> dict[str, Any]:
    ...
    require(
        ...
        f'<color rgb="{EXPECTED_BAR_COLOR}"/>' in first_xml,
    ...
    require('<cfRule type="dataBar" priority="1">' in second_xml,
...
def verify_openpyxl_basic(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        require(rule.type == "dataBar", f"rule type mismatch: {rule.type!r}")
        require(rule.priority == 1, f"rule priority mismatch: {rule.priority!r}")
        require(len(rule.dataBar.cfvo) == 2, "openpyxl cfvo count mismatch")
        require(rule.dataBar.cfvo[0].type == "min", "first cfvo type mismatch")
        require(rule.dataBar.cfvo[1].type == "max", "second cfvo type mismatch")
        color = rule.dataBar.color.rgb
        require(color == EXPECTED_BAR_COLOR, f"openpyxl data bar color mismatch: {color!r}")
        return {"status": "opened", "sqref": sqref, "priority": rule.priority, "color": color}
    finally:
        workbook.close()
...
def verify_openpyxl_multi_range(path: Path) -> dict[str, Any]:
    ...
    workbook = openpyxl.load_workbook(path, read_only=False, data_only=False)
    try:
        worksheet = workbook["DataBarRanges"]
        ranges = [
        ...
        rule = worksheet.conditional_formatting[cf_range][0]
        require(rule.type == "dataBar", f"multi-range rule type mismatch: {rule.type!r}")
        require(rule.priority == 1, f"multi-range rule priority mismatch: {rule.priority!r}")
        color = rule.dataBar.color.rgb
        require(color == EXPECTED_BAR_COLOR, f"multi-range data bar color mismatch: {color!r}")
        return {"status": "opened", "sqref": ranges[0], "color": color}
    finally:
        workbook.close()
...
def create_xlsxwriter_reference(path: Path) -> dict[str, Any]:
    ...
    try:
        worksheet = workbook.add_worksheet("DataBar")
    ...
    require("dataBar" in worksheet_xml, "XlsxWriter reference missing dataBar")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=Path(
            ...
            "fastxlsx-streaming-conditional-formatting-data-bar.xlsx"
        ),
        help="FastXLSX basic data-bar workbook to verify.",
    )
    parser.add_argument(
        ...
        help="FastXLSX data bar + relationship-backed metadata workbook.",
    )
    parser.add_argument(
        "--multi-range-input",
        type=Path,
        ...
        help="FastXLSX multi-range conditional-formatting data bar workbook.",
    )
    parser.add_argument(
        "--priorities-input",
        type=Path,
        default=Path(
            "build/windows-nmake-release/tests/"
            "fastxlsx-streaming-conditional-formatting-data-bar-priorities.xlsx"
        ),
        help="FastXLSX workbook that verifies mixed color-scale/data-bar priorities.",
    )
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=Path("build/qa/conditional-formatting-data-bars"),
    ...
    report = {
        ...
        "xlsx_libraries": {
            ...
            "xlsxwriter": create_xlsxwriter_reference(
                work_dir / "reference-xlsxwriter-conditional-formatting-data-bar.xlsx"
            ),
        },
    }
