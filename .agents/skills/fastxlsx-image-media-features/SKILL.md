...
`read_image_info()` 这一 PNG/JPEG 图片元数据 API；它通过默认 `stb` 依赖
读取格式、尺寸和通道。当前还已有
`WorksheetWriter::add_image(path, anchor)` 的 streaming-only new-workbook PNG/JPEG
...
当前还已有窄 `ImageOptions` metadata：`edit_as` 只写入 drawing XML 的
`xdr:twoCellAnchor editAs` attribute；非空 `name` / `description` 只写入
`xdr:cNvPr name` / `descr` attributes；空 `name` 保留生成的 `Picture N`，空
`description` 省略。它不是 `oneCellAnchor` / `absoluteAnchor` 元素支持、图片文件
...
- `WorksheetWriter::add_image()` 用 `read_image_info()` 验证 PNG/JPEG metadata，把
  原始图片字节复制到临时 file-backed media entry；它不解码完整像素，不把图片数据放入
  row/cell 热路径，也不触发 worksheet DOM。
- `ImageOptions` 只把 `edit_as` 枚举和 name/description 字符串复制进 worksheet
  image state，并在 drawing XML 输出阶段做 OpenXML token / XML attribute 输出；它不
  新增 relationship/content type/media 副作用，也不改变 anchor 坐标。
...
1. P17.0 依赖发现和图片元数据 helper：验证默认 `stb` 依赖的 vcpkg 解析、
   include 路径、license、CMake package/include 行为、CI 安装成本和失败模式。当前
   P17a 已有默认 `stb` 依赖和 `read_image_info()`，但只代表 PNG/JPEG
   格式、尺寸和通道读取。
...
3. P17.2 new-workbook-only 插入：当前基础切片是
   `WorksheetWriter::add_image(path, anchor)`，限制为 PNG/JPEG、一种 two-cell anchor
   ...
   drawing mutation。当前 `ImageOptions` 增量只在该 drawing XML 中写
   `xdr:twoCellAnchor editAs` 和 `xdr:cNvPr` 的 `name` / `descr`。
...
- 不要把 `stb` 可用写成完整图片功能已实现；当前只可写为
  `WorksheetWriter::add_image()` 的 new-workbook PNG/JPEG 基础切片。
...
- 不要把 `ImageOptions::edit_as` 写成 `oneCellAnchor` / `absoluteAnchor` 元素支持、
  row/column resize 几何计算、跨办公软件 UI 保证或 existing-workbook 图片编辑。不要把
...
- P17a 图片元数据阶段：默认构建下 `fastxlsx.image` 读取 PNG/JPEG 文件和内存尺寸、
  格式、通道通过，并覆盖 unsupported memory/file header、empty memory buffer、
...
- P17b 图片插入阶段：默认构建下验证 PNG/JPEG media、drawing、rels、
  ...
  图片 metadata 结构测试还应覆盖 `xdr:twoCellAnchor editAs` 的 `oneCell` /
  `absolute` / 默认 `twoCell`、`xdr:cNvPr name` / `descr`、XML attribute escape、
  空 description 省略、默认 `Picture N` 名称，以及 metadata 不改变 media/rels/content
  types/anchor 坐标语义。
...
- 结构测试：检查 `xl/media/*`、`xl/drawings/drawing*.xml`、
  ...
  drawing reference；如果涉及 `ImageOptions`，还要检查 `xdr:twoCellAnchor editAs`
  和 `xdr:cNvPr name` / `descr`。
