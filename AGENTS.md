...
- `include/fastxlsx/detail/cell_store.hpp` 和 `src/cell_store.cpp` 是当前
  ...
  sparse `CellStore`。它只提供 internal sparse storage、`CellValue` 转换、
  `cell_count()` 和估算内存输入；不代表 public `WorkbookEditor`、`WorksheetEditor`、
  random cell editing、guardrail enforcement、save-as / Patch handoff、
...
- `tests/test_minimal_xlsx.cpp` 通过 CTest 注册为 `fastxlsx.unit`，覆盖 XML escape、
  ...
  16385 列拒绝路径、`CellValue` public value 边界和 internal `CellStore` sparse
  storage 边界。
...
- `tests/test_package_editor.cpp` 通过 CTest 注册为 `fastxlsx.package_editor`，
  ...
  且不会凭空创建 `xl/calcChain.xml`。这只是 internal Patch MVP 回归，不是
...
- 把 `src/zip_store_writer.*`、`src/package_writer.*`、`src/package_reader.*` 或
  ...
  或 broad existing-file preservation。当前 package reader 只是 stored/no-compression
