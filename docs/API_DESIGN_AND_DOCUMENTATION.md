...
- Existing-workbook data-validation API 复用 Streaming 的 owning `DataValidationRule`。Add 必须单独声明 multi-range `sqref`、formula1/formula2 shape、prompt/error escaping 与已有 container/count/schema-order guardrail；remove 必须声明当前 effective source order 的 zero-based index、与 `WorksheetDataValidationView::index` 的一致性、strict bounded projection 前置审计、final-container removal 和 count normalization。两者均需写清 planned rename/added worksheet、same-session composition、failure-before-commit/retry，以及不创建 `.rels`/content type、不求值/请求重算。后续 materialized row/column insert/delete 必须从 effective strict projection bounded translate `sqref`：insert shift/expand，delete shift/clip/full removal，single-cell result 保留，empty rule/container 删除；rule/formula/prompt/error 不重写。Point edit、copy/move 与 linked objects 不同步，共享 rule 类型或 reader index 不代表共享 worksheet DOM 或完整 validation 对象模型。
...
- Row/column structural edit 的 merged-cell、root auto-filter、data-validation 与 hyperlink synchronization 必须使用 CellStore/session candidate + 当前有效 worksheet metadata plans 两阶段预构造。四类 metadata 分别 strict scan/translate/audit，再合并为一次 file-backed worksheet rewrite 和一次 `PackageEditor` commit；最后 session 与 public filter diagnostic map publish 必须 noexcept。Merged cells 只产生完整 container replacement/removal；auto-filter 只改 opening-tag `ref` 或移除 whole element；validation 只改 opening-tag `sqref`、删除 empty child/container，并在 child removal 时归一化 `count`；hyperlink 只改 opening-tag `ref`、删除 empty child/container，保留 location/target/display/tooltip。Validation source/final ranges 必须受 64K aggregate guardrail，validation/hyperlink source/final ranges 必须做全局 duplicate/overlap audit。External hyperlink relationship 仅在没有 surviving hyperlink、标准 external type/TargetMode 已审计且 worksheet-local relationship id 没有非-hyperlink 复用时删除；prospective manifest 必须用于重写后 worksheet audit。不能通过多次 public metadata API 拼接，否则后续失败会留下部分 package mutation。Metadata-only 空 CellStore 变化必须 dirty，失败 save 必须保留最终 cells/metadata retry。
