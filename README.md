...
- **Patch rewrite 现状**：最新同机 schema-v11/v5 profile 中，1,000,000-cell numeric/mixed-inline/formula + external hyperlink level-1 upsert 的 total median 为 0.847/1.222/0.849 秒，transform median 为 0.456/0.678/0.457 秒。Pass-through batching 将 transform action 与 output append 降低 99.82%，numeric/formula total median 较 schema-v10 baseline 降低 21.21%/15.69%；mixed-inline 从 1.205 秒变为 1.222 秒，只视为持平。Median process peak working set 为 8.609/9.609/8.609 MB，这些结果只证明记录 workload 的顺序 rewrite 与有界内存，不是任意随机编辑承诺。
...
- **Patch 下一瓶颈**：在吞吐配置 level 1 下，batching 已把 transform action/output append 降到约 2,100 次，但 source scan/action median 仍为 0.441/0.634/0.444 秒。Mixed-inline parser fast path 是下一优先级，temporary output IO 为次级；只有 size-oriented workload 在 matched level 下仍受压缩限制时，才评估其他 backend。
