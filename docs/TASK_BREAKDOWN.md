...
- **下一优先级**：在第二台机器复跑同 revision 的 fixed-shape/fixed-total Patch scaling，并为 Streaming 增加至少一个更大规模或不同机器的 validated bundle；release claim 继续使用 warmed repeated protocol，不以 dirty candidate 替代 bundle。逐 entry 数据继续优先解释 DEFLATE restart/write envelope，当前单机分布不足以授权并发、direct-zlib 或 public backend 变更。Streaming 只评估有明确 bounded-memory、failure propagation 和 paired evidence 方案的 generation/compression overlap；没有 paired wall/CPU/memory/compatibility evidence 时，不跳过 staged CRC failure gate。
