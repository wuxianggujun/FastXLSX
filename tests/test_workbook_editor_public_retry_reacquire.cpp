...
{
    {
        ...
        const std::filesystem::path noop_output =
            artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-delete-row-shift-noop.xlsx");
        ...
        check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
                !matching.has_pending_changes(),
            "delete-row no-op save should keep all recovery handles clean");
        check_retry_reacquire_safe_save_clean_state(
            editor, 4, "delete-row no-op save");
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_noop, "delete-row no-op save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_noop, "delete-row no-op save");
        check(fastxlsx::test::read_zip_entries(noop_output) == second_entries,
            "delete-row no-op output should match the second output");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "delete-row no-op save should leave the source package unchanged");
        ...
        fastxlsx::WorksheetEditor noop_sheet =
            noop_editor.worksheet("Data", options);
        check_retry_reopened_clean_state(
            noop_editor, noop_sheet, "delete-row no-op reopened output");
        check(noop_sheet.get_cell("A1").text_value() == "placeholder-a2",
            "delete-row no-op reopened output should read back shifted source-backed row");
        check(!noop_sheet.try_cell("B1").has_value(),
            "delete-row no-op reopened output should not read back deleted row number");
        check(noop_sheet.get_cell("B3").text_value() == "tail-b4",
            "delete-row no-op reopened output should read back shifted dirty cell");
        const fastxlsx::CellValue noop_formula = noop_sheet.get_cell("C3");
        check(noop_formula.kind() == fastxlsx::CellValueKind::Formula &&
                noop_formula.text_value() == "A1+B3",
            "delete-row no-op reopened output should read back translated formula");
        check_retry_cell_range_equals(noop_sheet.used_range(), 1, 1, 3, 3,
            "delete-row no-op reopened output should read back shifted sparse used range");
        const std::vector<fastxlsx::WorksheetCellSnapshot> noop_row_three =
            noop_sheet.row_cells(3);
        check(noop_row_three.size() == 2 &&
                noop_row_three[0].reference.row == 3 &&
                noop_row_three[0].reference.column == 2 &&
                noop_row_three[0].value.kind() == fastxlsx::CellValueKind::Text &&
                noop_row_three[0].value.text_value() == "tail-b4" &&
                noop_row_three[1].reference.row == 3 &&
                noop_row_three[1].reference.column == 3 &&
                noop_row_three[1].value.kind() == fastxlsx::CellValueKind::Formula &&
                noop_row_three[1].value.text_value() == "A1+B3",
            "delete-row no-op row_cells should expose shifted sparse cells");
        ...
        check(noop_column_three.size() == 1 &&
                ...
                noop_column_three[0].value.text_value() == "A1+B3",
            "delete-row no-op column_cells should expose shifted formula");
    }

    {
        ...
        const std::filesystem::path noop_output =
            artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-delete-column-shift-noop.xlsx");
        ...
        check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
                !matching.has_pending_changes(),
            "delete-column no-op save should keep all recovery handles clean");
        check_retry_reacquire_safe_save_clean_state(
            editor, 4, "delete-column no-op save");
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_noop, "delete-column no-op save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_noop, "delete-column no-op save");
        check(fastxlsx::test::read_zip_entries(noop_output) == second_entries,
            "delete-column no-op output should match the second output");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "delete-column no-op save should leave the source package unchanged");
        ...
        fastxlsx::WorksheetEditor noop_sheet =
            noop_editor.worksheet("Data", options);
        check_retry_reopened_clean_state(
            noop_editor, noop_sheet, "delete-column no-op reopened output");
        const fastxlsx::CellValue noop_number = noop_sheet.get_cell("A1");
        check(noop_number.kind() == fastxlsx::CellValueKind::Number &&
                noop_number.number_value() == 1.0,
            "delete-column no-op reopened output should read back shifted source-backed number");
        check(!noop_sheet.try_cell("A2").has_value(),
            "delete-column no-op reopened output should not read back deleted column text");
        check(noop_sheet.get_cell("C2").text_value() == "tail-d2",
            "delete-column no-op reopened output should read back shifted dirty cell");
        const fastxlsx::CellValue noop_formula = noop_sheet.get_cell("B1");
        check(noop_formula.kind() == fastxlsx::CellValueKind::Formula &&
                noop_formula.text_value() == "A2+C1",
            "delete-column no-op reopened output should read back translated formula");
        check_retry_cell_range_equals(noop_sheet.used_range(), 1, 1, 2, 3,
            "delete-column no-op reopened output should read back shifted sparse used range");
        ...
        check(noop_row_one.size() == 2 &&
                ...
                noop_row_one[1].reference.column == 2 &&
                noop_row_one[1].value.kind() == fastxlsx::CellValueKind::Formula &&
                noop_row_one[1].value.text_value() == "A2+C1",
            "delete-column no-op row_cells should expose shifted sparse cells");
        const std::vector<fastxlsx::WorksheetCellSnapshot> noop_column_two =
            noop_sheet.column_cells(2);
        check(noop_column_two.size() == 1 &&
                noop_column_two[0].reference.row == 1 &&
                noop_column_two[0].reference.column == 2 &&
                noop_column_two[0].value.kind() == fastxlsx::CellValueKind::Formula &&
                noop_column_two[0].value.text_value() == "A2+C1",
            "delete-column no-op column_cells should expose shifted formula");
    }
}
...
{
    {
        ...
        const std::filesystem::path post_noop_output =
            artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-delete-row-ref-formula-post-noop.xlsx");
        const std::filesystem::path final_noop_output =
            artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-delete-row-ref-formula-final-noop.xlsx");
        ...
        matching.set_cell(3, 4,
            fastxlsx::CellValue::text("delete-row-ref-formula-post-noop"));
        check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
                matching.has_pending_changes(),
            "delete-row ref formula post-noop edit should dirty all recovery handles");

        editor.save_as(post_noop_output);
        check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
                !matching.has_pending_changes(),
            "delete-row ref formula post-noop save should clean all recovery handles");
        check_retry_reacquire_safe_save_clean_state(
            editor, 5, "delete-row ref formula post-noop save");

        const auto post_noop_entries =
            fastxlsx::test::read_zip_entries(post_noop_output);
        check_contains(post_noop_entries.at("xl/worksheets/sheet1.xml"),
            R"(<c r="C3"><f>#REF!+A:A+#REF!+B3</f></c>)",
            "delete-row ref formula post-noop output should keep the translated formula");
        check_contains(post_noop_entries.at("xl/worksheets/sheet1.xml"),
            "delete-row-ref-formula-post-noop",
            "delete-row ref formula post-noop output should include the later edit");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "delete-row ref formula post-noop save should leave the source package unchanged");
        check(fastxlsx::test::read_zip_entries(second_output) == second_entries,
            "delete-row ref formula post-noop save should leave the second output unchanged");
        check(fastxlsx::test::read_zip_entries(noop_output) == noop_entries,
            "delete-row ref formula post-noop save should leave the no-op output unchanged");

        fastxlsx::WorkbookEditor post_noop_editor =
            fastxlsx::WorkbookEditor::open(post_noop_output);
        fastxlsx::WorksheetEditor post_noop_sheet =
            post_noop_editor.worksheet("Data", options);
        check_retry_reopened_clean_state(
            post_noop_editor, post_noop_sheet,
            "delete-row ref formula post-noop reopened output");
        const fastxlsx::CellValue post_noop_formula =
            post_noop_sheet.get_cell("C3");
        check(post_noop_formula.kind() == fastxlsx::CellValueKind::Formula &&
                post_noop_formula.text_value() == "#REF!+A:A+#REF!+B3",
            "delete-row ref formula post-noop reopened output should read back #REF! translation");
        check(post_noop_sheet.get_cell("D3").text_value() ==
                "delete-row-ref-formula-post-noop",
            "delete-row ref formula post-noop reopened output should read back the later edit");
        check_retry_cell_range_equals(post_noop_sheet.used_range(), 1, 1, 3, 4,
            "delete-row ref formula post-noop reopened output should expand sparse used range");
        ...
        check(post_noop_row_three.size() == 2 &&
                ...
                post_noop_row_three[1].value.kind() == fastxlsx::CellValueKind::Text &&
                post_noop_row_three[1].value.text_value() ==
                    "delete-row-ref-formula-post-noop",
            "delete-row ref formula post-noop row_cells should expose formula and later edit");

        const WorkbookEditorPublicCatalogSnapshot catalog_before_final_noop =
            workbook_editor_public_catalog_snapshot(editor);
        const WorkbookEditorPublicSaveStateSnapshot save_state_before_final_noop =
            workbook_editor_public_save_state_snapshot(editor);

        editor.save_as(final_noop_output);
        check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
                !matching.has_pending_changes(),
            "delete-row ref formula final no-op save should keep all recovery handles clean");
        check_retry_reacquire_safe_save_clean_state(
            editor, 5, "delete-row ref formula final no-op save");
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_final_noop,
            "delete-row ref formula final no-op save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_final_noop,
            "delete-row ref formula final no-op save");
        const auto final_noop_entries =
            fastxlsx::test::read_zip_entries(final_noop_output);
        check(final_noop_entries == post_noop_entries,
            "delete-row ref formula final no-op output should match the post-noop output");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "delete-row ref formula final no-op save should leave the source package unchanged");
        check(fastxlsx::test::read_zip_entries(second_output) == second_entries,
            "delete-row ref formula final no-op save should leave the second output unchanged");
        check(fastxlsx::test::read_zip_entries(noop_output) == noop_entries,
            "delete-row ref formula final no-op save should leave the first no-op output unchanged");
        check(fastxlsx::test::read_zip_entries(post_noop_output) == post_noop_entries,
            "delete-row ref formula final no-op save should leave the post-noop output unchanged");

        fastxlsx::WorkbookEditor final_noop_editor =
            fastxlsx::WorkbookEditor::open(final_noop_output);
        fastxlsx::WorksheetEditor final_noop_sheet =
            final_noop_editor.worksheet("Data", options);
        check_retry_reopened_clean_state(
            final_noop_editor, final_noop_sheet,
            "delete-row ref formula final no-op reopened output");
        const fastxlsx::CellValue final_noop_formula =
            final_noop_sheet.get_cell("C3");
        check(final_noop_formula.kind() == fastxlsx::CellValueKind::Formula &&
                final_noop_formula.text_value() == "#REF!+A:A+#REF!+B3",
            "delete-row ref formula final no-op reopened output should read back #REF! translation");
        check(final_noop_sheet.get_cell("D3").text_value() ==
                "delete-row-ref-formula-post-noop",
            "delete-row ref formula final no-op reopened output should keep the later edit");
        check_retry_cell_range_equals(final_noop_sheet.used_range(), 1, 1, 3, 4,
            "delete-row ref formula final no-op reopened output should keep expanded sparse range");
    }

    {
        ...
        const std::filesystem::path post_noop_output =
            artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-delete-column-ref-formula-post-noop.xlsx");
        const std::filesystem::path final_noop_output =
            artifact("fastxlsx-workbook-editor-public-worksheet-rename-back-failed-save-delete-column-ref-formula-final-noop.xlsx");
        ...
        matching.set_cell(1, 4,
            fastxlsx::CellValue::text("delete-column-ref-formula-post-noop"));
        check(sheet.has_pending_changes() && reacquired.has_pending_changes() &&
                matching.has_pending_changes(),
            "delete-column ref formula post-noop edit should dirty all recovery handles");

        editor.save_as(post_noop_output);
        check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
                !matching.has_pending_changes(),
            "delete-column ref formula post-noop save should clean all recovery handles");
        check_retry_reacquire_safe_save_clean_state(
            editor, 5, "delete-column ref formula post-noop save");

        const auto post_noop_entries =
            fastxlsx::test::read_zip_entries(post_noop_output);
        check_contains(post_noop_entries.at("xl/worksheets/sheet1.xml"),
            R"(<c r="C1"><f>#REF!+#REF!+1:1+C2</f></c>)",
            "delete-column ref formula post-noop output should keep the translated formula");
        check_contains(post_noop_entries.at("xl/worksheets/sheet1.xml"),
            "delete-column-ref-formula-post-noop",
            "delete-column ref formula post-noop output should include the later edit");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "delete-column ref formula post-noop save should leave the source package unchanged");
        check(fastxlsx::test::read_zip_entries(second_output) == second_entries,
            "delete-column ref formula post-noop save should leave the second output unchanged");
        check(fastxlsx::test::read_zip_entries(noop_output) == noop_entries,
            "delete-column ref formula post-noop save should leave the no-op output unchanged");
        ...
        check_retry_reopened_clean_state(
            post_noop_editor, post_noop_sheet,
            "delete-column ref formula post-noop reopened output");
        const fastxlsx::CellValue post_noop_number =
            post_noop_sheet.get_cell("A1");
        check(post_noop_number.kind() == fastxlsx::CellValueKind::Number &&
                post_noop_number.number_value() == 1.0,
            "delete-column ref formula post-noop reopened output should read shifted source number");
        const fastxlsx::CellValue post_noop_formula =
            post_noop_sheet.get_cell("C1");
        check(post_noop_formula.kind() == fastxlsx::CellValueKind::Formula &&
                post_noop_formula.text_value() == "#REF!+#REF!+1:1+C2",
            "delete-column ref formula post-noop reopened output should read back #REF! translation");
        check(post_noop_sheet.get_cell("D1").text_value() ==
                "delete-column-ref-formula-post-noop",
            "delete-column ref formula post-noop reopened output should read back the later edit");
        check_retry_cell_range_equals(post_noop_sheet.used_range(), 1, 1, 1, 4,
            "delete-column ref formula post-noop reopened output should expand sparse used range");
        const std::vector<fastxlsx::WorksheetCellSnapshot> post_noop_row_one =
            post_noop_sheet.row_cells(1);
        check(post_noop_row_one.size() == 3 &&
                ...
                post_noop_row_one[2].value.kind() == fastxlsx::CellValueKind::Text &&
                post_noop_row_one[2].value.text_value() ==
                    "delete-column-ref-formula-post-noop",
            "delete-column ref formula post-noop row_cells should expose number, formula, and later edit");
        ...
        check(!sheet.has_pending_changes() && !reacquired.has_pending_changes() &&
                !matching.has_pending_changes(),
            "delete-column ref formula final no-op save should keep all recovery handles clean");
        check_retry_reacquire_safe_save_clean_state(
            editor, 5, "delete-column ref formula final no-op save");
        check_workbook_editor_public_save_state_preserved(
            editor, save_state_before_final_noop,
            "delete-column ref formula final no-op save");
        check_workbook_editor_public_catalog_preserved(
            editor, catalog_before_final_noop,
            "delete-column ref formula final no-op save");
        const auto final_noop_entries =
            fastxlsx::test::read_zip_entries(final_noop_output);
        check(final_noop_entries == post_noop_entries,
            "delete-column ref formula final no-op output should match the post-noop output");
        check(fastxlsx::test::read_zip_entries(source) == source_entries,
            "delete-column ref formula final no-op save should leave the source package unchanged");
        check(fastxlsx::test::read_zip_entries(second_output) == second_entries,
            "delete-column ref formula final no-op save should leave the second output unchanged");
        check(fastxlsx::test::read_zip_entries(noop_output) == noop_entries,
            "delete-column ref formula final no-op save should leave the first no-op output unchanged");
        check(fastxlsx::test::read_zip_entries(post_noop_output) == post_noop_entries,
            "delete-column ref formula final no-op save should leave the post-noop output unchanged");
        ...
        check_retry_reopened_clean_state(
            final_noop_editor, final_noop_sheet,
            "delete-column ref formula final no-op reopened output");
        const fastxlsx::CellValue final_noop_number =
            final_noop_sheet.get_cell("A1");
        check(final_noop_number.kind() == fastxlsx::CellValueKind::Number &&
                final_noop_number.number_value() == 1.0,
            "delete-column ref formula final no-op reopened output should keep shifted source number");
        const fastxlsx::CellValue final_noop_formula =
            final_noop_sheet.get_cell("C1");
        check(final_noop_formula.kind() == fastxlsx::CellValueKind::Formula &&
                final_noop_formula.text_value() == "#REF!+#REF!+1:1+C2",
            "delete-column ref formula final no-op reopened output should read back #REF! translation");
        check(final_noop_sheet.get_cell("D1").text_value() ==
                "delete-column-ref-formula-post-noop",
            "delete-column ref formula final no-op reopened output should keep the later edit");
        check_retry_cell_range_equals(final_noop_sheet.used_range(), 1, 1, 1, 4,
            "delete-column ref formula final no-op reopened output should keep expanded sparse range");
    }
}
