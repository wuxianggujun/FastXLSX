...
- **已完成基线**：existing-workbook `WorkbookEditor::add_internal_hyperlink()` 已实现 worksheet-local internal hyperlink 窄切片。它通过两次 bounded worksheet event scan 规划并写出 `<hyperlink ref="..." location="..."/>`，支持既有/自闭合 `<hyperlinks>`、display/tooltip escaping、A1 单元格和 range overlap rejection、planned rename 与同会话新增 worksheet；不创建 `.rels`、content type 或 external relationship，不修改 cell value/style。失败在 public state 发布前返回，save failure 保留 retry，unknown package entries 与既有 worksheet relationships 保留；`pending_worksheet_edits()` 暴露每表 internal hyperlink count。
- 本切片已通过 production CTest 136/136、stored-only focused 1/1、no-images runtime smoke 0，以及 production/stored/no-images 三套 install consumer；三个默认 profile 的安装 config 均不含 direct-zlib export dependency。Office 未运行，保持 `not_run`。
- **已完成基线**：existing-workbook `WorkbookEditor::add_external_hyperlink()` 已实现 worksheet XML + worksheet `.rels` 的 external hyperlink 窄切片。它分配不冲突的 relationship id，追加 `TargetMode="External"`，支持既有/缺失 `.rels`、缺失 `xmlns:r`、display/tooltip escaping、planned rename/added worksheet、重复/重叠 ref 和 schema 顺序校验；不创建 content type，不做 target reachability、relationship repair/pruning 或 cell/formula/definedName/table/drawing 同步。失败在 public state 发布前返回，save failure 保留 retry；`pending_worksheet_edits()` 暴露每表 external hyperlink count。
- 本切片已通过 production CTest 136/136、stored-only focused 1/1、no-images runtime smoke 0，以及 production/stored/no-images 三套 install consumer；三个默认 profile 的安装 config 均不含 direct-zlib export dependency。Office 未运行，保持 `not_run`。
- **下一功能主线**：推进 existing-workbook data-validation 编辑。先定义 rule API、`sqref`、formula1/formula2、XML ordering、与 row/cell mutation 的边界及失败恢复；不要把 hyperlink relationship mutation 泛化成通用 worksheet metadata DOM。
